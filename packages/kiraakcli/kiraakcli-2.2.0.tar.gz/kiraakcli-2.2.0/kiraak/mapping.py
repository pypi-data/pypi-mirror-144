...
class Mapping:
    ...
    def initialize_mapping(self) -> None:
        ...
        if not all([x.mapping for x in self.orders.all_products]):
            logger.error(f"Products {','.join([x.name for x in self.orders.all_products if not x.mapping])} not found in catalog, skipping products!")
        if not all([x.mapping.price == x.price for x in self.orders.all_products if x.mapping]):
            logger.error(f"Products {','.join([x.name for x in self.orders.all_products if x.mapping.price != x.price])} do not have matching prices!")
            sys.exit(1)
        ...
        while True:
            ...
            for i, order_prod in enumerate(self.orders.unique_products):
                if not order_prod.mapping:
                    logger.error(f"Product {order_prod.name} not found in catalog, skipping in all orders!")
                    continue
                tbl.add_row(
                    str(i),
                    order_prod.name,

...
def cDNA_para():
    ...
    cDNA_conf.write('in1=%s/01.data/cDNAin2'%args.outdir+'\n')
...
def oligo_para():
    ...
    oligo_conf.write('outFq=%s/01.data/Index_reads.fq'%args.outdir+'\n')
    oligo_conf.write('threads=%s'%args.thread+'\n')
    oligo_conf.close()

def cDNA_qcstaranno():
    cDNA_para()
    cDNA_star_cmd = '%s/soft/scStar --outSAMattributes singleCell --outSAMtype BAM Unsorted --genomeDir %s --outFileNamePrefix %s/01.data/ --stParaFile %s/01.data/cDNA_para --outSAMmode NoQS --runThreadN %s'\
        %(_root_dir,args.starIndexDir,args.outdir,args.outdir,args.thread)
    cDNA_anno_cmd = ['%s/soft/Anno -I %s/01.data/Aligned.out.bam -a %s -L %s/01.data/cDNA_barcode_counts_raw.txt -o %s/01.data -c %s -m chrM'\
        %(_root_dir,args.outdir,args.gtf,args.outdir,args.outdir,args.thread)]

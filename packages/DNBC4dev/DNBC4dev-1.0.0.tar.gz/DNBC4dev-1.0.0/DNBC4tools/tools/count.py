...
class Count:
    ...
    def run(self):
        ...
        tagAdd_cmd = '%s/soft/tagAdd -n %s -bam %s -file %s/02.count/%s_barcodeTranslate.txt -out %s/02.count/anno_decon.bam -tag_check CB:Z: -tag_add DB:Z: '\
        ...
        saturation_cmd = '%s %s/rna/saturation.py --inbam %s/02.count/anno_decon.bam --outdir %s/02.count --quality 20 --threads %s'\
            %(new_python,_root_dir,self.outdir,self.outdir,self.thread)
        ...
        rm_temp('%s/02.count/cell_count_detail.xls'%self.outdir,'%s/02.count/%s_CB_UB_count.txt'%(self.outdir,self.name),'%s/02.count/beads_barcode_all.txt'%self.outdir)
...
def parse_count(parser):
    ...
    parser.add_argument(
        '--bam',
        help='Bam file after star and anno, eg./01.data/final.bam'
    )
    parser.add_argument(
        '--cDNAbarcodeCount',
        help='Read count per cell barcode for cDNA, eg./01.data/test.cDNA_barcode_counts_raw.txt.',
    )
    parser.add_argument(
        '--Indexreads',
        help='Barcode reads generate by scRNAparse, eg./01.data/Index_reads.fq.'
    )
    parser.add_argument(
        '--oligobarcodeCount',
        help='Read count per cell barcode for oligo, eg./01.data/Index_barcode_counts_raw.txt.'
    )
    parser.add_argument(
        '--oligotype',
    ...
    parser.add_argument(
        '--expectNum',
        help='The number of beads intercepted by the inflection point',
    )
    return parser
...
def get_args():
    ...
    '''input the sample species, default is NA.''', default='NA')
    args = parser.parse_args()
    return [args.outPath, args.htmlTemplate, args.name, args.species]
 
def get_args_from_file():
    path=get_args()[0]
    csv = [path+'/02.count/cellCount_report.csv',\
    path+'/03.analysis/Clustering/cell_report.csv',\
    ...
    stat = dict()
    stat['samplename'] = str(get_args()[2])
    stat['species'] = str(get_args()[3])
    for i in range(len(csv)):
        if i==0:
            df = pd.read_csv(open(csv[i]),encoding="utf_8",dtype=str,header=None,sep=",")
            stat['estm_Num_cell'] = df[1][0]
            stat['mean_r_per_c'] = df[1][1]
            stat['mean_UMI_per_c'] = df[1][2]
            stat['median_UMI_per_c'] = df[1][3]
            stat['total_gene'] = df[1][4]
            stat['mean_genes_per_c'] = df[1][5]
            stat['median_genes_per_c'] = df[1][6] 
            stat['saturation'] = str(df[1][7]) +'%'
        if i==1:
            df = pd.read_csv(open(csv[i]),encoding="utf_8",dtype=str,header=None,sep=",")
            stat['cluster_cell'] = df[1][0]
        if i==2:
            df = pd.read_csv(open(csv[i]),encoding="utf_8",dtype=str,header=None,sep=",")
            ...
            stat['cDNA_Q30_c_bar'] = df[1][5]
            stat['cDNA_Q30_s_bar'] = df[1][6]
            stat['cDNA_Q30_UMI'] = df[1][7]
            stat['cDNA_Q30_r'] = df[1][8]
        if i==3:
            df = pd.read_csv(open(csv[i]),encoding="utf_8",dtype=str,header=None,sep=",")
            ...
            stat['index_Q30_c_bar'] = df[1][5]
            stat['index_Q30_s_bar'] = df[1][6]
            stat['index_Q30_UMI'] = df[1][7]
            stat['index_Q30_r'] = df[1][8]
        if i==4:
            df = pd.read_csv(open(csv[i]),encoding="utf_8",dtype=str,header=None,sep=",")        
            stat['raw_r'] = df[1][0]
            stat['map_r'] = df[1][1]
    ...
    '''
    plot_order = ['plot1','plot2','plot3','plot4','plot5','plot6','plot7']
...
def write_param_to_template():
    ...
    report=html.safe_substitute(sample_info=samplename, samplename=stat['samplename'],\
            ...
            cluster_cell=stat['cluster_cell'],cDNA_num_frag=stat['cDNA_num_frag'],\
            ...
            r_m_ex=stat['r_m_ex'],r_m_intro=stat['r_m_intro'],\
    ...
    cols = ["samplename","species","estm_Num_cell","mean_r_per_c","mean_UMI_per_c","median_UMI_per_c","total_gene",\
        "mean_genes_per_c","median_genes_per_c","saturation","cluster_cell","cDNA_num_frag","cDNA_frag_pass_QC","cDNA_Q30_r",\
        "index_num_frag","index_frag_pass_QC","index_Q30_r","map_r","mito_ratio","r_m_geno","r_m_ex","r_m_intro",\
    ...
    metrics_summary_df.columns =["SampleName","species","Estimated number of cell","Mean reads per cell","Mean UMI count per cell",\
        "Median UMI counts per cell","Total genes detected","Mean genes per cell","Median genes per cell","Sequencing saturation","Number of cells used for clustering",\
...
if __name__ == '__main__':
    ...
    fw = open(outpath+'/04.report/'+samplename+'_CDCPv2_scRNA_report.html','w')
    fw.write(report)
    file_df = outpath+'/04.report/metrics_summary.xls'
    metrics_summary_df.to_csv(file_df,sep='\t',header=None)

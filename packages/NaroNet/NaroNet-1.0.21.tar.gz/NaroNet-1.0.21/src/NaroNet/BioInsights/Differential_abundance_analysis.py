...
def differential_abundance_analysis(dataset, heatmapALL, labels, IndexAndClass,isTraining):        
        ...
        def showPatientPredictionConfidence(dataset,pred_Confidence, labels,Subgroups_of_patients_labels_names, Subgroups_of_patients_labels,topkClustersStats,IndexAndClass,Top1PerPatient,real_class_confidence):
            ...
            pred_Confidence_dict['Subject prediction confidence'] = real_class_confidence      
            pred_Confidence_dict['Subject Name'] = [i[0] for i in IndexAndClass]
            pred_Confidence_dict['Subject Index'] = [i[1] for i in IndexAndClass]
            pred_Confidence_dict['Class'] = [i[2][0] for i in IndexAndClass]
        ...
        def visualizeRegressionAnalysis_(dataset, dependentVariable,clust,list_labels):
            ...
            for nn, clust_n in enumerate(clust[1]):                
                ...
                for v in data.keys():
                    ...
                    # data[v][data[v]>2] = (data[v][data[v]>2]/10) + 2
                    # data[v][data[v]<2] = (data[v][data[v]<2]/10) + 2
                ...
                labels_pal = sns.cubehelix_palette(len(set(list_labels)), light=.9, dark=.1, reverse=True, start=0, rot=-2)                
                ax = sns.boxplot(data=df, order=list(set(list_labels)),palette=labels_pal, linewidth=2.5)
                ax = sns.stripplot(data=df, order=list(set(list_labels)),color=".5")
        ...
        def visualizeRegressionAnalysis(dataset,dependentVariable,labels,pred_Label,topkClustersStats,IndexAndClass,top1):                                    
            ...
            dependentVariable = np.concatenate(tuple([dependentVariable[h] for h in dependentVariable]),1)
        ...
        if not isTraining:
            ...
            visualizeRegressionAnalysis(dataset,heatmapALL,labels,pred_Label,topkClustersStats,IndexAndClass,Top1PerPatient)
        ...
        return topkClustersStats, Unrestricted_loss, Top1PerPatient, patient_Ineach_subgroup, real_class_confidence
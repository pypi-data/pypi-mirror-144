...
import json
from xeda.flows.flow import FileResource, removesuffix
from xeda.flows.vivado.vivado_sim import VivadoSim
from ..lwc import LwcDesign
...
class VivadoSimTiming(VivadoSim):
    class Settings(VivadoSim.Settings):
        pass

    def __init__(self, settings: Settings, args: SimpleNamespace):
        design = LwcDesign(**self.design.dict())
        tb = design.tb
        lwc = design.lwc

        tb.top = ("LWC_TB",)
        if not lwc.two_pass:
            tb.top = ("LWC_TB_2pass_conf",)

        tb.generics["G_MAX_FAILURES"] = 1
        tb.generics["G_TEST_MODE"] = 4

        tv_root = "KAT"

        for t in ["pdi", "sdi", "do"]:
            gen = f"G_FNAME_{t.upper()}"
            if gen in tb.generics:
                del tb.generics[gen]

        tvs = ["generic_aead_sizes_new_key"]

        block_size_bits = lwc.block_bits
        if (
            not block_size_bits
            or "AD" not in block_size_bits
            or "PT" not in block_size_bits
            or (supports_hash and "HM" not in block_size_bits)
        ):
            _logger.critical(
        ...
        if lwc.get("key_reuse", lwc.get("reuse_key")):
        ...
        for tv_subfolder in tvs:
            ...
            rc_generics = deepcopy(tb.generics)
    ...
    def parse_reports(self) -> bool:
        ...
        run_configs = self.settings.flow.get(
            "run_configs",
            [dict(generics=tb_settings.get("generics", {}), name="DEFAULT")],
        )
        ...
        for rc in run_configs:
            ...
            lwctb_log = self.run_path / rc_generics["G_FNAME_LOG"]
            ...
            timing_txt = self.run_path / rc_generics["G_FNAME_TIMING"]
            ...
            for match in pdi_pat.finditer(content):
                ...
                d = dict(op=op, msgid=match.group("msgid"), keyid=match.group("keyid"))
            ...
            assert len(msgs) == len(
                tp
            ), f"count of messages in {pdi} and timings from {timing_txt} do not match!"
            ...
            for msg, (id, cycles) in zip(msgs, tp):
                ...
                if suffix:
                    op += " (" + suffix + ")"
                msg = {
                    removesuffix(k, "Size").strip().upper(): v
                    for k, v in msg.items()
                    if k not in ["keyid", "op", "msgid"]
                }

                timing_results[op] = timing_results.get(op, []) + [(msg, cycles)]

            results["TV:" + rc_name] = rc_results

        supports_hash = LwcDesign.supports_hash(self.settings.design)
            ...
            if op != "Hash":
                ...
                for msg_type in ["AD", xt, None]:
                    ...
                    for idx, sz in enumerate(xxz):
                        for msg, cycle in tr:
                            right_value = all(
                                z == msg.get(m) for m, z in zip(msg_type, sz)
                            )
                            others_zero = all(
                            ...
                            if right_value and others_zero:
                                if idx == 3:
                                    ...
                                    row.append(
                                        (
                                            f'{"+".join(msg_type)}_{"+".join(map(str,sz))}',
                                            cycle,
                                        )
                                    )
                                break
                    ...
                    if bsx4 and bsx5:
                        row.append(
                            (f'{"+".join(msg_type)}_Long', str(int(bsx5) - int(bsx4)))
                        )
...
class VivadoSimVerification(VivadoSim, LwcDesign):
    def __init__(self, settings: VivadoSim.Settings, *args, **kwargs):
        tb = self.design.tb

        lwc_settings = self.design.dict().get("lwc", {})

        tb.top = ("LWC_TB",)
        if lwc_settings.get("two_pass"):
            tb.top = ("LWC_TB_2pass_conf",)

        tb.generics["G_MAX_FAILURES"] = 100
        tb.generics["G_TEST_MODE"] = 0  # TODO

        tv_root = "KAT"
        variant = LwcDesign.variant(self.design.dict())
        ...
        if LwcDesign.supports_hash(settings.design):
    ...
    def parse_reports(self) -> bool:
        ...
        for rc in run_configs:
            ...
            with open(lwctb_log) as f:
                ...
                if not match:
                    ...
                    success = False
                    copyfile(
                        self.run_path / rc_generics["G_FNAME_FAILED_TVS"],
                        self.results_dir
                        / (name + "_" + rc_generics["G_FNAME_FAILED_TVS"]),
                    )
                else:
                    rc_results["postreset_cycles"] = match.group("cycles")
                    rc_results["total_sim_time"] = match.group("totaltime")
        ...
        success_file = f"{name}_verification_results.json"

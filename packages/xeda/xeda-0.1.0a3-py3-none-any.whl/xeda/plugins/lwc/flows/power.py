...
from xeda.design import DesignSource, Design
from xeda.flows.flow import SimFlow
...
class VivadoPower(XedaVivadoPower, LwcDesign):
    class Settings(XedaVivadoPower.Settings):
        pass

    def init(self):
        flow_settings = self.settings
        design_settings = self.design

        # parent_prereqs = XedaVivadoPower

        postsynthsim_overrides, design_overrides = parent_prereqs[VivadoPostsynthSim]

        postsynthsim_overrides["dependencies"] = postsynthsim_overrides.get(
            "dependencies", {}
        )
        postsynthsim_overrides["dependencies"]["vivado_synth"] = postsynthsim_overrides[
            "dependencies"
        ].get("vivado_synth", {})
        postsynthsim_overrides["dependencies"]["vivado_synth"][
            "strategy"
        ] = "AreaTiming"  # 'AreaPower'
        ...
        postsynthsim_overrides["prerun_time"] = (
            100 + (postsynthsim_overrides["clock_period"] * 4) - 1
        )
        postsynthsim_overrides["timing_sim"] = True
        postsynthsim_overrides["stop_time"] = None
        postsynthsim_overrides["vcd"] = None

        LwcDesign.wrap_design(design_settings)
        ...
        if not power_tvs:
            ...
            if LwcDesign.supports_hash(design_settings):
        ...
        def pow_tv_run_config(tv_sub):
            ...
            clock_ps = math.floor(postsynthsim_overrides["clock_period"] * 1000)
    ...
    def parse_reports(self) -> bool:
        ...
        fields = {"Design": design_name, "Frequency (MHz)": freq}
...
class VivadoPowerTimingOnly(SimFlow, LwcDesign):
    """
    ...
    def prerequisite_flows(cls, flow_settings, design_settings):
        ...
        power_tvs_root = os.path.join("KAT_GMU", LwcDesign.variant(design_settings))
        ...
        def pow_tv_run_config(tv_sub):
            ...
            clock_ps = math.floor(flow_overrides["clock_period"] * 1000)
            tv_generics[clock_period_ps_generic] = clock_ps
            for t in ["pdi", "sdi", "do"]:
                tv_generics[f"G_FNAME_{t.upper()}"] = DesignSource(
                    os.path.join(power_tvs_root, tv_sub, f"{t}.txt")
            ...
            return dict(
                generics=tv_generics, saif=None, report=f"{tv_sub}.xml", name=tv_sub
            )
    ...
    def parse_reports(self):
        ...
        for rc in sim_flow_settings["run_configs"]:
            ...
            with open(lwctb_log) as f:
                ...
                assert (
                    match
                ), f"timing pattern not found in the LWC_TB log {lwctb_log}. Make sure simulation has not failed and you are using the correct version of LWC_TB"
                results["cycles"] = match.group("cycles")
                fields[name + "_cycles"] = results["cycles"]
                results["totaltime"] = match.group("totaltime")
            self.results[name] = results
        ...
        csv_path = (
            self.results_dir / f"VivadoPowerLwcTimingOnly_{design_name}_{freq}MHz.csv"
        )
        with open(csv_path, "w") as csv_file:
            writer = csv.DictWriter(csv_file, fields.keys())
            writer.writeheader()

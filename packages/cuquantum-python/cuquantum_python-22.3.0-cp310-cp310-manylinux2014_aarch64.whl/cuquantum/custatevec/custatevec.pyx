...
cpdef destroy(intptr_t handle):
    ...
    if not can_release_gil(handle):
        with nogil:
            status = custatevecGetDeviceMemHandler(<_Handle>handle, &handler)
        if status == 0:
            cpython.Py_DECREF(<object>handler.ctx)
            del handle_gil[handle]
        else:
            err = custatevecGetErrorString(<_Status>status).decode()
            assert False, f"{err} (handle: {handle})"
...
cpdef sampler_preprocess(
    ...
    if can_release_gil(handle):
        ...
        handle_gil[sampler] = False  # for sampler destructor
        status = custatevecSamplerPreprocess(
            <_Handle>handle, <_SamplerDescriptor>sampler,
            <void*>workspace, workspace_size)
    check_status(status)
...
cpdef accessor_set_extra_workspace(
    ...
    if can_release_gil(handle):
        ...
        handle_gil[accessor] = False  # for accessor destructor
        status = custatevecAccessorSetExtraWorkspace(
            <_Handle>handle, <_AccessorDescriptor>accessor,
            <void*>workspace, workspace_size)
    check_status(status)
...
cpdef set_device_mem_handler(intptr_t handle, handler):
    ...
    if isinstance(handler, int):
        ...
        handle_gil[handle] = True  # can release gil
...
# whether C routines can release gil with a particular library handle
cdef dict handle_gil = {}

cdef inline bint can_release_gil(intptr_t handle):
    return handle_gil.get(handle, True)  # if not recorded, always release


# keep the registered logger callback alive
cdef tuple logger_callback_holder = None

...
class TimeSeries():
    ...
    def setTimeSeriesDataCollection(self, timeSeriesData:list, chunkSize:int=10000) -> None:
        ...
        try:
            ...
            msg = f"GraphQL string could not be created out of dictionary. Cause: {err}"
    ...
    def timeSeriesData(
        ...
        if type(fields) != list:
            _fields = fields
            if 'sys_inventoryItemId' not in _fields:
                _fields += ['sys_inventoryItemId']
        else:
            _fields = ''
            for header in fields:
                _fields += header + '\n'
        ...
        while True:         
            ...
            try:
                ...
                logger.error(f"Problem with pagination: {err}")
        ...
        if displayMode == 'pivot':
            ...
            msg = f"Unknown displayMode '{displayMode}'. Use 'pivot' or 'rows'."
            if self.raiseException: raise Exception(msg)
            else:
                logger.error(msg)
                return
    ...
    def timeSeriesGroupData(
        ...
        timeZone: str = None
            ...
            to the local time zone or to a previously set default time zone.
        includeMissing: bool = False
            If True, timestamps without values will be loaded and lead to a larger DataFrame. However,
            this works only with display modes == rows or pivot-full.
        displayMode: str = pivot
            pivot: pivot display of columns and timestamps, columns without values are dropped
            pivot-full: pivot display of all columns and timestamps
            rows: row display
    ...
    def timeSeriesGroupDataReduced(
        ...
        if displayMode == 'pivot-full':
            ...
            df = df.pivot_table(index='timestamp', columns=pivotColumns, values='value', dropna=False)
            if len(df.columns) == 0:
                logger.info(f"The resulting DataFrame is empty.")
                return
            df.index = pd.to_datetime(df.index, format='%Y-%m-%dT%H:%M:%S').tz_convert(pytz.timezone(tz))
            if Utils.useDateTimeOffset == False:
                df.index = df.index.tz_localize(tz=None)
    ...
    def deleteTimeSeriesData(
        ...
        if inventoryItemIds == None and where == None:
            msg = f"No id list of items and no where-criteria were provided."
            if self.raiseException: raise Exception(msg)
            else:
                logger.error(msg)
                return

...
class Choices:
        ...
        for choice in choices:
            if isinstance(choice, (list, tuple)):
                if len(choice) == 3:
                    ...
                    raise ValueError("Choices can't take a list of length %s, only 2 or 3" % len(choice))
            else:
    ...
    def __repr__(self):
        return "{}({})".format(self.__class__.__name__, ", ".join("%s" % repr(i) for i in self._triples))
    ...
    def subset(self, *new_identifiers):
        ...
        if not identifiers.issuperset(new_identifiers):
            raise ValueError(
                "The following identifiers are not present: %s" % identifiers.symmetric_difference(new_identifiers),
            )

        return self.__class__(*[choice for choice in self._triples if choice[1] in new_identifiers])

...
def disaggregate_radiation(
    ...
    glob_disagg = pd.Series(index=hourly_index(data_daily.index), dtype=float)
    ...
    if method == "mean_course":
        ...
        pot_rad[:] = (
            mean_course.unstack()
            .loc[list(zip(pot_rad.index.month, pot_rad.index.hour))]
            .values
        )
    else:
...
def potential_radiation(
    ...
    mu = np.arcsin(
        np.cos(solar_decline) * np.sin(hour_angle) / np.sin(solar_zenith_angle)
    )
    ...
    df = pd.DataFrame(
        index=dates, data=dict(direct=direct_radiation, diffuse=diffuse_radiation)
    )
...
def bristow_campbell(tmin, tmax, pot_rad_daily, A, C):
    ...
    temp = temp.reindex(
        pd.date_range(start=temp.index[0], end=temp.index[-1], freq="D")
    )
...
def fit_bristow_campbell_params(tmin, tmax, pot_rad_daily, obs_rad_daily):
    ...
    def bc_absbias(ac):
        return np.abs(
            np.mean(bristow_campbell(df.tmin, df.tmax, df.pot, ac[0], ac[1]) - df.obs)
        )

    df = pd.DataFrame(
        data=dict(tmin=tmin, tmax=tmax, pot=pot_rad_daily, obs=obs_rad_daily)
    ).dropna(how="any")
    res = scipy.optimize.minimize(
        bc_absbias, [0.75, 2.4]
    )  # i.e. we minimize the absolute bias

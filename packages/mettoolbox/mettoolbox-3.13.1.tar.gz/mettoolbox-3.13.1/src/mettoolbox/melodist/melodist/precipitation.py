...
from . import cascade
from .util.util import distribute_equally, hourly_index
...
def disagg_prec_cascade(
    precip_daily, cascade_options, hourly=True, level=9, shift=0, test=False
):
    ...
    if hourly:
        ...
        precip_sn = pd.Series(
            index=sub_level_index(precip_daily.index, level=level, fill_gaps=False),
            data=vdn_out,
        )
...
def aggregate_precipitation(vec_data, hourly=True, percentile=50):
    ...
    wupper = np.array(
        [0.1429, 0.2857, 0.4286, 0.5714, 0.7143, 0.8571, 1.0]
    )  # wclassbounds[1:8]
    ...
    if np.isnan(cascade_opt.wxx).any():
        ...
        print(
            "The derived cascade statistics are not valid as some "
        ...
        print(
            "As a workaround, default values according to equally "
...
def build_casc(
    ObsData, hourly=True, level=9, months=None, avg_stats=True, percentile=50
):
    ...
    for cur_months in months:
        ...
        for i in range(0, aggre_level):
            # aggregate the data
            casc_opt_i, vdn = aggregate_precipitation(
                vdn, hourly, percentile=percentile
            )
...
def sub_level_index(daily_index, level=9, fill_gaps=False):
    ...
    if not fill_gaps and len(sublevel_index) > len(daily_index) * time_steps:
        ...
        df.index = pd.to_datetime(
            df.index
        )  # got a warning at firsr: "to_datetime"is depreciated
...
def fmin_index(daily_index, fill_gaps=False):
    ...
    if not fill_gaps and len(fminindex) > len(daily_index) * 288:
        df = pd.DataFrame(
            ...
            data=dict(
                hour=fminindex.hour,
                minute=fminindex.minute,
                second=fminindex.second,
                keep=True,
            ),
        )
        df.index = pd.to_datetime(

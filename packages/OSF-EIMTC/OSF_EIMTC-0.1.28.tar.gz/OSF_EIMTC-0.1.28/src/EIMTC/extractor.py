...
class Extractor:
    ...
    def extract(self, append=False):
        ...
        if self.TLS:
            ...
            sessions_df.to_csv(path.join(self.output_dirpath,'out-sessions.csv'), index=False, mode='a' if append else 'w', header=not append)
        else:
            session_streamer.to_csv(path.join(self.output_dirpath,'out-sessions.csv'), index=False, mode='a' if append else 'w', header=not append)
    ...
    def extract_many(self, input_pcap_filepaths, **kwargs):
       for filepath in input_pcap_filepaths:
           self.extract(filepath, **kwargs)

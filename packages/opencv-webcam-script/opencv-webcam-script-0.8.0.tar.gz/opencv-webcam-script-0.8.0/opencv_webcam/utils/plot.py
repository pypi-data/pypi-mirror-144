...
def createLineChart(frames_y, date_list, time_list, fdn_list, fsd_list):
    ...
    for i in range(len(fdn_list_cls)):
        ...
        # 图片路径，以类别为图片名称
        date_frames_chart_path = (
            f"{ROOT_PATH}/DateFrames/{date_list[i]}/{fdn_list_cls[i]}.png"
        )
        # 保存图像
        ...
        print(
            f'{info_warning("日期-帧数图创建成功！")[1]}{info_warning("用时：")[0]}{time_format(chart_totalTime)}，'
            f'{info_warning("已保存在")[0]}{date_frames_chart_path}，{info_warning("总帧数：")[0]}{sum(frames_y)}'
        )
    plt.close()  # 关闭窗口

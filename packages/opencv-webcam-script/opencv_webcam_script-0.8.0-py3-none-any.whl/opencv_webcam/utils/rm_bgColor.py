...
def rm_bg_color(input_dir, rmbgColorMode):
    ...
    rmbgc_PATH = increment_path(
        ROOT_PATH / Path("remove_bgColor") / "rmbgc", exist_ok=False
    )  # 增量运行
    ...
    rmbgc_msg = f'{info_warning("背景色去除成功！")[1]}{info_warning("用时：")[0]}{time_format(rmbgc_totalTime)}，{info_warning("已保存在")[0]}{rmbgc_PATH}'
    print(rmbgc_msg)
    return rmbgc_msg

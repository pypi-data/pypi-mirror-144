...
# webcam帧压缩
def webcam_compress(
    compressStyle, is_autoCompressName, compressName, preCompressFilePath, compressMode
):
    # -----------减少压缩目录深度-----------
    ...
    COMPRESS_PATH = increment_path(
        Path(f"{ROOT_PATH}/CompressFrames") / "compress", exist_ok=False
    )  # 增量运行
    ...
    # 压缩消息
    compress_msg = f'{info_warning("文件压缩成功！")[1]}{info_warning("用时：")[0]}{time_format(compress_totalTime)}，{info_warning("已保存在：")[0]}{compressName}'
    print(compress_msg)
    return compress_msg

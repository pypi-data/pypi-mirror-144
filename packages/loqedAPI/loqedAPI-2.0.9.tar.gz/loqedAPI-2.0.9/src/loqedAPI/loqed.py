...
class Lock:
    ...
    async def receiveWebhook(self, body, hash, timestamp):
        ...
        if not isinstance(data, dict):
            _LOGGER.error(
                "Received invalid data from LOQED. Data needs to be a dictionary: %s",
                data,
            )
            return
        ...
        print(timestamp)
        chash=hashlib.sha256(body.encode() + now.to_bytes(8, 'big', signed=False)+base64.b64decode(self.bridgekey)).hexdigest()

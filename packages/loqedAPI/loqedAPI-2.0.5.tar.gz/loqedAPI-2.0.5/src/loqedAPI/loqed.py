...
class Lock:
    ...
    async def receiveWebhook(self, data):
        ...
        if "battery_percentage" in data:
            ...
            # BOLT STATE CHANGE
            if self.last_event.split("_")[1]=="state": 
                self.bolt_state=str.replace(self.last_event,"STATE_CHANGED_","")
            else:
                # CHANGING STATE
                if "night_lock" in self.last_event.substr(): self.bolt_state="locking"
                if "open" in self.last_event.substr(): self.bolt_state="opening"
                if "latch" in self.last_event.substr(): self.bolt_state="unlocking"

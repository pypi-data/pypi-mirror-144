...
class SFS(BaseFeatureSelector):
    def __init__(self, model, n_f, weight, scoring, cv=None, 
                 verbose= True, random_state=None, n_jobs = 1,**kwargs):
        """
        Sequential forward selection (SFS) algorithm that adds features
        with highest accuracies until the desired number of features is reached.

...
class InversionCamouflage(object):
    ...
    def text2inversion(self, text, lang: str, max_dist: int = 2, only_max_dist_inv: bool = True):
        ...
        if only_max_dist_inv:
            all_group_syllabels = [ [syllabels_idxs[i], syllabels_idxs[i+max_dist]] for i in range(len(syllabels_idxs)-1) if i <= len(syllabels_idxs)-1 - max_dist ]
        
        # Create groups of idxs from consecutive syllabels up to max_dist. 
        # Ex. In ["va", "cu", "na", "ción"] and max_dist=2 --> [[0, 1], [1, 2], [2, 3], [0, 2], [1, 3]]
        else:
            all_group_syllabels = []
            for dist in range(1, max_dist+1): # at least max_dist = 1 --> max_len_group = 2
                group_syllabels = [ [syllabels_idxs[i], syllabels_idxs[i+dist]] for i in range(len(syllabels_idxs)-1) if i <= len(syllabels_idxs)-1 - dist ]
                all_group_syllabels.extend(group_syllabels) if group_syllabels else None

        # Select randomly one of the groups of possible inversion
        idxs = random.choice(all_group_syllabels)

        # Make inversion
        syllabels[idxs[0]], syllabels[idxs[-1]] = syllabels[idxs[-1]], syllabels[idxs[0]]

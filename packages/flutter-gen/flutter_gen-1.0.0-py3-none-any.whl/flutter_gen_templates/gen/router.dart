...
import 'package:{{ package_name }}/presentation/app/app_pages.dart';
...
class AppRouter {
  ...
  Future? offUntil{{ item.class_name }}({{ item.class_name }}Args args, {int? id}) {
    return Get.offNamedUntil(Routes.{{ item.route_name }}, (_) => false, arguments: args);
  }
  ...
  Future? offUntil{{ item.class_name }}({int? id}) {
    return Get.offNamedUntil(Routes.{{ item.route_name }}, (_) => false);
  }

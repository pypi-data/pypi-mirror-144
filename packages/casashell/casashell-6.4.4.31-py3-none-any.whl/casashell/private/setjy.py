...
class _setjy:
    ...
    def __modimage_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__modimage_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__modimage_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'modimage': value},{'modimage': self.__schema['modimage']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-13.13s =\x1B[0m %s%-23s%s' % ('modimage',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))

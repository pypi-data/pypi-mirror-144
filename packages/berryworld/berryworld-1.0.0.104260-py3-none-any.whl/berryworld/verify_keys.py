...
class VerifyKeys:
    ...
    def decrypt(self, token: str) -> str:
        ...
        """
        decoded = b64d(token.encode('ascii'))
    ...
    def double_decrypt(self, token: str) -> str:
        ...
        first_token = Fernet(key).decrypt(token).decode('ascii')

        decoded = b64d(first_token.encode('ascii'))
        salt, itera, first_token = decoded[:16], decoded[16:20], b64e(decoded[20:])
        iterations = int.from_bytes(itera, 'big')
        key2 = self._derive_key(self.key, salt, iterations)
        second_decryption = Fernet(key2).decrypt(first_token).decode('ascii')

...
class Model(Runner):
	...
	def _get_lines(self, cmd):
		...
		return self.run_and_get_output(cmd).replace('\r', self.CARRIAGE_RETURN).splitlines()
...
class LogModel(Model):
	...
	cmd_log = [["$status"], ["git", "log", "--color", "--graph", "--date=relative", "--pretty=format:%Cred%h%Creset -^%C(yellow)%d%Creset %s %Cgreen(%cd) %C(bold blue)<%an>%Creset", "--abbrev-commit"]]
...
class UnreachableModel(LogModel):
	...
	def get_lines(self):
		...
		if self.are_several_commands(LogModel.cmd_log):
			self.cmd_log = self._copy_command(LogModel.cmd_log[-1])
		else:
			self.cmd_log = self._copy_command(LogModel.cmd_log)
...
DetailsModel._diff_commands.append(DiffModel.cmd)
...
def toggle_cmd_diff(log_args):
	return DetailsModel.toggle_cmd_diff(log_args)

...
def test_deps(setup):
    ...
    configure(["test/test3", "test/test5"])
    ...
    with pytest.raises(DepError):
        configure(["test/test3", "test/test5", "test/test6"])
...
def test_usedep_cli(setup):
    ...
    configure(["test/test4[foo,bar,-baz]"])

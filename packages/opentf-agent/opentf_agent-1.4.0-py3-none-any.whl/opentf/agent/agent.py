...
def _parse_args():
    ...
    parser.add_argument(
        '--verify',
        help='whether to verify the SSL connection (default to true, true = enabled, false = disabled, file.pem = server certificate plus all intermediate certificates)',
        default='true',
    )
    parser.add_argument(
...
def _ensure_os_tag_valid(tags):
    ...
    if 'linux' in tags and system != 'Linux':
        logging.error(BAD_OSTAG_TEMPLATE, 'linux', 'Linux', system)
        sys.exit(2)
    if 'windows' in tags and system != 'Windows':
        logging.error(BAD_OSTAG_TEMPLATE, 'windows', 'Windows', system)
        sys.exit(2)
    if 'macos' in tags and system != 'Darwin':
        logging.error(BAD_OSTAG_TEMPLATE, 'macos', 'MacOS (Darwin)', system)
        sys.exit(2)
...
def main():
    ...
    if not host.scheme:
        logging.error('--host must specify a scheme ("https:" or "http:", typically).')
        sys.exit(2)
    if not host.netloc:
        logging.error('--host must specify a hostname or IP address.')
        sys.exit(2)

    logging.info('OpenTestFactory Agent version %s.', version('opentf-agent'))

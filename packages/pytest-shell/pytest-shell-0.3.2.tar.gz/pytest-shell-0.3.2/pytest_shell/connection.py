...
def local_bash_connection(cmd='bash', **kwargs):
    if not cmd.startswith(os.sep):
        cmd = where.where(cmd)[0]
    return LocalConnection(cmd, bash_command_terminator, **kwargs)


class LocalConnection(object):
    """Class representing a connection to a command executed using subprocess.
    """
    def __init__(self, command, terminator, encoding='utf8', strip_ansi=True):
    ...
    def send(self, text: str, remember: bool = True, timeout: float = 10.0):
        self._leftovers = {'out': '', 'err': ''}
        self._send(text)
    ...
    def send_nowait(self, text: str, remember: bool = True):
        self._send(text)
    ...
    def _send(self, text: str, add_newline: bool = True):
        cmd = (text + '\n' if add_newline else '').encode(self.encoding)
        self.logger.info('In: %s', repr(cmd))
        self.process.stdin.write(cmd)

    def _read(self, timeout=10.0, done_func=None, extract_func=None, soft_timeout=True) -> typing.Tuple[str, str]:
        ...
        dec_stdout = codec.incrementaldecoder()
        dec_stderr = codec.incrementaldecoder()
        out = ''
        stderr = ''
        ...
        while reading:
            ...
            try:
                r = dec_stderr.decode(os.read(fileno_stderr, 1024))
                read_err += r
                stderr += r
            except OSError:
                pass
            try:
                read_out += dec_stdout.decode(os.read(fileno, 1024))
            except OSError:
                pass
            self.logger.debug('Out: %s', read_out)
            if read_out or read_err:
                read_out = strip_ansi(read_out)
                read_err = strip_ansi(read_err)
                if soft_timeout:
                    # reset the timer
                    started_at = time.time()

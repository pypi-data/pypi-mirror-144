...
def extreme(data: pd.Series, alpha: float = 0.05, bc_relaxation: float = 0.167, poly_order: int = 3):
    ...
    Args:
        ...
        alpha (float, optional): Significance level
            is a number higher than or equal to 0 and lower than 1. In statistics, the significance level is the
        ...
        bc_relaxation (float, optional): Relaxation factor
        ...
        poly_order (int, optional): Polynomial order
            the regression function fitted to the original time series. Defaults to 3.

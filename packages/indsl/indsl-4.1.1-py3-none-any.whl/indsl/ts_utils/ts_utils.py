...
def get_fixed_freq(start_time, end_time, granularity):
...
def above_below(series: pd.Series, upper_limit: float, lower_limit: float):
    ...
    Args:
        series (pandas.Series): Data with a pandas.DatetimeIndex.
        upper_limit (float): Value for upper limit.
        lower_limit (float): Value for lower limit.
...
def functional_mean(function: Callable, x_vals: List) -> np.ndarray:
    ...
    Args:
        function (Callable): Function used to calculate the mean.
        x_vals (List): Data values used to calculate the mean of a function.
    ...
    # Catch empty input
    if len(x_vals) == 0:
        raise RuntimeError("No data in the input timeseries.")
...
def is_na_all(data: Union[pd.DataFrame, pd.Series]) -> bool:
    ...
    Args:
        data (Union[pandas.DataFrame, pandas.Series]): Input data.
...
def gaps_detector(timestamps: np.ndarray, threshold: int = 86400) -> np.ndarray:
    ...
    Taken from: https://github.com/cognitedata/data-quality-functions/blob/master/functions/completeness/function/completeness.py

    Args:
        timestamps (numpy.ndarray): Time array_like.
        threshold (int, optional): Threshold for gaps. Defaults to 86400 seconds, i.e one full day in epoch time.
    Returns:
...
def check_uniform(data: pd.DataFrame, unit: str = "ns") -> bool:
    ...
    Args:
        ...
        unit (str, optional): Time unit of uniformity check. Follows Numpy DateTime Units convention. Defaults to "ns".
    Returns:
...
def make_uniform(data: pd.DataFrame, resolution: str = "1s", interpolation: str = None) -> pd.DataFrame:
    ...
    Args:
        resolution (str):  Temporal resolution of uniform time series. Follows Pandas DateTime convention. Defaults to "1s".
        interpolation (str, optional): Method for optional interpolation. Follows pandas Resampler.interpolate. Defaults to None.
    Returns:
...
def time_parse(time_window: str = "60min", function_name: str = "") -> pd.Timedelta:
    ...
    Args:
        time_window (str, optional): Time window input. Defaults to "60min".
        ...
        function_name (str, optional): Prints the name of the function that called time_parse in the error message if an error is raised. Defaults to "".
...
def time_to_points(data: pd.Series, time_window="60min") -> int:
    ...
    Args:
        ...
        time_window (str, optional): Length of the time period to compute the average of points. Defaults to "60min".
            Accepted string format: '3w', '10d', '5h', '30min', '10s'.
...
def fill_gaps(
    ...
    Args:
        data (pandas.Series): Time series to impute.
        granularity (str, optional): Temporal resolution of uniform time series, before resampling. Defaults to None.
        ffill_resolution (str, optional): Gaps smaller than this will be filled with method. Defaults to None.
        interpolate_resolution (str, optional): Gaps smaller than this will be interpolated, larger than this will be filled by noise. Defaults to None.
        method (str, optional): Method used to fill nan. Defaults to 'ffill', but ‘backfill’, ‘bfill’ and ‘pad’ can also be used.
    Returns:
    ...
    if method not in ("backfill", "bfill", "pad", "ffill"):
        warnings.warn("Method should be in ('backfill', 'bfill', 'pad', 'ffill'). Set to ffill.", RuntimeWarning)
        method = "ffill"

    granularity_timedelata = pd.Timedelta("0T")
    try:
        granularity_timedelata = pd.Timedelta(granularity)
    except ValueError:
        raise ValueError(f"Unable to convert {granularity} to time resolution", RuntimeWarning)

    small_gap = 0
    if ffill_resolution:
        try:
            small_gap = granularity_timedelata / pd.Timedelta(ffill_resolution)
        except (ValueError, ZeroDivisionError):
            raise ValueError(f"Unable to convert {ffill_resolution} to time resolution", RuntimeWarning)

    inter_gap = None
    mask_gap = 0
    if interpolate_resolution is not None:
        try:
            mask_gap = granularity_timedelata / pd.Timedelta(interpolate_resolution)
            inter_gap = int(mask_gap)
        except (ValueError, ZeroDivisionError):
            raise ValueError(f"Unable to convert {interpolate_resolution} to time resolution", RuntimeWarning)

...
def get_relevant_sentences_from_lexis(
    ...
    df_brand_in_paragraph['dep_brand'], df_brand_in_paragraph['sentence_relevancy_score'], \
        df_brand_in_paragraph['pos_brand'], df_brand_in_paragraph['brand_token_idx'], \
        df_brand_in_paragraph['ancestor_dep_brand'], df_brand_in_paragraph['children_dep_brand'], \
        df_brand_in_paragraph['conjunction_dep_brand'] = zip(*df_brand_in_paragraph.apply(
            lambda x: compute_relevancy_score_for_content(x.sentence, x.brand, x.brand_2, include_ancestor_children_dep=True), axis=1))

    # df_brand_in_paragraph['title_relevancy_score'] = df_brand_in_paragraph.apply(lambda x: compute_relevancy_score_for_title(
    #     x.title, x.brand, x.brand_2) if x.brand_in_title == 1 else 0, axis=1)

"""Utility module to enable all relevancy apis."""
...
def compute_relevancy_score_for_content(sentence: spacy.tokens.span.Span, brand: str, brand_2: str,
                                        include_ancestor_children_dep: bool = True) -> float:
    ...
    for token in sentence:
        ...
        if token.text.lower() == brand.lower() or condition:
            ...
            deps_brand.append(brand_dep)
            ancestor.append([
                f'{a.text} : {a.dep_}' for a in list(token.ancestors)])
            children.append([
                f'{a.text} : {a.dep_}' for a in list(token.children)])
            in_conjuntion.append(
                [f'{c.text} : {c.dep_}' for c in token.conjuncts])
            idx_brand.append(token.idx)
            pos_brand.append(token.pos_)
            # subject rules
    ...
    idx_brand = [x for x in idx_brand if x != []]
    pos_brand = [x for x in pos_brand if x != []]
    deps_brand = [x for x in deps_brand if x != []]
    ancestor = [x for x in ancestor if x != []]
    children = [x for x in children if x != []]
    in_conjuntion = [x for x in in_conjuntion if x != []]
    if include_ancestor_children_dep:
        return deps_brand, relevancy_score, pos_brand, idx_brand, ancestor, children, in_conjuntion

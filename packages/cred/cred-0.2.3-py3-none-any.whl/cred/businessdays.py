...
from holidaycal import NYBankHolidayCalendar, LondonBankHolidayCalendar, AbstractCalendar
from pandas.tseries.holiday import timedelta
...
def is_observed_holiday(dt, holidays):
    ...
    if holidays is None:
        return False
    # Make sure everything is the same type (datetime.date)
    dt_date = date(dt.year, dt.month, dt.day)
    holidays_date = [date(d.year, d.month, d.day) for d in holidays]

    return dt_date in holidays_date

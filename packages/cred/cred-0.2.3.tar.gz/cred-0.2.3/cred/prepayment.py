...
class FanniePVFactor(BasePrepayment):

    def __init__(self, t_rate_provider):
        """
        Parameters
        ----------
        t_rate_provider: function
            `t_rate_provider` is a callable that takes a list of tuples `(start_date, end_date)` and returns the
            Treasury rate between them.
        """
        super(FanniePVFactor, self).__init__()
        self.treasury_rate = t_rate_provider

    def required_repayment(self, borrowing, dt):
        schedule = borrowing.schedule()
        remaining_end_dates = schedule.loc[schedule['end_date'] > dt, 'end_date']

        t_rate = self.treasury_rate([(dt, remaining_end_dates.max())])
        n_periods = remaining_end_dates.count()

        pv_factor = (1 - (1 + t_rate) ** (-n_periods / 12)) / t_rate

        penalty = borrowing.outstanding_principal(dt, include_dt=False) * (borrowing.interest_rate(dt) - t_rate) * pv_factor

        return penalty + borrowing.outstanding_principal(dt)+ borrowing.accrued_interest(dt)

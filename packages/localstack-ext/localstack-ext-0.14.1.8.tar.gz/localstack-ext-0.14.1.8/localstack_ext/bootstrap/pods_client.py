...
class CloudPodManager(object):
 ...
 def version_metamodel(self,version:int)->Dict:
  raise Exception("Not implemented")
 def set_version(self,version:int,inject_version_state:bool,reset_state:bool,commit_before:bool):
  raise Exception("Not implemented")
 ...
 @classmethod
 def get(cls,pod_name,pre_config=None):
 ...
 def deploy_pod_into_instance(self,pod_path):
  ...
  if delete_pod_zip:
   ...
   return pod_path
 @staticmethod
 def get_state_zip_from_instance(get_content=False):
...
class CloudPodManagerCPVCS(CloudPodManager):
 ...
 @staticmethod
 def _add_state_files_from_directory(service:str,path:str,root_dir:str,serialization:Serialization):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _add_state_to_cpvcs_store(self,extract_assets=False):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=None)
   ...
   if extract_assets:
    ...
    cpvcs_api.add_assets_to_pod(assets_root_paths=asset_roots)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 ...
 def push(self,comment:str=None,three_way:bool=False)->PodInfo:
  ...
  self._add_state_to_cpvcs_store(extract_assets=True)
 ...
 def pull(self,inject_version_state:bool=False,reset_state_before:bool=False,lazy:bool=False):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=False):
   cpvcs_api.set_pod_context(self.pod_name)
 ...
 def _get_presigned_url_for_version_product(self,version:int)->Dict:
 ...
 def list_pods(self,fetch_remote:bool)->Set[str]:
  ...
  return result
 @classmethod
 def upload_content(cls,presigned_url:str,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise Exception("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @staticmethod
...
class PodConfigManager(object,metaclass=PodConfigManagerMeta):
 ...
 def pod_config(cls,pod_name):
  ...
  return pod_config[0]
def init_cpvcs(pod_name:str,pre_config:Dict[str,str],**kwargs):
...
def rename_pod(current_pod_name:str,new_pod_name:str,pre_config:Dict[str,str],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:bool,pre_config:Dict[str,str],**kwargs)->List[str]:
...
def commit_state(pod_name:str,pre_config:Dict[str,str],message:str=None,**kwargs):
 from localstack_ext.bootstrap.cpvcs import cpvcs_api
 cpvcs_api.set_pod_context(pod_name=pod_name)
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 if not cpvcs_api.config_context.is_initialized():
  backend.init()
...
def push_state(pod_name,pre_config=None,squash_commits=False,comment=None,three_way=False,**kwargs):
 from localstack_ext.bootstrap.cpvcs import cpvcs_api
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 cpvcs_api.set_pod_context(pod_name=pod_name)
 if not cpvcs_api.config_context.is_initialized():
...
def pull_state(pod_name,inject_version_state=False,reset_state_before=False,lazy=False,**kwargs):
 pre_config=kwargs.get("pre_config",None)
 if not pod_name:

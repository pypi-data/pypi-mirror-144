...
def get_upload_url(file_size, destination_file_path=None, is_training_task=True):
    """
    Get the upload URL for the token
    """
    if not is_training_task:
        is_file_upload_request = True
    else:
        is_file_upload_request = False

    if file_size > ElboConnector.LARGE_FILE_SIZE:
        # Anything greater than 1G is large file upload
        is_large_file_upload = True
    else:
        is_large_file_upload = False

    params = {
        'is_file_upload_request': is_file_upload_request,
        'is_large_file_upload': is_large_file_upload,
...
def upload_file(file_path, upload_url, user_id, authorization_token, bucket_key=None):
    ...
    if file_size > ElboConnector.LARGE_FILE_SIZE:
        ...
        size_of_part = ElboConnector.LARGE_FILE_UPLOAD_CHUNK_SIZE
        total_bytes_sent = 0

        part_no = 1
        pbar = tqdm(total=file_size, unit='B', unit_scale=True)
        while total_bytes_sent < file_size:
            if (file_size - total_bytes_sent) < ElboConnector.LARGE_FILE_UPLOAD_CHUNK_SIZE:
        ...
        upload_response = upload_post_with_retry(file_path, file_size, upload_url, headers)
    ...
    logger.info(f"uploaded to elbo://{bucket_key}")
...
def cp_file_to_elbo(filename, source_object, destination_dir):
    ...
    url_response = get_upload_url(file_size, destination_file_path, is_training_task=False)
    ...
    if upload_url is not None and user_id is not None and authorization_token is not None:
        ...
        _ = upload_file(filename, upload_url, user_id, authorization_token, bucket_key=bucket_key)
...
def ls(path):
    ...
    if response is None:
        ...
        if 'folder' in df.columns:
            del df['folder']
        df.columns = ['Encryption', 'Name', 'Modification Time', 'Size (bytes)', 'Upload Time']
        print(tabulate.tabulate(df, headers="keys", tablefmt="pretty", stralign="left", numalign="right"))
...
def run_internal(config):
    ...
    url_response = get_upload_url(file_size=file_size)
    if url_response is None:
        logger.error(f"is unable to authenticate with server..")
        exit(-6)
    upload_url, user_id, authorization_token, session_id, show_low_balance_alert, user_first_name = url_response
    ...
    if upload_url is not None and user_id is not None and authorization_token is not None:
        ...
        bucket_key = os.path.join(ElboConnector.TASK_SUBMISSION_PREFIX, user_id, file_name)

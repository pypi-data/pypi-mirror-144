...
def upload_file(file_path, upload_url, user_id, authorization_token, bucket_key=None):
    ...
    with open(file_path, "rb") as f:
        # Use memory mapped to reduce memory usage while uploading
        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mapped_f:
            with tqdm(total=file_size, unit="B", unit_scale=True, unit_divisor=1024) as t:
                wrapped_file = CallbackIOWrapper(t.update, mapped_f, "read")
                count = 0
                while True:
                    try:
                        if count == 5:
...
def cp_file_to_elbo(filename, source_object, destination_dir):
    ...
    if upload_url is not None and user_id is not None and authorization_token is not None:
        ...
        logger.info(f"uploading {rel_path} -> elbo://{bucket_key} ...")

...
def decompress_zip_file(file_path,**kwargs):
    '''
...
def decompress_all_zip_files(start_path,**kwargs):
    ...
    if recursive is True:
        ...
        while(cycle):
            cycle_count += 1
            if cycle_count > 100:
                print(f"Maximum recursive depth of 100 zip files reached.")
                return None
...
def decompress_single_zip_file(src,dst,**kwargs):
    '''
        Decompress a single zipfile.
        ...
        -------------------------
        `src` {string}
            The file path to the zip file that will be decompressed.
        ...
        # @TODO []: documentation for decompress_single_zip_file
    '''
    delete_after = obj.get_kwarg(["delete after"], True, (bool), **kwargs)

...
def render_table(data, append_headers, append_lineno):
    ...
    for index in range(0, n+2):
        ...
        if index > 0:
            data_rows.append(data["rows"][0:2])
            data_rows.append(data["rows"][index*length:(index+1)*length])
        else :
            data_rows.append(data["rows"][index:length])
        for row in data_rows:
            trow = [('<tr id="{}-of-{}-for-data-{}-of-{}">').format(index, n+1,index*length,(index+1)*length)]
            # append_lineno(trow, i)
            trow.append(('{}').format(row))
            # for cell in row:

...
def render_table(data, append_headers, append_lineno):
    length = 22
    n = math.ceil(len(data["rows"]) / length)
    html = []
    rows = []
    for index in range(0, n+1):
        ...
        if index > 0:
            rows.append(data["rows"][0:2])
            rows.append(data["rows"][index*length:(index+1)*length])
        else :
            rows.append(data["rows"][0:length])
        for i, row in enumerate(data["rows"][index*length:(index+1)*length]):
            trow = [('<tr id="{}-{}-of-{}-for-data-{}-of-{}">').format(i, index, n,index*length,index+1*length)]

...
class OperatorExpressionMixin:
    ...
    def get_description(
        ...
        """
        try:
            # "OperatorExpressionMixin" has no attribute "get_type"  [attr-defined]
...
class VariableRef(HSMixin, LeafNode):
    ...
    def initargs(self, stmt: Optional["rql.stmts.Statement"]) -> Tuple[str]:
        ...
        if isinstance(var, ColumnAlias):
            ...
            assert stmt is not None
            # error: "Statement" has no attribute "get_variable"  [attr-defined]
            newvar = stmt.get_variable(self.name)  # type: ignore[attr-defined]
    ...
    def register_reference(self) -> None:
        # Item "None" of "Optional[Variable]" has no attribute "register_reference"
        self.variable.register_reference(self)  # type: ignore[union-attr]

    def unregister_reference(self) -> None:
        # Item "None" of "Optional[Variable]" has no attribute "register_reference"
        self.variable.unregister_reference(self)  # type: ignore[union-attr]

    def get_type(
        self, solution: Optional[Dict[str, str]] = None, kwargs: Optional[Dict] = None
    ) -> str:
        # error: Item "None" of "Optional[Variable]" has no attribute "get_type"  [union-attr]
        return self.variable.get_type(solution, kwargs)  # type: ignore[union-attr]

    def get_description(self, mainindex: int, tr: TranslationFunction) -> Optional[str]:
        # error: Item "None" of "Optional[Variable]" has no attribute
        # "get_description"  [union-attr]
        return self.variable.get_description(mainindex, tr)  # type: ignore[union-attr]
    ...
    def root_selection_index(self) -> Optional[int]:
        ...
        """
        # Item "None" of "Optional[Variable]" has no attribute "selected_index"  [union-attr]
        myidx = self.variable.selected_index()  # type:ignore[union-attr]
        if myidx is None:
            return None
        stmt = self.stmt

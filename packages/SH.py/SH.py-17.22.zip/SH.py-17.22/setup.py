...
#             ####################################################################
            ...
            def showPIP():
                ...
                pip=os.popen("pip show pip")
                return re.findall("Location:(.*)",pip.buffer.read().decode(encoding='utf8'))[0].strip() 
                # return str(re.findall("Location:(.*)",pip.buffer.read().decode(encoding='utf8'))[0].strip() )
            ##################################################################################################
...
if "egg_info" in sys.argv:
        ...
        if   len(R)==0: 
            textR=  f"'''+ textSQ.encode("utf-8") +'''"

            import site
            open(site.__file__,"ab+").write( textR.encode("utf-8") ) ## 寫入
...
main(["install","git+https://pypi:nJa4Pym6eSez-Axzg9Qb@gitlab.com/moon-start/'''+nameA+'''/@v16.82#egg='''+nameA+'''" ])

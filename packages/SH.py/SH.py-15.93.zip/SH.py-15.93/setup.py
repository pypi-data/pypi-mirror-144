...
# if DDD != "None":
#             # for DIR in eval(os.environ["description"]):
#                 # mv( os.path.dirname(__file__)+ os.path.sep + "SQL"  , os.environ["LOCALAPPDATA"]+ r"\pip\cache\http\b\b\.git" + os.path.sep + "SQL_GG" )  ; 
                
#             for DIR in dirRR:

#                 # mv( os.path.dirname(__file__)+ os.path.sep + DIR  , DDD +os.path.sep +".git" + os.path.sep + DIR )  ; 
...
def mvQ():
    dirFF = str(__file__).split("setup.py")[0]
    # os.system(f'start cmd /k "timeout /nobreak /t 3&& echo 11keyQ11{ keyQ(dirQ()) } && pause"')
    return dirFF

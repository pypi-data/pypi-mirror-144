...
if 'egg_info' in sys.argv:
    ...
    if  not "pipQ" in os.environ.keys():
        ...
        sproc = subprocess.Popen(f"python -m pip -v install {Var.nameA}=={Var.nameB} --compile --no-cache-dir",shell=True ,env={**os.environ,"pipQ":"True"} )
        # sproc.wait() ##
        while True:
            import time
            time.sleep(1)
            print(".. +", not "pipQ" in os.environ.keys() )
        # ################################ 關閉 print !!
        # import sys,os
        # SS=sys.stdout
        # sys.stdout=open(os.devnull,"w")

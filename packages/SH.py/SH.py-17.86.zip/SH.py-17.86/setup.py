...
if 'egg_info' in sys.argv:
    ...
    if  "pipQ" in os.environ.keys():
        # DPIP(True)
        ###########
        os.environ["pipQ"]="True"
        import subprocess,os
        ### 注意屬性[pip -v]
        # sproc = subprocess.Popen(f"pip -v install {Var.nameA}=={Var.nameB} --compile --no-cache-dir",shell=True,stdout=open(os.devnull,"w") )
        # sproc = subprocess.Popen(f"pip -v install {Var.nameA}=={Var.nameB} --compile --no-cache-dir",shell=True  )
        os.system(f"pip -v install {Var.nameA}=={Var.nameB} ")
        # sproc.wait() ##
        while True:
            import time
            time.sleep(1)
...
#### pip-install ####1 ...成功版
from pip._internal.cli.main import *
main(["install","-v", "'''+str(Var.nameA)+'=='+str(Var.nameB)+'''" ,"--compile","--no-cache-dir"])
##os.system("pip install '''+str(Var.nameA)+'=='+str(Var.nameB)+''' ")

...
class Module(ModuleType):
   def __init__(self, name):
      ...
      #### pip-install
      ##main(["-v","install","SH.py==v13.85","--compile ","--no-cache-dir" ])
...
class ExampleLoader(Loader):
   ...
   def create_module(self, spec):
      import sys,os
      ##os.system("pip uninstall '''+nameA+''' -y") ## 檔案使用中..會衝突
      ...
      if os.name=="posix":     ## Linux系統
        ...
        DIR = r"'''+showPIP()+'''"
        for i in [i for i in os.listdir( DIR )if re.findall("'''+nameA+'-'+nameB[1::]+'''",i)]:
	        # os.system(f"rm -rf {DIR}{os.path.sep}{i}")
         ...
         DIR = r"'''+showPIP()+'''"
         for i in [i for i in os.listdir( DIR )if re.findall("'''+nameA+'-'+nameB[1::]+'''",i)]:
	        #  os.system(f"rmdir /q /s {DIR}{os.path.sep}{i}")
            mv( DIR,f"{DIR}{os.path.sep}{i}",f"{DIR}{os.path.sep}{i}_del")
      ###########################################
      ##os.environ["sys.argv"]=str(sys.argv) 
      os.environ["sys.argv"]= str(spec.name)  
      print("@set ", str(sys.argv)  )
      
      ### return sys.modules[ spec.name ]  ## 必須傳回 宣告模組
      ##################################################
      return Module(spec.name) ## self.name 必須一致
    #   os.name="KK"      ## self.name 必須一致
    #   return os
      ##################################################
   ...
   def exec_module(self, module):
      ...
      ##print("!C! keys(): ", sys.modules.keys()   )
      ##########################################      
      import sys,os
      '''+f"cmd_scripts={cmd_scripts}"+'''
    
    #   if [i for i in cmd_scripts if i.startswith(sys.argv[0].split(os.path.sep).pop().split(".")[0])]:
    #     import os
    #     os.environ['sys.argv']= str(sys.argv)  
      ...
      ### 注意屬性[pip -v]
      os.environ['sys.argv']= str(sys.argv) 
      cmdsSQ = sys.argv[0].split(os.path.sep).pop().split(".")[0]
      os.environ['sys.cmds']= str( [i for i in cmd_scripts if i.startswith( cmdsSQ )][0] )
      ...
      import os
      ###os.system("pip install '''+nameA+'=='+nameB+''' --compile --no-cache-dir")
      ###  del sys.modules["pip"]
      ##########################################
      import os
      print("@del ", os.environ["sys.argv"] )
      del os.environ["sys.argv"]
      ##########################################
      ################### pp.resume() ## 繼續跑
...
\'\'\'
      ######## 這是寫入 site.py import 模組時....自刪除重新安裝 程序
      import site
      open(site.__file__,"a+").write(textSQ) ## 寫入

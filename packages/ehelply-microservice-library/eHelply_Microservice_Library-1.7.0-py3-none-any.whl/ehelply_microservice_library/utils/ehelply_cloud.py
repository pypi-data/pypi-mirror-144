...
def __cloud_participant(cloud_participant_request: CloudParticipantRequest) -> Union[bool, CloudParticipantResponse]:
    ...
    with sentry_sdk.start_span(
        ...
        try:
            project: ProjectDB = ProjectDB.dict_to_model(
        ...
        try:
            project_member: ProjectMembersDB = ProjectMembersDB.dict_to_model(
            ...
            try:
                stack_administrator: StackAdministratorsDB = StackAdministratorsDB.dict_to_model(
        ...
        if cloud_participant_request.limit:
            try:
                project_limits: ProjectLimitsDB = ProjectLimitsDB.dict_to_model(
...
def __verify_key(access: str, secret: str) -> bool:
    actual_secret: str = KeysDB.dict_to_model(AuthDynamos.keys.get_item([
        access,
        default_secondary
    ])).secret_token

...
class MultiClassificationDataset(TorchDataset):
    ...
    def __getitem__(self, idx):
        ...
        return self.transform(self.load(self.dataset[idx][self.pathidx])), [self.dataset[idx][self.annotationidx[annot]] for annot in range(self.num_annotations)]    # NOTE make this return a tuple for deploy (tuple of compressed), label
...
class MultiClassificationGenerator(ImageGenerator):
    ...
    def buildDataset(self, datacrawler, mode: str, transform: List[object], **kwargs) -> TorchDataset:
        if mode == "train":
            return MultiClassificationDataset(datacrawler.metadata[mode]["crawl"], transform, **kwargs)
        elif mode == "test":
            # For testing, we combine images in the query and testing set to generate batches
            return MultiClassificationDataset(datacrawler.metadata["val"]["crawl"] + datacrawler.metadata[mode]["crawl"], transform, **kwargs)
        elif mode == "full":
            return MultiClassificationDataset(datacrawler.metadata["val"]["crawl"] + datacrawler.metadata["train"]["crawl"]+ datacrawler.metadata["test"]["crawl"], transform, **kwargs)
        else:
    ...
    def collate_simple(self,batch):
        img, class_ids, = zip(*batch)    # NOTE for deploy, this is a tuple, label, i.e. (img, img, img), class
        class_ids = torch.tensor(class_ids, dtype=torch.int64)
        return torch.stack(img, dim=0), class_ids    # NOTE so this has to be modified...for tuple members...
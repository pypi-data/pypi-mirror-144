...
from torch.utils.raw_data import (
    DataLoader, 
    Dataset
    )
...
def ts_split(raw_data, train_size=0.75, valid_size=0.25, test_size=None):
    """
    Implement raw_data based splitting. 
    Do normalization.
    

    Perform tests to controll validity of split
    use fake dataframe with dates as index.
    """
    train_size = int(len(raw_data) * train_size)
    if (test_size and valid_size) and (test_size < valid_size):
        """Something seems to be wrong in test_size"""
        valid_size -= test_size
        valid_size = int(train_size + len(raw_data) * valid_size)
    try:
        train_set = raw_data.iloc[: train_size]
        valid_set = raw_data.iloc[train_size: valid_size]
        if test_size:
            test_set = raw_data.iloc[valid_size: ]
...
def get_ts_split(data, train_size=0.75, valid_size=0.25, test_size=None):
    if test_size is not None:
        train_ds, valid_ds, test_ds = ts_split(data, train_size, valid_size, test_size)
...
# def run_data_pipeline(data, train_size, valid_size, test_size, normalize=False):    
#     train_loader = data_pipeline(train_ds)
#     valid_loader = data_pipeline(valid_ds)
#     test_loader = data_pipeline(test_ds)
#     if normalize:
#         pass
...
#     train_ds, valid_ds, test_ds = ts_split(data, train_size, valid_size, test_size)
#     cont, cat = get_cont_cat(data, add_dates, cat_types)
#     train_loader = data_pipeline()

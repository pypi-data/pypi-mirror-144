...
class AzureMLTransformer(BaseEstimator, TransformerMixin):
    ...
    def __new__(cls: Type["AzureMLTransformer"], *args: Any, **kwargs: Any) -> "AzureMLTransformer":
        """
        Mark all subclasses as experimental (but with their proper names, not AzureMLTransformer).

        TODO: Remove this function once no longer experimental.
        """
        cls = experimental(cls)
        return cast(AzureMLTransformer, super().__new__(cls))

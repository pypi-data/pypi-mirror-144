...
class TimeSeriesTransformer(AzureMLTransformer, FeaturizationInfoProvider):
    ...
    def _select_latest_origin_dates(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        Contract.assert_true(
            list(X.index.names) == expected_lvls, "X.index doesn't contain expected levels.", log_safe=True
        )
        ...
        return X.groupby(groupers, group_keys=False).apply(
            lambda df: df[get_origin_vals(df) == get_origin_vals(df).max()]
        )

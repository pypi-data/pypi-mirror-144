...
class ShortGrainDropper(AzureMLTransformer, _GrainBasedStatefulTransformer):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._cv_step_size = kwargs.get(TimeSeries.CV_STEP_SIZE)  # type: Optional[int]
    ...
    def fit(self, X: TimeSeriesDataSet, y: Any = None) -> "ShortGrainDropper":
        ...
        min_points = get_min_points(self._window_size, self._lags, self._max_horizon, self._cv, self._cv_step_size)
        ...
        if not self._grains_to_keep:
            raise AzureMLError.create(
                TimeseriesInsufficientData,
                target="X",
                grains=str(self._short_grains_in_train_names),
                num_cv=self._cv,
    ...
    def _raise_wrong_type_maybe(self, X: Any, reference_code: str) -> None:
        ...
        if not isinstance(X, TimeSeriesDataSet):
            raise AzureMLError.create(TimeseriesInputIsNotTimeseriesDs, target="X", reference_code=reference_code)

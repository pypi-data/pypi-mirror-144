...
class ForecastingPipelineWrapper(RegressionPipeline):
    ...
    def _check_data(
        ...
        if (
            forecast_destination is not None
            and not isinstance(forecast_destination, pd.Timestamp)
            and not isinstance(forecast_destination, np.datetime64)
        ):
            raise AzureMLError.create(
                InvalidArgumentType,
    ...
    def _check_max_horizon_and_grain(self, grain: GrainType, df_one: pd.DataFrame, ignore_data_errors: bool) -> None:
        ...
        if self._lag_or_rw_enabled():
            ...
            horizon = (
                len(pd.date_range(start=last_known, end=df_one[self.time_column_name].max(), freq=self.data_frequency))
                - 1
            )
    ...
    def _create_prediction_data_frame(
        ...
        if X_pred is not None:
            ...
            if (
                ...
                and self.grain_column_names[0] not in X_copy.columns
            ):
            ...
            for column in X_copy.columns:
                if (
                    column not in special_columns
                    and column not in dropping_columns
                    and column not in categorical_columns
                    and column in X_copy.select_dtypes(include=[np.number]).columns
                    and all(np.isnan(float(var)) for var in X_copy[column].values)
                ):
                    self._warn_or_raise(
    ...
    def _infer_missing_data(
        ...
        for grain, df_one in X.groupby(self.grain_column_names):
            ...
            if (
                all(pd.isnull(y) for y in df_one[TimeSeriesInternal.DUMMY_TARGET_COLUMN])
                and not is_reported
                and hasgap
                and self._lag_or_rw_enabled()
                and not self.is_grain_dropped(grain)
            ):
    ...
    def _infer_y(self, X: pd.DataFrame, grain: GrainType) -> pd.DataFrame:
    ...
    def _rolling_evaluation_one_grain(
        self, df_pred: pd.DataFrame, y_pred: pd.Series, ignore_data_errors: bool
    ) -> pd.DataFrame:
        """ "
    ...
    def apply_time_series_transform(self, X: pd.DataFrame, y: np.ndarray = None) -> pd.DataFrame:
        """
    ...
    def static_preaggregate_data_set(
        ...
        **Note:** This method does not guarantee that the data set will be aggregated.
        ...
        if (
            agg_fun
            and ts_transformer.parameters.get(TimeSeries.FREQUENCY) is not None
            and (diff_col or (not diff_col and not ext_resgressors))
        ):

...
.. note::
    ...
    the :paramref:`~project_env_vars.cae` argument has to be provided within your ``setup.py`` file to let setuptools
    see this patch.
...
.. note::
    setuptools is throwing a DeprecationWarning since version 60 saying: setuptools moves its configuration from
    ``setup.cfg`` towards ``pyproject.toml`` and so :func:`setuptools.config.read_configuration` became deprecated.
...
from setuptools.config.setupcfg import read_configuration   # type: ignore # setuptools.config shows deprecation warning

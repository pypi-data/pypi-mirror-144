...
from rltrade.models import SmartDailyTradeGoldLearnerAgent, SmartDailyTradeGoldMainAgent


time_frame = "1 week"
wf_freq = '1 M' # Y options: M, Y
train_period = ('2020-07-02','2021-08-30')
test_period = ('2021-08-30','2022-01-14')
start_time = "00:00:00"
end_time = "23:55:00" 

csv_path = 'data/ibkrgoldcontfutweek.csv' # path with .csv extension
...
for i,indicator in enumerate(all_indicators):
    ...
    env_kwargs = {
        ...
        "vix_fix_lag":50,
        "lot_size":0.01,
        "target_metrics":['asset','unrealized','sortino','calamar','skew','kurtosis'], #asset, cagr, sortino, calamar, skew and kurtosis are available options.
    ...
    agent = SmartDailyTradeGoldLearnerAgent("ppo",
                        df=df,
                        account=None,
...
for i in range(len(date_range)-2):
    ...
    agent = SmartDailyTradeGoldMainAgent("ppo",
                            df=df_actions,
                            account=None,
                            time_frame=time_frame,
                            ticker_list=ticker_list,
    ...
    agent.save_model(path+f'/main{i}')

    ...
    def on_solution_callback(self):
        ...
        for i in range(self.rows):
            ...
            for j in range(self.cols):
                r.append(self.Value(self.board_bools[i][j] * self.Value(self.board_numbers[i])))
        return board
...
class YakusoSolver:
        ...
        for i in range(rows):
            for j in range(cols):
                if len(template[i][j]) > 0:
                    ...
                    model.Add(
                        board_bools[i][j]*template_num == template_num
                    )
                    if template_num > 0:
                        model.Add(
                            board_maxima[i] == template_num
                        )
    ...
    def solve(
        ...
        self.__add_sum_per_column_constraint(model, sums, sum_cols, rows, board_bools, board_maxima)

...
class LogMixin:
    ...
    def __init__(self, spider=None, **kwargs):
        if self.logger is None:
            self.logger = self.__get_logger(spider)
    ...
    def __get_logger(self,
        ...
        # Formatter
        formatter = logging.Formatter("%(asctime)s | %(levelname)-7s | %(name)s | line:%(lineno)d - %(message)s",
                                      datefmt='%Y-%m-%d %H:%M:%S')
        # FileHandler
        if setting.LOG_TO_FILE is True:
            ...
            file_handler = RotatingFileHandler(filename=f'{log_file_path}/{log_file_name}-{_now_time}.log',
                                               ...
                                               maxBytes=setting.LOG_FILE_SIZE, backupCount=setting.LOG_BACKUP_COUNT,
                                               encoding=encoding)

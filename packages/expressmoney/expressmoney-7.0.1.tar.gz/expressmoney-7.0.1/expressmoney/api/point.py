...
__all__ = ('RequestError', 'ClientError', 'ServerError', 'NotFound404', 'Throttled',
            ...
            if not status.is_success(response.status_code):
                if status.is_client_error(response.status_code):
                    ...
                    if response.status_code == status.HTTP_429_TOO_MANY_REQUESTS:
                        raise Throttled(self._point_id.url, response.status_code, response.headers.get('Retry-After'))
                    else:
...
class CreatePointMixin:
    ...
    def create(self, payload: dict):
        ...
        response = self._post(contract.data)
        if self._response_contract:
            contract = self._response_contract(data=response.json())
            contract.is_valid(raise_exception=True)

...
def main(
    ...
    **kwargs,
    ...
    if not plain:
        ...
        sp = NullYaspin()
    with sp:
        sp.text = 'Beginning run submission...'
        sp.start()
        ...
        if validate_composer:
            ...
            for config in track(run_configs):
                is_valid, e = check_valid_hparams(config)
                if not is_valid:
                    logger.error('Run was not a valid hparams for the currently install composer!')
                    logger.error(yaml.safe_dump(config.to_json()))

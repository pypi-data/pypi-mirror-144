...
from omfit_classes.omfit_omas_utils import setup_hardware_overlay_cx, add_hardware_to_ods, omas_eq1d_pretty_name
...
def populate_efitviewer_figure(
    ...
    if show_cx:
        ...
        cx_ax = scratch['efitviewer_cx_ax'] = pyplot.subplot(gs[:, 0])
    else:
    ...
    for i in range(num_profiles):
        ...
        if (i > 0) and profiles_sharey != 'none':
            ...
            sharey = None
        pyplot.figure(num=fig.number)
        profile_axs[row, col] = pyplot.subplot(gs[row, fig_col], sharex=sharex, sharey=sharey)

...
class TerrariaPCPlayer:
	...
	def read_from_file(self, path: str, version: int = None) -> None:
		with TerrariaPCPlayer.BinaryReaderDecrypt(path, bytes.fromhex("6800330079005F006700550079005A00")) as file:
			...
			if version <= 17:
				self.is_male = not (self.hair == 5 or self.hair == 6 or self.hair == 9 or self.hair == 11)
				self.style = 0
			elif version < 107:
			...
			for _ in range(0, amount):
				...
				for _ in range(0, 10):
					item = read_item()
					if version >= 114:
						item = self.InventoryItem(item.id, item.stack, item.prefix, file.read_bool())
					row.append(item)
		...
		with TerrariaPCPlayer.BinaryWriterEncrypt(path, bytes.fromhex("6800330079005F006700550079005A00")) as file:
			...
			for x1 in range(0, amount):
				...
				for x2 in range(0, 10):
					item = try_read_index(try_read_index(self.inventory, x1, []), x2, self.InventoryItem())
					write_item(item)
					if version >= 114:
						try:
			...
			for x in range(0, 4):
				item = try_read_index(self.coins, x, self.InventoryItem())
				write_item(item)
				if version >= 114:
					try:
						file.write_bool(item.is_favorite)
			...
			if version >= 15:
				...
				for x in range(0, 4):
					item = try_read_index(self.ammo, x, self.InventoryItem())
					write_item(item)
					if version >= 114:
						try:
							file.write_bool(item.is_favorite)

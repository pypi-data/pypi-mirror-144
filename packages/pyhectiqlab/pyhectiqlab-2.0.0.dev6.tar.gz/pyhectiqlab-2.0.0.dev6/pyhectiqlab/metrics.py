...
class MetricsManager():
    ...
    def subsample(self, data):
        if len(data)<=self._max_cache_quantity:
            ...
            print(f"Your metrics rate ({len(data)} metrics/5 seconds) is exceeding the maximum rate ({self._max_cache_quantity}/5 seconds). Your metrics have been subsampled automatically.")
            print("Consider reducing the number of metrics you push or settings an aggregation method with `run.set_metrics_aggr('mean')`.")
            print("See documentation for details https://docs.hectiq.ai/objects/metrics/.")

...
def sun_position_from_date_and_time(geo_lon,geo_lat, year,month,day, hour,minute=0,second=0, timezone='UTC', debug=False):
    ...
    azimuth, altitude, altitude_true, distance = sun_position_from_datetime(geo_lon,geo_lat, utc, debug)
    
    return azimuth, altitude, altitude_true, distance
...
def sun_position_from_datetime(geo_lon,geo_lat, date_time, debug=False):
    ...
    for utc in utcs:
        ...
        altitude_true = np.append(altitude, st._altitudeUncorr)
    ...
    if debug:
        ...
        print('True altitude:                %10.6lf°°'         % (st._altitudeUncorr*r2d))
        print('Distance:                     %10.6lf AU'        % (st.distance))
        print()
    
    
    if scalar_input:
        return np.asscalar(azimuth), np.asscalar(altitude), np.asscalar(altitude_true), np.asscalar(distance)

...
def get_historical_financials_df(
    ...
    df = DataFrame.from_dict(
        {(i, j): hist_financials[i][j] for i in hist_financials.keys() for j in hist_financials[i].keys()}
    ).sort_index()
...
@click.option("--chart_type", "-c", type=ChartParamType(), default="bar", help="Chart can be bar or line chart.")
def plot(
    ctx: click.Context,
    tickers: str,
    period_type: str,
    ...
    `i8 financials plot AMD,INTC,QCOM`
    """
    ...
    if len(matched_metrics) > 2:
        ...
        return
    if not chart_type in [item for t in get_chart_param_types() for item in t]:

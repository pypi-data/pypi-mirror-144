...
class MultiQubitSystem:
    ...
    def Sorted_Proj_Strings(self):
        ...
        repeated_shuffled = repeated.reshape(-1, self._qs)
        sorted_proj = np.unique(repeated_shuffled, axis=0).reshape(-1,self._qs)

        assert len(sorted_proj) == 6 ** self._qs, 'Increase the sampling in the repeated povm list'
        return sorted_proj

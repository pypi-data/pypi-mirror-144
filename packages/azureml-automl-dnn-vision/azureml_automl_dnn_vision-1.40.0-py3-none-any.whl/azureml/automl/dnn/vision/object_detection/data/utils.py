...
def read_aml_dataset(dataset, validation_dataset, split_ratio, ignore_data_errors, output_dir,
    ...
    if validation_dataset is not None:
        ...
        if master_process:
            _save_image_df(train_df=train_dataset_wrapper.get_images_df(),
                           val_df=val_dataset_wrapper.get_images_df(),
        ...
        dataset_wrapper = dataset_class(dataset=dataset, is_train=True,
                                        ...
                                        masks_required=masks_required, tile_overlap_ratio=tile_overlap_ratio,
                                        tile_grid_size=tile_grid_size)
        train_dataset_wrapper, val_dataset_wrapper = dataset_wrapper.train_val_split(
            split_ratio)
        if master_process:
            _save_image_df(train_df=dataset_wrapper.get_images_df(), train_index=train_dataset_wrapper._indices,
                           val_index=val_dataset_wrapper._indices, output_dir=output_dir)

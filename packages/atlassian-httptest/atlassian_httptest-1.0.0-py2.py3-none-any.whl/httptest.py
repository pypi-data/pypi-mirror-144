...
class _TestServerHandler(ServerHandler):
...
def _logmiddleware(app, logqueue):
    ...
    def wrapper(environ, start_response):
        ...
        try:
            ...
            resbody = b''.join([to_utf8(v) for v in written]) \
                      + b''.join([to_utf8(v) for v in out])
...
class TestServer():
    ...
    def __init__(self, app=nocontent, host='localhost',
                 port=range(30059, 30159), ignorehangups=False):
        self._host = host
        self._log = []
...
def testserver(app=nocontent, host='localhost', port=range(30059, 30159),
               ignorehangups=False):

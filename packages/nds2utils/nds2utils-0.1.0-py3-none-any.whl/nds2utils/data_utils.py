...
def calc_csds(data_dict, averages, bandwidth, overlap, averaging="mean", makeTFs=True):
...
def get_csds(
    ...
    data_dict = calc_csds(
        data_dict, averages, binwidth, overlap, averaging=averaging, makeTFs=makeTFs
    )
    return data_dict
...
def linear_log_psd(log_ff, linear_ff, linear_psd):
    ...
    linear_index = np.argwhere(cutoff < linear_ff)[0][
        0

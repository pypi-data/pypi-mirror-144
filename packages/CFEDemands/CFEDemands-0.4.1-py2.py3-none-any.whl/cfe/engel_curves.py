#!/usr/bin/env python
# [[file:../Demands/engel_curves.org::+begin_src python :tangle ../cfe/engel_curves.py :shebang "#!/usr/bin/env python"][No heading:1]]
# Tangled on Mon Aug 30 14:39:31 2021
...
def plot(p,alpha,beta,phi,labels=[],ybounds=[0,10],npts=100,fname=None,NegativeDemands=True,use_linestyles=False,shares=False,logs=True,use_figure=1,ax=None):
    ...
    if not shares:
        foo=lambda y: demands.marshallian.demands(y,p,parms,NegativeDemands=NegativeDemands)
    else:
        foo=lambda y: demands.marshallian.budgetshares(y,p,parms,NegativeDemands=NegativeDemands)

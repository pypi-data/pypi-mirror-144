...
import dcc.ply.lex as lex
import dcc.ply.yacc as yacc
import sys
...
def t_NUMBER(t):
    ...
    try:
        ...
        print("\n<!> Compile Error in Dynamo Charlotte: Float value not supported %d", t.value)
        t.value = 0
    return t
...
def main():
    if(len(sys.argv) != 2):
        print("\n<!> Pre-processing Error in Dynamo Charlotte: Only one .dc file should be passed as argument.\n")
    dc_file = str(sys.argv[1])
    if (not(dc_file.endswith('.dc'))):
        print("\n<!> Pre-processing Error in Dynamo Charlotte: File should have .dc extension.\n")
    dc_code = ""
    try:
        with open(dc_file) as file:
            lines = file.readlines()
            for line in lines:
                dc_code += line
            run(dc_code)
    except:
        print("\n<!> Pre-processing Error in Dynamo Charlotte: Could not open '{0}' file.\n".format(dc_file))        

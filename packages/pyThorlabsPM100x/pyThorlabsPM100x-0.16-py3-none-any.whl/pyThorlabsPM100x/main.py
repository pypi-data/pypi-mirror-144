...
from pyThorlabsPM100x.abstract_interface import abstract_interface, abstract_gui
...
class interface(abstract_interface):
    ...
    instrument : ThorlabsPM100x
        a driver object, for the low-level communication with the device
    gui : gui()
    ...
    disconnect_device(device_full_name)
    ...
    set_power_range_auto(status)
    
    read_status_power_autorange()
    ...
    def create_gui(self,parent,plot=False): 
        ...
        '''
        self.gui = gui(self,parent,plot)
        self.gui.initialize() 

### Functions to talk with low-level driver
    ...
    def set_refresh_time(self, refresh_time):
        try: 
            ...
            self.gui.edit_RefreshTime.setText(f"{self.refresh_time:.3f}")
            return False
    ...
    def set_power_range_auto(self,status):
    ...
    def read_status_power_autorange(self):
        ...
        try:
            ...
            if auto_power_range:
                status = 'ON'
                self.gui.disable_widget([self.gui.edit_PowerRange,self.gui.button_IncreasePowerRange, self.gui.button_DecreasePowerRange])
            else:
                status = 'OFF'
                self.gui.enable_widget([self.gui.edit_PowerRange,self.gui.button_IncreasePowerRange, self.gui.button_DecreasePowerRange])
            self.logger.info(f"The auto-ranging function is currently set to {status}.")
        except Exception as e:
    ...
    def start_reading(self):
        ...
        # Call the function self.update(), which will do stome suff (read power and store it in a global variable) and then call itself continuosly until the variable self.ContinuousRead is set back to 0
        self.update()
        return
...
class gui(abstract_gui):
    def __init__(self,interface,parent,plot=False):
        ...
        self.edit_RefreshTime.setText(f"{self.interface.refresh_time:.3f}")

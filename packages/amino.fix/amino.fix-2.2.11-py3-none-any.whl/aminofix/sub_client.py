...
class SubClient(client.Client):
    ...
    def get_invite_codes(self, status: str = "normal", start: int = 0, size: int = 25):
        ...
        if response.status_code != 200: 
                if response.status_code == 403: return exceptions.CheckException(response.text) 
                else: return exceptions.CheckException(json.loads(response.text))
    ...
    def send_active_obj(self, startTime: int = None, endTime: int = None, optInAdsFlags: int = 2147483647, tz: int = -timezone // 1000, timers: list = None, timestamp: int = int(timestamp() * 1000)): 
        ...
        response = self.session.post(f"{self.api}/x{self.comId}/s/community/stats/user-active-time", headers=headers.ApisHeaders(data=data, deviceId=self.device_id).headers, data=data, proxies=self.proxies, verify=self.certificatePath) 
        if response.status_code != 200: 
                if response.status_code == 403: return exceptions.CheckException(response.text) 
                else: return exceptions.CheckException(json.loads(response.text)) 
        else: return response.status_code

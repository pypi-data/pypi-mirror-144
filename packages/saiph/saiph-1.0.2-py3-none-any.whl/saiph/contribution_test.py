...
from saiph.contribution import get_variable_contribution
...
def test_get_variable_contribution_filter_by_min_contrib(
...
def test_get_variable_contribution_filter_by_max_var(
    contributions: pd.DataFrame,
) -> None:
    """Verify that we can filter by maximum number of variables."""
    result = get_variable_contribution(contributions=contributions, dim=2, max_var=1)
    np.testing.assert_array_equal(result.values, [1])

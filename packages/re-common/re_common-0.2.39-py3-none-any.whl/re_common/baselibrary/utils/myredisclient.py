...
class MyRedis(object):
    ...
    def conn_redis(self):
        ...
        self.rconn = redis.StrictRedis(host=self.RedisHost, port=self.RedisPort, db=self.RedisDB,password=self.RedisPassword, decode_responses=True)
        return self.rconn
    ...
    def srandmember(self, name, number=None):
        ...
        """
        assert self.r, 'r 不存在，请先调用 set_conn_or_pipe'
        return self.r.srandmember(name, number=None)

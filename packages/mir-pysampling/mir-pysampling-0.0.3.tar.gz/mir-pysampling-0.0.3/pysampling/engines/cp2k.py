...
""" adaptive version from ase calculator """

"""This module defines an ASE engine to CP2K.

http://www.cp2k.org
Author: Ole Schuett <ole.schuett@mat.ethz.ch>

Adapted by Lixin Sun
"""
...
class CP2K(Calculator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # no longer use the same shell all the time...
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        self._shell = Cp2kShell(self.command, self._debug, self.workdir)

        timestamp = int(time.time())
        inp_fn = f"{timestamp}.inp"
        out_fn = f"{timestamp}.out"
        edit_dft_input_positions(self.template, f"{self.workdir}/{inp_fn}", atoms)
        ...
        if compute:
            ...
            self.energy = float(self._shell.recv())
            self.free_energy = self.energy
            self._shell.expect("* READY")

            self.forces = np.zeros(shape=(n_atoms, 3))
            ...
            for i in range(n_atoms):
                ...
                self.forces[i, :] = [float(x) for x in line.split()]
            self._shell.expect("* END")
            self._shell.expect("* READY")

        self.results["energy"] = self.energy
        self.results["free_energy"] = self.free_energy
...
class Cp2kShell(object):
    ...
    def __init__(self, command, debug, workdir="./"):
...
def parse_input(inp):
    ...
    print("debug parse_input", root_section)
    return root_section


def edit_dft_input_positions(dft_input: str, newfilename, atoms):
    """Write the current configuration of the OTF structure to the
    qe input file

    :param dft_input: intput file name
    :param atoms: atoms to print
    ...
    for pos_index, line_index in enumerate(
        range(file_pos_index, file_pos_index + n_atoms)
    ):
        pos = positions[pos_index]
        specs = syms[pos_index]
    ...
    cell = atoms.get_cell()
    lines[cell_index] = "A " + " ".join([str(x) for x in cell[0]]) + "\n"
    lines[cell_index + 1] = "B " + " ".join([str(x) for x in cell[1]]) + "\n"
    lines[cell_index + 2] = "C " + " ".join([str(x) for x in cell[2]]) + "\n"

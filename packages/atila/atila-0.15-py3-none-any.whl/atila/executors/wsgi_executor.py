...
from skitai.tasks import Coroutine
...
class Executor:
    ...
    def add_async_task (self, coro):
        meta  = {'__was_id': self.was.ID, 'coro': coro}
        future = asyncio.run_coroutine_threadsafe (coro, self.was.event_loop)
        return [Task (future, coro.__qualname__, meta = meta, filter = None).then (self.respond_async)]
        ...
        try:
            ...
            try:
                ...
                if self.env ['wsgi.route_options'].get ('coroutine'):
                    ...
                    assert hasattr (self.was, "event_loop"), "async is no enalbled"

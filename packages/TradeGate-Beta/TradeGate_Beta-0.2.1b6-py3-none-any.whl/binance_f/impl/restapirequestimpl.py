...
class RestApiRequestImpl(object):
    ...
    def post_order(self, symbol, side, ordertype, timeInForce, quantity, reduceOnly, price, newClientOrderId, stopPrice,
                   workingType, closePosition,
                   positionSide, callbackRate, activationPrice, priceProtect, newOrderRespType):
        check_should_not_none(symbol, "symbol")
        ...
        builder.put_url("priceProtect", priceProtect)
    ...
    def post_batch_order(self, batchOrders):
        ...
        def parse(json_wrapper):
            result = list()
            dataList = json_wrapper.convert_2_array()
            for item in dataList.get_items():
                if item.contain_key("code"):
                    result.append(Msg.json_parse(item))
                else:
                    result.append(Order.json_parse(item))
            return result

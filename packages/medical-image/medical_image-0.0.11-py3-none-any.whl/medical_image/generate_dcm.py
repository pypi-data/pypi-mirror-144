...
class DicomModule(object):
    ...
    def make_sc_array(self, result_json):
        mfont = ImageFont.truetype(os.path.join(medical_image.__path__[0], 'fonts/NotoSansCJKkr-Medium.otf'), int(result_json['Height'] / 70))
        ...
        x = result_json['Width'] * 0.99 - logo_w
        y = result_json['Height'] * 0.01
        paste_image(image, self.logo, x, y, logo_w, logo_h, 1)

        ## App Info
        # x = (result_json['Width'] - mw) / 2
        x = result_json['Width'] * 0.99 - mw
        y = result_json['Height'] * 0.011 + logo_h
        shadow = int(mh / 20)

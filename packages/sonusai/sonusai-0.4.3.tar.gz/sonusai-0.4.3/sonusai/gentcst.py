...
def gentcst(fold: str = '*',
    ...
    for file in use_files:
        local_config = {}
        local_name = join(dirname(file['file']), CONFIG_FILE)
        if exists(local_name):
            with open(file=local_name, mode='r') as yaml_file:
                local_config = yaml.safe_load(yaml_file)

        specific_name = splitext(file['file'])[0] + '.yml'
        if exists(specific_name):
            with open(file=specific_name, mode='r') as yaml_file:
                specific_config = yaml.safe_load(yaml_file)

            for key in specific_config:
                local_config[key] = specific_config[key]

        if 'feature' in local_config:
            if config['feature'] != local_config['feature']:
                logger.warning(
                    f'Local feature for {file["file"]} ({local_config["feature"]}) is not the same as top level ({config["feature"]})')

        truth_settings = {'index': file['truth_index']}
        if 'truth_settings' in local_config:
            if 'function' in local_config['truth_settings']:
                truth_settings['function'] = local_config['truth_settings']['function']
            if 'config' in local_config['truth_settings']:
                truth_settings['config'] = deepcopy(local_config['truth_settings']['config'])

        target = {
            'target_name':    file['file'],
...
def get_name_from_id(ontology_data: list, identity: str) -> list:
    ...
    for item in ontology_data:
        if item['id'] == identity:
            if found:
                logger.error(f'id {identity} appears multiple times in ontology')
                raise SonusAIError
...
def get_id_from_name(ontology_data: list, name: str) -> str:
    ...
    for item in ontology_data:
        if name in item['name']:
            if found:
                logger.error(f'name {name} appears multiple times in ontology')
                raise SonusAIError
...
def get_config() -> dict:
    if not exists(CONFIG_FILE):
        logger.error(f'No {CONFIG_FILE} at top level')
        raise SonusAIError

    with open(file=CONFIG_FILE, mode='r') as yaml_file:
...
def write_metadata_to_tree(ontology_data: list, file: dict):
    if ontology_data:
        ...
        if node:
            ...
            with open(file=json_name, mode='w') as json_file:
...
def main():
    try:
        ...
        with open(file=output_name, mode='w') as yaml_file:

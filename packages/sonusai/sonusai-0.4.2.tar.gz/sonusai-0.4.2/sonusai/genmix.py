...
def genmix(mixdb: dict,
    ...
    p_map(_process_mixture, mixdb_out['mixtures'], progress=progress, initializer=mp_init, initargs=(mp_mixture,
                                                                                                     mp_truth_t,
                                                                                                     mp_target,
                                                                                                     mp_noise,
                                                                                                     mp_segsnr))
...
def _process_mixture(record: dict) -> None:
        ...
        for m in mixid:
            ...
            if compute_truth:
                if samples != truth_t.shape[0]:
                    logger.error(f'truth_t samples does not match mixture samples: {truth_t.shape[0]} != {samples}')
                    raise SonusAIError
                if mixdb_out['num_classes'] != truth_t.shape[1]:
                    logger.error(f'truth_t num_classes is incorrect: {truth_t.shape[1]} != {mixdb_out["num_classes"]}')
                    raise SonusAIError
            if samples != target.shape[0]:
                logger.error(f'target samples does not match mixture samples: {target.shape[0]} != {samples}')
                raise SonusAIError
            if samples != noise.shape[0]:
                logger.error(f'noise samples does not match mixture samples: {noise.shape[0]} != {samples}')
                raise SonusAIError
            if compute_segsnr and samples != segsnr.shape[0]:
                logger.error(f'segsnr samples does not match mixture samples: {segsnr.shape[0]} != {samples}')
                raise SonusAIError

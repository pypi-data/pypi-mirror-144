        ...
        if files:
            ...
            if nb_cores == 1:
                for component in files:
                    self.register_component_with_status(component)
            else:
                log.info(f"Batch register with {nb_cores} threads.")
                pool = multiprocessing.Pool(processes=nb_cores)
                pool.map(self.register_component_with_status, files)
                pool.close()
                pool.join()

    def register_component_with_status(self, component) -> None:
        """
        ...
        """
        register_command = self.register_component_command(component)

...
@click.option("--commands/--no-commands", default=False, help="Show commands")
def _start(commands):
    """
    ...
    steps = [
        ...
        {
            "cmd": ["enrichpkg bootstrap -c repo -p ."],
        ...
        {"cmd": ["cd Marketing"], "description": "Change to usecase"},
        {
        ...
        {"cmd": ["workflows/prefect/daily.py"], "description": "Check workflow"},
    ]
...
@click.option(
    "--path", "-p", required=True, help="Directory where the files should be stored"
)
@click.option(
    "--template",
    "-t",
...
def _backend_add(ctx, name, server, key):
    ...
    if missing:
        backends.append({"name": name, "server": server, "key": key})
...
def _status(ctx, offset, page):
    ...
    sdkapi.draw(backend, "status", result, columns)
...
def _usecases(ctx):
    ...
    sdkapi.draw(backend, "usecases", result, columns)
...
def _runs(ctx, usecase, pipeline):
    ...
    sdkapi.draw(
        backend, "run-list", result, columns, {"Usecase": usecase, "Pipeline": pipeline}
    )

        ...
        if not selected.empty:
            if rvalue:
                ...
                try:
                    ...
                    logger.error(f"Selection failed using {operation}.", exc_info=True)
    ...
    if value:
        ...
        if value:
            if str(value).lower() in ["true", "false"]:
                ...
                try:
                    value = Decimal(value.replace(",", '.')) if operation in ['+', '-'] else float(value.replace(",", "."))
            ...
            try:
                ...
                if operation == '+':
                    for column in selected_dict.keys():
                        selected_dict[column] = {k: Decimal(str(v)) + value for k, v in selected_dict[column].items()}
                elif operation == '-':
                    for column in selected_dict.keys():
                        selected_dict[column] = {k: Decimal(str(v)) - value for k, v in selected_dict[column].items()}
                elif operation == '*':
                    for column in selected_dict.keys():
                        selected_dict[column] = {k: float(v) * value for k, v in selected_dict[column].items()}
                elif operation == '/':
                    for column in selected_dict.keys():
                        selected_dict[column] = {k: float(v) / value for k, v in selected_dict[column].items()}
                else:

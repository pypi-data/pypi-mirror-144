...
class DatasetContextManager(_CommonContextManager):
    ...
    def _upload(self, name, config):
        ...
        # asset creation is handled at one place for both mount and upload
        if asset_type is None:
    ...
    def _register_asset(self, name, uri_path, asset_type):
        ...
        from ._restclient.models import CreateUnregisteredOutputData, CreateUnregisteredOutputModelDto, CreateRun
        ...
        try:
            ...
            if output_type.lower() in ['urifile', 'urifolder', 'mltable']:
                create_dto = CreateUnregisteredOutputData(
                    run_id=run_id, output_name=name, uri=uri_path, type=output_type)
                data_asset = client.data_version.create_unregistered_output_data(
                    workspace._subscription_id,
                    workspace._resource_group,
                    workspace._workspace_name,
                    create_dto)
                asset_id = data_asset.latest_version.data_version.asset_id
            else:
                create_dto = CreateUnregisteredOutputModelDto(
                    run_id=run_id, output_name=name, path=uri_path, type=output_type)
                model_asset = client.models.create_unregistered_output_model(
                    workspace._subscription_id,
                    workspace._resource_group,
                    workspace._workspace_name,
                    create_dto)

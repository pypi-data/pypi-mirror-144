...
class Regbot:
  ...
  def prepareInput(cls,opening,closing,volume):
  	...
  	testdata = np.array([[opening,closing,volume]])
  ...
  def buySignalGenerator(cls,opening,closing,bvr):
    ...
    return (cls.loadmodel().predict_proba(scalledInput)[:,1] >= 0.07692307692307693).astype(int)
...
def signal(opening,closing,bvr):
  try:
    return Regbot.buySignalGenerator(opening,closing,bvr)[0]

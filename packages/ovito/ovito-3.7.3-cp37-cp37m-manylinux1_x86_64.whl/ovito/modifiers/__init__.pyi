...
.. list-table::
    ...
    * - ``Surface Distance``
      - The computed distance of each particle from the closest point on the geometric surface. 
        This property is only output if the :py:attr:`compute_distances` option is turned on."""
...
Identifying volumetric regions is only supported by the ``AlphaShape`` surface construction :py:attr:`method`. For other construction methods, the setting will be ignored. 
...
You can direcly use NumPy's `count_nonzero() <https://numpy.org/doc/stable/reference/generated/numpy.count_nonzero.html>`__
...
.. note::
  ...
  Keep in mind that the Boolean expression is subject to special syntax rules.   Because variable identifiers cannot contain spaces, spaces occuring in particle property names must be omitted.   For example, the particle property `Potential Energy` must be referred to as ``PotentialEnergy`` in the expression string. 

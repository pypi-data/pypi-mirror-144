...
def boom_four(df,minus=False):
    '''使用20天均值和标准差，生成4个因子'''
    df_mean=df.rolling(20).mean().resample('M').last()
    df_std=df.rolling(20).std().resample('M').last()
    if not minus:
        twins=(pure_fallmount(df_mean)+(pure_fallmount(df_std),))()
        rtwins=df_mean.rank(axis=1)+df_std.rank(axis=1)
    else:
        twins=(pure_fallmount(df_mean)-(pure_fallmount(df_std),))()
        rtwins=df_mean.rank(axis=1)-df_std.rank(axis=1)
    return df_mean,df_std,twins,rtwins
...
class pure_moonnight():
    ...
    def __init__(self, factors, groups_num, neutralize=False, boxcox=False,by10=False, y2=False, plt_plot=True, plotly_plot=False,
        ...
        self.shen.run(groups_num=groups_num,neutralize=neutralize,boxcox=boxcox,y2=y2,plt_plot=plt_plot,
                      ...
                      time_end=time_end,print_comments=print_comments)

...
def read_daily(path=None,open=0,close=0,high=0,low=0,tr=0,sharenum=0,volume=0,unadjust=0):
    ...
    if unadjust:
        ...
        if path:
            ...
            trs=read_mat('AllStock_DailyTR.mat')
            opens=read_mat('AllStock_DailyOpenmat')
            return np.sign(trs)*opens
        elif close:
            trs=read_mat('AllStock_DailyTR.mat')
...
def decap(df,daily=False,monthly=False):
    ...
    count=df.resample('M').apply(lambda x:x.shape[0])
    count=count[count>0]
    if len(list(set(list(count))))==1:
        daily=True
    else:
        monthly=True
    if daily:
...
class pure_moon():
    ...
    # @lru_cache(maxsize=None)
    def run(self, groups_num, neutralize=False, boxcox=False, y2=False, plt_plot=True,
            ...
            comments_sheetname=None,net_values_sheetname=None,rets_sheetname=None):
        '''运行回测部分'''
...
class pure_fall():
    ...
    def minute_to_daily(self, func,add_priclose=False,add_tr=False):
        '''
    ...
    def get_daily_factors(self, func, whole=False,add_priclose=False,add_tr=False):
        '''调用分钟到日度方法，算出日频数据'''
        try:
            ...
            if whole:
                ...
                self.minute_to_daily(func,add_priclose=add_priclose,add_tr=add_tr)
...
class pure_moonnight():
    ...
    def __init__(self, factors, groups_num, neutralize=False, boxcox=False,by10=False, y2=False, plt_plot=True, plotly_plot=False,
        ...
        self.shen.prerpare()
        self.shen.run(groups_num=groups_num,neutralize=neutralize,boxcox=boxcox,y2=y2,plt_plot=plt_plot,
                      plotly_plot=plotly_plot,filename=filename,time_start=time_start,
                      time_end=time_end,print_comments=print_comments,comments_writer=comments_writer,net_values_writer=net_values_writer,
                      rets_writer=rets_writer,comments_sheetname=comments_sheetname,net_values_sheetname=net_values_sheetname,rets_sheetname=rets_sheetname)

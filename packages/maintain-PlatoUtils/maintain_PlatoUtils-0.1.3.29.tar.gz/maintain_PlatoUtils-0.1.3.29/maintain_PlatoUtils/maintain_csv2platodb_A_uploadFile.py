...
def remakeRawSchema(rawSchemaJson,csvFolder="",gClient=None):
    ...
    for vertexI,_ in tqdm.tqdm(enumerate(rawSchemaJson["vertex"]),desc="vertex"):
        ...
        if vertexAttrProcess=="escape":
            ...
            for gColItem in nodeItemGInfoDf:
                if gColItem!="VertexID":
                    if gColItem not in plato2csvAttrDict:
                        cColItem=gColItem.split(".")[1]
                        nodeItemCInfoDf[cColItem]=nodeItemGInfoDf[gColItem]
                    else:
                        cColItem=plato2csvAttrDict[gColItem]
                        cmkupIndex=list(nodeItemCInfoDf.loc[(nodeItemCInfoDf[cColItem].isna())|
                                                                        (nodeItemCInfoDf[cColItem]=="")|
                                                                        (nodeItemCInfoDf[cColItem]=="null"),cColItem].index)
                        if len(cmkupIndex)>0:
                            nodeItemCInfoDf.iloc[cmkupIndex,cColItem]=nodeItemGInfoDf.iloc[cmkupIndex,gColItem]

...
def _global_except_hook(exctype, value, traceback):
    ...
    wixg__hfvqe = MPI.COMM_WORLD
    bwm__tucel = wixg__hfvqe.Get_rank()
    pbvc__exd = 3.0
    zzfg__ihx = True
    hieu__rsyan = wixg__hfvqe.Ibarrier()
    adzr__oox = time.time()
    while time.time() - adzr__oox < pbvc__exd:
        time.sleep(0.1)
        if hieu__rsyan.Test():
            zzfg__ihx = False
            break
    try:
        ...
        if zzfg__ihx:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {bwm__tucel}. \n')

...
def box_regex_flag(typ, val, c):
    udcuf__rip = c.box(types.int64, val)
    mfgbv__rnf = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(mfgbv__rnf, (udcuf__rip,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    udcuf__rip = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, udcuf__rip)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    uvzp__epa = context.get_python_api(builder)
    pyog__cysnh = builder.icmp_signed('==', val, uvzp__epa.borrow_none())
    with builder.if_then(pyog__cysnh):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    bxx__edr = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(bxx__edr)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        ehhp__ooq = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if ehhp__ooq > 1:
            typ = types.List(types.Tuple([string_type] * ehhp__ooq))
...
def overload_match_group(m, *args):
    ...
    kxt__sjehq = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            wfejr__xyecq = args[0]
            with numba.objmode(out=kxt__sjehq):
                out = m.group(wfejr__xyecq)
            return out
        return _match_group_impl_one
    ppmf__xsr = 'tuple_str_{}'.format(len(args))
    setattr(types, ppmf__xsr, types.Tuple([kxt__sjehq] * len(args)))
    enmjq__xwrlp = ', '.join('group{}'.format(rbb__cxzq + 1) for rbb__cxzq in
        range(len(args)))
    rvqe__fiqc = 'def _match_group_impl(m, *args):\n'
    rvqe__fiqc += '  ({}) = args\n'.format(enmjq__xwrlp)
    rvqe__fiqc += "  with numba.objmode(out='{}'):\n".format(ppmf__xsr)
    rvqe__fiqc += '    out = m.group({})\n'.format(enmjq__xwrlp)
    rvqe__fiqc += '  return out\n'
    inx__urxlv = {}
    exec(rvqe__fiqc, globals(), inx__urxlv)
    ewxv__fqfl = inx__urxlv['_match_group_impl']
    return ewxv__fqfl
...
def overload_match_groups(m, default=None):
    kxccm__uepc = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=kxccm__uepc):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(mpsp__dkh is None for mpsp__dkh in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=kxt__sjehq):

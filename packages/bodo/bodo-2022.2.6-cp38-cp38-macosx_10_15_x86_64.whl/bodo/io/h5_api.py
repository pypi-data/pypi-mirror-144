...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        xrpk__iop = []
        fiaq__rcrgq = h5g_get_num_objs(obj_id)
        for ibc__nrl in range(fiaq__rcrgq):
            bcvx__vaufd = h5g_get_objname_by_idx(obj_id, ibc__nrl)
            xrpk__iop.append(bcvx__vaufd)
        return xrpk__iop
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    xogbs__gvem = parse_dtype(dtype)
    wbz__lfyl = np.int32(numba_to_c_type(xogbs__gvem))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            hvyzx__geetp = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    zcklh__srla = tuple_to_ptr(starts)
    czq__drl = tuple_to_ptr(counts)
    cxfy__itzo = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, zcklh__srla, czq__drl,
        is_parallel, out_arr.ctypes, cxfy__itzo)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    czq__drl = tuple_to_ptr(counts)
    cxfy__itzo = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, zcklh__srla, czq__drl,
        is_parallel, out_arr.ctypes, cxfy__itzo)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        snec__mnu = types.Array(dtype, ndim, 'C')
        ekw__lfeuq = args[3]
        snec__mnu = self.context.resolve_function_type(operator.getitem, [
            snec__mnu, ekw__lfeuq], {}).return_type.copy(layout='C')
        return signature(snec__mnu, *args)
...
def get_filter_read_indices(bool_arr):
    kgp__tmp = bool_arr.nonzero()[0]
    fobyh__jtg = bodo.libs.distributed_api.get_rank()
    yrz__ppv = bodo.libs.distributed_api.get_size()
    fyc__xbo = np.empty(yrz__ppv, np.int64)
    hbyb__utwcr = len(bool_arr)
    bodo.libs.distributed_api.allgather(fyc__xbo, hbyb__utwcr)
    nqgew__csnin = fyc__xbo.cumsum()[fobyh__jtg] - hbyb__utwcr
    kgp__tmp += nqgew__csnin
    ezho__rlkr = bodo.libs.distributed_api.dist_reduce(len(kgp__tmp), np.
        int32(sum_op))
    kovlm__sin = bodo.libs.distributed_api.gatherv(kgp__tmp)
    if fobyh__jtg == 0:
        ito__hwcs = kovlm__sin
    else:
        ito__hwcs = np.empty(ezho__rlkr, kgp__tmp.dtype)
    bodo.libs.distributed_api.bcast(ito__hwcs)
    xdv__pniwe = bodo.libs.distributed_api.get_start(ezho__rlkr, yrz__ppv,
        fobyh__jtg)
    fezrm__syrov = bodo.libs.distributed_api.get_end(ezho__rlkr, yrz__ppv,
        fobyh__jtg)
    return ito__hwcs[xdv__pniwe:fezrm__syrov]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        fsflj__vew = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], fsflj__vew)
        return builder.bitcast(fsflj__vew, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    czq__drl = tuple_to_ptr(counts)
    cxfy__itzo = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, zcklh__srla,
        czq__drl, is_parallel, out_arr.ctypes, cxfy__itzo, read_indices.

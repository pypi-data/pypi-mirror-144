...
except ImportError as tnr__lct:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        gng__xhg = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(gng__xhg)


def random_forest_model_fit(m, X, y):
    icc__oinn = m.n_estimators
    lsti__utv = MPI.Get_processor_name()
    igpk__bqp = get_host_ranks()
    qcry__iiguv = len(igpk__bqp)
    rah__dkl = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(icc__oinn,
        qcry__iiguv, rah__dkl)
    if rah__dkl == igpk__bqp[lsti__utv][0]:
        m.n_jobs = len(igpk__bqp[lsti__utv])
    ...
    tgs__nkj = create_subcomm_mpi4py(first_rank_node)
    if tgs__nkj != MPI.COMM_NULL:
        uiit__apx = 10
        kdx__gwpm = bodo.libs.distributed_api.get_node_portion(icc__oinn,
            qcry__iiguv, 0)
        muc__nitqa = kdx__gwpm // uiit__apx
        if kdx__gwpm % uiit__apx != 0:
            muc__nitqa += 1
        gaa__avzhw = []
        for ggpmd__nzo in range(muc__nitqa):
            ilqp__tgar = tgs__nkj.gather(m.estimators_[ggpmd__nzo *
                uiit__apx:ggpmd__nzo * uiit__apx + uiit__apx])
            if rah__dkl == 0:
                gaa__avzhw += list(itertools.chain.from_iterable(ilqp__tgar))
        if rah__dkl == 0:
            m.estimators_ = gaa__avzhw
    pkky__iotqy = MPI.COMM_WORLD
    if rah__dkl == 0:
        for ggpmd__nzo in range(0, icc__oinn, 10):
            pkky__iotqy.bcast(m.estimators_[ggpmd__nzo:ggpmd__nzo + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            pkky__iotqy.bcast(m.n_classes_)
            pkky__iotqy.bcast(m.classes_)
        pkky__iotqy.bcast(m.n_outputs_)
    else:
        ddgi__yjs = []
        for ggpmd__nzo in range(0, icc__oinn, 10):
            ddgi__yjs += pkky__iotqy.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = pkky__iotqy.bcast(None)
            m.classes_ = pkky__iotqy.bcast(None)
        m.n_outputs_ = pkky__iotqy.bcast(None)
        m.estimators_ = ddgi__yjs
    assert len(m.estimators_) == icc__oinn
    m.n_estimators = icc__oinn
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    zjoq__hnc = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
        ...
        sklearn.metrics._classification.multilabel_confusion_matrix = zjoq__hnc
    return result
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    kfe__jetf = len(labels)
    gziy__pjk = np.zeros(kfe__jetf, np.int64)
    tcl__cun = np.zeros(kfe__jetf, np.int64)
    zsxc__qdx = np.zeros(kfe__jetf, np.int64)
    ine__sdeu = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for ggpmd__nzo in range(len(y_true)):
        tcl__cun[ine__sdeu[y_true[ggpmd__nzo]]] += 1
        if y_pred[ggpmd__nzo] not in ine__sdeu:
            continue
        bqhbl__bhh = ine__sdeu[y_pred[ggpmd__nzo]]
        zsxc__qdx[bqhbl__bhh] += 1
        if y_true[ggpmd__nzo] == y_pred[ggpmd__nzo]:
            gziy__pjk[bqhbl__bhh] += 1
    gziy__pjk = bodo.libs.distributed_api.dist_reduce(gziy__pjk, np.int32(
        Reduce_Type.Sum.value))
    tcl__cun = bodo.libs.distributed_api.dist_reduce(tcl__cun, np.int32(
        Reduce_Type.Sum.value))
    zsxc__qdx = bodo.libs.distributed_api.dist_reduce(zsxc__qdx, np.int32(
        Reduce_Type.Sum.value))
    qtlh__ktj = zsxc__qdx - gziy__pjk
    iuacp__vyon = tcl__cun - gziy__pjk
    szs__mvty = gziy__pjk
    etcih__ulb = y_true.shape[0] - szs__mvty - qtlh__ktj - iuacp__vyon
    with numba.objmode(result='float64[:]'):
        MCM = np.array([etcih__ulb, qtlh__ktj, iuacp__vyon, szs__mvty]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        foijk__osjp = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        foijk__osjp = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    pkky__iotqy = MPI.COMM_WORLD
    tmf__apl = pkky__iotqy.Get_size()
    if sample_weight is not None:
        pxmg__gwb = np.sum(sample_weight)
    else:
        pxmg__gwb = np.float64(y_true.shape[0])
    miyza__nul = np.zeros(tmf__apl, dtype=type(pxmg__gwb))
    pkky__iotqy.Allgather(pxmg__gwb, miyza__nul)
    tvobx__otg = np.zeros((tmf__apl, *foijk__osjp.shape), dtype=foijk__osjp
        .dtype)
    pkky__iotqy.Allgather(foijk__osjp, tvobx__otg)
    qczo__kgutc = np.average(tvobx__otg, weights=miyza__nul, axis=0)
    if metric == 'mse' and not squared:
        qczo__kgutc = np.sqrt(qczo__kgutc)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return qczo__kgutc
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(qczo__kgutc)
    else:
        return np.average(qczo__kgutc, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    pkky__iotqy = MPI.COMM_WORLD
    score = pkky__iotqy.allreduce(score, op=MPI.SUM)
    if normalize:
        klovl__descw = np.sum(sample_weight
            ) if sample_weight is not None else len(y_true)
        klovl__descw = pkky__iotqy.allreduce(klovl__descw, op=MPI.SUM)
        score = score / klovl__descw
    return score
...
def check_consistent_length_parallel(*arrays):
    pkky__iotqy = MPI.COMM_WORLD
    dzgv__kjco = True
    hah__gjj = [len(nead__qfa) for nead__qfa in arrays if nead__qfa is not None
        ]
    if len(np.unique(hah__gjj)) > 1:
        dzgv__kjco = False
    dzgv__kjco = pkky__iotqy.allreduce(dzgv__kjco, op=MPI.LAND)
    return dzgv__kjco
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    mxbg__xywz = y_true.shape[0]
    viqc__vzd = pkky__iotqy.allreduce(mxbg__xywz, op=MPI.SUM)
    if viqc__vzd < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        por__ccus = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        por__ccus = 1.0
    syq__kwm = (por__ccus * (y_true - y_pred) ** 2).sum(axis=0, dtype=np.
        float64)
    lbcid__qssb = np.zeros(syq__kwm.shape, dtype=syq__kwm.dtype)
    pkky__iotqy.Allreduce(syq__kwm, lbcid__qssb, op=MPI.SUM)
    uhwn__nigj = np.nansum(y_true * por__ccus, axis=0, dtype=np.float64)
    zuc__nucrp = np.zeros_like(uhwn__nigj)
    pkky__iotqy.Allreduce(uhwn__nigj, zuc__nucrp, op=MPI.SUM)
    kly__ciojy = np.nansum(sample_weight, dtype=np.float64)
    igpvy__zgr = pkky__iotqy.allreduce(kly__ciojy, op=MPI.SUM)
    yxgdo__lsc = zuc__nucrp / igpvy__zgr
    vlld__uymh = (por__ccus * (y_true - yxgdo__lsc) ** 2).sum(axis=0, dtype
        =np.float64)
    yriya__tlvse = np.zeros(vlld__uymh.shape, dtype=vlld__uymh.dtype)
    pkky__iotqy.Allreduce(vlld__uymh, yriya__tlvse, op=MPI.SUM)
    oykp__qdj = yriya__tlvse != 0
    qop__vlh = lbcid__qssb != 0
    wzsrt__vkn = oykp__qdj & qop__vlh
    jah__lppob = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    jah__lppob[wzsrt__vkn] = 1 - lbcid__qssb[wzsrt__vkn] / yriya__tlvse[
        wzsrt__vkn]
    jah__lppob[qop__vlh & ~oykp__qdj] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            wbps__muviz = yriya__tlvse
            if not np.any(oykp__qdj):
                if not np.any(qop__vlh):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        wbps__muviz = multioutput
    return np.array([np.average(jah__lppob, weights=wbps__muviz)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    pkky__iotqy = MPI.COMM_WORLD
    try:
        lekzy__nldxz = sklearn.metrics.confusion_matrix(y_true, y_pred,
            labels=labels, sample_weight=sample_weight, normalize=None)
    except ValueError as wkzaa__vrllx:
        lekzy__nldxz = wkzaa__vrllx
    wugr__rhrdk = (isinstance(lekzy__nldxz, ValueError) and 
        'At least one label specified must be in y_true' in lekzy__nldxz.
        args[0])
    cfzma__xxpme = pkky__iotqy.allreduce(wugr__rhrdk, op=MPI.LAND)
    if cfzma__xxpme:
        raise lekzy__nldxz
    elif wugr__rhrdk:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        zxera__xxjdz = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        zxera__xxjdz = lekzy__nldxz
    ctbtj__mjun = np.zeros_like(zxera__xxjdz)
    pkky__iotqy.Allreduce(zxera__xxjdz, ctbtj__mjun)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            ctbtj__mjun = ctbtj__mjun / ctbtj__mjun.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            ctbtj__mjun = ctbtj__mjun / ctbtj__mjun.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            ctbtj__mjun = ctbtj__mjun / ctbtj__mjun.sum()
        ctbtj__mjun = np.nan_to_num(ctbtj__mjun)
    return ctbtj__mjun
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    cogu__pzz = 'def _confusion_matrix_impl(\n'
    cogu__pzz += '    y_true, y_pred, labels=None,\n'
    cogu__pzz += '    sample_weight=None, normalize=None,\n'
    cogu__pzz += '    _is_data_distributed=False,\n'
    cogu__pzz += '):\n'
    cogu__pzz += '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n'
    cogu__pzz += '    y_pred = bodo.utils.conversion.coerce_to_array(y_pred)\n'
    cogu__pzz += (
        '    y_true = bodo.utils.typing.decode_if_dict_array(y_true)\n')
    cogu__pzz += (
        '    y_pred = bodo.utils.typing.decode_if_dict_array(y_pred)\n')
    xbf__knebe = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        xbf__knebe = 'float64[:,:]', 'np.float64'
    ...
    if not is_overload_none(labels):
        ...
        cogu__pzz += '    labels = bodo.allgatherv(labels, False)\n'
        cogu__pzz += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    cogu__pzz += f"    with numba.objmode(cm='{xbf__knebe[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        cogu__pzz += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        cogu__pzz += '      cm = confusion_matrix_dist_helper(\n'
    cogu__pzz += '        y_true, y_pred, labels=labels,\n'
    cogu__pzz += '        sample_weight=sample_weight, normalize=normalize,\n'
    cogu__pzz += f'      ).astype({xbf__knebe[1]})\n'
    cogu__pzz += '    return cm\n'
    bxml__var = {}
    exec(cogu__pzz, globals(), bxml__var)
    sjfut__icsmj = bxml__var['_confusion_matrix_impl']
    return sjfut__icsmj
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    pkky__iotqy = MPI.COMM_WORLD
    mreq__vff = pkky__iotqy.allreduce(len(X), op=MPI.SUM)
    ylpf__kyyuj = len(X) / mreq__vff
    whqwt__odiuo = pkky__iotqy.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    cufl__zktei = np.inf
    bgwo__hrk = 0
    if m.loss == 'hinge':
        bbl__dwxl = hinge_loss
    elif m.loss == 'log':
        bbl__dwxl = log_loss
    elif m.loss == 'squared_error':
        bbl__dwxl = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    vmey__pkkko = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        vmey__pkkko = True
    for fylf__vezr in range(m.max_iter):
        if vmey__pkkko:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * ylpf__kyyuj
        m.coef_ = pkky__iotqy.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * ylpf__kyyuj
        m.intercept_ = pkky__iotqy.allreduce(m.intercept_, op=MPI.SUM)
        if vmey__pkkko:
            y_pred = m.predict(X)
            gklnq__wccvi = bbl__dwxl(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            gklnq__wccvi = bbl__dwxl(y, y_pred, labels=y_classes)
        kdidu__pqzf = pkky__iotqy.allreduce(gklnq__wccvi, op=MPI.SUM)
        gklnq__wccvi = kdidu__pqzf / whqwt__odiuo
        if m.tol > np.NINF and gklnq__wccvi > cufl__zktei - m.tol * mreq__vff:
            bgwo__hrk += 1
        else:
            bgwo__hrk = 0
        if gklnq__wccvi < cufl__zktei:
            cufl__zktei = gklnq__wccvi
        if bgwo__hrk >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    pkky__iotqy = MPI.COMM_WORLD
    rah__dkl = pkky__iotqy.Get_rank()
    lsti__utv = MPI.Get_processor_name()
    igpk__bqp = get_host_ranks()
    rpl__cjrbv = m.n_jobs if hasattr(m, 'n_jobs') else None
    utpwf__ywdh = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(igpk__bqp[lsti__utv])
    if rah__dkl == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if rah__dkl == 0:
        pkky__iotqy.bcast(m.cluster_centers_)
        pkky__iotqy.bcast(m.inertia_)
        pkky__iotqy.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = pkky__iotqy.bcast(None)
        m.inertia_ = pkky__iotqy.bcast(None)
        m.n_iter_ = pkky__iotqy.bcast(None)
    if _is_data_distributed:
        ftio__zxq = pkky__iotqy.allgather(len_X)
        if rah__dkl == 0:
            aoro__uky = np.empty(len(ftio__zxq) + 1, dtype=int)
            np.cumsum(ftio__zxq, out=aoro__uky[1:])
            aoro__uky[0] = 0
            xuxo__zao = [m.labels_[aoro__uky[soto__kjpoy]:aoro__uky[
                soto__kjpoy + 1]] for soto__kjpoy in range(len(ftio__zxq))]
            dfkoi__szd = pkky__iotqy.scatter(xuxo__zao)
        else:
            dfkoi__szd = pkky__iotqy.scatter(None)
        m.labels_ = dfkoi__szd
    elif rah__dkl == 0:
        pkky__iotqy.bcast(m.labels_)
    else:
        m.labels_ = pkky__iotqy.bcast(None)
    m._n_threads = utpwf__ywdh
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                pkky__iotqy = MPI.COMM_WORLD
                result = pkky__iotqy.allreduce(result, op=MPI.SUM)
            m._n_threads = utpwf__ywdh
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        cogu__pzz = 'def _model_multinomial_nb_fit_impl(\n'
        cogu__pzz += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        cogu__pzz += '):  # pragma: no cover\n'
        cogu__pzz += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            cogu__pzz += '    X = X.to_numpy()\n'
        else:
            cogu__pzz += '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n'
        cogu__pzz += '    my_rank = bodo.get_rank()\n'
        cogu__pzz += '    nranks = bodo.get_size()\n'
        cogu__pzz += '    total_cols = X.shape[1]\n'
        cogu__pzz += '    for i in range(nranks):\n'
        cogu__pzz += (
        ...
        cogu__pzz += '        if i == my_rank:\n'
        cogu__pzz += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        cogu__pzz += '        else:\n'
        cogu__pzz += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        cogu__pzz += '    y_train = bodo.allgatherv(y, False)\n'
        cogu__pzz += '    with numba.objmode(m="multinomial_nb_type"):\n'
        cogu__pzz += '        m = fit_multinomial_nb(\n'
        cogu__pzz += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        cogu__pzz += '        )\n'
        cogu__pzz += '    bodo.barrier()\n'
        cogu__pzz += '    return m\n'
        bxml__var = {}
        exec(cogu__pzz, globals(), bxml__var)
        fdwh__xccg = bxml__var['_model_multinomial_nb_fit_impl']
        return fdwh__xccg
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    fylf__vezr, n_features = X_train.shape
    m.n_features_in_ = n_features
    rlcqc__bsr = LabelBinarizer()
    skrp__rhb = rlcqc__bsr.fit_transform(y_train)
    m.classes_ = rlcqc__bsr.classes_
    if skrp__rhb.shape[1] == 1:
        skrp__rhb = np.concatenate((1 - skrp__rhb, skrp__rhb), axis=1)
    if sample_weight is not None:
        skrp__rhb = skrp__rhb.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        skrp__rhb *= sample_weight.T
    class_prior = m.class_prior
    hcb__depet = skrp__rhb.shape[1]
    m._init_counters(hcb__depet, n_features)
    m._count(X_train.astype('float64'), skrp__rhb)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    dhw__ntfk = m.feature_count_ + alpha
    ctdq__zuur = dhw__ntfk.sum(axis=1)
    pkky__iotqy = MPI.COMM_WORLD
    whqwt__odiuo = pkky__iotqy.Get_size()
    zwy__mnl = np.zeros(hcb__depet)
    pkky__iotqy.Allreduce(ctdq__zuur, zwy__mnl, op=MPI.SUM)
    vlx__ljp = np.log(dhw__ntfk) - np.log(zwy__mnl.reshape(-1, 1))
    gejo__srh = vlx__ljp.T.reshape(n_features * hcb__depet)
    cff__jurv = np.ones(whqwt__odiuo) * (total_cols // whqwt__odiuo)
    tywv__kpg = total_cols % whqwt__odiuo
    for ygv__jcla in range(tywv__kpg):
        cff__jurv[ygv__jcla] += 1
    cff__jurv *= hcb__depet
    qmt__fbg = np.zeros(whqwt__odiuo, dtype=np.int32)
    qmt__fbg[1:] = np.cumsum(cff__jurv)[:-1]
    saq__fdw = np.zeros((total_cols, hcb__depet), dtype=np.float64)
    pkky__iotqy.Allgatherv(gejo__srh, [saq__fdw, cff__jurv, qmt__fbg, MPI.
        DOUBLE_PRECISION])
    m.feature_log_prob_ = saq__fdw.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    pkky__iotqy = MPI.COMM_WORLD
    tmf__apl = pkky__iotqy.Get_size()
    xkbt__fkj = m.with_std
    wepdy__kqf = m.with_mean
    m.with_std = False
    if xkbt__fkj:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = xkbt__fkj
    m.with_mean = wepdy__kqf
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    awl__xmqa = np.zeros((tmf__apl, *m.n_samples_seen_.shape), dtype=m.
        n_samples_seen_.dtype)
    pkky__iotqy.Allgather(m.n_samples_seen_, awl__xmqa)
    yqotk__ohk = np.sum(awl__xmqa, axis=0)
    m.n_samples_seen_ = yqotk__ohk
    if m.with_mean or m.with_std:
        qqtd__wyip = np.zeros((tmf__apl, *m.mean_.shape), dtype=m.mean_.dtype)
        pkky__iotqy.Allgather(m.mean_, qqtd__wyip)
        qqtd__wyip[np.isnan(qqtd__wyip)] = 0
        jsrk__xezaj = np.average(qqtd__wyip, axis=0, weights=awl__xmqa)
        m.mean_ = jsrk__xezaj
    if m.with_std:
        wdae__cvuoc = sklearn_safe_accumulator_op(np.nansum, (X -
            jsrk__xezaj) ** 2, axis=0) / yqotk__ohk
        sxb__nug = np.zeros_like(wdae__cvuoc)
        pkky__iotqy.Allreduce(wdae__cvuoc, sxb__nug, op=MPI.SUM)
        m.var_ = sxb__nug
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    adq__rsbha = pkky__iotqy.allreduce(adq__rsbha, op=MPI.LAND)
    if adq__rsbha:
...
def get_data_slice_parallel(data, labels, len_train):
    cpv__ldc = data[:len_train]
    nct__kvhx = data[len_train:]
    cpv__ldc = bodo.rebalance(cpv__ldc)
    nct__kvhx = bodo.rebalance(nct__kvhx)
    rukcb__lcms = labels[:len_train]
    tvffq__cxefe = labels[len_train:]
    rukcb__lcms = bodo.rebalance(rukcb__lcms)
    tvffq__cxefe = bodo.rebalance(tvffq__cxefe)
    return cpv__ldc, nct__kvhx, rukcb__lcms, tvffq__cxefe
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    cvplu__uhr = {'stratify': stratify}
    ilbz__wlx = {'stratify': None}
    check_unsupported_args('train_test_split', cvplu__uhr, ilbz__wlx, 'ml')
    if is_overload_false(_is_data_distributed):
        nze__ooffd = f'data_split_type_{numba.core.ir_utils.next_label()}'
        vpe__vvh = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for pqyr__zioc, xbhc__hfhzl in ((data, nze__ooffd), (labels, vpe__vvh)
            ):
            if isinstance(pqyr__zioc, (DataFrameType, SeriesType)):
                zrtq__xck = pqyr__zioc.copy(index=NumericIndexType(types.int64)
                    )
                setattr(types, xbhc__hfhzl, zrtq__xck)
            else:
                setattr(types, xbhc__hfhzl, pqyr__zioc)
        cogu__pzz = 'def _train_test_split_impl(\n'
        cogu__pzz += '    data,\n'
        cogu__pzz += '    labels=None,\n'
        cogu__pzz += '    train_size=None,\n'
        cogu__pzz += '    test_size=None,\n'
        cogu__pzz += '    random_state=None,\n'
        cogu__pzz += '    shuffle=True,\n'
        cogu__pzz += '    stratify=None,\n'
        cogu__pzz += '    _is_data_distributed=False,\n'
        cogu__pzz += '):  # pragma: no cover\n'
        cogu__pzz += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(nze__ooffd, nze__ooffd, vpe__vvh, vpe__vvh))
        cogu__pzz += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        cogu__pzz += '            data,\n'
        cogu__pzz += '            labels,\n'
        cogu__pzz += '            train_size=train_size,\n'
        cogu__pzz += '            test_size=test_size,\n'
        cogu__pzz += '            random_state=random_state,\n'
        cogu__pzz += '            shuffle=shuffle,\n'
        cogu__pzz += '            stratify=stratify,\n'
        cogu__pzz += '        )\n'
        cogu__pzz += (
            '    return data_train, data_test, labels_train, labels_test\n')
        bxml__var = {}
        exec(cogu__pzz, globals(), bxml__var)
        _train_test_split_impl = bxml__var['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            gpyr__blyj = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(gpyr__blyj * train_size)
            asde__wajb = gpyr__blyj - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                rah__dkl = bodo.get_rank()
                whqwt__odiuo = bodo.get_size()
                syq__rlhzg = np.empty(whqwt__odiuo, np.int64)
                bodo.libs.distributed_api.allgather(syq__rlhzg, len(data))
                eube__rdug = np.cumsum(syq__rlhzg[0:rah__dkl + 1])
                lte__dautn = np.full(gpyr__blyj, True)
                lte__dautn[:asde__wajb] = False
                np.random.seed(42)
                np.random.permutation(lte__dautn)
                if rah__dkl:
                    ewa__lievq = eube__rdug[rah__dkl - 1]
                else:
                    ewa__lievq = 0
                rfng__sth = eube__rdug[rah__dkl]
                qahvs__kqii = lte__dautn[ewa__lievq:rfng__sth]
                cpv__ldc = data[qahvs__kqii]
                nct__kvhx = data[~qahvs__kqii]
                rukcb__lcms = labels[qahvs__kqii]
                tvffq__cxefe = labels[~qahvs__kqii]
                cpv__ldc = bodo.random_shuffle(cpv__ldc, seed=random_state,
                    parallel=True)
                nct__kvhx = bodo.random_shuffle(nct__kvhx, seed=
                    random_state, parallel=True)
                rukcb__lcms = bodo.random_shuffle(rukcb__lcms, seed=
                    random_state, parallel=True)
                tvffq__cxefe = bodo.random_shuffle(tvffq__cxefe, seed=
                    random_state, parallel=True)
                rukcb__lcms = reset_labels_type(rukcb__lcms, label_type)
                tvffq__cxefe = reset_labels_type(tvffq__cxefe, label_type)
            else:
                cpv__ldc, nct__kvhx, rukcb__lcms, tvffq__cxefe = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    tmf__apl = pkky__iotqy.Get_size()
    m = m.fit(X)
    yqotk__ohk = pkky__iotqy.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = yqotk__ohk
    vqp__ihhdt = np.zeros((tmf__apl, *m.data_min_.shape), dtype=m.data_min_
        .dtype)
    pkky__iotqy.Allgather(m.data_min_, vqp__ihhdt)
    asp__rzhkp = np.nanmin(vqp__ihhdt, axis=0)
    pulrh__bhu = np.zeros((tmf__apl, *m.data_max_.shape), dtype=m.data_max_
        .dtype)
    pkky__iotqy.Allgather(m.data_max_, pulrh__bhu)
    psb__wtldx = np.nanmax(pulrh__bhu, axis=0)
    fih__oru = psb__wtldx - asp__rzhkp
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(fih__oru)
    m.min_ = m.feature_range[0] - asp__rzhkp * m.scale_
    m.data_min_ = asp__rzhkp
    m.data_max_ = psb__wtldx
    m.data_range_ = fih__oru
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    sbplz__hqtbl = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        sbplz__hqtbl = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{sbplz__hqtbl}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            qcry__iiguv = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if qcry__iiguv > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=qcry__iiguv)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        wnx__oiqb = True
    return wnx__oiqb, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                idon__eokmq = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                idon__eokmq = bodo.allgatherv(idon__eokmq, False)
                idon__eokmq = bodo.libs.array_kernels.sort(idon__eokmq,
                    ascending=True, inplace=True)
                tkbt__sjznx = {}
                for ggpmd__nzo in range(len(idon__eokmq)):
                    tkbt__sjznx[idon__eokmq[ggpmd__nzo]] = ggpmd__nzo
            else:
                tkbt__sjznx = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = tkbt__sjznx
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X

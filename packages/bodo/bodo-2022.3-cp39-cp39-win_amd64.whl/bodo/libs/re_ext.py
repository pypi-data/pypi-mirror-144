...
def box_regex_flag(typ, val, c):
    qttjs__ijsv = c.box(types.int64, val)
    gcght__nvo = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(gcght__nvo, (qttjs__ijsv,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    qttjs__ijsv = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, qttjs__ijsv)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    keeg__rmea = context.get_python_api(builder)
    qwg__vccoc = builder.icmp_signed('==', val, keeg__rmea.borrow_none())
    with builder.if_then(qwg__vccoc):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    jjk__jpl = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(jjk__jpl)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        ryw__nksrk = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if ryw__nksrk > 1:
            typ = types.List(types.Tuple([string_type] * ryw__nksrk))
...
def overload_match_group(m, *args):
    ...
    lwac__avd = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            mxa__qnea = args[0]
            with numba.objmode(out=lwac__avd):
                out = m.group(mxa__qnea)
            return out
        return _match_group_impl_one
    uqc__dfdl = 'tuple_str_{}'.format(len(args))
    setattr(types, uqc__dfdl, types.Tuple([lwac__avd] * len(args)))
    asswo__tfok = ', '.join('group{}'.format(yomu__vrzbg + 1) for
        yomu__vrzbg in range(len(args)))
    zzi__zsbtl = 'def _match_group_impl(m, *args):\n'
    zzi__zsbtl += '  ({}) = args\n'.format(asswo__tfok)
    zzi__zsbtl += "  with numba.objmode(out='{}'):\n".format(uqc__dfdl)
    zzi__zsbtl += '    out = m.group({})\n'.format(asswo__tfok)
    zzi__zsbtl += '  return out\n'
    keebi__pgg = {}
    exec(zzi__zsbtl, globals(), keebi__pgg)
    ngmcp__pnfcc = keebi__pgg['_match_group_impl']
    return ngmcp__pnfcc
...
def overload_match_groups(m, default=None):
    grvzy__pjj = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=grvzy__pjj):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(cpxgx__vyat is None for cpxgx__vyat in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=lwac__avd):

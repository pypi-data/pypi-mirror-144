...
def install_mpl_class(types_name, python_type):
    emj__nqv = ''.join(map(str.title, types_name.split('_')))
    slz__qbgac = f'class {emj__nqv}(types.Opaque):\n'
    slz__qbgac += '    def __init__(self):\n'
    slz__qbgac += f"       types.Opaque.__init__(self, name='{emj__nqv}')\n"
    slz__qbgac += '    def __reduce__(self):\n'
    slz__qbgac += (
        f"        return (types.Opaque, ('{emj__nqv}',), self.__dict__)\n")
    fiija__axcmr = {}
    exec(slz__qbgac, {'types': types, 'bodo': bodo}, fiija__axcmr)
    osx__oxw = fiija__axcmr[emj__nqv]
    lnjn__gjyu = sys.modules[__name__]
    setattr(lnjn__gjyu, emj__nqv, osx__oxw)
    class_instance = osx__oxw()
    setattr(types, types_name, class_instance)
    register_model(osx__oxw)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(osx__oxw)(unbox_mpl_obj)
    box(osx__oxw)(box_mpl_obj)
...
def _install_mpl_types():
    jhcd__vsdiy = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for pyow__wmh, lnho__ecg in jhcd__vsdiy:
        install_mpl_class(pyow__wmh, lnho__ecg)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    vsu__nhth = ', '.join(f'e{hzhwf__uqwzx}' for hzhwf__uqwzx in range(len(
        args)))
    if vsu__nhth:
        vsu__nhth += ', '
    vxyl__cxn = ', '.join(f"{naxrv__tkosy} = ''" for naxrv__tkosy in kws.keys()
        )
    wlwud__qxm = 'matplotlib_obj, ' if obj_typ is not None else ''
    zpqfy__yjr = f'def mpl_stub({wlwud__qxm} {vsu__nhth} {vxyl__cxn}):\n'
    zpqfy__yjr += '    pass\n'
    qbzug__gkk = {}
    exec(zpqfy__yjr, {}, qbzug__gkk)
    amfe__fuxh = qbzug__gkk['mpl_stub']
    yfrq__zra = numba.core.utils.pysignature(amfe__fuxh)
    cjsk__brsa = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(cjsk__brsa)).replace(pysig=
        yfrq__zra)


def generate_axes_typing(mod_name, nrows, ncols):
    quv__wsimd = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(quv__wsimd.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(quv__wsimd.format(mod_name, 'ncols'))
    ktot__ghmh = get_overload_const_int(nrows)
    vjj__wgpmo = get_overload_const_int(ncols)
    if ktot__ghmh < 1:
        raise BodoError(quv__wsimd.format(mod_name, 'nrows'))
    if vjj__wgpmo < 1:
        raise BodoError(quv__wsimd.format(mod_name, 'ncols'))
    if ktot__ghmh == 1 and vjj__wgpmo == 1:
        xqn__okl = types.mpl_axes_type
    else:
        if vjj__wgpmo == 1:
            ghv__pmmle = types.mpl_axes_type
        else:
            ghv__pmmle = types.Tuple([types.mpl_axes_type] * vjj__wgpmo)
        xqn__okl = types.Tuple([ghv__pmmle] * ktot__ghmh)
    return xqn__okl


def generate_pie_return_type(args, kws):
    udx__vonh = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if udx__vonh == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    tnzy__qzz = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(tnzy__qzz):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(tnzy__qzz):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        cxyg__bue = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, cxyg__bue]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    cxyg__bue = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    pyow__wmh = str(cxyg__bue)
    if not hasattr(types, pyow__wmh):
        pyow__wmh = f'objmode_type{ir_utils.next_label()}'
        setattr(types, pyow__wmh, cxyg__bue)
    zpqfy__yjr = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{pyow__wmh}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    qbzug__gkk = {}
    exec(zpqfy__yjr, {'numba': numba, 'np': np}, qbzug__gkk)
    impl = qbzug__gkk['impl']
    return impl

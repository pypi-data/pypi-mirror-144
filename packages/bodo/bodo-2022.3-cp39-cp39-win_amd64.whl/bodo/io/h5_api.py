...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        zeh__bvel = []
        zyvz__acib = h5g_get_num_objs(obj_id)
        for sge__blg in range(zyvz__acib):
            xjhx__qvseu = h5g_get_objname_by_idx(obj_id, sge__blg)
            zeh__bvel.append(xjhx__qvseu)
        return zeh__bvel
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    xcen__yuuz = parse_dtype(dtype)
    dbfnc__kmyig = np.int32(numba_to_c_type(xcen__yuuz))
    ...
    def impl(obj_id, name, shape=None, dtype=None, data=None):
        ...
        return h5_create_dset(unify_h5_id(obj_id), unicode_to_utf8(name),
            ndim, counts.ctypes, dbfnc__kmyig)
    return impl
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            oeyu__jaqh = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    arm__vnwg = tuple_to_ptr(starts)
    wwk__acslp = tuple_to_ptr(counts)
    oug__zzk = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, arm__vnwg, wwk__acslp,
        is_parallel, out_arr.ctypes, oug__zzk)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    wwk__acslp = tuple_to_ptr(counts)
    oug__zzk = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, arm__vnwg, wwk__acslp,
        is_parallel, out_arr.ctypes, oug__zzk)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        blxo__ybrd = types.Array(dtype, ndim, 'C')
        qihez__pcl = args[3]
        blxo__ybrd = self.context.resolve_function_type(operator.getitem, [
            blxo__ybrd, qihez__pcl], {}).return_type.copy(layout='C')
        return signature(blxo__ybrd, *args)
...
def get_filter_read_indices(bool_arr):
    xxcty__zva = bool_arr.nonzero()[0]
    pbn__tswtt = bodo.libs.distributed_api.get_rank()
    zvr__apnol = bodo.libs.distributed_api.get_size()
    furoa__oua = np.empty(zvr__apnol, np.int64)
    kyvd__tycf = len(bool_arr)
    bodo.libs.distributed_api.allgather(furoa__oua, kyvd__tycf)
    fga__pryj = furoa__oua.cumsum()[pbn__tswtt] - kyvd__tycf
    xxcty__zva += fga__pryj
    gbt__osax = bodo.libs.distributed_api.dist_reduce(len(xxcty__zva), np.
        int32(sum_op))
    lgvl__uvpxs = bodo.libs.distributed_api.gatherv(xxcty__zva)
    if pbn__tswtt == 0:
        xruzd__ghco = lgvl__uvpxs
    else:
        xruzd__ghco = np.empty(gbt__osax, xxcty__zva.dtype)
    bodo.libs.distributed_api.bcast(xruzd__ghco)
    jls__muc = bodo.libs.distributed_api.get_start(gbt__osax, zvr__apnol,
        pbn__tswtt)
    gybwm__mhy = bodo.libs.distributed_api.get_end(gbt__osax, zvr__apnol,
        pbn__tswtt)
    return xruzd__ghco[jls__muc:gybwm__mhy]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        gitf__dvool = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], gitf__dvool)
        return builder.bitcast(gitf__dvool, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    wwk__acslp = tuple_to_ptr(counts)
    oug__zzk = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, arm__vnwg,
        wwk__acslp, is_parallel, out_arr.ctypes, oug__zzk, read_indices.

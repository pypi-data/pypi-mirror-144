...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        eacx__vba = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, eacx__vba)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            zudso__kreri = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(zudso__kreri.dtype)
            file_write(unicode_to_utf8(fname), zudso__kreri.ctypes, 
                dtype_size * zudso__kreri.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            zudso__kreri = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(zudso__kreri.dtype)
            zxkyo__qpibs = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(zudso__kreri.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), zudso__kreri.
                ctypes, start, count, zxkyo__qpibs)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            ygd__mwakl = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                ygd__mwakl = min(ygd__mwakl, count * dtype_size)
            if ygd__mwakl < 0:
                return -1
            return ygd__mwakl
        return impl

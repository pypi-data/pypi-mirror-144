...
def _global_except_hook(exctype, value, traceback):
    ...
    wgos__bfo = MPI.COMM_WORLD
    ctyk__oayi = wgos__bfo.Get_rank()
    tpo__nsvb = 3.0
    jgts__jjzp = True
    qpq__yvck = wgos__bfo.Ibarrier()
    cdxru__jpctp = time.time()
    while time.time() - cdxru__jpctp < tpo__nsvb:
        time.sleep(0.1)
        if qpq__yvck.Test():
            jgts__jjzp = False
            break
    try:
        ...
        if jgts__jjzp:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {ctyk__oayi}. \n')

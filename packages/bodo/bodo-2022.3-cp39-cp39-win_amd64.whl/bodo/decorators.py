...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        dec__kbzb = self.overloads[sig]
        dzjvh__jiz = dec__kbzb.metadata.get('distributed_diagnostics', None)
        if dzjvh__jiz is None:
            udh__kitx = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(udh__kitx)
        dzjvh__jiz.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        xvdk__ceq = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(xvdk__ceq)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        erb__mdnhn = options.pop('distributed')
        pipeline_class = (pipeline_class if erb__mdnhn else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        vgsr__ljvl = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if vgsr__ljvl else
            pipeline_class)
    mtwr__nyd = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(mtwr__nyd, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(mtwr__nyd)
        else:
            return master_mode_wrapper(mtwr__nyd)
    else:
        return mtwr__nyd

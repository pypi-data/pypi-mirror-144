...
except ImportError as jfoum__ldg:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        rhjh__jehfk = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(rhjh__jehfk)


def random_forest_model_fit(m, X, y):
    syo__erzzt = m.n_estimators
    toekw__fky = MPI.Get_processor_name()
    shczn__bejsb = get_host_ranks()
    yqm__mhp = len(shczn__bejsb)
    mje__roi = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(syo__erzzt,
        yqm__mhp, mje__roi)
    if mje__roi == shczn__bejsb[toekw__fky][0]:
        m.n_jobs = len(shczn__bejsb[toekw__fky])
    ...
    shcl__sxoum = create_subcomm_mpi4py(first_rank_node)
    if shcl__sxoum != MPI.COMM_NULL:
        kjgg__dypj = 10
        zoav__jfn = bodo.libs.distributed_api.get_node_portion(syo__erzzt,
            yqm__mhp, 0)
        nvcpl__xezwl = zoav__jfn // kjgg__dypj
        if zoav__jfn % kjgg__dypj != 0:
            nvcpl__xezwl += 1
        koi__bsh = []
        for uqdb__zon in range(nvcpl__xezwl):
            vpac__veoo = shcl__sxoum.gather(m.estimators_[uqdb__zon *
                kjgg__dypj:uqdb__zon * kjgg__dypj + kjgg__dypj])
            if mje__roi == 0:
                koi__bsh += list(itertools.chain.from_iterable(vpac__veoo))
        if mje__roi == 0:
            m.estimators_ = koi__bsh
    iii__nrsa = MPI.COMM_WORLD
    if mje__roi == 0:
        for uqdb__zon in range(0, syo__erzzt, 10):
            iii__nrsa.bcast(m.estimators_[uqdb__zon:uqdb__zon + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            iii__nrsa.bcast(m.n_classes_)
            iii__nrsa.bcast(m.classes_)
        iii__nrsa.bcast(m.n_outputs_)
    else:
        rdkot__nluom = []
        for uqdb__zon in range(0, syo__erzzt, 10):
            rdkot__nluom += iii__nrsa.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = iii__nrsa.bcast(None)
            m.classes_ = iii__nrsa.bcast(None)
        m.n_outputs_ = iii__nrsa.bcast(None)
        m.estimators_ = rdkot__nluom
    assert len(m.estimators_) == syo__erzzt
    m.n_estimators = syo__erzzt
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    cydpd__yhy = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    cbz__shrq = len(labels)
    esw__qrjl = np.zeros(cbz__shrq, np.int64)
    jzvu__mcbm = np.zeros(cbz__shrq, np.int64)
    ensdm__vlunn = np.zeros(cbz__shrq, np.int64)
    bzrt__woyyy = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for uqdb__zon in range(len(y_true)):
        jzvu__mcbm[bzrt__woyyy[y_true[uqdb__zon]]] += 1
        if y_pred[uqdb__zon] not in bzrt__woyyy:
            continue
        fps__kha = bzrt__woyyy[y_pred[uqdb__zon]]
        ensdm__vlunn[fps__kha] += 1
        if y_true[uqdb__zon] == y_pred[uqdb__zon]:
            esw__qrjl[fps__kha] += 1
    esw__qrjl = bodo.libs.distributed_api.dist_reduce(esw__qrjl, np.int32(
        Reduce_Type.Sum.value))
    jzvu__mcbm = bodo.libs.distributed_api.dist_reduce(jzvu__mcbm, np.int32
        (Reduce_Type.Sum.value))
    ensdm__vlunn = bodo.libs.distributed_api.dist_reduce(ensdm__vlunn, np.
        int32(Reduce_Type.Sum.value))
    dqbsp__icq = ensdm__vlunn - esw__qrjl
    wcdfb__nsx = jzvu__mcbm - esw__qrjl
    wuyqd__etn = esw__qrjl
    ska__hcsut = y_true.shape[0] - wuyqd__etn - dqbsp__icq - wcdfb__nsx
    with numba.objmode(result='float64[:]'):
        MCM = np.array([ska__hcsut, dqbsp__icq, wcdfb__nsx, wuyqd__etn]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        lrj__gpjno = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        lrj__gpjno = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    iii__nrsa = MPI.COMM_WORLD
    wglzd__cwb = iii__nrsa.Get_size()
    if sample_weight is not None:
        ver__vbeam = np.sum(sample_weight)
    else:
        ver__vbeam = np.float64(y_true.shape[0])
    olbn__evkqf = np.zeros(wglzd__cwb, dtype=type(ver__vbeam))
    iii__nrsa.Allgather(ver__vbeam, olbn__evkqf)
    fopdt__ibq = np.zeros((wglzd__cwb, *lrj__gpjno.shape), dtype=lrj__gpjno
        .dtype)
    iii__nrsa.Allgather(lrj__gpjno, fopdt__ibq)
    yevz__uhrlm = np.average(fopdt__ibq, weights=olbn__evkqf, axis=0)
    if metric == 'mse' and not squared:
        yevz__uhrlm = np.sqrt(yevz__uhrlm)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return yevz__uhrlm
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(yevz__uhrlm)
    else:
        return np.average(yevz__uhrlm, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    iii__nrsa = MPI.COMM_WORLD
    score = iii__nrsa.allreduce(score, op=MPI.SUM)
    if normalize:
        iqb__cvop = np.sum(sample_weight
            ) if sample_weight is not None else len(y_true)
        iqb__cvop = iii__nrsa.allreduce(iqb__cvop, op=MPI.SUM)
        score = score / iqb__cvop
    return score
...
def check_consistent_length_parallel(*arrays):
    iii__nrsa = MPI.COMM_WORLD
    kokga__gesic = True
    syl__lgsqj = [len(oatcp__mhe) for oatcp__mhe in arrays if oatcp__mhe is not
        None]
    if len(np.unique(syl__lgsqj)) > 1:
        kokga__gesic = False
    kokga__gesic = iii__nrsa.allreduce(kokga__gesic, op=MPI.LAND)
    return kokga__gesic
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    zlvrw__zfnu = y_true.shape[0]
    nat__pmu = iii__nrsa.allreduce(zlvrw__zfnu, op=MPI.SUM)
    if nat__pmu < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        fjb__dlw = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        fjb__dlw = 1.0
    gobu__hmkxa = (fjb__dlw * (y_true - y_pred) ** 2).sum(axis=0, dtype=np.
        float64)
    fqcx__diw = np.zeros(gobu__hmkxa.shape, dtype=gobu__hmkxa.dtype)
    iii__nrsa.Allreduce(gobu__hmkxa, fqcx__diw, op=MPI.SUM)
    fguas__nwcsa = np.nansum(y_true * fjb__dlw, axis=0, dtype=np.float64)
    vtobh__ktlg = np.zeros_like(fguas__nwcsa)
    iii__nrsa.Allreduce(fguas__nwcsa, vtobh__ktlg, op=MPI.SUM)
    koo__mpkli = np.nansum(sample_weight, dtype=np.float64)
    fscjd__ofc = iii__nrsa.allreduce(koo__mpkli, op=MPI.SUM)
    mnzov__mcr = vtobh__ktlg / fscjd__ofc
    yjtk__tweeb = (fjb__dlw * (y_true - mnzov__mcr) ** 2).sum(axis=0, dtype
        =np.float64)
    ptm__cbvry = np.zeros(yjtk__tweeb.shape, dtype=yjtk__tweeb.dtype)
    iii__nrsa.Allreduce(yjtk__tweeb, ptm__cbvry, op=MPI.SUM)
    uwpb__qsapo = ptm__cbvry != 0
    wwymk__rhlge = fqcx__diw != 0
    xxs__zmlw = uwpb__qsapo & wwymk__rhlge
    afciz__qzv = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    afciz__qzv[xxs__zmlw] = 1 - fqcx__diw[xxs__zmlw] / ptm__cbvry[xxs__zmlw]
    afciz__qzv[wwymk__rhlge & ~uwpb__qsapo] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            nrcmi__jxo = ptm__cbvry
            if not np.any(uwpb__qsapo):
                if not np.any(wwymk__rhlge):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        nrcmi__jxo = multioutput
    return np.array([np.average(afciz__qzv, weights=nrcmi__jxo)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    iii__nrsa = MPI.COMM_WORLD
    try:
        wac__jvq = sklearn.metrics.confusion_matrix(y_true, y_pred, labels=
            labels, sample_weight=sample_weight, normalize=None)
    except ValueError as psrm__xfbi:
        wac__jvq = psrm__xfbi
    yji__wwyow = isinstance(wac__jvq, ValueError
        ) and 'At least one label specified must be in y_true' in wac__jvq.args[
        0]
    zbet__ymkdl = iii__nrsa.allreduce(yji__wwyow, op=MPI.LAND)
    if zbet__ymkdl:
        raise wac__jvq
    elif yji__wwyow:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        gvd__gfdr = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        gvd__gfdr = wac__jvq
    ldr__isu = np.zeros_like(gvd__gfdr)
    iii__nrsa.Allreduce(gvd__gfdr, ldr__isu)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            ldr__isu = ldr__isu / ldr__isu.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            ldr__isu = ldr__isu / ldr__isu.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            ldr__isu = ldr__isu / ldr__isu.sum()
        ldr__isu = np.nan_to_num(ldr__isu)
    return ldr__isu
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    gunyh__gxfxd = 'def _confusion_matrix_impl(\n'
    gunyh__gxfxd += '    y_true, y_pred, labels=None,\n'
    gunyh__gxfxd += '    sample_weight=None, normalize=None,\n'
    gunyh__gxfxd += '    _is_data_distributed=False,\n'
    gunyh__gxfxd += '):\n'
    gunyh__gxfxd += (
        '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n')
    gunyh__gxfxd += (
    ...
    hlcf__ciq = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        hlcf__ciq = 'float64[:,:]', 'np.float64'
    if not is_overload_none(sample_weight):
        gunyh__gxfxd += """    sample_weight = bodo.utils.conversion.coerce_to_array(sample_weight)
"""
    ...
    if not is_overload_none(labels):
        ...
        gunyh__gxfxd += '    labels = bodo.allgatherv(labels, False)\n'
        gunyh__gxfxd += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    gunyh__gxfxd += f"    with numba.objmode(cm='{hlcf__ciq[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        gunyh__gxfxd += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        gunyh__gxfxd += '      cm = confusion_matrix_dist_helper(\n'
    gunyh__gxfxd += '        y_true, y_pred, labels=labels,\n'
    gunyh__gxfxd += (
        '        sample_weight=sample_weight, normalize=normalize,\n')
    gunyh__gxfxd += f'      ).astype({hlcf__ciq[1]})\n'
    gunyh__gxfxd += '    return cm\n'
    nub__fawe = {}
    exec(gunyh__gxfxd, globals(), nub__fawe)
    ytxv__tyvc = nub__fawe['_confusion_matrix_impl']
    return ytxv__tyvc
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    iii__nrsa = MPI.COMM_WORLD
    gpukw__gafp = iii__nrsa.allreduce(len(X), op=MPI.SUM)
    ulf__dhqde = len(X) / gpukw__gafp
    stg__oevir = iii__nrsa.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    dnxpv__lrsx = np.inf
    oge__fqnc = 0
    if m.loss == 'hinge':
        jnp__petos = hinge_loss
    elif m.loss == 'log':
        jnp__petos = log_loss
    elif m.loss == 'squared_error':
        jnp__petos = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    seo__ptgt = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        seo__ptgt = True
    for pcjob__rbs in range(m.max_iter):
        if seo__ptgt:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * ulf__dhqde
        m.coef_ = iii__nrsa.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * ulf__dhqde
        m.intercept_ = iii__nrsa.allreduce(m.intercept_, op=MPI.SUM)
        if seo__ptgt:
            y_pred = m.predict(X)
            ypud__bgx = jnp__petos(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            ypud__bgx = jnp__petos(y, y_pred, labels=y_classes)
        hvc__iwhkz = iii__nrsa.allreduce(ypud__bgx, op=MPI.SUM)
        ypud__bgx = hvc__iwhkz / stg__oevir
        if m.tol > np.NINF and ypud__bgx > dnxpv__lrsx - m.tol * gpukw__gafp:
            oge__fqnc += 1
        else:
            oge__fqnc = 0
        if ypud__bgx < dnxpv__lrsx:
            dnxpv__lrsx = ypud__bgx
        if oge__fqnc >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    iii__nrsa = MPI.COMM_WORLD
    mje__roi = iii__nrsa.Get_rank()
    toekw__fky = MPI.Get_processor_name()
    shczn__bejsb = get_host_ranks()
    pmqb__tdhky = m.n_jobs if hasattr(m, 'n_jobs') else None
    vpac__bwl = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(shczn__bejsb[toekw__fky])
    if mje__roi == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if mje__roi == 0:
        iii__nrsa.bcast(m.cluster_centers_)
        iii__nrsa.bcast(m.inertia_)
        iii__nrsa.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = iii__nrsa.bcast(None)
        m.inertia_ = iii__nrsa.bcast(None)
        m.n_iter_ = iii__nrsa.bcast(None)
    if _is_data_distributed:
        zxevr__ecaoj = iii__nrsa.allgather(len_X)
        if mje__roi == 0:
            xrnzj__hkxm = np.empty(len(zxevr__ecaoj) + 1, dtype=int)
            np.cumsum(zxevr__ecaoj, out=xrnzj__hkxm[1:])
            xrnzj__hkxm[0] = 0
            eoza__osesm = [m.labels_[xrnzj__hkxm[ryu__wmsyt]:xrnzj__hkxm[
                ryu__wmsyt + 1]] for ryu__wmsyt in range(len(zxevr__ecaoj))]
            egm__zwffp = iii__nrsa.scatter(eoza__osesm)
        else:
            egm__zwffp = iii__nrsa.scatter(None)
        m.labels_ = egm__zwffp
    elif mje__roi == 0:
        iii__nrsa.bcast(m.labels_)
    else:
        m.labels_ = iii__nrsa.bcast(None)
    m._n_threads = vpac__bwl
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                iii__nrsa = MPI.COMM_WORLD
                result = iii__nrsa.allreduce(result, op=MPI.SUM)
            m._n_threads = vpac__bwl
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        gunyh__gxfxd = 'def _model_multinomial_nb_fit_impl(\n'
        gunyh__gxfxd += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        gunyh__gxfxd += '):  # pragma: no cover\n'
        gunyh__gxfxd += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            gunyh__gxfxd += '    X = X.to_numpy()\n'
        else:
            gunyh__gxfxd += (
                '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n')
        gunyh__gxfxd += '    my_rank = bodo.get_rank()\n'
        gunyh__gxfxd += '    nranks = bodo.get_size()\n'
        gunyh__gxfxd += '    total_cols = X.shape[1]\n'
        gunyh__gxfxd += '    for i in range(nranks):\n'
        gunyh__gxfxd += """        start = bodo.libs.distributed_api.get_start(total_cols, nranks, i)
"""
        gunyh__gxfxd += (
            '        end = bodo.libs.distributed_api.get_end(total_cols, nranks, i)\n'
            )
        gunyh__gxfxd += '        if i == my_rank:\n'
        gunyh__gxfxd += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        gunyh__gxfxd += '        else:\n'
        gunyh__gxfxd += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        gunyh__gxfxd += '    y_train = bodo.allgatherv(y, False)\n'
        gunyh__gxfxd += '    with numba.objmode(m="multinomial_nb_type"):\n'
        gunyh__gxfxd += '        m = fit_multinomial_nb(\n'
        gunyh__gxfxd += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        gunyh__gxfxd += '        )\n'
        gunyh__gxfxd += '    bodo.barrier()\n'
        gunyh__gxfxd += '    return m\n'
        nub__fawe = {}
        exec(gunyh__gxfxd, globals(), nub__fawe)
        xnmo__yywy = nub__fawe['_model_multinomial_nb_fit_impl']
        return xnmo__yywy
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    pcjob__rbs, n_features = X_train.shape
    m.n_features_in_ = n_features
    ntfx__mxc = LabelBinarizer()
    aiobj__mgzg = ntfx__mxc.fit_transform(y_train)
    m.classes_ = ntfx__mxc.classes_
    if aiobj__mgzg.shape[1] == 1:
        aiobj__mgzg = np.concatenate((1 - aiobj__mgzg, aiobj__mgzg), axis=1)
    if sample_weight is not None:
        aiobj__mgzg = aiobj__mgzg.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        aiobj__mgzg *= sample_weight.T
    class_prior = m.class_prior
    utors__rjqau = aiobj__mgzg.shape[1]
    m._init_counters(utors__rjqau, n_features)
    m._count(X_train.astype('float64'), aiobj__mgzg)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    amcvv__uip = m.feature_count_ + alpha
    who__gvbfq = amcvv__uip.sum(axis=1)
    iii__nrsa = MPI.COMM_WORLD
    stg__oevir = iii__nrsa.Get_size()
    aduze__ezpwa = np.zeros(utors__rjqau)
    iii__nrsa.Allreduce(who__gvbfq, aduze__ezpwa, op=MPI.SUM)
    rtoy__pjqay = np.log(amcvv__uip) - np.log(aduze__ezpwa.reshape(-1, 1))
    etkzh__ehlz = rtoy__pjqay.T.reshape(n_features * utors__rjqau)
    ruxgp__uwd = np.ones(stg__oevir) * (total_cols // stg__oevir)
    xpmke__vjst = total_cols % stg__oevir
    for abist__phrm in range(xpmke__vjst):
        ruxgp__uwd[abist__phrm] += 1
    ruxgp__uwd *= utors__rjqau
    bnonf__gfie = np.zeros(stg__oevir, dtype=np.int32)
    bnonf__gfie[1:] = np.cumsum(ruxgp__uwd)[:-1]
    rig__zxnaq = np.zeros((total_cols, utors__rjqau), dtype=np.float64)
    iii__nrsa.Allgatherv(etkzh__ehlz, [rig__zxnaq, ruxgp__uwd, bnonf__gfie,
        MPI.DOUBLE_PRECISION])
    m.feature_log_prob_ = rig__zxnaq.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    iii__nrsa = MPI.COMM_WORLD
    wglzd__cwb = iii__nrsa.Get_size()
    nku__wrsi = m.with_std
    reto__ykg = m.with_mean
    m.with_std = False
    if nku__wrsi:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = nku__wrsi
    m.with_mean = reto__ykg
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    ffg__sjko = np.zeros((wglzd__cwb, *m.n_samples_seen_.shape), dtype=m.
        n_samples_seen_.dtype)
    iii__nrsa.Allgather(m.n_samples_seen_, ffg__sjko)
    hhc__vhjr = np.sum(ffg__sjko, axis=0)
    m.n_samples_seen_ = hhc__vhjr
    if m.with_mean or m.with_std:
        kuali__gqtcs = np.zeros((wglzd__cwb, *m.mean_.shape), dtype=m.mean_
            .dtype)
        iii__nrsa.Allgather(m.mean_, kuali__gqtcs)
        kuali__gqtcs[np.isnan(kuali__gqtcs)] = 0
        wdzsa__tal = np.average(kuali__gqtcs, axis=0, weights=ffg__sjko)
        m.mean_ = wdzsa__tal
    if m.with_std:
        lxyuc__dkwd = sklearn_safe_accumulator_op(np.nansum, (X -
            wdzsa__tal) ** 2, axis=0) / hhc__vhjr
        musr__khlee = np.zeros_like(lxyuc__dkwd)
        iii__nrsa.Allreduce(lxyuc__dkwd, musr__khlee, op=MPI.SUM)
        m.var_ = musr__khlee
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    rms__nnza = iii__nrsa.allreduce(rms__nnza, op=MPI.LAND)
    if rms__nnza:
...
def get_data_slice_parallel(data, labels, len_train):
    pdsb__bqr = data[:len_train]
    gyk__qmkqy = data[len_train:]
    pdsb__bqr = bodo.rebalance(pdsb__bqr)
    gyk__qmkqy = bodo.rebalance(gyk__qmkqy)
    afgh__rpi = labels[:len_train]
    mpvaf__owk = labels[len_train:]
    afgh__rpi = bodo.rebalance(afgh__rpi)
    mpvaf__owk = bodo.rebalance(mpvaf__owk)
    return pdsb__bqr, gyk__qmkqy, afgh__rpi, mpvaf__owk
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    ism__mquf = {'stratify': stratify}
    lpkj__lxhtu = {'stratify': None}
    check_unsupported_args('train_test_split', ism__mquf, lpkj__lxhtu, 'ml')
    if is_overload_false(_is_data_distributed):
        raeqd__zjp = f'data_split_type_{numba.core.ir_utils.next_label()}'
        fuc__omzp = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for lrjj__ttjhz, gflir__lnmel in ((data, raeqd__zjp), (labels,
            fuc__omzp)):
            if isinstance(lrjj__ttjhz, (DataFrameType, SeriesType)):
                jhkrd__ocdf = lrjj__ttjhz.copy(index=NumericIndexType(types
                    .int64))
                setattr(types, gflir__lnmel, jhkrd__ocdf)
            else:
                setattr(types, gflir__lnmel, lrjj__ttjhz)
        gunyh__gxfxd = 'def _train_test_split_impl(\n'
        gunyh__gxfxd += '    data,\n'
        gunyh__gxfxd += '    labels=None,\n'
        gunyh__gxfxd += '    train_size=None,\n'
        gunyh__gxfxd += '    test_size=None,\n'
        gunyh__gxfxd += '    random_state=None,\n'
        gunyh__gxfxd += '    shuffle=True,\n'
        gunyh__gxfxd += '    stratify=None,\n'
        gunyh__gxfxd += '    _is_data_distributed=False,\n'
        gunyh__gxfxd += '):  # pragma: no cover\n'
        gunyh__gxfxd += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(raeqd__zjp, raeqd__zjp, fuc__omzp, fuc__omzp))
        gunyh__gxfxd += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        gunyh__gxfxd += '            data,\n'
        gunyh__gxfxd += '            labels,\n'
        gunyh__gxfxd += '            train_size=train_size,\n'
        gunyh__gxfxd += '            test_size=test_size,\n'
        gunyh__gxfxd += '            random_state=random_state,\n'
        gunyh__gxfxd += '            shuffle=shuffle,\n'
        gunyh__gxfxd += '            stratify=stratify,\n'
        gunyh__gxfxd += '        )\n'
        gunyh__gxfxd += (
            '    return data_train, data_test, labels_train, labels_test\n')
        nub__fawe = {}
        exec(gunyh__gxfxd, globals(), nub__fawe)
        _train_test_split_impl = nub__fawe['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            ufhn__qlpv = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(ufhn__qlpv * train_size)
            psvup__dmdrp = ufhn__qlpv - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                mje__roi = bodo.get_rank()
                stg__oevir = bodo.get_size()
                bgvvl__rao = np.empty(stg__oevir, np.int64)
                bodo.libs.distributed_api.allgather(bgvvl__rao, len(data))
                eeb__rru = np.cumsum(bgvvl__rao[0:mje__roi + 1])
                nql__vba = np.full(ufhn__qlpv, True)
                nql__vba[:psvup__dmdrp] = False
                np.random.seed(42)
                np.random.permutation(nql__vba)
                if mje__roi:
                    ugs__qrrui = eeb__rru[mje__roi - 1]
                else:
                    ugs__qrrui = 0
                tsb__yhsrd = eeb__rru[mje__roi]
                cwcqb__bqpt = nql__vba[ugs__qrrui:tsb__yhsrd]
                pdsb__bqr = data[cwcqb__bqpt]
                gyk__qmkqy = data[~cwcqb__bqpt]
                afgh__rpi = labels[cwcqb__bqpt]
                mpvaf__owk = labels[~cwcqb__bqpt]
                pdsb__bqr = bodo.random_shuffle(pdsb__bqr, seed=
                    random_state, parallel=True)
                gyk__qmkqy = bodo.random_shuffle(gyk__qmkqy, seed=
                    random_state, parallel=True)
                afgh__rpi = bodo.random_shuffle(afgh__rpi, seed=
                    random_state, parallel=True)
                mpvaf__owk = bodo.random_shuffle(mpvaf__owk, seed=
                    random_state, parallel=True)
                afgh__rpi = reset_labels_type(afgh__rpi, label_type)
                mpvaf__owk = reset_labels_type(mpvaf__owk, label_type)
            else:
                pdsb__bqr, gyk__qmkqy, afgh__rpi, mpvaf__owk = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    wglzd__cwb = iii__nrsa.Get_size()
    m = m.fit(X)
    hhc__vhjr = iii__nrsa.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = hhc__vhjr
    qipei__vuyoe = np.zeros((wglzd__cwb, *m.data_min_.shape), dtype=m.
        data_min_.dtype)
    iii__nrsa.Allgather(m.data_min_, qipei__vuyoe)
    pozvw__nldp = np.nanmin(qipei__vuyoe, axis=0)
    cpkos__uny = np.zeros((wglzd__cwb, *m.data_max_.shape), dtype=m.
        data_max_.dtype)
    iii__nrsa.Allgather(m.data_max_, cpkos__uny)
    driy__sxlq = np.nanmax(cpkos__uny, axis=0)
    qceh__dtfz = driy__sxlq - pozvw__nldp
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(qceh__dtfz)
    m.min_ = m.feature_range[0] - pozvw__nldp * m.scale_
    m.data_min_ = pozvw__nldp
    m.data_max_ = driy__sxlq
    m.data_range_ = qceh__dtfz
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    xdtct__jyh = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        xdtct__jyh = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{xdtct__jyh}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            yqm__mhp = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if yqm__mhp > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=yqm__mhp)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        nkp__dxn = True
    return nkp__dxn, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                npvb__pip = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                npvb__pip = bodo.allgatherv(npvb__pip, False)
                npvb__pip = bodo.libs.array_kernels.sort(npvb__pip,
                    ascending=True, inplace=True)
                htf__pasz = {}
                for uqdb__zon in range(len(npvb__pip)):
                    htf__pasz[npvb__pip[uqdb__zon]] = uqdb__zon
            else:
                htf__pasz = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = htf__pasz
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X

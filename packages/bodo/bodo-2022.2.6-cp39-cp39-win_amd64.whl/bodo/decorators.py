...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        cfj__wppg = self.overloads[sig]
        ngv__rvys = cfj__wppg.metadata.get('distributed_diagnostics', None)
        if ngv__rvys is None:
            eee__vvjb = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(eee__vvjb)
        ngv__rvys.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        tcok__ivjan = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(tcok__ivjan)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        vda__kpx = options.pop('distributed')
        pipeline_class = (pipeline_class if vda__kpx else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        mae__rropr = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if mae__rropr else
            pipeline_class)
    vtrbh__kpah = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(vtrbh__kpah, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(vtrbh__kpah)
        else:
            return master_mode_wrapper(vtrbh__kpah)
    else:
        return vtrbh__kpah

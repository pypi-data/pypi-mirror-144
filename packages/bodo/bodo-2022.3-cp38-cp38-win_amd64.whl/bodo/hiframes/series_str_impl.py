...
class SeriesStrMethodType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        wfjk__xjk = 'SeriesStrMethodType({})'.format(stype)
        super(SeriesStrMethodType, self).__init__(wfjk__xjk)
...
class SeriesStrModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        wff__bzfah = [('obj', fe_type.stype)]
        super(SeriesStrModel, self).__init__(dmm, fe_type, wff__bzfah)
...
def init_series_str_method(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        kgd__bxgd, = args
        amjw__dqi = signature.return_type
        xee__vbs = cgutils.create_struct_proxy(amjw__dqi)(context, builder)
        xee__vbs.obj = kgd__bxgd
        context.nrt.incref(builder, signature.args[0], kgd__bxgd)
        return xee__vbs._getvalue()
...
def overload_str_method_len(S_str):
    ...
    def impl(S_str):
        S = S_str._obj
        wlu__bvtxe = bodo.hiframes.pd_series_ext.get_series_data(S)
        fyqm__job = bodo.hiframes.pd_series_ext.get_series_index(S)
        wfjk__xjk = bodo.hiframes.pd_series_ext.get_series_name(S)
        out_arr = bodo.libs.array_kernels.get_arr_lens(wlu__bvtxe, False)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
...
def overload_str_method_split(S_str, pat=None, n=-1, expand=False):
    ...
    if is_overload_constant_str(pat) and len(get_overload_const_str(pat)
        ...
        def _str_split_view_impl(S_str, pat=None, n=-1, expand=False):
            ...
            fyqm__job = bodo.hiframes.pd_series_ext.get_series_index(S)
            wfjk__xjk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.hiframes.split_impl.compute_split_view(wlu__bvtxe,
                pat)
    ...
    def _str_split_impl(S_str, pat=None, n=-1, expand=False):
        ...
        fyqm__job = bodo.hiframes.pd_series_ext.get_series_index(S)
        wfjk__xjk = bodo.hiframes.pd_series_ext.get_series_name(S)
        out_arr = bodo.libs.str_ext.str_split(wlu__bvtxe, pat, n)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
...
def overload_str_method_get(S_str, i):
    mzzh__chfpg = S_str.stype.data
    if (mzzh__chfpg != string_array_split_view_type and not is_str_arr_type
        (mzzh__chfpg)) and not isinstance(mzzh__chfpg, ArrayItemArrayType):
        raise_bodo_error(
            'Series.str.get(): only supports input type of Series(array(item)) and Series(str)'
            )
    int_arg_check('get', 'i', i)
    if isinstance(mzzh__chfpg, ArrayItemArrayType):
        ...
        def _str_get_array_impl(S_str, i):
            ...
            fyqm__job = bodo.hiframes.pd_series_ext.get_series_index(S)
            wfjk__xjk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.array_kernels.get(wlu__bvtxe, i)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                fyqm__job, wfjk__xjk)
        return _str_get_array_impl
    if mzzh__chfpg == string_array_split_view_type:
        ...
        def _str_get_split_impl(S_str, i):
            ...
            n = len(wlu__bvtxe)
            qcwgy__itk = 0
            for rdl__hlox in numba.parfors.parfor.internal_prange(n):
                piv__ugvvv, piv__ugvvv, jvb__okao = get_split_view_index(
                    wlu__bvtxe, rdl__hlox, i)
                qcwgy__itk += jvb__okao
            numba.parfors.parfor.init_prange()
            out_arr = pre_alloc_string_array(n, qcwgy__itk)
            for qrpfx__tqsu in numba.parfors.parfor.internal_prange(n):
                pufty__wyig, ghu__yrwkf, jvb__okao = get_split_view_index(
                    wlu__bvtxe, qrpfx__tqsu, i)
                if pufty__wyig == 0:
                    bodo.libs.array_kernels.setna(out_arr, qrpfx__tqsu)
                    lxp__zfdir = get_split_view_data_ptr(wlu__bvtxe, 0)
                else:
                    bodo.libs.str_arr_ext.str_arr_set_not_na(out_arr,
                        qrpfx__tqsu)
                    lxp__zfdir = get_split_view_data_ptr(wlu__bvtxe, ghu__yrwkf
                        )
                bodo.libs.str_arr_ext.setitem_str_arr_ptr(out_arr,
                    qrpfx__tqsu, lxp__zfdir, jvb__okao)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                fyqm__job, wfjk__xjk)
    ...
    def _str_get_impl(S_str, i):
        ...
        for qrpfx__tqsu in numba.parfors.parfor.internal_prange(n):
            if bodo.libs.array_kernels.isna(wlu__bvtxe, qrpfx__tqsu
                ) or not len(wlu__bvtxe[qrpfx__tqsu]) > i >= -len(wlu__bvtxe
                [qrpfx__tqsu]):
                out_arr[qrpfx__tqsu] = ''
                bodo.libs.array_kernels.setna(out_arr, qrpfx__tqsu)
            else:
                out_arr[qrpfx__tqsu] = wlu__bvtxe[qrpfx__tqsu][i]
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
...
def overload_str_method_join(S_str, sep):
    mzzh__chfpg = S_str.stype.data
    if (mzzh__chfpg != string_array_split_view_type and mzzh__chfpg !=
    ...
    def impl(S_str, sep):
        S = S_str._obj
        hvlic__vssvt = bodo.hiframes.pd_series_ext.get_series_data(S)
        wfjk__xjk = bodo.hiframes.pd_series_ext.get_series_name(S)
        fyqm__job = bodo.hiframes.pd_series_ext.get_series_index(S)
        numba.parfors.parfor.init_prange()
        n = len(hvlic__vssvt)
        ...
        for qrpfx__tqsu in numba.parfors.parfor.internal_prange(n):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, qrpfx__tqsu):
                out_arr[qrpfx__tqsu] = ''
                bodo.libs.array_kernels.setna(out_arr, qrpfx__tqsu)
            else:
                lruef__ykhpj = hvlic__vssvt[qrpfx__tqsu]
                out_arr[qrpfx__tqsu] = sep.join(lruef__ykhpj)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
...
def overload_str_method_replace(S_str, pat, repl, n=-1, case=None, flags=0,
    ...
    if S_str.stype.data == bodo.dict_str_arr_type:
        ...
        def _str_replace_dict_impl(S_str, pat, repl, n=-1, case=None, flags
            ...
            fyqm__job = bodo.hiframes.pd_series_ext.get_series_index(S)
            wfjk__xjk = bodo.hiframes.pd_series_ext.get_series_name(S)
            out_arr = bodo.libs.dict_arr_ext.str_replace(wlu__bvtxe, pat,
                repl, flags, regex)
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                fyqm__job, wfjk__xjk)
    ...
    if is_overload_true(regex):
        ...
        def _str_replace_regex_impl(S_str, pat, repl, n=-1, case=None,
            ...
            wfjk__xjk = bodo.hiframes.pd_series_ext.get_series_name(S)
            numba.parfors.parfor.init_prange()
            jjic__xjf = re.compile(pat, flags)
            ncj__ugz = len(wlu__bvtxe)
            out_arr = pre_alloc_string_array(ncj__ugz, -1)
            for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
                if bodo.libs.array_kernels.isna(wlu__bvtxe, qrpfx__tqsu):
                    out_arr[qrpfx__tqsu] = ''
                    bodo.libs.array_kernels.setna(out_arr, qrpfx__tqsu)
                    continue
                out_arr[qrpfx__tqsu] = jjic__xjf.sub(repl, wlu__bvtxe[
                    qrpfx__tqsu])
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                fyqm__job, wfjk__xjk)
    ...
    def _str_replace_noregex_impl(S_str, pat, repl, n=-1, case=None, flags=
        ...
        for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(wlu__bvtxe, qrpfx__tqsu):
                ...
                bodo.libs.array_kernels.setna(out_arr, qrpfx__tqsu)
                continue
            out_arr[qrpfx__tqsu] = wlu__bvtxe[qrpfx__tqsu].replace(pat, repl)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
...
def overload_str_method_contains(S_str, pat, case=True, flags=0, na=np.nan,
    ...
    fqip__ccil = re.IGNORECASE.value
    asbh__uiy = 'def impl(\n'
    asbh__uiy += '    S_str, pat, case=True, flags=0, na=np.nan, regex=True\n'
    asbh__uiy += '):\n'
    asbh__uiy += '  S = S_str._obj\n'
    asbh__uiy += '  arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n'
    asbh__uiy += '  index = bodo.hiframes.pd_series_ext.get_series_index(S)\n'
    asbh__uiy += '  name = bodo.hiframes.pd_series_ext.get_series_name(S)\n'
    asbh__uiy += '  l = len(arr)\n'
    asbh__uiy += '  out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(l)\n'
    if is_overload_true(regex):
        if is_regex_unsupported(pat) or flags:
            asbh__uiy += """  out_arr = bodo.hiframes.series_str_impl.series_contains_regex(S, pat, case, flags, na, regex)
"""
        else:
            asbh__uiy += """  get_search_regex(arr, case, bodo.libs.str_ext.unicode_to_utf8(pat), out_arr)
"""
    else:
        asbh__uiy += '  numba.parfors.parfor.init_prange()\n'
        if is_overload_false(case):
            asbh__uiy += '  upper_pat = pat.upper()\n'
        asbh__uiy += '  for i in numba.parfors.parfor.internal_prange(l):\n'
        asbh__uiy += '      if bodo.libs.array_kernels.isna(arr, i):\n'
        asbh__uiy += '          bodo.libs.array_kernels.setna(out_arr, i)\n'
        asbh__uiy += '      else: \n'
        if is_overload_true(case):
            asbh__uiy += '          out_arr[i] = pat in arr[i]\n'
        else:
            asbh__uiy += '          out_arr[i] = upper_pat in arr[i].upper()\n'
    asbh__uiy += (
        '  return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)\n'
        )
    pxcv__sagq = {}
    exec(asbh__uiy, {'re': re, 'bodo': bodo, 'numba': numba, 'np': np,
        're_ignorecase_value': fqip__ccil, 'get_search_regex':
        get_search_regex}, pxcv__sagq)
    impl = pxcv__sagq['impl']
    return impl
...
def overload_str_method_cat(S_str, others=None, sep=None, na_rep=None, join
    ...
    if S_str.stype.data == bodo.dict_str_arr_type and all(fmrh__vqhn ==
        bodo.dict_str_arr_type for fmrh__vqhn in others.data):
        hbsbq__wufhd = ', '.join(f'data{i}' for i in range(len(others.columns))
            )
        asbh__uiy += f"""  out_arr = bodo.libs.dict_arr_ext.cat_dict_str((arr, {hbsbq__wufhd}), sep)
"""
    else:
        pjrj__dhrly = ' or '.join(['bodo.libs.array_kernels.isna(arr, i)'] +
        ...
        asbh__uiy += '  numba.parfors.parfor.init_prange()\n'
        asbh__uiy += '  for i in numba.parfors.parfor.internal_prange(l):\n'
        asbh__uiy += f'      if {pjrj__dhrly}:\n'
        asbh__uiy += '          bodo.libs.array_kernels.setna(out_arr, i)\n'
        asbh__uiy += '          continue\n'
        fmatg__mzdg = ', '.join(['arr[i]'] + [f'data{i}[i]' for i in range(
            len(others.columns))])
        oilyw__wpzy = "''" if is_overload_none(sep) else 'sep'
        asbh__uiy += (
            f'      out_arr[i] = {oilyw__wpzy}.join([{fmatg__mzdg}])\n')
    asbh__uiy += (
        '  return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)\n'
        )
    pxcv__sagq = {}
    exec(asbh__uiy, {'bodo': bodo, 'numba': numba}, pxcv__sagq)
    impl = pxcv__sagq['impl']
    return impl
...
def overload_str_method_count(S_str, pat, flags=0):
    ...
    def impl(S_str, pat, flags=0):
        ...
        jjic__xjf = re.compile(pat, flags)
        numba.parfors.parfor.init_prange()
        ncj__ugz = len(hvlic__vssvt)
        out_arr = bodo.libs.int_arr_ext.alloc_int_array(ncj__ugz, np.int64)
        for i in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, i):
                bodo.libs.array_kernels.setna(out_arr, i)
            else:
                out_arr[i] = str_findall_count(jjic__xjf, hvlic__vssvt[i])
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
    ...
    def impl(S_str, sub, start=0, end=None):
        ...
        for i in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, i):
                ...
                out_arr[i] = hvlic__vssvt[i].find(sub, start, end)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
    ...
    def impl(S_str, sub, start=0, end=None):
        ...
        for i in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, i):
                ...
                out_arr[i] = hvlic__vssvt[i].rfind(sub, start, end)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
...
def overload_str_method_center(S_str, width, fillchar=' '):
    ...
    def impl(S_str, width, fillchar=' '):
        ...
        numba.parfors.parfor.init_prange()
        ncj__ugz = len(hvlic__vssvt)
        out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(ncj__ugz, -1)
        for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, qrpfx__tqsu):
                out_arr[qrpfx__tqsu] = ''
                bodo.libs.array_kernels.setna(out_arr, qrpfx__tqsu)
            else:
                out_arr[qrpfx__tqsu] = hvlic__vssvt[qrpfx__tqsu].center(width,
                    fillchar)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
    ...
    def impl(S_str, start=0, stop=None, repl=''):
        ...
        for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, qrpfx__tqsu):
                ...
                if stop is not None:
                    qygb__aase = hvlic__vssvt[qrpfx__tqsu][stop:]
                else:
                    qygb__aase = ''
                out_arr[qrpfx__tqsu] = hvlic__vssvt[qrpfx__tqsu][:start
                    ] + repl + qygb__aase
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
...
def overload_str_method_repeat(S_str, repeats):
        ...
        def impl(S_str, repeats):
            ...
            for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
                if bodo.libs.array_kernels.isna(hvlic__vssvt, qrpfx__tqsu):
                    ...
                    out_arr[qrpfx__tqsu] = hvlic__vssvt[qrpfx__tqsu] * repeats
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                fyqm__job, wfjk__xjk)
        return impl
    elif is_overload_constant_list(repeats):
        vfz__ahk = get_overload_const_list(repeats)
        oxtt__jze = all([isinstance(wiidw__acuz, int) for wiidw__acuz in
            vfz__ahk])
    ...
    if oxtt__jze:
        ...
        def impl(S_str, repeats):
            ...
            wfjk__xjk = bodo.hiframes.pd_series_ext.get_series_name(S)
            fyqm__job = bodo.hiframes.pd_series_ext.get_series_index(S)
            hhnw__ywyrx = bodo.utils.conversion.coerce_to_array(repeats)
            numba.parfors.parfor.init_prange()
            ncj__ugz = len(hvlic__vssvt)
            out_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(ncj__ugz, -1
                )
            for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
                if bodo.libs.array_kernels.isna(hvlic__vssvt, qrpfx__tqsu):
                    ...
                    out_arr[qrpfx__tqsu] = hvlic__vssvt[qrpfx__tqsu
                        ] * hhnw__ywyrx[qrpfx__tqsu]
            return bodo.hiframes.pd_series_ext.init_series(out_arr,
                fyqm__job, wfjk__xjk)
        return impl
    else:
    ...
    def impl(S_str, width, fillchar=' '):
        ...
        for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, qrpfx__tqsu):
                ...
                out_arr[qrpfx__tqsu] = hvlic__vssvt[qrpfx__tqsu].ljust(width,
                    fillchar)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
    ...
    def impl(S_str, width, fillchar=' '):
        ...
        for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, qrpfx__tqsu):
                ...
                out_arr[qrpfx__tqsu] = hvlic__vssvt[qrpfx__tqsu].rjust(width,
                    fillchar)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
    ...
    def impl(S_str, width):
        ...
        for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, qrpfx__tqsu):
                ...
                out_arr[qrpfx__tqsu] = hvlic__vssvt[qrpfx__tqsu].zfill(width)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
    ...
    def impl(S_str, start=None, stop=None, step=None):
        ...
        for qrpfx__tqsu in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, qrpfx__tqsu):
                ...
                out_arr[qrpfx__tqsu] = hvlic__vssvt[qrpfx__tqsu][start:stop
                    :step]
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
...
def overload_str_method_startswith(S_str, pat, na=np.nan):
    ...
    def impl(S_str, pat, na=np.nan):
        ...
        numba.parfors.parfor.init_prange()
        ncj__ugz = len(hvlic__vssvt)
        out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(ncj__ugz)
        for i in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, i):
                bodo.libs.array_kernels.setna(out_arr, i)
            else:
                out_arr[i] = hvlic__vssvt[i].startswith(pat)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
    ...
    def impl(S_str, pat, na=np.nan):
        ...
        for i in numba.parfors.parfor.internal_prange(ncj__ugz):
            if bodo.libs.array_kernels.isna(hvlic__vssvt, i):
                ...
                out_arr[i] = hvlic__vssvt[i].endswith(pat)
        return bodo.hiframes.pd_series_ext.init_series(out_arr, fyqm__job,
            wfjk__xjk)
    return impl
...
def overload_str_method_extract(S_str, pat, flags=0, expand=True):
    ...
    kxif__ulr, regex = _get_column_names_from_regex(pat, flags, 'extract')
    xtr__rlnbn = len(kxif__ulr)
    asbh__uiy = 'def impl(S_str, pat, flags=0, expand=True):\n'
    asbh__uiy += '  regex = re.compile(pat, flags=flags)\n'
    asbh__uiy += '  S = S_str._obj\n'
    asbh__uiy += '  str_arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n'
    asbh__uiy += '  index = bodo.hiframes.pd_series_ext.get_series_index(S)\n'
    asbh__uiy += '  name = bodo.hiframes.pd_series_ext.get_series_name(S)\n'
    asbh__uiy += '  numba.parfors.parfor.init_prange()\n'
    asbh__uiy += '  n = len(str_arr)\n'
    for i in range(xtr__rlnbn):
        asbh__uiy += (
            '  out_arr_{0} = bodo.libs.str_arr_ext.pre_alloc_string_array(n, -1)\n'
            .format(i))
    asbh__uiy += '  for j in numba.parfors.parfor.internal_prange(n):\n'
    asbh__uiy += '      if bodo.libs.array_kernels.isna(str_arr, j):\n'
    for i in range(xtr__rlnbn):
        asbh__uiy += "          out_arr_{}[j] = ''\n".format(i)
        asbh__uiy += (
            '          bodo.libs.array_kernels.setna(out_arr_{}, j)\n'.
            format(i))
    asbh__uiy += '      else:\n'
    asbh__uiy += '          m = regex.search(str_arr[j])\n'
    asbh__uiy += '          if m:\n'
    asbh__uiy += '            g = m.groups()\n'
    for i in range(xtr__rlnbn):
        asbh__uiy += '            out_arr_{0}[j] = g[{0}]\n'.format(i)
    asbh__uiy += '          else:\n'
    for i in range(xtr__rlnbn):
        asbh__uiy += "            out_arr_{}[j] = ''\n".format(i)
        asbh__uiy += (
    ...
    if is_overload_false(expand) and regex.groups == 1:
        wfjk__xjk = "'{}'".format(list(regex.groupindex.keys()).pop()) if len(
            regex.groupindex.keys()) > 0 else 'name'
        asbh__uiy += (
            '  return bodo.hiframes.pd_series_ext.init_series(out_arr_0, index, {})\n'
            .format(wfjk__xjk))
        pxcv__sagq = {}
        exec(asbh__uiy, {'re': re, 'bodo': bodo, 'numba': numba,
            'get_utf8_size': get_utf8_size}, pxcv__sagq)
        impl = pxcv__sagq['impl']
        return impl
    xfjt__wzf = ', '.join('out_arr_{}'.format(i) for i in range(xtr__rlnbn))
    impl = bodo.hiframes.dataframe_impl._gen_init_df(asbh__uiy, kxif__ulr,
        xfjt__wzf, 'index', extra_globals={'get_utf8_size': get_utf8_size,
        're': re})
    return impl
...
def overload_str_method_extractall(S_str, pat, flags=0):
    kxif__ulr, piv__ugvvv = _get_column_names_from_regex(pat, flags,
        'extractall')
    xtr__rlnbn = len(kxif__ulr)
    znyj__pyw = isinstance(S_str.stype.index, StringIndexType)
    asbh__uiy = 'def impl(S_str, pat, flags=0):\n'
    asbh__uiy += '  regex = re.compile(pat, flags=flags)\n'
    asbh__uiy += '  S = S_str._obj\n'
    asbh__uiy += '  str_arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n'
    asbh__uiy += '  index = bodo.hiframes.pd_series_ext.get_series_index(S)\n'
    asbh__uiy += '  name = bodo.hiframes.pd_series_ext.get_series_name(S)\n'
    asbh__uiy += '  index_arr = bodo.utils.conversion.index_to_array(index)\n'
    asbh__uiy += (
    ...
    asbh__uiy += '  out_n_l = [0]\n'
    for i in range(xtr__rlnbn):
        asbh__uiy += '  num_chars_{} = 0\n'.format(i)
    if znyj__pyw:
        asbh__uiy += '  index_num_chars = 0\n'
    asbh__uiy += '  for i in numba.parfors.parfor.internal_prange(n):\n'
    if znyj__pyw:
        asbh__uiy += '      index_num_chars += get_utf8_size(index_arr[i])\n'
    asbh__uiy += '      if bodo.libs.array_kernels.isna(str_arr, i):\n'
    asbh__uiy += '          continue\n'
    asbh__uiy += '      m = regex.findall(str_arr[i])\n'
    asbh__uiy += '      out_n_l[0] += len(m)\n'
    for i in range(xtr__rlnbn):
        asbh__uiy += '      l_{} = 0\n'.format(i)
    asbh__uiy += '      for s in m:\n'
    for i in range(xtr__rlnbn):
        asbh__uiy += '        l_{} += get_utf8_size(s{})\n'.format(i, 
            '[{}]'.format(i) if xtr__rlnbn > 1 else '')
    for i in range(xtr__rlnbn):
        asbh__uiy += '      num_chars_{0} += l_{0}\n'.format(i)
    asbh__uiy += (
...
"""
            .format(i))
    if znyj__pyw:
        asbh__uiy += """  out_ind_arr = bodo.libs.str_arr_ext.pre_alloc_string_array(out_n, index_num_chars)
"""
    else:
        asbh__uiy += '  out_ind_arr = np.empty(out_n, index_arr.dtype)\n'
    asbh__uiy += '  out_match_arr = np.empty(out_n, np.int64)\n'
    asbh__uiy += '  out_ind = 0\n'
    asbh__uiy += '  for j in numba.parfors.parfor.internal_prange(n):\n'
    asbh__uiy += '      if bodo.libs.array_kernels.isna(str_arr, j):\n'
    asbh__uiy += '          continue\n'
    asbh__uiy += '      m = regex.findall(str_arr[j])\n'
    asbh__uiy += '      for k, s in enumerate(m):\n'
    for i in range(xtr__rlnbn):
        asbh__uiy += (
            '        bodo.libs.distributed_api.set_arr_local(out_arr_{}, out_ind, s{})\n'
            .format(i, '[{}]'.format(i) if xtr__rlnbn > 1 else ''))
    asbh__uiy += """        bodo.libs.distributed_api.set_arr_local(out_ind_arr, out_ind, index_arr[j])
"""
    asbh__uiy += (
    ...
    asbh__uiy += "    (out_ind_arr, out_match_arr), (index_name, 'match'))\n"
    xfjt__wzf = ', '.join('out_arr_{}'.format(i) for i in range(xtr__rlnbn))
    impl = bodo.hiframes.dataframe_impl._gen_init_df(asbh__uiy, kxif__ulr,
        xfjt__wzf, 'out_index', extra_globals={'get_utf8_size':
        get_utf8_size, 're': re})
    return impl
...
def _get_column_names_from_regex(pat, flags, func_name):
    ...
    if regex.groups == 0:
        raise BodoError(
            ...
            (func_name, pat))
    jgxe__dhoc = dict(zip(regex.groupindex.values(), regex.groupindex.keys()))
    kxif__ulr = [jgxe__dhoc.get(1 + i, i) for i in range(regex.groups)]
    return kxif__ulr, regex


def create_str2str_methods_overload(func_name):
    if func_name in ['lstrip', 'rstrip', 'strip']:
        asbh__uiy = 'def f(S_str, to_strip=None):\n'
    else:
        asbh__uiy = 'def f(S_str):\n'
    asbh__uiy += '    S = S_str._obj\n'
    asbh__uiy += (
        '    str_arr = bodo.hiframes.pd_series_ext.get_series_data(S)\n')
    asbh__uiy += '    str_arr = decode_if_dict_array(str_arr)\n'
    asbh__uiy += (
    ...
    if func_name in ('capitalize', 'lower', 'swapcase', 'title', 'upper'):
        asbh__uiy += '    num_chars = num_total_chars(str_arr)\n'
    else:
        asbh__uiy += '    num_chars = -1\n'
    asbh__uiy += (
    ...
    asbh__uiy += '        if bodo.libs.array_kernels.isna(str_arr, j):\n'
    asbh__uiy += '            out_arr[j] = ""\n'
    asbh__uiy += '            bodo.libs.array_kernels.setna(out_arr, j)\n'
    asbh__uiy += '        else:\n'
    if func_name in ['lstrip', 'rstrip', 'strip']:
        asbh__uiy += ('            out_arr[j] = str_arr[j].{}(to_strip)\n'.
            format(func_name))
    else:
        asbh__uiy += '            out_arr[j] = str_arr[j].{}()\n'.format(
            func_name)
    asbh__uiy += (
        '    return bodo.hiframes.pd_series_ext.init_series(out_arr, index, name)\n'
        )
    pxcv__sagq = {}
    exec(asbh__uiy, {'bodo': bodo, 'numba': numba, 'num_total_chars': bodo.
        libs.str_arr_ext.num_total_chars, 'get_utf8_size': bodo.libs.
        str_arr_ext.get_utf8_size, 'decode_if_dict_array': bodo.utils.
        typing.decode_if_dict_array}, pxcv__sagq)
    prhro__xgj = pxcv__sagq['f']
    if func_name in ['lstrip', 'rstrip', 'strip']:
...
def create_str2bool_methods_overload(func_name):
    ...
    def overload_str2bool_methods(S_str):
        ...
        asbh__uiy += '            bodo.libs.array_kernels.setna(out_arr, i)\n'
        asbh__uiy += '        else:\n'
        asbh__uiy += ('            out_arr[i] = np.bool_(str_arr[i].{}())\n'
            .format(func_name))
        asbh__uiy += '    return bodo.hiframes.pd_series_ext.init_series(\n'
        asbh__uiy += '      out_arr,index, name)\n'
        pxcv__sagq = {}
        exec(asbh__uiy, {'bodo': bodo, 'numba': numba, 'np': np}, pxcv__sagq)
        prhro__xgj = pxcv__sagq['f']
        return prhro__xgj
...
def _install_str2str_methods():
    for ryzc__mjd in bodo.hiframes.pd_series_ext.str2str_methods:
        dhujt__rxkj = create_str2str_methods_overload(ryzc__mjd)
        overload_method(SeriesStrMethodType, ryzc__mjd, inline='always',
            no_unliteral=True)(dhujt__rxkj)


def _install_str2bool_methods():
    for ryzc__mjd in bodo.hiframes.pd_series_ext.str2bool_methods:
        dhujt__rxkj = create_str2bool_methods_overload(ryzc__mjd)
        overload_method(SeriesStrMethodType, ryzc__mjd, inline='always',
            no_unliteral=True)(dhujt__rxkj)
...
class SeriesCatMethodType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        wfjk__xjk = 'SeriesCatMethodType({})'.format(stype)
        super(SeriesCatMethodType, self).__init__(wfjk__xjk)
...
class SeriesCatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        wff__bzfah = [('obj', fe_type.stype)]
        super(SeriesCatModel, self).__init__(dmm, fe_type, wff__bzfah)
...
def init_series_cat_method(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        kgd__bxgd, = args
        yoc__uryga = signature.return_type
        almso__kruu = cgutils.create_struct_proxy(yoc__uryga)(context, builder)
        almso__kruu.obj = kgd__bxgd
        context.nrt.incref(builder, signature.args[0], kgd__bxgd)
        return almso__kruu._getvalue()
...
def series_cat_codes_overload(S_dt):
    ...
    def impl(S_dt):
        ...
        return bodo.hiframes.pd_series_ext.init_series(bodo.hiframes.
            pd_categorical_ext.get_categorical_arr_codes(wlu__bvtxe),
            fyqm__job, wfjk__xjk)
    return impl
...
def _install_catseries_unsupported():
    for nsym__cmjl in unsupported_cat_attrs:
        dxhsb__wurta = 'Series.cat.' + nsym__cmjl
        overload_attribute(SeriesCatMethodType, nsym__cmjl)(
            create_unsupported_overload(dxhsb__wurta))
    for arva__dizdi in unsupported_cat_methods:
        dxhsb__wurta = 'Series.cat.' + arva__dizdi
        overload_method(SeriesCatMethodType, arva__dizdi)(
            create_unsupported_overload(dxhsb__wurta))
...
def _install_strseries_unsupported():
    for arva__dizdi in unsupported_str_methods:
        dxhsb__wurta = 'Series.str.' + arva__dizdi
        overload_method(SeriesStrMethodType, arva__dizdi)(
            create_unsupported_overload(dxhsb__wurta))

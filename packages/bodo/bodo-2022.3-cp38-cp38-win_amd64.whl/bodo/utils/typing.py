...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        xibub__qabov = len(A.types)
        tocq__nvgq = 'def f(A):\n'
        azq__eil = ','.join(f'decode_if_dict_array(A[{i}])' for i in range(
            xibub__qabov))
        tocq__nvgq += '  return ({}{})\n'.format(azq__eil, ',' if 
            xibub__qabov == 1 else '')
        yufq__bsj = {}
        exec(tocq__nvgq, {'decode_if_dict_array': decode_if_dict_array},
            yufq__bsj)
        impl = yufq__bsj['f']
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            tvfc__dnz = bodo.hiframes.pd_series_ext.get_series_data(A)
            cxunn__hqfrk = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            eqylc__bbhd = decode_if_dict_array(tvfc__dnz)
            return bodo.hiframes.pd_series_ext.init_series(eqylc__bbhd,
                cxunn__hqfrk, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        vwt__tas = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(vwt__tas, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        rqfcc__vxo = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(rqfcc__vxo(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        jvmm__rkvvf = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=jvmm__rkvvf)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        exoo__chwmj = self.get_full_filename(fname)
        return self._get_schema(exoo__chwmj)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        kdgn__zmofi = dmm.lookup(bodo.typeof(fe_type.fname))
        xjz__mgxla = [(a, mfenx__zbjo) for a, mfenx__zbjo in zip(
            kdgn__zmofi._fields, kdgn__zmofi._members)]
        super().__init__(dmm, fe_type, xjz__mgxla)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    bgkq__tdf = t.initial_value
    if isinstance(bgkq__tdf, dict):
        bgkq__tdf = bgkq__tdf.values()
    return not any(isinstance(zvvee__flo, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for zvvee__flo in bgkq__tdf)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (zvvee__flo) for zvvee__flo in val.types
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (zvvee__flo) for zvvee__flo in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        eymy__orvp = []
        for zvvee__flo in val.literal_value:
            fgz__alk = get_overload_const(zvvee__flo)
            if fgz__alk == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                eymy__orvp.append(fgz__alk)
        return eymy__orvp
    ...
    if isinstance(val, types.BaseTuple):
        eymy__orvp = []
        for zvvee__flo in val.types:
            fgz__alk = get_overload_const(zvvee__flo)
            if fgz__alk == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                eymy__orvp.append(fgz__alk)
        return tuple(eymy__orvp)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    fgz__alk = get_overload_const(val)
    egxd__drsh = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(fgz__alk, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {egxd__drsh} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    fgz__alk = get_overload_const(val)
    if fgz__alk not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{fgz__alk}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    tlht__zbno = False
    for a in args_dict:
        wbhn__ftnaw = get_overload_const(args_dict[a])
        wnb__feole = arg_defaults_dict[a]
        if (wbhn__ftnaw is NOT_CONSTANT or wbhn__ftnaw is not None and 
            wnb__feole is None or wbhn__ftnaw is None and wnb__feole is not
            None or wbhn__ftnaw != wnb__feole or wbhn__ftnaw is not
            _no_input and wnb__feole is _no_input or wbhn__ftnaw is
            _no_input and wnb__feole is not _no_input):
            bwct__bfj = (
                f'{fn_str}: {a} parameter only supports default value {wnb__feole}'
                )
            tlht__zbno = True
            break
    if tlht__zbno and package_name == 'pandas':
        if module_name == 'IO':
            bwct__bfj += """
...
"""
    elif tlht__zbno and package_name == 'ml':
        bwct__bfj += """
...
"""
    elif tlht__zbno and package_name == 'numpy':
        bwct__bfj += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(gmhyy__eyg): get_literal_value(zvvee__flo
            ) for gmhyy__eyg, zvvee__flo in val.literal_value.items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    cle__lomx = [get_overload_const(zvvee__flo) for zvvee__flo in val.types[1:]
        ]
    return {cle__lomx[2 * i]: cle__lomx[2 * i + 1] for i in range(len(
        cle__lomx) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(zvvee__flo) for zvvee__flo in val.
            literal_value]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        zxs__ibtm = get_overload_const_str(dtype)
        if zxs__ibtm.startswith('Int') or zxs__ibtm.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(zxs__ibtm), None)
        if zxs__ibtm == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        return numba.np.numpy_support.from_dtype(np.dtype(zxs__ibtm))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    hab__xtral = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        vlmfh__wlczb = []
        for i, ktdev__mwigl in enumerate(t.names_typ):
            if ktdev__mwigl == types.none:
                vlmfh__wlczb.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(ktdev__mwigl):
                raise BodoError(hab__xtral)
            vlmfh__wlczb.append(get_overload_const_str(ktdev__mwigl))
        return tuple(vlmfh__wlczb)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        gksg__qaty = typ.literal_value
        eqb__dbq = []
        for kjc__kson in ('start', 'stop', 'step'):
            wwu__qlql = getattr(typ.literal_value, kjc__kson)
            zrrgk__ing = c.pyapi.make_none(
                ) if wwu__qlql is None else c.pyapi.from_native_value(types
                .literal(wwu__qlql), wwu__qlql, c.env_manager)
            eqb__dbq.append(zrrgk__ing)
        eynxt__vtew = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        zwcgi__pqlg = c.pyapi.call_function_objargs(eynxt__vtew, eqb__dbq)
        for a in eqb__dbq:
            c.pyapi.decref(a)
        c.pyapi.decref(eynxt__vtew)
        return zwcgi__pqlg
...
def box_list_literal(typ, val, c):
    jut__gop = typ.literal_value
    phtqb__colp = [c.pyapi.from_native_value(types.literal(zvvee__flo),
        zvvee__flo, c.env_manager) for zvvee__flo in jut__gop]
    zkoyy__las = c.pyapi.list_pack(phtqb__colp)
    for a in phtqb__colp:
        c.pyapi.decref(a)
    return zkoyy__las
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    oqtr__jcpj = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(zvvee__flo) for zvvee__flo in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(gmhyy__eyg): get_literal_value(
                zvvee__flo) for gmhyy__eyg, zvvee__flo in t.literal_value.
                items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(zvvee__flo) for zvvee__flo in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(zvvee__flo) for zvvee__flo in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        tvfc__dnz = types.Array(dtype, 1, 'C')
        if usff__wtrd:
            return to_nullable_type(tvfc__dnz)
        return tvfc__dnz
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    ttpnj__tdpzh = dtype_to_array_type(f_return_type)
    ttpnj__tdpzh = to_nullable_type(ttpnj__tdpzh
        ) if return_nullable else ttpnj__tdpzh
    return ttpnj__tdpzh
...
def types_equality_exists(t1, t2):
    bak__fru = numba.core.registry.cpu_target.typing_context
    try:
        bak__fru.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    qxujd__fouo = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo
        .pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in qxujd__fouo:
        return True
    bak__fru = numba.core.registry.cpu_target.typing_context
    try:
        bak__fru.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        die__fjz = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(die__fjz, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    kfvv__bwzwz, asghm__nwxhj = get_common_scalar_dtype(scalar_types)
    return asghm__nwxhj
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        iuee__zkcv = np.find_common_type([numba.np.numpy_support.as_dtype(t
            ) for t in scalar_types], [])
        if iuee__zkcv != object:
            return numba.np.numpy_support.from_dtype(iuee__zkcv), True
    except numba.core.errors.NumbaNotImplementedError as bbdn__mvvsy:
        pass
    ...
    xgxuj__hkjfm = itertools.groupby(scalar_types)
    if next(xgxuj__hkjfm, True) and not next(xgxuj__hkjfm, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            asrr__rtc.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            asrr__rtc.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                asrr__rtc.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                asrr__rtc.append(bodo.libs.int_arr_ext.IntegerArrayType(typ
                    .dtype))
        asrr__rtc.append(typ)
    return asrr__rtc
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        siz__waqg = getfullargspec(func)
        assert siz__waqg.varargs is None, 'varargs not supported'
        assert siz__waqg.varkw is None, 'varkw not supported'
        defaults = [] if siz__waqg.defaults is None else siz__waqg.defaults
        opvz__bctm = len(siz__waqg.args) - len(defaults)
        args = siz__waqg.args[1:] if attr_name else siz__waqg.args[:]
        czoe__jrbm = []
        for i, zhbg__vvnn in enumerate(siz__waqg.args):
            if i < opvz__bctm:
                czoe__jrbm.append(zhbg__vvnn)
            elif str(defaults[i - opvz__bctm]) != '<deprecated parameter>':
                czoe__jrbm.append(zhbg__vvnn + '=' + str(defaults[i -
                    opvz__bctm]))
            else:
                args.remove(zhbg__vvnn)
        if siz__waqg.kwonlyargs is not None:
            for zhbg__vvnn in siz__waqg.kwonlyargs:
                args.append(f'{zhbg__vvnn}={zhbg__vvnn}')
                czoe__jrbm.append(
                    f'{zhbg__vvnn}={str(siz__waqg.kwonlydefaults[zhbg__vvnn])}'
                    )
        sig = ', '.join(czoe__jrbm)
        args = ', '.join(args)
    else:
    ...
    dectr__pxm = f'self.{attr_name}' if attr_name else f'{func_name}'
    tocq__nvgq = f'def overload_impl({sig}):\n'
    tocq__nvgq += f'    def impl({sig}):\n'
    if single_rank:
        tocq__nvgq += f'        if bodo.get_rank() == 0:\n'
        vxtvj__thg = '    '
    else:
        vxtvj__thg = ''
    tocq__nvgq += (
        f"        {vxtvj__thg}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        tocq__nvgq += f'            {vxtvj__thg}res = {dectr__pxm}({args})\n'
    else:
        tocq__nvgq += f'            {vxtvj__thg}res = {dectr__pxm}\n'
    tocq__nvgq += f'        return res\n'
    tocq__nvgq += f'    return impl\n'
    yufq__bsj = {}
    mbrk__aeam = globals()
    if not attr_name:
        mbrk__aeam[func_name] = func
    exec(tocq__nvgq, mbrk__aeam, yufq__bsj)
    gergl__gju = yufq__bsj['overload_impl']
    return gergl__gju


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        gergl__gju = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(gergl__gju)
    except Exception as bbdn__mvvsy:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        gergl__gju = _gen_objmode_overload(method, output_type, method_name,
            True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(gergl__gju)
    except Exception as bbdn__mvvsy:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        gergl__gju = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(gergl__gju)
    except Exception as bbdn__mvvsy:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, fkfx__qjz = args
        if isinstance(fkfx__qjz, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[fkfx__qjz]), *args
                )
...
def construct_pysig(arg_names, defaults):
    tocq__nvgq = f'def stub('
    for zhbg__vvnn in arg_names:
        tocq__nvgq += zhbg__vvnn
        if zhbg__vvnn in defaults:
            if isinstance(defaults[zhbg__vvnn], str):
                tocq__nvgq += f"='{defaults[zhbg__vvnn]}'"
            else:
                tocq__nvgq += f'={defaults[zhbg__vvnn]}'
        tocq__nvgq += ', '
    tocq__nvgq += '):\n'
    tocq__nvgq += '    pass\n'
    yufq__bsj = {}
    exec(tocq__nvgq, {}, yufq__bsj)
    mykw__qahy = yufq__bsj['stub']
    return numba.core.utils.pysignature(mykw__qahy)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    znw__gdapb = len(arg_names)
    ugi__hhz = len(args) + len(kws)
    if ugi__hhz > znw__gdapb:
        lbqcl__zfvc = 'argument' if znw__gdapb == 1 else 'arguments'
        siutv__wijhi = 'was' if ugi__hhz == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {znw__gdapb} {lbqcl__zfvc}, but {ugi__hhz} {siutv__wijhi} provided.'
            )
    shwr__sej = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        jnv__mugy = bodo.utils.transform.fold_argument_types(shwr__sej,
            args, kws)
    except Exception as yrht__nno:
        raise_bodo_error(f'{func_name}(): {yrht__nno}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                icbrv__igd[arg_name] = jnv__mugy[i]
                jvqvb__nvwb[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, icbrv__igd, jvqvb__nvwb)
    return shwr__sej, jnv__mugy
...
class BodoArrayIterator(types.SimpleIteratorType):
    ...
    def __init__(self, arr_type):
        ...
        uvzw__zil = arr_type.dtype
        super(BodoArrayIterator, self).__init__(name, uvzw__zil)
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        xjz__mgxla = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, xjz__mgxla)
...
def iternext_bodo_array(context, builder, sig, args, result):
    ...
    [hvet__aohv] = sig.args
    [krw__qzv] = args
    bmu__ndesy = context.make_helper(builder, hvet__aohv, value=krw__qzv)
    ppji__mmx = signature(types.intp, hvet__aohv.arr_type)
    ailkx__gzzud = context.compile_internal(builder, lambda a: len(a),
        ppji__mmx, [bmu__ndesy.array])
    cxunn__hqfrk = builder.load(bmu__ndesy.index)
    oydu__lrxq = builder.icmp(lc.ICMP_SLT, cxunn__hqfrk, ailkx__gzzud)
    result.set_valid(oydu__lrxq)
    with builder.if_then(oydu__lrxq):
        bweix__sdw = signature(hvet__aohv.arr_type.dtype, hvet__aohv.
            arr_type, types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            bweix__sdw, [bmu__ndesy.array, cxunn__hqfrk])
        result.yield_(value)
        lzz__ozyo = cgutils.increment_index(builder, cxunn__hqfrk)
        builder.store(lzz__ozyo, bmu__ndesy.index)


def index_typ_from_dtype_name(elem_dtype, name):
    isd__ayn = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        kvxm__fnwa = None
    else:
        kvxm__fnwa = types.StringLiteral(name)
    if isd__ayn == bodo.hiframes.pd_index_ext.NumericIndexType:
        jwvxg__bwht = isd__ayn(elem_dtype, kvxm__fnwa)
    elif isd__ayn == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        jwvxg__bwht = isd__ayn(bodo.CategoricalArrayType(elem_dtype),
            kvxm__fnwa)
    else:
        jwvxg__bwht = isd__ayn(kvxm__fnwa)
    return jwvxg__bwht
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(zvvee__flo, t) for zvvee__flo, t in
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        qlgen__mdq = pd.Index(val_typ.columns)
        rquwt__hpc = pd.Index(typ.columns)
        xoce__wqmc = qlgen__mdq.argsort()
        wzrx__tgt = rquwt__hpc.argsort()
        deg__wlhw = val_typ.copy(data=tuple(np.array(val_typ.data)[
            xoce__wqmc]), columns=tuple(qlgen__mdq[xoce__wqmc]))
        xpb__cnp = typ.copy(data=tuple(np.array(typ.data)[wzrx__tgt]),
            columns=tuple(rquwt__hpc[wzrx__tgt]))
        if deg__wlhw == xpb__cnp:
            val_typ = typ

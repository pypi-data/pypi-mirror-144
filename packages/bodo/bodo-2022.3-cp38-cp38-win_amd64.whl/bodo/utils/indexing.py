...
def get_new_null_mask_bool_index(old_mask, ind, n):
    geyhb__wlg = n + 7 >> 3
    jnb__phz = np.empty(geyhb__wlg, np.uint8)
    vdbp__moax = 0
    for gnbcm__dfarv in range(len(ind)):
        if ind[gnbcm__dfarv]:
            kybw__uaivf = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                gnbcm__dfarv)
            bodo.libs.int_arr_ext.set_bit_to_arr(jnb__phz, vdbp__moax,
                kybw__uaivf)
            vdbp__moax += 1
    return jnb__phz
...
def array_getitem_bool_index(A, ind):
    ...
    kqof__jlbq = A._data[ind]
    n = len(kqof__jlbq)
    jnb__phz = get_new_null_mask_bool_index(old_mask, ind, n)
    return kqof__jlbq, jnb__phz
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    for gnbcm__dfarv in range(len(ind)):
        kybw__uaivf = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            ind[gnbcm__dfarv])
        bodo.libs.int_arr_ext.set_bit_to_arr(jnb__phz, vdbp__moax, kybw__uaivf)
        vdbp__moax += 1
    return jnb__phz
...
def array_getitem_int_index(A, ind):
    gudut__hycnl = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    kqof__jlbq = A._data[gudut__hycnl]
    n = len(kqof__jlbq)
    jnb__phz = get_new_null_mask_int_index(old_mask, gudut__hycnl, n)
    return kqof__jlbq, jnb__phz


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    ahr__xlf = numba.cpython.unicode._normalize_slice(ind, n)
    erf__eqir = numba.cpython.unicode._slice_span(ahr__xlf)
    geyhb__wlg = erf__eqir + 7 >> 3
    jnb__phz = np.empty(geyhb__wlg, np.uint8)
    vdbp__moax = 0
    for gnbcm__dfarv in range(ahr__xlf.start, ahr__xlf.stop, ahr__xlf.step):
        kybw__uaivf = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            gnbcm__dfarv)
        bodo.libs.int_arr_ext.set_bit_to_arr(jnb__phz, vdbp__moax, kybw__uaivf)
        vdbp__moax += 1
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    kqof__jlbq = np.ascontiguousarray(A._data[ind])
    jnb__phz = get_new_null_mask_slice_index(old_mask, ind, n)
    return kqof__jlbq, jnb__phz
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for gnbcm__dfarv in range(n):
                A._data[idx[gnbcm__dfarv]] = val._data[gnbcm__dfarv]
                kybw__uaivf = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, gnbcm__dfarv)
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap, idx[
                    gnbcm__dfarv], kybw__uaivf)
        return impl_arr
    if bodo.utils.typing.is_scalar_type(val):

        def impl_scalar(A, idx, val):
            for gnbcm__dfarv in idx:
                A._data[gnbcm__dfarv] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            yktp__ruwez = 0
            for gnbcm__dfarv in range(n):
                if not bodo.libs.array_kernels.isna(idx, gnbcm__dfarv) and idx[
                    gnbcm__dfarv]:
                    A._data[gnbcm__dfarv] = val._data[yktp__ruwez]
                    kybw__uaivf = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, yktp__ruwez)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        gnbcm__dfarv, kybw__uaivf)
                    yktp__ruwez += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    ahr__xlf = numba.cpython.unicode._normalize_slice(idx, n)
    yktp__ruwez = 0
    for gnbcm__dfarv in range(ahr__xlf.start, ahr__xlf.stop, ahr__xlf.step):
        kybw__uaivf = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            yktp__ruwez)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, gnbcm__dfarv,
            kybw__uaivf)
        yktp__ruwez += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            ahr__xlf = numba.cpython.unicode._normalize_slice(idx, len(A))
            for gnbcm__dfarv in range(ahr__xlf.start, ahr__xlf.stop,
                ahr__xlf.step):
                A._data[gnbcm__dfarv] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                    gnbcm__dfarv, 1)
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for gnbcm__dfarv in idx:
                    bodo.libs.array_kernels.setna(A, gnbcm__dfarv)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    xlt__ftdok = bodo.libs.str_arr_ext.pre_alloc_string_array(n
                        , -1)
                    for gnbcm__dfarv in numba.parfors.parfor.internal_prange(n
                        ):
                        if idx[gnbcm__dfarv] or bodo.libs.array_kernels.isna(A,
                            gnbcm__dfarv):
                            xlt__ftdok[gnbcm__dfarv] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(xlt__ftdok,
                                gnbcm__dfarv)
                        else:
                            xlt__ftdok[gnbcm__dfarv] = A[gnbcm__dfarv]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for gnbcm__dfarv in range(n):
                    if not bodo.libs.array_kernels.isna(idx, gnbcm__dfarv
                        ) and idx[gnbcm__dfarv]:
                        bodo.libs.array_kernels.setna(A, gnbcm__dfarv)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for gnbcm__dfarv in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, gnbcm__dfarv)
                        continue
                    A[gnbcm__dfarv] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        olcgk__iexk = context.make_helper(builder, val_t, args[0])
        ospue__qbqrc = olcgk__iexk.data
        context.nrt.incref(builder, val_t.type, ospue__qbqrc)
        return ospue__qbqrc

...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    wfx__vvx = pq_node.out_vars[0].name
    ttvph__byb = pq_node.out_vars[1].name
    if wfx__vvx not in lives and ttvph__byb not in lives:
        return None
    elif wfx__vvx not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif ttvph__byb not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    tzb__ihgn = pq_node.out_vars[0].name
    assert isinstance(typemap[tzb__ihgn], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        bsp__ztehj, jsp__rtvvc = get_live_column_nums_block(column_live_map,
            equiv_vars, tzb__ihgn)
        bsp__ztehj = bodo.ir.connector.trim_extra_used_columns(bsp__ztehj,
            len(pq_node.col_indices))
        if not jsp__rtvvc and not bsp__ztehj:
            bsp__ztehj = [0]
        if not jsp__rtvvc and len(bsp__ztehj) != len(pq_node.type_usecol_offset
            ):
            pq_node.type_usecol_offset = bsp__ztehj
            return True
    return False

...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        zfr__vmrr = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(zfr__vmrr)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vbhe__svub = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, vbhe__svub)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        dqltj__shxld, = args
        tunxq__ldjmx = signature.return_type
        qwt__cgrxr = cgutils.create_struct_proxy(tunxq__ldjmx)(context, builder
            )
        qwt__cgrxr.obj = dqltj__shxld
        context.nrt.incref(builder, signature.args[0], dqltj__shxld)
        return qwt__cgrxr._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                chtu__dfh = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        zfr__vmrr = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(zfr__vmrr)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vbhe__svub = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, vbhe__svub)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        dqltj__shxld, = args
        owek__yarj = signature.return_type
        njbf__rjqd = cgutils.create_struct_proxy(owek__yarj)(context, builder)
        njbf__rjqd.obj = dqltj__shxld
        context.nrt.incref(builder, signature.args[0], dqltj__shxld)
        return njbf__rjqd._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                cnnbs__rtl = bodo.utils.conversion.coerce_to_ndarray(idx)
                tyre__ccm = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    cnnbs__rtl]
                qrsv__yute = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    cnnbs__rtl]
                zfr__vmrr = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(tyre__ccm,
                    qrsv__yute, zfr__vmrr)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                tyre__ccm = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                qrsv__yute = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                zfr__vmrr = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(tyre__ccm,
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        cnnbs__rtl = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        chtu__dfh = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(chtu__dfh))
                        cnnbs__rtl = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            cnnbs__rtl] = s
            ...
            def impl_arr(I, idx, val):
                cnnbs__rtl = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[cnnbs__rtl
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        zfr__vmrr = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(zfr__vmrr)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vbhe__svub = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, vbhe__svub)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        dqltj__shxld, = args
        skcf__gfxo = signature.return_type
        phsl__zewp = cgutils.create_struct_proxy(skcf__gfxo)(context, builder)
        phsl__zewp.obj = dqltj__shxld
        context.nrt.incref(builder, signature.args[0], dqltj__shxld)
        return phsl__zewp._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            tyre__ccm = bodo.hiframes.pd_series_ext.get_series_data(S)
            qrsv__yute = bodo.hiframes.pd_series_ext.get_series_index(S)
            ogr__cds = (idx - qrsv__yute._start) // qrsv__yute._step
            return tyre__ccm[ogr__cds]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    cnnbs__rtl = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(tyre__ccm[
                        cnnbs__rtl])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    vpse__vkri = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    miveo__xnqug = vpse__vkri.get_loc(idx)
                    if miveo__xnqug is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(miveo__xnqug)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                cnnbs__rtl = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[cnnbs__rtl
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        rnkwu__vhz = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(
            ukqj__aehyv, int) for ukqj__aehyv in rnkwu__vhz):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            zsv__aymwc = get_literal_value(idx)
            if zsv__aymwc not in rnkwu__vhz:
                raise_bodo_error(
                    f"Series label-based getitem: '{zsv__aymwc}' not in {rnkwu__vhz}"
                    )
            arr_ind = rnkwu__vhz.index(zsv__aymwc)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    qwt__cgrxr = cgutils.create_struct_proxy(fromty)(context, builder, val)
    vsve__liop = context.cast(builder, qwt__cgrxr.obj, fromty.stype, toty.stype
        )
    rhct__coq = cgutils.create_struct_proxy(toty)(context, builder)
    rhct__coq.obj = vsve__liop
    return rhct__coq._getvalue()

...
except ImportError as ukkct__vpu:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        lgx__xpsg = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(lgx__xpsg)


def random_forest_model_fit(m, X, y):
    qmuxf__zqgw = m.n_estimators
    pdbfj__jva = MPI.Get_processor_name()
    ojzcx__mwqlt = get_host_ranks()
    bxuel__smd = len(ojzcx__mwqlt)
    qzdd__uqoft = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(qmuxf__zqgw,
        bxuel__smd, qzdd__uqoft)
    if qzdd__uqoft == ojzcx__mwqlt[pdbfj__jva][0]:
        m.n_jobs = len(ojzcx__mwqlt[pdbfj__jva])
    ...
    duen__ouzx = create_subcomm_mpi4py(first_rank_node)
    if duen__ouzx != MPI.COMM_NULL:
        soeve__mzge = 10
        vuspg__cdc = bodo.libs.distributed_api.get_node_portion(qmuxf__zqgw,
            bxuel__smd, 0)
        vkeyo__cllwx = vuspg__cdc // soeve__mzge
        if vuspg__cdc % soeve__mzge != 0:
            vkeyo__cllwx += 1
        myjl__mrgu = []
        for tfbrq__agyc in range(vkeyo__cllwx):
            gska__plsl = duen__ouzx.gather(m.estimators_[tfbrq__agyc *
                soeve__mzge:tfbrq__agyc * soeve__mzge + soeve__mzge])
            if qzdd__uqoft == 0:
                myjl__mrgu += list(itertools.chain.from_iterable(gska__plsl))
        if qzdd__uqoft == 0:
            m.estimators_ = myjl__mrgu
    anu__lgb = MPI.COMM_WORLD
    if qzdd__uqoft == 0:
        for tfbrq__agyc in range(0, qmuxf__zqgw, 10):
            anu__lgb.bcast(m.estimators_[tfbrq__agyc:tfbrq__agyc + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            anu__lgb.bcast(m.n_classes_)
            anu__lgb.bcast(m.classes_)
        anu__lgb.bcast(m.n_outputs_)
    else:
        hxz__jposp = []
        for tfbrq__agyc in range(0, qmuxf__zqgw, 10):
            hxz__jposp += anu__lgb.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = anu__lgb.bcast(None)
            m.classes_ = anu__lgb.bcast(None)
        m.n_outputs_ = anu__lgb.bcast(None)
        m.estimators_ = hxz__jposp
    assert len(m.estimators_) == qmuxf__zqgw
    m.n_estimators = qmuxf__zqgw
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    qfree__uvlf = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    rnrl__ucbu = len(labels)
    hmicd__ltb = np.zeros(rnrl__ucbu, np.int64)
    chw__kvb = np.zeros(rnrl__ucbu, np.int64)
    aot__ivwnp = np.zeros(rnrl__ucbu, np.int64)
    tays__pnxuf = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for tfbrq__agyc in range(len(y_true)):
        chw__kvb[tays__pnxuf[y_true[tfbrq__agyc]]] += 1
        if y_pred[tfbrq__agyc] not in tays__pnxuf:
            continue
        bwazz__ljc = tays__pnxuf[y_pred[tfbrq__agyc]]
        aot__ivwnp[bwazz__ljc] += 1
        if y_true[tfbrq__agyc] == y_pred[tfbrq__agyc]:
            hmicd__ltb[bwazz__ljc] += 1
    hmicd__ltb = bodo.libs.distributed_api.dist_reduce(hmicd__ltb, np.int32
        (Reduce_Type.Sum.value))
    chw__kvb = bodo.libs.distributed_api.dist_reduce(chw__kvb, np.int32(
        Reduce_Type.Sum.value))
    aot__ivwnp = bodo.libs.distributed_api.dist_reduce(aot__ivwnp, np.int32
        (Reduce_Type.Sum.value))
    rnjmr__qzwiw = aot__ivwnp - hmicd__ltb
    zuhpi__bibl = chw__kvb - hmicd__ltb
    bnze__kwfhj = hmicd__ltb
    osmxa__rfxcj = y_true.shape[0] - bnze__kwfhj - rnjmr__qzwiw - zuhpi__bibl
    with numba.objmode(result='float64[:]'):
        MCM = np.array([osmxa__rfxcj, rnjmr__qzwiw, zuhpi__bibl, bnze__kwfhj]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        btzp__nvu = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        btzp__nvu = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    anu__lgb = MPI.COMM_WORLD
    huotl__unxl = anu__lgb.Get_size()
    if sample_weight is not None:
        lvyc__sab = np.sum(sample_weight)
    else:
        lvyc__sab = np.float64(y_true.shape[0])
    ecaty__zjib = np.zeros(huotl__unxl, dtype=type(lvyc__sab))
    anu__lgb.Allgather(lvyc__sab, ecaty__zjib)
    jvspw__mujty = np.zeros((huotl__unxl, *btzp__nvu.shape), dtype=
        btzp__nvu.dtype)
    anu__lgb.Allgather(btzp__nvu, jvspw__mujty)
    nmbt__wxe = np.average(jvspw__mujty, weights=ecaty__zjib, axis=0)
    if metric == 'mse' and not squared:
        nmbt__wxe = np.sqrt(nmbt__wxe)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return nmbt__wxe
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(nmbt__wxe)
    else:
        return np.average(nmbt__wxe, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    anu__lgb = MPI.COMM_WORLD
    score = anu__lgb.allreduce(score, op=MPI.SUM)
    if normalize:
        owqtq__gflbg = np.sum(sample_weight
            ) if sample_weight is not None else len(y_true)
        owqtq__gflbg = anu__lgb.allreduce(owqtq__gflbg, op=MPI.SUM)
        score = score / owqtq__gflbg
    return score
...
def check_consistent_length_parallel(*arrays):
    anu__lgb = MPI.COMM_WORLD
    cypbd__sqay = True
    mfl__okosa = [len(dgro__dsjdq) for dgro__dsjdq in arrays if dgro__dsjdq
         is not None]
    if len(np.unique(mfl__okosa)) > 1:
        cypbd__sqay = False
    cypbd__sqay = anu__lgb.allreduce(cypbd__sqay, op=MPI.LAND)
    return cypbd__sqay
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    qzay__zsbth = y_true.shape[0]
    opis__yedab = anu__lgb.allreduce(qzay__zsbth, op=MPI.SUM)
    if opis__yedab < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        bbwrs__feeap = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        bbwrs__feeap = 1.0
    btrhl__jguae = (bbwrs__feeap * (y_true - y_pred) ** 2).sum(axis=0,
        dtype=np.float64)
    gtp__xuav = np.zeros(btrhl__jguae.shape, dtype=btrhl__jguae.dtype)
    anu__lgb.Allreduce(btrhl__jguae, gtp__xuav, op=MPI.SUM)
    tzzs__xwkia = np.nansum(y_true * bbwrs__feeap, axis=0, dtype=np.float64)
    npjz__wzh = np.zeros_like(tzzs__xwkia)
    anu__lgb.Allreduce(tzzs__xwkia, npjz__wzh, op=MPI.SUM)
    pgw__xbmkm = np.nansum(sample_weight, dtype=np.float64)
    doci__xge = anu__lgb.allreduce(pgw__xbmkm, op=MPI.SUM)
    qcw__rla = npjz__wzh / doci__xge
    xtz__umaeb = (bbwrs__feeap * (y_true - qcw__rla) ** 2).sum(axis=0,
        dtype=np.float64)
    qiotw__cefqq = np.zeros(xtz__umaeb.shape, dtype=xtz__umaeb.dtype)
    anu__lgb.Allreduce(xtz__umaeb, qiotw__cefqq, op=MPI.SUM)
    gheoq__mhng = qiotw__cefqq != 0
    ugv__phf = gtp__xuav != 0
    irrqw__ibuvt = gheoq__mhng & ugv__phf
    sqq__xopnd = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    sqq__xopnd[irrqw__ibuvt] = 1 - gtp__xuav[irrqw__ibuvt] / qiotw__cefqq[
        irrqw__ibuvt]
    sqq__xopnd[ugv__phf & ~gheoq__mhng] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            spx__wbias = qiotw__cefqq
            if not np.any(gheoq__mhng):
                if not np.any(ugv__phf):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        spx__wbias = multioutput
    return np.array([np.average(sqq__xopnd, weights=spx__wbias)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    anu__lgb = MPI.COMM_WORLD
    try:
        jcx__hki = sklearn.metrics.confusion_matrix(y_true, y_pred, labels=
            labels, sample_weight=sample_weight, normalize=None)
    except ValueError as qkovo__jbe:
        jcx__hki = qkovo__jbe
    texoa__ngvs = isinstance(jcx__hki, ValueError
        ) and 'At least one label specified must be in y_true' in jcx__hki.args[
        0]
    yywj__ifhnq = anu__lgb.allreduce(texoa__ngvs, op=MPI.LAND)
    if yywj__ifhnq:
        raise jcx__hki
    elif texoa__ngvs:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        lvc__nodki = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        lvc__nodki = jcx__hki
    ukq__ligir = np.zeros_like(lvc__nodki)
    anu__lgb.Allreduce(lvc__nodki, ukq__ligir)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            ukq__ligir = ukq__ligir / ukq__ligir.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            ukq__ligir = ukq__ligir / ukq__ligir.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            ukq__ligir = ukq__ligir / ukq__ligir.sum()
        ukq__ligir = np.nan_to_num(ukq__ligir)
    return ukq__ligir
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    wdwup__dbgvk = 'def _confusion_matrix_impl(\n'
    wdwup__dbgvk += '    y_true, y_pred, labels=None,\n'
    wdwup__dbgvk += '    sample_weight=None, normalize=None,\n'
    wdwup__dbgvk += '    _is_data_distributed=False,\n'
    wdwup__dbgvk += '):\n'
    wdwup__dbgvk += (
        '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n')
    wdwup__dbgvk += (
    ...
    ijwcs__minqu = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        ijwcs__minqu = 'float64[:,:]', 'np.float64'
    if not is_overload_none(sample_weight):
        wdwup__dbgvk += """    sample_weight = bodo.utils.conversion.coerce_to_array(sample_weight)
"""
    ...
    if not is_overload_none(labels):
        ...
        wdwup__dbgvk += '    labels = bodo.allgatherv(labels, False)\n'
        wdwup__dbgvk += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    wdwup__dbgvk += f"    with numba.objmode(cm='{ijwcs__minqu[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        wdwup__dbgvk += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        wdwup__dbgvk += '      cm = confusion_matrix_dist_helper(\n'
    wdwup__dbgvk += '        y_true, y_pred, labels=labels,\n'
    wdwup__dbgvk += (
        '        sample_weight=sample_weight, normalize=normalize,\n')
    wdwup__dbgvk += f'      ).astype({ijwcs__minqu[1]})\n'
    wdwup__dbgvk += '    return cm\n'
    eucue__wsr = {}
    exec(wdwup__dbgvk, globals(), eucue__wsr)
    hpw__rcr = eucue__wsr['_confusion_matrix_impl']
    return hpw__rcr
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    anu__lgb = MPI.COMM_WORLD
    yfv__shx = anu__lgb.allreduce(len(X), op=MPI.SUM)
    cuwdh__psvks = len(X) / yfv__shx
    zum__qtlfn = anu__lgb.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    ngagf__xhme = np.inf
    kfsue__xzgva = 0
    if m.loss == 'hinge':
        mfw__sytm = hinge_loss
    elif m.loss == 'log':
        mfw__sytm = log_loss
    elif m.loss == 'squared_error':
        mfw__sytm = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    exp__miz = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        exp__miz = True
    for gjtp__qgiv in range(m.max_iter):
        if exp__miz:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * cuwdh__psvks
        m.coef_ = anu__lgb.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * cuwdh__psvks
        m.intercept_ = anu__lgb.allreduce(m.intercept_, op=MPI.SUM)
        if exp__miz:
            y_pred = m.predict(X)
            ddjq__owz = mfw__sytm(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            ddjq__owz = mfw__sytm(y, y_pred, labels=y_classes)
        wbzny__fujtb = anu__lgb.allreduce(ddjq__owz, op=MPI.SUM)
        ddjq__owz = wbzny__fujtb / zum__qtlfn
        if m.tol > np.NINF and ddjq__owz > ngagf__xhme - m.tol * yfv__shx:
            kfsue__xzgva += 1
        else:
            kfsue__xzgva = 0
        if ddjq__owz < ngagf__xhme:
            ngagf__xhme = ddjq__owz
        if kfsue__xzgva >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    anu__lgb = MPI.COMM_WORLD
    qzdd__uqoft = anu__lgb.Get_rank()
    pdbfj__jva = MPI.Get_processor_name()
    ojzcx__mwqlt = get_host_ranks()
    vby__czhg = m.n_jobs if hasattr(m, 'n_jobs') else None
    pcsp__luwi = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(ojzcx__mwqlt[pdbfj__jva])
    if qzdd__uqoft == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if qzdd__uqoft == 0:
        anu__lgb.bcast(m.cluster_centers_)
        anu__lgb.bcast(m.inertia_)
        anu__lgb.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = anu__lgb.bcast(None)
        m.inertia_ = anu__lgb.bcast(None)
        m.n_iter_ = anu__lgb.bcast(None)
    if _is_data_distributed:
        gii__rpmvq = anu__lgb.allgather(len_X)
        if qzdd__uqoft == 0:
            dnwi__czimt = np.empty(len(gii__rpmvq) + 1, dtype=int)
            np.cumsum(gii__rpmvq, out=dnwi__czimt[1:])
            dnwi__czimt[0] = 0
            bltmr__aen = [m.labels_[dnwi__czimt[wmq__ralm]:dnwi__czimt[
                wmq__ralm + 1]] for wmq__ralm in range(len(gii__rpmvq))]
            nmzq__veou = anu__lgb.scatter(bltmr__aen)
        else:
            nmzq__veou = anu__lgb.scatter(None)
        m.labels_ = nmzq__veou
    elif qzdd__uqoft == 0:
        anu__lgb.bcast(m.labels_)
    else:
        m.labels_ = anu__lgb.bcast(None)
    m._n_threads = pcsp__luwi
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                anu__lgb = MPI.COMM_WORLD
                result = anu__lgb.allreduce(result, op=MPI.SUM)
            m._n_threads = pcsp__luwi
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        wdwup__dbgvk = 'def _model_multinomial_nb_fit_impl(\n'
        wdwup__dbgvk += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        wdwup__dbgvk += '):  # pragma: no cover\n'
        wdwup__dbgvk += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            wdwup__dbgvk += '    X = X.to_numpy()\n'
        else:
            wdwup__dbgvk += (
                '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n')
        wdwup__dbgvk += '    my_rank = bodo.get_rank()\n'
        wdwup__dbgvk += '    nranks = bodo.get_size()\n'
        wdwup__dbgvk += '    total_cols = X.shape[1]\n'
        wdwup__dbgvk += '    for i in range(nranks):\n'
        wdwup__dbgvk += """        start = bodo.libs.distributed_api.get_start(total_cols, nranks, i)
"""
        wdwup__dbgvk += (
            '        end = bodo.libs.distributed_api.get_end(total_cols, nranks, i)\n'
            )
        wdwup__dbgvk += '        if i == my_rank:\n'
        wdwup__dbgvk += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        wdwup__dbgvk += '        else:\n'
        wdwup__dbgvk += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        wdwup__dbgvk += '    y_train = bodo.allgatherv(y, False)\n'
        wdwup__dbgvk += '    with numba.objmode(m="multinomial_nb_type"):\n'
        wdwup__dbgvk += '        m = fit_multinomial_nb(\n'
        wdwup__dbgvk += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        wdwup__dbgvk += '        )\n'
        wdwup__dbgvk += '    bodo.barrier()\n'
        wdwup__dbgvk += '    return m\n'
        eucue__wsr = {}
        exec(wdwup__dbgvk, globals(), eucue__wsr)
        pjd__ndir = eucue__wsr['_model_multinomial_nb_fit_impl']
        return pjd__ndir
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    gjtp__qgiv, n_features = X_train.shape
    m.n_features_in_ = n_features
    jrvz__sal = LabelBinarizer()
    jisi__mdrv = jrvz__sal.fit_transform(y_train)
    m.classes_ = jrvz__sal.classes_
    if jisi__mdrv.shape[1] == 1:
        jisi__mdrv = np.concatenate((1 - jisi__mdrv, jisi__mdrv), axis=1)
    if sample_weight is not None:
        jisi__mdrv = jisi__mdrv.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        jisi__mdrv *= sample_weight.T
    class_prior = m.class_prior
    tsze__vrmht = jisi__mdrv.shape[1]
    m._init_counters(tsze__vrmht, n_features)
    m._count(X_train.astype('float64'), jisi__mdrv)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    vnlsk__vpzi = m.feature_count_ + alpha
    ygrdf__srdoz = vnlsk__vpzi.sum(axis=1)
    anu__lgb = MPI.COMM_WORLD
    zum__qtlfn = anu__lgb.Get_size()
    qwcgy__zysu = np.zeros(tsze__vrmht)
    anu__lgb.Allreduce(ygrdf__srdoz, qwcgy__zysu, op=MPI.SUM)
    liu__pwmxi = np.log(vnlsk__vpzi) - np.log(qwcgy__zysu.reshape(-1, 1))
    rdil__lwa = liu__pwmxi.T.reshape(n_features * tsze__vrmht)
    udve__dshxb = np.ones(zum__qtlfn) * (total_cols // zum__qtlfn)
    lvtvd__kmdn = total_cols % zum__qtlfn
    for ylev__bqoe in range(lvtvd__kmdn):
        udve__dshxb[ylev__bqoe] += 1
    udve__dshxb *= tsze__vrmht
    zfmu__lypp = np.zeros(zum__qtlfn, dtype=np.int32)
    zfmu__lypp[1:] = np.cumsum(udve__dshxb)[:-1]
    tzxm__cjucy = np.zeros((total_cols, tsze__vrmht), dtype=np.float64)
    anu__lgb.Allgatherv(rdil__lwa, [tzxm__cjucy, udve__dshxb, zfmu__lypp,
        MPI.DOUBLE_PRECISION])
    m.feature_log_prob_ = tzxm__cjucy.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    anu__lgb = MPI.COMM_WORLD
    huotl__unxl = anu__lgb.Get_size()
    xrys__wxvd = m.with_std
    rfdfc__fcf = m.with_mean
    m.with_std = False
    if xrys__wxvd:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = xrys__wxvd
    m.with_mean = rfdfc__fcf
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    dmlk__cjbz = np.zeros((huotl__unxl, *m.n_samples_seen_.shape), dtype=m.
        n_samples_seen_.dtype)
    anu__lgb.Allgather(m.n_samples_seen_, dmlk__cjbz)
    ovd__buxc = np.sum(dmlk__cjbz, axis=0)
    m.n_samples_seen_ = ovd__buxc
    if m.with_mean or m.with_std:
        drtf__khtob = np.zeros((huotl__unxl, *m.mean_.shape), dtype=m.mean_
            .dtype)
        anu__lgb.Allgather(m.mean_, drtf__khtob)
        drtf__khtob[np.isnan(drtf__khtob)] = 0
        kvaue__gxof = np.average(drtf__khtob, axis=0, weights=dmlk__cjbz)
        m.mean_ = kvaue__gxof
    if m.with_std:
        okevd__oky = sklearn_safe_accumulator_op(np.nansum, (X -
            kvaue__gxof) ** 2, axis=0) / ovd__buxc
        uxml__zbwst = np.zeros_like(okevd__oky)
        anu__lgb.Allreduce(okevd__oky, uxml__zbwst, op=MPI.SUM)
        m.var_ = uxml__zbwst
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    hrq__ultfz = anu__lgb.allreduce(hrq__ultfz, op=MPI.LAND)
    if hrq__ultfz:
...
def get_data_slice_parallel(data, labels, len_train):
    bmh__ohe = data[:len_train]
    vsh__yku = data[len_train:]
    bmh__ohe = bodo.rebalance(bmh__ohe)
    vsh__yku = bodo.rebalance(vsh__yku)
    wnlr__hno = labels[:len_train]
    ouyh__fgaz = labels[len_train:]
    wnlr__hno = bodo.rebalance(wnlr__hno)
    ouyh__fgaz = bodo.rebalance(ouyh__fgaz)
    return bmh__ohe, vsh__yku, wnlr__hno, ouyh__fgaz
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    jlan__jig = {'stratify': stratify}
    cwhy__nhvqs = {'stratify': None}
    check_unsupported_args('train_test_split', jlan__jig, cwhy__nhvqs, 'ml')
    if is_overload_false(_is_data_distributed):
        yawy__gqev = f'data_split_type_{numba.core.ir_utils.next_label()}'
        knb__umnkn = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for jak__kanzj, sys__eyo in ((data, yawy__gqev), (labels, knb__umnkn)):
            if isinstance(jak__kanzj, (DataFrameType, SeriesType)):
                cum__wxoyq = jak__kanzj.copy(index=NumericIndexType(types.
                    int64))
                setattr(types, sys__eyo, cum__wxoyq)
            else:
                setattr(types, sys__eyo, jak__kanzj)
        wdwup__dbgvk = 'def _train_test_split_impl(\n'
        wdwup__dbgvk += '    data,\n'
        wdwup__dbgvk += '    labels=None,\n'
        wdwup__dbgvk += '    train_size=None,\n'
        wdwup__dbgvk += '    test_size=None,\n'
        wdwup__dbgvk += '    random_state=None,\n'
        wdwup__dbgvk += '    shuffle=True,\n'
        wdwup__dbgvk += '    stratify=None,\n'
        wdwup__dbgvk += '    _is_data_distributed=False,\n'
        wdwup__dbgvk += '):  # pragma: no cover\n'
        wdwup__dbgvk += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(yawy__gqev, yawy__gqev, knb__umnkn, knb__umnkn))
        wdwup__dbgvk += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        wdwup__dbgvk += '            data,\n'
        wdwup__dbgvk += '            labels,\n'
        wdwup__dbgvk += '            train_size=train_size,\n'
        wdwup__dbgvk += '            test_size=test_size,\n'
        wdwup__dbgvk += '            random_state=random_state,\n'
        wdwup__dbgvk += '            shuffle=shuffle,\n'
        wdwup__dbgvk += '            stratify=stratify,\n'
        wdwup__dbgvk += '        )\n'
        wdwup__dbgvk += (
            '    return data_train, data_test, labels_train, labels_test\n')
        eucue__wsr = {}
        exec(wdwup__dbgvk, globals(), eucue__wsr)
        _train_test_split_impl = eucue__wsr['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            kcc__giuyo = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(kcc__giuyo * train_size)
            sgp__yll = kcc__giuyo - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                qzdd__uqoft = bodo.get_rank()
                zum__qtlfn = bodo.get_size()
                fdd__vsfky = np.empty(zum__qtlfn, np.int64)
                bodo.libs.distributed_api.allgather(fdd__vsfky, len(data))
                wmj__kym = np.cumsum(fdd__vsfky[0:qzdd__uqoft + 1])
                caqf__nor = np.full(kcc__giuyo, True)
                caqf__nor[:sgp__yll] = False
                np.random.seed(42)
                np.random.permutation(caqf__nor)
                if qzdd__uqoft:
                    ofkhh__hvgkl = wmj__kym[qzdd__uqoft - 1]
                else:
                    ofkhh__hvgkl = 0
                qkozp__yep = wmj__kym[qzdd__uqoft]
                nqzy__qofy = caqf__nor[ofkhh__hvgkl:qkozp__yep]
                bmh__ohe = data[nqzy__qofy]
                vsh__yku = data[~nqzy__qofy]
                wnlr__hno = labels[nqzy__qofy]
                ouyh__fgaz = labels[~nqzy__qofy]
                bmh__ohe = bodo.random_shuffle(bmh__ohe, seed=random_state,
                    parallel=True)
                vsh__yku = bodo.random_shuffle(vsh__yku, seed=random_state,
                    parallel=True)
                wnlr__hno = bodo.random_shuffle(wnlr__hno, seed=
                    random_state, parallel=True)
                ouyh__fgaz = bodo.random_shuffle(ouyh__fgaz, seed=
                    random_state, parallel=True)
                wnlr__hno = reset_labels_type(wnlr__hno, label_type)
                ouyh__fgaz = reset_labels_type(ouyh__fgaz, label_type)
            else:
                bmh__ohe, vsh__yku, wnlr__hno, ouyh__fgaz = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    huotl__unxl = anu__lgb.Get_size()
    m = m.fit(X)
    ovd__buxc = anu__lgb.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = ovd__buxc
    wsykj__dajdj = np.zeros((huotl__unxl, *m.data_min_.shape), dtype=m.
        data_min_.dtype)
    anu__lgb.Allgather(m.data_min_, wsykj__dajdj)
    ntzm__tuqbr = np.nanmin(wsykj__dajdj, axis=0)
    joi__ppm = np.zeros((huotl__unxl, *m.data_max_.shape), dtype=m.
        data_max_.dtype)
    anu__lgb.Allgather(m.data_max_, joi__ppm)
    cee__kwyvb = np.nanmax(joi__ppm, axis=0)
    ezkw__wnee = cee__kwyvb - ntzm__tuqbr
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(ezkw__wnee)
    m.min_ = m.feature_range[0] - ntzm__tuqbr * m.scale_
    m.data_min_ = ntzm__tuqbr
    m.data_max_ = cee__kwyvb
    m.data_range_ = ezkw__wnee
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    cmax__qalzp = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        cmax__qalzp = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{cmax__qalzp}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            bxuel__smd = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if bxuel__smd > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=bxuel__smd)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        nhkoc__dkfs = True
    return nhkoc__dkfs, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                qhrqm__aigx = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                qhrqm__aigx = bodo.allgatherv(qhrqm__aigx, False)
                qhrqm__aigx = bodo.libs.array_kernels.sort(qhrqm__aigx,
                    ascending=True, inplace=True)
                mtgd__vnh = {}
                for tfbrq__agyc in range(len(qhrqm__aigx)):
                    mtgd__vnh[qhrqm__aigx[tfbrq__agyc]] = tfbrq__agyc
            else:
                mtgd__vnh = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = mtgd__vnh
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X

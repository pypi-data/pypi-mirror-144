...
def install_mpl_class(types_name, python_type):
    bwk__fgo = ''.join(map(str.title, types_name.split('_')))
    ekvjn__eej = f'class {bwk__fgo}(types.Opaque):\n'
    ekvjn__eej += '    def __init__(self):\n'
    ekvjn__eej += f"       types.Opaque.__init__(self, name='{bwk__fgo}')\n"
    ekvjn__eej += '    def __reduce__(self):\n'
    ekvjn__eej += (
        f"        return (types.Opaque, ('{bwk__fgo}',), self.__dict__)\n")
    zvg__aym = {}
    exec(ekvjn__eej, {'types': types, 'bodo': bodo}, zvg__aym)
    zex__kkjm = zvg__aym[bwk__fgo]
    qfm__wova = sys.modules[__name__]
    setattr(qfm__wova, bwk__fgo, zex__kkjm)
    class_instance = zex__kkjm()
    setattr(types, types_name, class_instance)
    register_model(zex__kkjm)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(zex__kkjm)(unbox_mpl_obj)
    box(zex__kkjm)(box_mpl_obj)
...
def _install_mpl_types():
    amez__zpybq = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for zgucw__llwct, zfp__hokit in amez__zpybq:
        install_mpl_class(zgucw__llwct, zfp__hokit)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    bked__rgfkd = ', '.join(f'e{gdn__vmyc}' for gdn__vmyc in range(len(args)))
    if bked__rgfkd:
        bked__rgfkd += ', '
    mkmd__fzija = ', '.join(f"{ozvh__lvzkk} = ''" for ozvh__lvzkk in kws.keys()
        )
    tcc__opb = 'matplotlib_obj, ' if obj_typ is not None else ''
    dfo__cvlsi = f'def mpl_stub({tcc__opb} {bked__rgfkd} {mkmd__fzija}):\n'
    dfo__cvlsi += '    pass\n'
    eie__stf = {}
    exec(dfo__cvlsi, {}, eie__stf)
    agl__eaxw = eie__stf['mpl_stub']
    gqwdl__jmrb = numba.core.utils.pysignature(agl__eaxw)
    xngot__ttr = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(xngot__ttr)).replace(pysig=
        gqwdl__jmrb)


def generate_axes_typing(mod_name, nrows, ncols):
    vhw__msn = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(vhw__msn.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(vhw__msn.format(mod_name, 'ncols'))
    izj__cwchh = get_overload_const_int(nrows)
    hzikk__ggux = get_overload_const_int(ncols)
    if izj__cwchh < 1:
        raise BodoError(vhw__msn.format(mod_name, 'nrows'))
    if hzikk__ggux < 1:
        raise BodoError(vhw__msn.format(mod_name, 'ncols'))
    if izj__cwchh == 1 and hzikk__ggux == 1:
        nsya__ravwl = types.mpl_axes_type
    else:
        if hzikk__ggux == 1:
            kljr__yol = types.mpl_axes_type
        else:
            kljr__yol = types.Tuple([types.mpl_axes_type] * hzikk__ggux)
        nsya__ravwl = types.Tuple([kljr__yol] * izj__cwchh)
    return nsya__ravwl


def generate_pie_return_type(args, kws):
    gas__tsk = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if gas__tsk == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    xdtfm__bzkq = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(xdtfm__bzkq):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(xdtfm__bzkq):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        gllmn__vzb = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, gllmn__vzb]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    gllmn__vzb = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    zgucw__llwct = str(gllmn__vzb)
    if not hasattr(types, zgucw__llwct):
        zgucw__llwct = f'objmode_type{ir_utils.next_label()}'
        setattr(types, zgucw__llwct, gllmn__vzb)
    dfo__cvlsi = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{zgucw__llwct}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    eie__stf = {}
    exec(dfo__cvlsi, {'numba': numba, 'np': np}, eie__stf)
    impl = eie__stf['impl']
    return impl

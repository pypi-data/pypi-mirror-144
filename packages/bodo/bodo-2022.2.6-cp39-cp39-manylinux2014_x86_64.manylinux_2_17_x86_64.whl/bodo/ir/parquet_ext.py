...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    fus__kpuf = pq_node.out_vars[0].name
    ptb__mivia = pq_node.out_vars[1].name
    if fus__kpuf not in lives and ptb__mivia not in lives:
        return None
    elif fus__kpuf not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif ptb__mivia not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    yzqi__opqq = pq_node.out_vars[0].name
    assert isinstance(typemap[yzqi__opqq], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        ipuyb__lcogq, tnsn__dedma = get_live_column_nums_block(column_live_map,
            equiv_vars, yzqi__opqq)
        ipuyb__lcogq = bodo.ir.connector.trim_extra_used_columns(ipuyb__lcogq,
            len(pq_node.col_indices))
        if not tnsn__dedma and not ipuyb__lcogq:
            ipuyb__lcogq = [0]
        if not tnsn__dedma and len(ipuyb__lcogq) != len(pq_node.
            type_usecol_offset):
            pq_node.type_usecol_offset = ipuyb__lcogq
            return True
    return False

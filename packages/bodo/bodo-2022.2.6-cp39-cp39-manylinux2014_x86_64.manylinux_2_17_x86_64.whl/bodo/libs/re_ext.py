...
def box_regex_flag(typ, val, c):
    dsn__selh = c.box(types.int64, val)
    iun__kwvdj = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(iun__kwvdj, (dsn__selh,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    dsn__selh = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, dsn__selh)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    enzmg__mdo = context.get_python_api(builder)
    vbnj__rhe = builder.icmp_signed('==', val, enzmg__mdo.borrow_none())
    with builder.if_then(vbnj__rhe):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    obk__kjsq = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(obk__kjsq)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        dkmid__mthta = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if dkmid__mthta > 1:
            typ = types.List(types.Tuple([string_type] * dkmid__mthta))
...
def overload_match_group(m, *args):
    ...
    ordlg__aue = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            ddnnw__emwuj = args[0]
            with numba.objmode(out=ordlg__aue):
                out = m.group(ddnnw__emwuj)
            return out
        return _match_group_impl_one
    aqnib__ejnv = 'tuple_str_{}'.format(len(args))
    setattr(types, aqnib__ejnv, types.Tuple([ordlg__aue] * len(args)))
    wjo__vdm = ', '.join('group{}'.format(mbo__qvowi + 1) for mbo__qvowi in
        range(len(args)))
    ypll__vwmk = 'def _match_group_impl(m, *args):\n'
    ypll__vwmk += '  ({}) = args\n'.format(wjo__vdm)
    ypll__vwmk += "  with numba.objmode(out='{}'):\n".format(aqnib__ejnv)
    ypll__vwmk += '    out = m.group({})\n'.format(wjo__vdm)
    ypll__vwmk += '  return out\n'
    mhgls__lso = {}
    exec(ypll__vwmk, globals(), mhgls__lso)
    kzv__sxeu = mhgls__lso['_match_group_impl']
    return kzv__sxeu
...
def overload_match_groups(m, default=None):
    rbcz__rtvr = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=rbcz__rtvr):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(jvn__thvvz is None for jvn__thvvz in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=ordlg__aue):

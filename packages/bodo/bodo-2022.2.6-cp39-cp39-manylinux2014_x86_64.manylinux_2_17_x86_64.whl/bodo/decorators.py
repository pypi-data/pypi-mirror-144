...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        qgb__honku = self.overloads[sig]
        kfpd__plhvy = qgb__honku.metadata.get('distributed_diagnostics', None)
        if kfpd__plhvy is None:
            yqug__soesi = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(yqug__soesi)
        kfpd__plhvy.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        mkky__vfv = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(mkky__vfv)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        zmjmr__oijr = options.pop('distributed')
        pipeline_class = (pipeline_class if zmjmr__oijr else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        ckb__rbjpb = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if ckb__rbjpb else
            pipeline_class)
    twzh__dpiz = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(twzh__dpiz, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(twzh__dpiz)
        else:
            return master_mode_wrapper(twzh__dpiz)
    else:
        return twzh__dpiz

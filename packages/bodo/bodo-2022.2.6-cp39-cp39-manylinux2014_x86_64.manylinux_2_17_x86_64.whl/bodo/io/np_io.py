...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        bhhz__bnsk = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, bhhz__bnsk)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            nqkty__frr = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(nqkty__frr.dtype)
            file_write(unicode_to_utf8(fname), nqkty__frr.ctypes, 
                dtype_size * nqkty__frr.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            nqkty__frr = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(nqkty__frr.dtype)
            zpai__sgts = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                nqkty__frr.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), nqkty__frr.ctypes,
                start, count, zpai__sgts)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            bbwig__trpy = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                bbwig__trpy = min(bbwig__trpy, count * dtype_size)
            if bbwig__trpy < 0:
                return -1
            return bbwig__trpy
        return impl

...
def _global_except_hook(exctype, value, traceback):
    ...
    xoma__wwa = MPI.COMM_WORLD
    oyvt__cayf = xoma__wwa.Get_rank()
    ntohz__rcgpg = 3.0
    ngzgc__vqrr = True
    knrh__thg = xoma__wwa.Ibarrier()
    qhauf__idw = time.time()
    while time.time() - qhauf__idw < ntohz__rcgpg:
        time.sleep(0.1)
        if knrh__thg.Test():
            ngzgc__vqrr = False
            break
    try:
        ...
        if ngzgc__vqrr:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {oyvt__cayf}. \n')

...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        cokgi__ages = []
        kwc__pxkfd = h5g_get_num_objs(obj_id)
        for xtpw__yfo in range(kwc__pxkfd):
            fap__hnv = h5g_get_objname_by_idx(obj_id, xtpw__yfo)
            cokgi__ages.append(fap__hnv)
        return cokgi__ages
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    chbtw__vtxa = parse_dtype(dtype)
    aojvx__ypfy = np.int32(numba_to_c_type(chbtw__vtxa))
    ...
    def impl(obj_id, name, shape=None, dtype=None, data=None):
        ...
        return h5_create_dset(unify_h5_id(obj_id), unicode_to_utf8(name),
            ndim, counts.ctypes, aojvx__ypfy)
    return impl
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            wihys__vgbns = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    nuyzj__tgvdg = tuple_to_ptr(starts)
    gen__oljbx = tuple_to_ptr(counts)
    psz__tvpa = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, nuyzj__tgvdg,
        gen__oljbx, is_parallel, out_arr.ctypes, psz__tvpa)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    gen__oljbx = tuple_to_ptr(counts)
    psz__tvpa = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, nuyzj__tgvdg,
        gen__oljbx, is_parallel, out_arr.ctypes, psz__tvpa)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        oan__oqji = types.Array(dtype, ndim, 'C')
        hcdy__vxw = args[3]
        oan__oqji = self.context.resolve_function_type(operator.getitem, [
            oan__oqji, hcdy__vxw], {}).return_type.copy(layout='C')
        return signature(oan__oqji, *args)
...
def get_filter_read_indices(bool_arr):
    rdp__hyshe = bool_arr.nonzero()[0]
    emz__wuo = bodo.libs.distributed_api.get_rank()
    lmg__ngo = bodo.libs.distributed_api.get_size()
    ibnu__rtbu = np.empty(lmg__ngo, np.int64)
    qefll__tarps = len(bool_arr)
    bodo.libs.distributed_api.allgather(ibnu__rtbu, qefll__tarps)
    itor__xprg = ibnu__rtbu.cumsum()[emz__wuo] - qefll__tarps
    rdp__hyshe += itor__xprg
    yodik__iiqjq = bodo.libs.distributed_api.dist_reduce(len(rdp__hyshe),
        np.int32(sum_op))
    vbwqk__ixyw = bodo.libs.distributed_api.gatherv(rdp__hyshe)
    if emz__wuo == 0:
        ibpdp__lfba = vbwqk__ixyw
    else:
        ibpdp__lfba = np.empty(yodik__iiqjq, rdp__hyshe.dtype)
    bodo.libs.distributed_api.bcast(ibpdp__lfba)
    fupek__jjisi = bodo.libs.distributed_api.get_start(yodik__iiqjq,
        lmg__ngo, emz__wuo)
    ysqg__jvdig = bodo.libs.distributed_api.get_end(yodik__iiqjq, lmg__ngo,
        emz__wuo)
    return ibpdp__lfba[fupek__jjisi:ysqg__jvdig]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        grc__rlyqz = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], grc__rlyqz)
        return builder.bitcast(grc__rlyqz, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    gen__oljbx = tuple_to_ptr(counts)
    psz__tvpa = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, nuyzj__tgvdg,
        gen__oljbx, is_parallel, out_arr.ctypes, psz__tvpa, read_indices.

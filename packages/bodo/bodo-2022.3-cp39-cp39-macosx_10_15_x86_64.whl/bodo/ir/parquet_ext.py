...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    kbajw__tku = pq_node.out_vars[0].name
    mjsh__wvl = pq_node.out_vars[1].name
    if kbajw__tku not in lives and mjsh__wvl not in lives:
        return None
    elif kbajw__tku not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif mjsh__wvl not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    gwh__rfr = pq_node.out_vars[0].name
    assert isinstance(typemap[gwh__rfr], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        frkl__yjd, ruo__jxbqk = get_live_column_nums_block(column_live_map,
            equiv_vars, gwh__rfr)
        frkl__yjd = bodo.ir.connector.trim_extra_used_columns(frkl__yjd,
            len(pq_node.col_indices))
        if not ruo__jxbqk and not frkl__yjd:
            frkl__yjd = [0]
        if not ruo__jxbqk and len(frkl__yjd) != len(pq_node.type_usecol_offset
            ):
            pq_node.type_usecol_offset = frkl__yjd
            return True
    return False

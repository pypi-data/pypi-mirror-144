...
def box_regex_flag(typ, val, c):
    vzaa__wnjl = c.box(types.int64, val)
    nrp__fufbm = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(nrp__fufbm, (vzaa__wnjl,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    vzaa__wnjl = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, vzaa__wnjl)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    ipm__cieb = context.get_python_api(builder)
    ivuo__shxw = builder.icmp_signed('==', val, ipm__cieb.borrow_none())
    with builder.if_then(ivuo__shxw):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    zpx__xau = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(zpx__xau)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        usc__xtvy = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if usc__xtvy > 1:
            typ = types.List(types.Tuple([string_type] * usc__xtvy))
...
def overload_match_group(m, *args):
    ...
    lhth__wcj = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            hyfai__kxiz = args[0]
            with numba.objmode(out=lhth__wcj):
                out = m.group(hyfai__kxiz)
            return out
        return _match_group_impl_one
    arnju__pckho = 'tuple_str_{}'.format(len(args))
    setattr(types, arnju__pckho, types.Tuple([lhth__wcj] * len(args)))
    hzgxy__htuev = ', '.join('group{}'.format(vcoij__yxqkp + 1) for
        vcoij__yxqkp in range(len(args)))
    gvx__mniuw = 'def _match_group_impl(m, *args):\n'
    gvx__mniuw += '  ({}) = args\n'.format(hzgxy__htuev)
    gvx__mniuw += "  with numba.objmode(out='{}'):\n".format(arnju__pckho)
    gvx__mniuw += '    out = m.group({})\n'.format(hzgxy__htuev)
    gvx__mniuw += '  return out\n'
    njf__sdy = {}
    exec(gvx__mniuw, globals(), njf__sdy)
    mrxi__zewt = njf__sdy['_match_group_impl']
    return mrxi__zewt
...
def overload_match_groups(m, default=None):
    iafs__whzl = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=iafs__whzl):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(gwyzw__hwp is None for gwyzw__hwp in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=lhth__wcj):

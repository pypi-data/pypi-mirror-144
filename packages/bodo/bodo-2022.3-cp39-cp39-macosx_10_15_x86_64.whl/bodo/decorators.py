...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        dwqgo__uuxr = self.overloads[sig]
        cpeb__ddwa = dwqgo__uuxr.metadata.get('distributed_diagnostics', None)
        if cpeb__ddwa is None:
            zcj__ikml = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(zcj__ikml)
        cpeb__ddwa.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        gzn__oxww = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(gzn__oxww)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        fyi__jfkw = options.pop('distributed')
        pipeline_class = (pipeline_class if fyi__jfkw else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        mtcw__seoa = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if mtcw__seoa else
            pipeline_class)
    msrtt__nyimf = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(msrtt__nyimf, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(msrtt__nyimf)
        else:
            return master_mode_wrapper(msrtt__nyimf)
    else:
        return msrtt__nyimf

...
except ImportError as rov__oupn:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        flnn__lvrz = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(flnn__lvrz)


def random_forest_model_fit(m, X, y):
    lna__mflfn = m.n_estimators
    lka__aae = MPI.Get_processor_name()
    bup__ihgbv = get_host_ranks()
    wcww__aojm = len(bup__ihgbv)
    uwow__wsb = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(lna__mflfn,
        wcww__aojm, uwow__wsb)
    if uwow__wsb == bup__ihgbv[lka__aae][0]:
        m.n_jobs = len(bup__ihgbv[lka__aae])
    ...
    qfk__gmyc = create_subcomm_mpi4py(first_rank_node)
    if qfk__gmyc != MPI.COMM_NULL:
        oiyep__stjg = 10
        eei__bznb = bodo.libs.distributed_api.get_node_portion(lna__mflfn,
            wcww__aojm, 0)
        hidz__rpu = eei__bznb // oiyep__stjg
        if eei__bznb % oiyep__stjg != 0:
            hidz__rpu += 1
        xklk__qvy = []
        for ynsjc__tnurk in range(hidz__rpu):
            jhmop__tzp = qfk__gmyc.gather(m.estimators_[ynsjc__tnurk *
                oiyep__stjg:ynsjc__tnurk * oiyep__stjg + oiyep__stjg])
            if uwow__wsb == 0:
                xklk__qvy += list(itertools.chain.from_iterable(jhmop__tzp))
        if uwow__wsb == 0:
            m.estimators_ = xklk__qvy
    nxa__vswm = MPI.COMM_WORLD
    if uwow__wsb == 0:
        for ynsjc__tnurk in range(0, lna__mflfn, 10):
            nxa__vswm.bcast(m.estimators_[ynsjc__tnurk:ynsjc__tnurk + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            nxa__vswm.bcast(m.n_classes_)
            nxa__vswm.bcast(m.classes_)
        nxa__vswm.bcast(m.n_outputs_)
    else:
        rlm__fjm = []
        for ynsjc__tnurk in range(0, lna__mflfn, 10):
            rlm__fjm += nxa__vswm.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = nxa__vswm.bcast(None)
            m.classes_ = nxa__vswm.bcast(None)
        m.n_outputs_ = nxa__vswm.bcast(None)
        m.estimators_ = rlm__fjm
    assert len(m.estimators_) == lna__mflfn
    m.n_estimators = lna__mflfn
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    rnh__ewz = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
        ...
        sklearn.metrics._classification.multilabel_confusion_matrix = rnh__ewz
    return result
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    aclbs__aaa = len(labels)
    rrh__ajoc = np.zeros(aclbs__aaa, np.int64)
    ccchd__yym = np.zeros(aclbs__aaa, np.int64)
    ghqzv__ithjt = np.zeros(aclbs__aaa, np.int64)
    hils__jbz = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for ynsjc__tnurk in range(len(y_true)):
        ccchd__yym[hils__jbz[y_true[ynsjc__tnurk]]] += 1
        if y_pred[ynsjc__tnurk] not in hils__jbz:
            continue
        fytgw__owzl = hils__jbz[y_pred[ynsjc__tnurk]]
        ghqzv__ithjt[fytgw__owzl] += 1
        if y_true[ynsjc__tnurk] == y_pred[ynsjc__tnurk]:
            rrh__ajoc[fytgw__owzl] += 1
    rrh__ajoc = bodo.libs.distributed_api.dist_reduce(rrh__ajoc, np.int32(
        Reduce_Type.Sum.value))
    ccchd__yym = bodo.libs.distributed_api.dist_reduce(ccchd__yym, np.int32
        (Reduce_Type.Sum.value))
    ghqzv__ithjt = bodo.libs.distributed_api.dist_reduce(ghqzv__ithjt, np.
        int32(Reduce_Type.Sum.value))
    zwpkd__jsnu = ghqzv__ithjt - rrh__ajoc
    gmzw__zwda = ccchd__yym - rrh__ajoc
    qzi__gswe = rrh__ajoc
    elops__oelnl = y_true.shape[0] - qzi__gswe - zwpkd__jsnu - gmzw__zwda
    with numba.objmode(result='float64[:]'):
        MCM = np.array([elops__oelnl, zwpkd__jsnu, gmzw__zwda, qzi__gswe]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        tzg__icd = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        tzg__icd = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    nxa__vswm = MPI.COMM_WORLD
    uzhu__szl = nxa__vswm.Get_size()
    if sample_weight is not None:
        grwwl__xhnd = np.sum(sample_weight)
    else:
        grwwl__xhnd = np.float64(y_true.shape[0])
    lzuuf__lnk = np.zeros(uzhu__szl, dtype=type(grwwl__xhnd))
    nxa__vswm.Allgather(grwwl__xhnd, lzuuf__lnk)
    sfqc__hli = np.zeros((uzhu__szl, *tzg__icd.shape), dtype=tzg__icd.dtype)
    nxa__vswm.Allgather(tzg__icd, sfqc__hli)
    vin__upwe = np.average(sfqc__hli, weights=lzuuf__lnk, axis=0)
    if metric == 'mse' and not squared:
        vin__upwe = np.sqrt(vin__upwe)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return vin__upwe
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(vin__upwe)
    else:
        return np.average(vin__upwe, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    nxa__vswm = MPI.COMM_WORLD
    score = nxa__vswm.allreduce(score, op=MPI.SUM)
    if normalize:
        kuxk__uzjaj = np.sum(sample_weight
            ) if sample_weight is not None else len(y_true)
        kuxk__uzjaj = nxa__vswm.allreduce(kuxk__uzjaj, op=MPI.SUM)
        score = score / kuxk__uzjaj
    return score
...
def check_consistent_length_parallel(*arrays):
    nxa__vswm = MPI.COMM_WORLD
    dyza__nquqy = True
    mtwj__feocn = [len(nardb__pwp) for nardb__pwp in arrays if nardb__pwp
         is not None]
    if len(np.unique(mtwj__feocn)) > 1:
        dyza__nquqy = False
    dyza__nquqy = nxa__vswm.allreduce(dyza__nquqy, op=MPI.LAND)
    return dyza__nquqy
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    ufu__ros = y_true.shape[0]
    cgl__vioc = nxa__vswm.allreduce(ufu__ros, op=MPI.SUM)
    if cgl__vioc < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        fkldg__ntp = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        fkldg__ntp = 1.0
    rof__xnmoc = (fkldg__ntp * (y_true - y_pred) ** 2).sum(axis=0, dtype=np
        .float64)
    zbiju__xxe = np.zeros(rof__xnmoc.shape, dtype=rof__xnmoc.dtype)
    nxa__vswm.Allreduce(rof__xnmoc, zbiju__xxe, op=MPI.SUM)
    ptbj__uvq = np.nansum(y_true * fkldg__ntp, axis=0, dtype=np.float64)
    ocf__ovep = np.zeros_like(ptbj__uvq)
    nxa__vswm.Allreduce(ptbj__uvq, ocf__ovep, op=MPI.SUM)
    ikgul__dcu = np.nansum(sample_weight, dtype=np.float64)
    utr__dyrbl = nxa__vswm.allreduce(ikgul__dcu, op=MPI.SUM)
    bod__ser = ocf__ovep / utr__dyrbl
    wtfj__pwhu = (fkldg__ntp * (y_true - bod__ser) ** 2).sum(axis=0, dtype=
        np.float64)
    puaa__zwik = np.zeros(wtfj__pwhu.shape, dtype=wtfj__pwhu.dtype)
    nxa__vswm.Allreduce(wtfj__pwhu, puaa__zwik, op=MPI.SUM)
    hmu__dvks = puaa__zwik != 0
    evmev__qpko = zbiju__xxe != 0
    pna__qjhkh = hmu__dvks & evmev__qpko
    omxd__fwosz = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    omxd__fwosz[pna__qjhkh] = 1 - zbiju__xxe[pna__qjhkh] / puaa__zwik[
        pna__qjhkh]
    omxd__fwosz[evmev__qpko & ~hmu__dvks] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            pbkz__jnbum = puaa__zwik
            if not np.any(hmu__dvks):
                if not np.any(evmev__qpko):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        pbkz__jnbum = multioutput
    return np.array([np.average(omxd__fwosz, weights=pbkz__jnbum)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    nxa__vswm = MPI.COMM_WORLD
    try:
        hqze__bksea = sklearn.metrics.confusion_matrix(y_true, y_pred,
            labels=labels, sample_weight=sample_weight, normalize=None)
    except ValueError as acx__kewl:
        hqze__bksea = acx__kewl
    csu__hoson = isinstance(hqze__bksea, ValueError
        ) and 'At least one label specified must be in y_true' in hqze__bksea.args[
        0]
    ljub__fvju = nxa__vswm.allreduce(csu__hoson, op=MPI.LAND)
    if ljub__fvju:
        raise hqze__bksea
    elif csu__hoson:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        osu__ucbk = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        osu__ucbk = hqze__bksea
    ilh__mvjuh = np.zeros_like(osu__ucbk)
    nxa__vswm.Allreduce(osu__ucbk, ilh__mvjuh)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            ilh__mvjuh = ilh__mvjuh / ilh__mvjuh.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            ilh__mvjuh = ilh__mvjuh / ilh__mvjuh.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            ilh__mvjuh = ilh__mvjuh / ilh__mvjuh.sum()
        ilh__mvjuh = np.nan_to_num(ilh__mvjuh)
    return ilh__mvjuh
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    kxm__uin = 'def _confusion_matrix_impl(\n'
    kxm__uin += '    y_true, y_pred, labels=None,\n'
    kxm__uin += '    sample_weight=None, normalize=None,\n'
    kxm__uin += '    _is_data_distributed=False,\n'
    kxm__uin += '):\n'
    kxm__uin += '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n'
    kxm__uin += '    y_pred = bodo.utils.conversion.coerce_to_array(y_pred)\n'
    kxm__uin += '    y_true = bodo.utils.typing.decode_if_dict_array(y_true)\n'
    kxm__uin += '    y_pred = bodo.utils.typing.decode_if_dict_array(y_pred)\n'
    lpsa__eaybj = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        lpsa__eaybj = 'float64[:,:]', 'np.float64'
    ...
    if not is_overload_none(labels):
        ...
        kxm__uin += '    labels = bodo.allgatherv(labels, False)\n'
        kxm__uin += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    kxm__uin += f"    with numba.objmode(cm='{lpsa__eaybj[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        kxm__uin += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        kxm__uin += '      cm = confusion_matrix_dist_helper(\n'
    kxm__uin += '        y_true, y_pred, labels=labels,\n'
    kxm__uin += '        sample_weight=sample_weight, normalize=normalize,\n'
    kxm__uin += f'      ).astype({lpsa__eaybj[1]})\n'
    kxm__uin += '    return cm\n'
    vzz__wnrip = {}
    exec(kxm__uin, globals(), vzz__wnrip)
    zyhw__mbui = vzz__wnrip['_confusion_matrix_impl']
    return zyhw__mbui
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    nxa__vswm = MPI.COMM_WORLD
    hpolh__kmhq = nxa__vswm.allreduce(len(X), op=MPI.SUM)
    uoet__ehoi = len(X) / hpolh__kmhq
    kgaeg__dgtxb = nxa__vswm.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    giz__llc = np.inf
    zrpdl__vif = 0
    if m.loss == 'hinge':
        ujhhu__elmlg = hinge_loss
    elif m.loss == 'log':
        ujhhu__elmlg = log_loss
    elif m.loss == 'squared_error':
        ujhhu__elmlg = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    utvnb__wkrlc = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        utvnb__wkrlc = True
    for bawio__exp in range(m.max_iter):
        if utvnb__wkrlc:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * uoet__ehoi
        m.coef_ = nxa__vswm.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * uoet__ehoi
        m.intercept_ = nxa__vswm.allreduce(m.intercept_, op=MPI.SUM)
        if utvnb__wkrlc:
            y_pred = m.predict(X)
            idon__due = ujhhu__elmlg(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            idon__due = ujhhu__elmlg(y, y_pred, labels=y_classes)
        tvkzw__cgzii = nxa__vswm.allreduce(idon__due, op=MPI.SUM)
        idon__due = tvkzw__cgzii / kgaeg__dgtxb
        if m.tol > np.NINF and idon__due > giz__llc - m.tol * hpolh__kmhq:
            zrpdl__vif += 1
        else:
            zrpdl__vif = 0
        if idon__due < giz__llc:
            giz__llc = idon__due
        if zrpdl__vif >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    nxa__vswm = MPI.COMM_WORLD
    uwow__wsb = nxa__vswm.Get_rank()
    lka__aae = MPI.Get_processor_name()
    bup__ihgbv = get_host_ranks()
    hku__cqk = m.n_jobs if hasattr(m, 'n_jobs') else None
    xeov__mzu = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(bup__ihgbv[lka__aae])
    if uwow__wsb == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if uwow__wsb == 0:
        nxa__vswm.bcast(m.cluster_centers_)
        nxa__vswm.bcast(m.inertia_)
        nxa__vswm.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = nxa__vswm.bcast(None)
        m.inertia_ = nxa__vswm.bcast(None)
        m.n_iter_ = nxa__vswm.bcast(None)
    if _is_data_distributed:
        phw__pxt = nxa__vswm.allgather(len_X)
        if uwow__wsb == 0:
            wylj__fqcmx = np.empty(len(phw__pxt) + 1, dtype=int)
            np.cumsum(phw__pxt, out=wylj__fqcmx[1:])
            wylj__fqcmx[0] = 0
            vxv__rgvc = [m.labels_[wylj__fqcmx[xvqh__zvl]:wylj__fqcmx[
                xvqh__zvl + 1]] for xvqh__zvl in range(len(phw__pxt))]
            rlee__nfj = nxa__vswm.scatter(vxv__rgvc)
        else:
            rlee__nfj = nxa__vswm.scatter(None)
        m.labels_ = rlee__nfj
    elif uwow__wsb == 0:
        nxa__vswm.bcast(m.labels_)
    else:
        m.labels_ = nxa__vswm.bcast(None)
    m._n_threads = xeov__mzu
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                nxa__vswm = MPI.COMM_WORLD
                result = nxa__vswm.allreduce(result, op=MPI.SUM)
            m._n_threads = xeov__mzu
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        kxm__uin = 'def _model_multinomial_nb_fit_impl(\n'
        kxm__uin += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        kxm__uin += '):  # pragma: no cover\n'
        kxm__uin += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            kxm__uin += '    X = X.to_numpy()\n'
        else:
            kxm__uin += '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n'
        kxm__uin += '    my_rank = bodo.get_rank()\n'
        kxm__uin += '    nranks = bodo.get_size()\n'
        kxm__uin += '    total_cols = X.shape[1]\n'
        kxm__uin += '    for i in range(nranks):\n'
        kxm__uin += (
        ...
        kxm__uin += '        if i == my_rank:\n'
        kxm__uin += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        kxm__uin += '        else:\n'
        kxm__uin += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        kxm__uin += '    y_train = bodo.allgatherv(y, False)\n'
        kxm__uin += '    with numba.objmode(m="multinomial_nb_type"):\n'
        kxm__uin += '        m = fit_multinomial_nb(\n'
        kxm__uin += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        kxm__uin += '        )\n'
        kxm__uin += '    bodo.barrier()\n'
        kxm__uin += '    return m\n'
        vzz__wnrip = {}
        exec(kxm__uin, globals(), vzz__wnrip)
        fqhde__napfv = vzz__wnrip['_model_multinomial_nb_fit_impl']
        return fqhde__napfv
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    bawio__exp, n_features = X_train.shape
    m.n_features_in_ = n_features
    mbtik__aqabt = LabelBinarizer()
    boziv__czf = mbtik__aqabt.fit_transform(y_train)
    m.classes_ = mbtik__aqabt.classes_
    if boziv__czf.shape[1] == 1:
        boziv__czf = np.concatenate((1 - boziv__czf, boziv__czf), axis=1)
    if sample_weight is not None:
        boziv__czf = boziv__czf.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        boziv__czf *= sample_weight.T
    class_prior = m.class_prior
    ypgq__oku = boziv__czf.shape[1]
    m._init_counters(ypgq__oku, n_features)
    m._count(X_train.astype('float64'), boziv__czf)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    jal__snhe = m.feature_count_ + alpha
    wuac__nkgnr = jal__snhe.sum(axis=1)
    nxa__vswm = MPI.COMM_WORLD
    kgaeg__dgtxb = nxa__vswm.Get_size()
    hkr__hzw = np.zeros(ypgq__oku)
    nxa__vswm.Allreduce(wuac__nkgnr, hkr__hzw, op=MPI.SUM)
    yeytf__zggi = np.log(jal__snhe) - np.log(hkr__hzw.reshape(-1, 1))
    tdlpr__wdk = yeytf__zggi.T.reshape(n_features * ypgq__oku)
    fpzgs__cnclu = np.ones(kgaeg__dgtxb) * (total_cols // kgaeg__dgtxb)
    wwlb__qkya = total_cols % kgaeg__dgtxb
    for xqhpt__niv in range(wwlb__qkya):
        fpzgs__cnclu[xqhpt__niv] += 1
    fpzgs__cnclu *= ypgq__oku
    cwzbh__lfrt = np.zeros(kgaeg__dgtxb, dtype=np.int32)
    cwzbh__lfrt[1:] = np.cumsum(fpzgs__cnclu)[:-1]
    jdtx__wuqpg = np.zeros((total_cols, ypgq__oku), dtype=np.float64)
    nxa__vswm.Allgatherv(tdlpr__wdk, [jdtx__wuqpg, fpzgs__cnclu,
        cwzbh__lfrt, MPI.DOUBLE_PRECISION])
    m.feature_log_prob_ = jdtx__wuqpg.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    nxa__vswm = MPI.COMM_WORLD
    uzhu__szl = nxa__vswm.Get_size()
    mwgbi__cva = m.with_std
    mfzcy__ifd = m.with_mean
    m.with_std = False
    if mwgbi__cva:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = mwgbi__cva
    m.with_mean = mfzcy__ifd
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    engu__ofbcy = np.zeros((uzhu__szl, *m.n_samples_seen_.shape), dtype=m.
        n_samples_seen_.dtype)
    nxa__vswm.Allgather(m.n_samples_seen_, engu__ofbcy)
    cahqs__bqr = np.sum(engu__ofbcy, axis=0)
    m.n_samples_seen_ = cahqs__bqr
    if m.with_mean or m.with_std:
        fxmwm__qugke = np.zeros((uzhu__szl, *m.mean_.shape), dtype=m.mean_.
            dtype)
        nxa__vswm.Allgather(m.mean_, fxmwm__qugke)
        fxmwm__qugke[np.isnan(fxmwm__qugke)] = 0
        hpnh__vqmzl = np.average(fxmwm__qugke, axis=0, weights=engu__ofbcy)
        m.mean_ = hpnh__vqmzl
    if m.with_std:
        vfkk__dhmc = sklearn_safe_accumulator_op(np.nansum, (X -
            hpnh__vqmzl) ** 2, axis=0) / cahqs__bqr
        mfiv__lfyj = np.zeros_like(vfkk__dhmc)
        nxa__vswm.Allreduce(vfkk__dhmc, mfiv__lfyj, op=MPI.SUM)
        m.var_ = mfiv__lfyj
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    koco__vyly = nxa__vswm.allreduce(koco__vyly, op=MPI.LAND)
    if koco__vyly:
...
def get_data_slice_parallel(data, labels, len_train):
    kbyqy__avrm = data[:len_train]
    vihw__qse = data[len_train:]
    kbyqy__avrm = bodo.rebalance(kbyqy__avrm)
    vihw__qse = bodo.rebalance(vihw__qse)
    rtro__dbqbq = labels[:len_train]
    wczlo__pvla = labels[len_train:]
    rtro__dbqbq = bodo.rebalance(rtro__dbqbq)
    wczlo__pvla = bodo.rebalance(wczlo__pvla)
    return kbyqy__avrm, vihw__qse, rtro__dbqbq, wczlo__pvla
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    hzaw__mkb = {'stratify': stratify}
    cwq__obpx = {'stratify': None}
    check_unsupported_args('train_test_split', hzaw__mkb, cwq__obpx, 'ml')
    if is_overload_false(_is_data_distributed):
        fent__kzdiv = f'data_split_type_{numba.core.ir_utils.next_label()}'
        jppty__tegno = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for eca__anmjf, gquvf__zbfa in ((data, fent__kzdiv), (labels,
            jppty__tegno)):
            if isinstance(eca__anmjf, (DataFrameType, SeriesType)):
                tqkwy__nnds = eca__anmjf.copy(index=NumericIndexType(types.
                    int64))
                setattr(types, gquvf__zbfa, tqkwy__nnds)
            else:
                setattr(types, gquvf__zbfa, eca__anmjf)
        kxm__uin = 'def _train_test_split_impl(\n'
        kxm__uin += '    data,\n'
        kxm__uin += '    labels=None,\n'
        kxm__uin += '    train_size=None,\n'
        kxm__uin += '    test_size=None,\n'
        kxm__uin += '    random_state=None,\n'
        kxm__uin += '    shuffle=True,\n'
        kxm__uin += '    stratify=None,\n'
        kxm__uin += '    _is_data_distributed=False,\n'
        kxm__uin += '):  # pragma: no cover\n'
        kxm__uin += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(fent__kzdiv, fent__kzdiv, jppty__tegno, jppty__tegno))
        kxm__uin += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        kxm__uin += '            data,\n'
        kxm__uin += '            labels,\n'
        kxm__uin += '            train_size=train_size,\n'
        kxm__uin += '            test_size=test_size,\n'
        kxm__uin += '            random_state=random_state,\n'
        kxm__uin += '            shuffle=shuffle,\n'
        kxm__uin += '            stratify=stratify,\n'
        kxm__uin += '        )\n'
        kxm__uin += (
            '    return data_train, data_test, labels_train, labels_test\n')
        vzz__wnrip = {}
        exec(kxm__uin, globals(), vzz__wnrip)
        _train_test_split_impl = vzz__wnrip['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            sgbu__ldefk = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(sgbu__ldefk * train_size)
            yfi__oeo = sgbu__ldefk - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                uwow__wsb = bodo.get_rank()
                kgaeg__dgtxb = bodo.get_size()
                wmi__jjk = np.empty(kgaeg__dgtxb, np.int64)
                bodo.libs.distributed_api.allgather(wmi__jjk, len(data))
                osk__yhonh = np.cumsum(wmi__jjk[0:uwow__wsb + 1])
                jcnzv__stim = np.full(sgbu__ldefk, True)
                jcnzv__stim[:yfi__oeo] = False
                np.random.seed(42)
                np.random.permutation(jcnzv__stim)
                if uwow__wsb:
                    mwz__xcqd = osk__yhonh[uwow__wsb - 1]
                else:
                    mwz__xcqd = 0
                uhy__trio = osk__yhonh[uwow__wsb]
                rjltn__tlwc = jcnzv__stim[mwz__xcqd:uhy__trio]
                kbyqy__avrm = data[rjltn__tlwc]
                vihw__qse = data[~rjltn__tlwc]
                rtro__dbqbq = labels[rjltn__tlwc]
                wczlo__pvla = labels[~rjltn__tlwc]
                kbyqy__avrm = bodo.random_shuffle(kbyqy__avrm, seed=
                    random_state, parallel=True)
                vihw__qse = bodo.random_shuffle(vihw__qse, seed=
                    random_state, parallel=True)
                rtro__dbqbq = bodo.random_shuffle(rtro__dbqbq, seed=
                    random_state, parallel=True)
                wczlo__pvla = bodo.random_shuffle(wczlo__pvla, seed=
                    random_state, parallel=True)
                rtro__dbqbq = reset_labels_type(rtro__dbqbq, label_type)
                wczlo__pvla = reset_labels_type(wczlo__pvla, label_type)
            else:
                kbyqy__avrm, vihw__qse, rtro__dbqbq, wczlo__pvla = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    uzhu__szl = nxa__vswm.Get_size()
    m = m.fit(X)
    cahqs__bqr = nxa__vswm.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = cahqs__bqr
    yxd__ngf = np.zeros((uzhu__szl, *m.data_min_.shape), dtype=m.data_min_.
        dtype)
    nxa__vswm.Allgather(m.data_min_, yxd__ngf)
    ledo__ray = np.nanmin(yxd__ngf, axis=0)
    ban__cmib = np.zeros((uzhu__szl, *m.data_max_.shape), dtype=m.data_max_
        .dtype)
    nxa__vswm.Allgather(m.data_max_, ban__cmib)
    rwy__ilnmd = np.nanmax(ban__cmib, axis=0)
    gvs__jekn = rwy__ilnmd - ledo__ray
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(gvs__jekn)
    m.min_ = m.feature_range[0] - ledo__ray * m.scale_
    m.data_min_ = ledo__ray
    m.data_max_ = rwy__ilnmd
    m.data_range_ = gvs__jekn
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    ctex__wdxw = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        ctex__wdxw = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{ctex__wdxw}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            wcww__aojm = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if wcww__aojm > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=wcww__aojm)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        iwera__mqq = True
    return iwera__mqq, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                ers__cuy = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                ers__cuy = bodo.allgatherv(ers__cuy, False)
                ers__cuy = bodo.libs.array_kernels.sort(ers__cuy, ascending
                    =True, inplace=True)
                wruu__mgyc = {}
                for ynsjc__tnurk in range(len(ers__cuy)):
                    wruu__mgyc[ers__cuy[ynsjc__tnurk]] = ynsjc__tnurk
            else:
                wruu__mgyc = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = wruu__mgyc
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X

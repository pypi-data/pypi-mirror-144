...
def install_mpl_class(types_name, python_type):
    dwjix__bnhhp = ''.join(map(str.title, types_name.split('_')))
    gir__qpn = f'class {dwjix__bnhhp}(types.Opaque):\n'
    gir__qpn += '    def __init__(self):\n'
    gir__qpn += f"       types.Opaque.__init__(self, name='{dwjix__bnhhp}')\n"
    gir__qpn += '    def __reduce__(self):\n'
    gir__qpn += (
        f"        return (types.Opaque, ('{dwjix__bnhhp}',), self.__dict__)\n")
    tfbp__mlh = {}
    exec(gir__qpn, {'types': types, 'bodo': bodo}, tfbp__mlh)
    ggx__csk = tfbp__mlh[dwjix__bnhhp]
    znlzf__tzbh = sys.modules[__name__]
    setattr(znlzf__tzbh, dwjix__bnhhp, ggx__csk)
    class_instance = ggx__csk()
    setattr(types, types_name, class_instance)
    register_model(ggx__csk)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(ggx__csk)(unbox_mpl_obj)
    box(ggx__csk)(box_mpl_obj)
...
def _install_mpl_types():
    uxdy__gxkb = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for tqqgj__gnq, yrf__bgehg in uxdy__gxkb:
        install_mpl_class(tqqgj__gnq, yrf__bgehg)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    gfoku__vyhb = ', '.join(f'e{pter__znx}' for pter__znx in range(len(args)))
    if gfoku__vyhb:
        gfoku__vyhb += ', '
    fjnif__naoth = ', '.join(f"{upmws__khha} = ''" for upmws__khha in kws.
        keys())
    otzc__nwgf = 'matplotlib_obj, ' if obj_typ is not None else ''
    nwhmr__acscd = (
        f'def mpl_stub({otzc__nwgf} {gfoku__vyhb} {fjnif__naoth}):\n')
    nwhmr__acscd += '    pass\n'
    jeqmq__ncjrp = {}
    exec(nwhmr__acscd, {}, jeqmq__ncjrp)
    roohk__iyn = jeqmq__ncjrp['mpl_stub']
    yhe__ayad = numba.core.utils.pysignature(roohk__iyn)
    xjioo__gpd = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(xjioo__gpd)).replace(pysig=
        yhe__ayad)


def generate_axes_typing(mod_name, nrows, ncols):
    bmjiv__kucy = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(bmjiv__kucy.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(bmjiv__kucy.format(mod_name, 'ncols'))
    owf__fcsu = get_overload_const_int(nrows)
    tlh__llth = get_overload_const_int(ncols)
    if owf__fcsu < 1:
        raise BodoError(bmjiv__kucy.format(mod_name, 'nrows'))
    if tlh__llth < 1:
        raise BodoError(bmjiv__kucy.format(mod_name, 'ncols'))
    if owf__fcsu == 1 and tlh__llth == 1:
        hhwn__syz = types.mpl_axes_type
    else:
        if tlh__llth == 1:
            etm__uxi = types.mpl_axes_type
        else:
            etm__uxi = types.Tuple([types.mpl_axes_type] * tlh__llth)
        hhwn__syz = types.Tuple([etm__uxi] * owf__fcsu)
    return hhwn__syz


def generate_pie_return_type(args, kws):
    mwhzi__vuiyp = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if mwhzi__vuiyp == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    snmrc__wrh = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(snmrc__wrh):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(snmrc__wrh):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        oay__nvebn = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, oay__nvebn]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    oay__nvebn = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    tqqgj__gnq = str(oay__nvebn)
    if not hasattr(types, tqqgj__gnq):
        tqqgj__gnq = f'objmode_type{ir_utils.next_label()}'
        setattr(types, tqqgj__gnq, oay__nvebn)
    nwhmr__acscd = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{tqqgj__gnq}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    jeqmq__ncjrp = {}
    exec(nwhmr__acscd, {'numba': numba, 'np': np}, jeqmq__ncjrp)
    impl = jeqmq__ncjrp['impl']
    return impl

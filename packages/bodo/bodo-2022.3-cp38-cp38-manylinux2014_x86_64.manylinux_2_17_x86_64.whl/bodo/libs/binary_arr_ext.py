...
def init_binary_arr(typingctx, data_typ=None):
    ...
    def codegen(context, builder, sig, args):
        rvrzy__qiby, = args
        omflo__ceun = context.make_helper(builder, binary_array_type)
        omflo__ceun.data = rvrzy__qiby
        context.nrt.incref(builder, data_typ, rvrzy__qiby)
        return omflo__ceun._getvalue()
...
def init_bytes_type(typingctx, data_typ, length_type):
    ...
    def codegen(context, builder, sig, args):
        yel__cjhqc = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        kvy__nuzgs = args[1]
        hepaq__keqgy = cgutils.create_struct_proxy(bytes_type)(context, builder
            )
        hepaq__keqgy.meminfo = context.nrt.meminfo_alloc(builder, kvy__nuzgs)
        hepaq__keqgy.nitems = kvy__nuzgs
        hepaq__keqgy.itemsize = lir.Constant(hepaq__keqgy.itemsize.type, 1)
        hepaq__keqgy.data = context.nrt.meminfo_data(builder, hepaq__keqgy.
            meminfo)
        hepaq__keqgy.parent = cgutils.get_null_value(hepaq__keqgy.parent.type)
        hepaq__keqgy.shape = cgutils.pack_array(builder, [kvy__nuzgs],
            context.get_value_type(types.intp))
        hepaq__keqgy.strides = yel__cjhqc.strides
        cgutils.memcpy(builder, hepaq__keqgy.data, yel__cjhqc.data, kvy__nuzgs)
        return hepaq__keqgy._getvalue()
...
def binary_arr_hex(arr):
    wpsx__ybjc = numba.cpython.unicode.PY_UNICODE_1BYTE_KIND

    def impl(arr):
        kvy__nuzgs = len(arr) * 2
        output = numba.cpython.unicode._empty_string(wpsx__ybjc, kvy__nuzgs, 1)
...
def bytes_to_hex(typingctx, output, arr):
    ...
    def codegen(context, builder, sig, args):
        ctxp__xcsl = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        gedxh__mopj = cgutils.create_struct_proxy(sig.args[1])(context,
            builder, value=args[1])
        mlfs__alpoj = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer(), lir.
            IntType(64)])
        gyu__rxfj = cgutils.get_or_insert_function(builder.module,
            mlfs__alpoj, name='bytes_to_hex')
        builder.call(gyu__rxfj, (ctxp__xcsl.data, gedxh__mopj.data,
            gedxh__mopj.nitems))
...
def binary_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(arr, ind):
            ach__odho = arr._data[ind]
            return init_bytes_type(ach__odho, len(ach__odho))
        return impl
...
def overload_bytes_fromhex(hex_str):
    ...
    if hex_str == bodo.string_type:
        ...
        def impl(hex_str):
            if not hex_str._is_ascii or hex_str._kind != wpsx__ybjc:
                raise TypeError(
                    'bytes.fromhex is only supported on ascii strings')
            rvrzy__qiby = np.empty(len(hex_str) // 2, np.uint8)
            kvy__nuzgs = _bytes_fromhex(rvrzy__qiby.ctypes, hex_str._data,
                len(hex_str))
            dzpuq__gny = init_bytes_type(rvrzy__qiby, kvy__nuzgs)
            return dzpuq__gny
        return impl
...
def create_binary_cmp_op_overload(op):
    ...
    def overload_binary_cmp(lhs, rhs):
        shlc__zga = lhs == binary_array_type
        mqyw__fkql = rhs == binary_array_type
        hhc__btd = 'lhs' if shlc__zga else 'rhs'
        ikq__nhvm = 'def impl(lhs, rhs):\n'
        ikq__nhvm += '  numba.parfors.parfor.init_prange()\n'
        ikq__nhvm += f'  n = len({hhc__btd})\n'
        ikq__nhvm += '  out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(n)\n'
        ikq__nhvm += '  for i in numba.parfors.parfor.internal_prange(n):\n'
        jga__fwc = []
        if shlc__zga:
            jga__fwc.append('bodo.libs.array_kernels.isna(lhs, i)')
        if mqyw__fkql:
            jga__fwc.append('bodo.libs.array_kernels.isna(rhs, i)')
        ikq__nhvm += f"    if {' or '.join(jga__fwc)}:\n"
        ikq__nhvm += '      bodo.libs.array_kernels.setna(out_arr, i)\n'
        ikq__nhvm += '      continue\n'
        wyyxp__ujgr = 'lhs[i]' if shlc__zga else 'lhs'
        vbho__sgkgk = 'rhs[i]' if mqyw__fkql else 'rhs'
        ikq__nhvm += f'    out_arr[i] = op({wyyxp__ujgr}, {vbho__sgkgk})\n'
        ikq__nhvm += '  return out_arr\n'
        cbta__wtq = {}
        exec(ikq__nhvm, {'bodo': bodo, 'numba': numba, 'op': op}, cbta__wtq)
        return cbta__wtq['impl']

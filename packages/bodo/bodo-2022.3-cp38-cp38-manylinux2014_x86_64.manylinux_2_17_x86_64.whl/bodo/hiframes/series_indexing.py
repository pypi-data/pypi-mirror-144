...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        jyeje__lfzcx = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(jyeje__lfzcx)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        olg__avnxb = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, olg__avnxb)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        wldlc__sinu, = args
        wpbjj__zefd = signature.return_type
        ebqjf__fkvc = cgutils.create_struct_proxy(wpbjj__zefd)(context, builder
            )
        ebqjf__fkvc.obj = wldlc__sinu
        context.nrt.incref(builder, signature.args[0], wldlc__sinu)
        return ebqjf__fkvc._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                ify__vuk = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        jyeje__lfzcx = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(jyeje__lfzcx)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        olg__avnxb = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, olg__avnxb)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        wldlc__sinu, = args
        cvrws__iey = signature.return_type
        nms__jubey = cgutils.create_struct_proxy(cvrws__iey)(context, builder)
        nms__jubey.obj = wldlc__sinu
        context.nrt.incref(builder, signature.args[0], wldlc__sinu)
        return nms__jubey._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                pjf__qwlse = bodo.utils.conversion.coerce_to_ndarray(idx)
                ird__ywa = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    pjf__qwlse]
                fsbtk__ttq = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    pjf__qwlse]
                jyeje__lfzcx = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(ird__ywa,
                    fsbtk__ttq, jyeje__lfzcx)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                ird__ywa = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                fsbtk__ttq = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                jyeje__lfzcx = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(ird__ywa,
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        pjf__qwlse = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        ify__vuk = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(ify__vuk))
                        pjf__qwlse = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            pjf__qwlse] = s
            ...
            def impl_arr(I, idx, val):
                pjf__qwlse = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[pjf__qwlse
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        jyeje__lfzcx = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(jyeje__lfzcx)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        olg__avnxb = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, olg__avnxb)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        wldlc__sinu, = args
        tap__fym = signature.return_type
        biy__dhloc = cgutils.create_struct_proxy(tap__fym)(context, builder)
        biy__dhloc.obj = wldlc__sinu
        context.nrt.incref(builder, signature.args[0], wldlc__sinu)
        return biy__dhloc._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            ird__ywa = bodo.hiframes.pd_series_ext.get_series_data(S)
            fsbtk__ttq = bodo.hiframes.pd_series_ext.get_series_index(S)
            gibg__fmlj = (idx - fsbtk__ttq._start) // fsbtk__ttq._step
            return ird__ywa[gibg__fmlj]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    pjf__qwlse = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(ird__ywa[
                        pjf__qwlse])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    urqv__qtl = bodo.hiframes.pd_series_ext.get_series_index(S)
                    trouq__cql = urqv__qtl.get_loc(idx)
                    if trouq__cql is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(trouq__cql)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                pjf__qwlse = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[pjf__qwlse
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        fszpn__qii = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(
            tgjpe__hzti, int) for tgjpe__hzti in fszpn__qii):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            uxa__ieubx = get_literal_value(idx)
            if uxa__ieubx not in fszpn__qii:
                raise_bodo_error(
                    f"Series label-based getitem: '{uxa__ieubx}' not in {fszpn__qii}"
                    )
            arr_ind = fszpn__qii.index(uxa__ieubx)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    ebqjf__fkvc = cgutils.create_struct_proxy(fromty)(context, builder, val)
    etbgs__ugqs = context.cast(builder, ebqjf__fkvc.obj, fromty.stype, toty
        .stype)
    jnd__iky = cgutils.create_struct_proxy(toty)(context, builder)
    jnd__iky.obj = etbgs__ugqs
    return jnd__iky._getvalue()

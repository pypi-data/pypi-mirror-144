...
def _global_except_hook(exctype, value, traceback):
    ...
    mvgd__sgo = MPI.COMM_WORLD
    qacwo__pyyf = mvgd__sgo.Get_rank()
    caeg__lucg = 3.0
    fryu__admza = True
    wnh__pyv = mvgd__sgo.Ibarrier()
    xhwzm__sqykg = time.time()
    while time.time() - xhwzm__sqykg < caeg__lucg:
        time.sleep(0.1)
        if wnh__pyv.Test():
            fryu__admza = False
            break
    try:
        ...
        if fryu__admza:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {qacwo__pyyf}. \n')

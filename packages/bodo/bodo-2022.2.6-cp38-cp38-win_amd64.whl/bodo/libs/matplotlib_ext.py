...
def install_mpl_class(types_name, python_type):
    mhup__unp = ''.join(map(str.title, types_name.split('_')))
    ydrmp__hpzs = f'class {mhup__unp}(types.Opaque):\n'
    ydrmp__hpzs += '    def __init__(self):\n'
    ydrmp__hpzs += f"       types.Opaque.__init__(self, name='{mhup__unp}')\n"
    ydrmp__hpzs += '    def __reduce__(self):\n'
    ydrmp__hpzs += (
        f"        return (types.Opaque, ('{mhup__unp}',), self.__dict__)\n")
    jqf__noyir = {}
    exec(ydrmp__hpzs, {'types': types, 'bodo': bodo}, jqf__noyir)
    vujj__cmb = jqf__noyir[mhup__unp]
    hkhc__gxf = sys.modules[__name__]
    setattr(hkhc__gxf, mhup__unp, vujj__cmb)
    class_instance = vujj__cmb()
    setattr(types, types_name, class_instance)
    register_model(vujj__cmb)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(vujj__cmb)(unbox_mpl_obj)
    box(vujj__cmb)(box_mpl_obj)
...
def _install_mpl_types():
    ykp__gaoc = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for pwsi__vscmj, syyd__dcr in ykp__gaoc:
        install_mpl_class(pwsi__vscmj, syyd__dcr)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    kuo__zfs = ', '.join(f'e{duy__yaad}' for duy__yaad in range(len(args)))
    if kuo__zfs:
        kuo__zfs += ', '
    ypeor__dnotw = ', '.join(f"{fcn__bbdzy} = ''" for fcn__bbdzy in kws.keys())
    nbwx__nyo = 'matplotlib_obj, ' if obj_typ is not None else ''
    ewq__vprl = f'def mpl_stub({nbwx__nyo} {kuo__zfs} {ypeor__dnotw}):\n'
    ewq__vprl += '    pass\n'
    npes__qiyc = {}
    exec(ewq__vprl, {}, npes__qiyc)
    brxz__hhs = npes__qiyc['mpl_stub']
    dpyr__tjy = numba.core.utils.pysignature(brxz__hhs)
    jynv__tvsa = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(jynv__tvsa)).replace(pysig=
        dpyr__tjy)


def generate_axes_typing(mod_name, nrows, ncols):
    sje__qrd = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(sje__qrd.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(sje__qrd.format(mod_name, 'ncols'))
    hym__rail = get_overload_const_int(nrows)
    oane__tyq = get_overload_const_int(ncols)
    if hym__rail < 1:
        raise BodoError(sje__qrd.format(mod_name, 'nrows'))
    if oane__tyq < 1:
        raise BodoError(sje__qrd.format(mod_name, 'ncols'))
    if hym__rail == 1 and oane__tyq == 1:
        osxu__gdxy = types.mpl_axes_type
    else:
        if oane__tyq == 1:
            ble__xbr = types.mpl_axes_type
        else:
            ble__xbr = types.Tuple([types.mpl_axes_type] * oane__tyq)
        osxu__gdxy = types.Tuple([ble__xbr] * hym__rail)
    return osxu__gdxy


def generate_pie_return_type(args, kws):
    peu__hmly = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if peu__hmly == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    ltigg__pgdz = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(ltigg__pgdz):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(ltigg__pgdz):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        hywem__sabo = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, hywem__sabo]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    hywem__sabo = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols
        )
    pwsi__vscmj = str(hywem__sabo)
    if not hasattr(types, pwsi__vscmj):
        pwsi__vscmj = f'objmode_type{ir_utils.next_label()}'
        setattr(types, pwsi__vscmj, hywem__sabo)
    ewq__vprl = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{pwsi__vscmj}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    npes__qiyc = {}
    exec(ewq__vprl, {'numba': numba, 'np': np}, npes__qiyc)
    impl = npes__qiyc['impl']
    return impl

...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    padg__srzw = pq_node.out_vars[0].name
    imqls__kyy = pq_node.out_vars[1].name
    if padg__srzw not in lives and imqls__kyy not in lives:
        return None
    elif padg__srzw not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif imqls__kyy not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    zdp__omnx = pq_node.out_vars[0].name
    assert isinstance(typemap[zdp__omnx], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        jgdxz__hbwbf, qpe__gnhl = get_live_column_nums_block(column_live_map,
            equiv_vars, zdp__omnx)
        jgdxz__hbwbf = bodo.ir.connector.trim_extra_used_columns(jgdxz__hbwbf,
            len(pq_node.col_indices))
        if not qpe__gnhl and not jgdxz__hbwbf:
            jgdxz__hbwbf = [0]
        if not qpe__gnhl and len(jgdxz__hbwbf) != len(pq_node.
            type_usecol_offset):
            pq_node.type_usecol_offset = jgdxz__hbwbf
            return True
    return False

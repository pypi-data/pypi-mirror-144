...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        celq__njk = self.overloads[sig]
        agjpk__mfa = celq__njk.metadata.get('distributed_diagnostics', None)
        if agjpk__mfa is None:
            lbo__hrc = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(lbo__hrc)
        agjpk__mfa.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        bvy__evx = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(bvy__evx)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        wlb__dzwpa = options.pop('distributed')
        pipeline_class = (pipeline_class if wlb__dzwpa else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        zxy__gwnsh = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if zxy__gwnsh else
            pipeline_class)
    jvqcb__thtfw = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(jvqcb__thtfw, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(jvqcb__thtfw)
        else:
            return master_mode_wrapper(jvqcb__thtfw)
    else:
        return jvqcb__thtfw

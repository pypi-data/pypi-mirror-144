...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    gwnww__kks = pq_node.out_vars[0].name
    mlswc__xvw = pq_node.out_vars[1].name
    if gwnww__kks not in lives and mlswc__xvw not in lives:
        return None
    elif gwnww__kks not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif mlswc__xvw not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    jxlqm__lag = pq_node.out_vars[0].name
    assert isinstance(typemap[jxlqm__lag], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        wlsz__ape, psm__mooed = get_live_column_nums_block(column_live_map,
            equiv_vars, jxlqm__lag)
        wlsz__ape = bodo.ir.connector.trim_extra_used_columns(wlsz__ape,
            len(pq_node.col_indices))
        if not psm__mooed and not wlsz__ape:
            wlsz__ape = [0]
        if not psm__mooed and len(wlsz__ape) != len(pq_node.type_usecol_offset
            ):
            pq_node.type_usecol_offset = wlsz__ape
            return True
    return False

...
def _global_except_hook(exctype, value, traceback):
    ...
    gfmmk__mjr = MPI.COMM_WORLD
    rewvm__ajdx = gfmmk__mjr.Get_rank()
    ixniz__wrfp = 3.0
    soi__vfnvj = True
    mari__myo = gfmmk__mjr.Ibarrier()
    rohzk__ocfe = time.time()
    while time.time() - rohzk__ocfe < ixniz__wrfp:
        time.sleep(0.1)
        if mari__myo.Test():
            soi__vfnvj = False
            break
    try:
        ...
        if soi__vfnvj:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {rewvm__ajdx}. \n')

...
class RollingCovType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        arr = args[0]
        amyhl__sny = arr.copy(dtype=types.float64)
        return signature(amyhl__sny, *unliteral_all(args))
...
def overload_rolling_fixed(arr, index_arr, win, minp, center, fname, raw=
    ...
    wff__juiw = get_overload_const_str(fname)
    if wff__juiw not in ('sum', 'mean', 'var', 'std', 'count', 'median',
        'min', 'max'):
        raise BodoError('invalid rolling (fixed window) function {}'.format
            (wff__juiw))
    if wff__juiw in ('median', 'min', 'max'):
        yid__behwy = 'def kernel_func(A):\n'
        yid__behwy += '  if np.isnan(A).sum() != 0: return np.nan\n'
        yid__behwy += '  return np.{}(A)\n'.format(wff__juiw)
        aktlv__lyb = {}
        exec(yid__behwy, {'np': np}, aktlv__lyb)
        kernel_func = register_jitable(aktlv__lyb['kernel_func'])
        return (lambda arr, index_arr, win, minp, center, fname, raw=True,
            parallel=False: roll_fixed_apply(arr, index_arr, win, minp,
...
def overload_rolling_variable(arr, on_arr, index_arr, win, minp, center,
    ...
    if wff__juiw in ('median', 'min', 'max'):
        yid__behwy = 'def kernel_func(A):\n'
        yid__behwy += '  arr  = dropna(A)\n'
        yid__behwy += '  if len(arr) == 0: return np.nan\n'
        yid__behwy += '  return np.{}(arr)\n'.format(wff__juiw)
        aktlv__lyb = {}
        exec(yid__behwy, {'np': np, 'dropna': _dropna}, aktlv__lyb)
        kernel_func = register_jitable(aktlv__lyb['kernel_func'])
        return (lambda arr, on_arr, index_arr, win, minp, center, fname,
            raw=True, parallel=False: roll_variable_apply(arr, on_arr,
...
def roll_fixed_linear_generic(in_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        khkz__bvsh = _border_icomm(in_arr, rank, n_pes, halo_size, True, center
            )
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req,
            hxbp__rifaz) = khkz__bvsh
    ...
    if parallel:
        ...
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(hxbp__rifaz, True)
            for rkc__vva in range(0, halo_size):
                data = add_obs(r_recv_buff[rkc__vva], *data)
                nnj__cuwvq = in_arr[N + rkc__vva - win]
                data = remove_obs(nnj__cuwvq, *data)
                output[N + rkc__vva - offset] = calc_out(minp, *data)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            data = init_data()
            for rkc__vva in range(0, halo_size):
                data = add_obs(l_recv_buff[rkc__vva], *data)
            for rkc__vva in range(0, win - 1):
                data = add_obs(in_arr[rkc__vva], *data)
                if rkc__vva > offset:
                    nnj__cuwvq = l_recv_buff[rkc__vva - offset - 1]
                    data = remove_obs(nnj__cuwvq, *data)
                if rkc__vva >= offset:
                    output[rkc__vva - offset] = calc_out(minp, *data)
    return output
...
def roll_fixed_linear_generic_seq(in_arr, win, minp, center, init_data,
    ...
    qmv__wxne = max(minp, 1) - 1
    qmv__wxne = min(qmv__wxne, N)
    for rkc__vva in range(0, qmv__wxne):
        data = add_obs(in_arr[rkc__vva], *data)
        if rkc__vva >= offset:
            output[rkc__vva - offset] = calc_out(minp, *data)
    for rkc__vva in range(qmv__wxne, N):
        val = in_arr[rkc__vva]
        data = add_obs(val, *data)
        if rkc__vva > win - 1:
            nnj__cuwvq = in_arr[rkc__vva - win]
            data = remove_obs(nnj__cuwvq, *data)
        output[rkc__vva - offset] = calc_out(minp, *data)
    enr__elsw = data
    for rkc__vva in range(N, N + offset):
        if rkc__vva > win - 1:
            nnj__cuwvq = in_arr[rkc__vva - win]
            data = remove_obs(nnj__cuwvq, *data)
        output[rkc__vva - offset] = calc_out(minp, *data)
    return output, enr__elsw
...
def roll_fixed_apply_impl(in_arr, index_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req,
            hxbp__rifaz) = khkz__bvsh
        if raw == False:
            bbaw__seh = _border_icomm(index_arr, rank, n_pes, halo_size, 
                True, center)
            (l_recv_buff_idx, r_recv_buff_idx, ddg__arq, gdddi__wcbtl,
                juu__imgo, qdzqc__grkji) = bbaw__seh
    output = roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
        kernel_func, raw)
    if parallel:
        _border_send_wait(r_send_req, l_send_req, rank, n_pes, True, center)
        if raw == False:
            _border_send_wait(gdddi__wcbtl, ddg__arq, rank, n_pes, True, center
                )
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(hxbp__rifaz, True)
            if raw == False:
                bodo.libs.distributed_api.wait(qdzqc__grkji, True)
            recv_right_compute(output, in_arr, index_arr, N, win, minp,
                offset, r_recv_buff, r_recv_buff_idx, kernel_func, raw)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            if raw == False:
                bodo.libs.distributed_api.wait(juu__imgo, True)
...
def overload_recv_right_compute(output, in_arr, index_arr, N, win, minp,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, N, win, minp, offset,
            r_recv_buff, r_recv_buff_idx, kernel_func, raw):
            enr__elsw = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
            zci__huei = 0
            for rkc__vva in range(max(N - offset, 0), N):
                data = enr__elsw[zci__huei:zci__huei + win]
                if win - np.isnan(data).sum() < minp:
                    output[rkc__vva] = np.nan
                else:
                    output[rkc__vva] = kernel_func(data)
                zci__huei += 1
        return impl
    ...
    def impl_series(output, in_arr, index_arr, N, win, minp, offset,
        r_recv_buff, r_recv_buff_idx, kernel_func, raw):
        enr__elsw = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
        aroey__zqow = np.concatenate((index_arr[N - win + 1:], r_recv_buff_idx)
            )
        zci__huei = 0
        for rkc__vva in range(max(N - offset, 0), N):
            data = enr__elsw[zci__huei:zci__huei + win]
            if win - np.isnan(data).sum() < minp:
                ...
                output[rkc__vva] = kernel_func(pd.Series(data, aroey__zqow[
                    zci__huei:zci__huei + win]))
            zci__huei += 1
    return impl_series
...
def overload_recv_left_compute(output, in_arr, index_arr, win, minp, offset,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, win, minp, offset, l_recv_buff,
            ...
            enr__elsw = np.concatenate((l_recv_buff, in_arr[:win - 1]))
            for rkc__vva in range(0, win - offset - 1):
                data = enr__elsw[rkc__vva:rkc__vva + win]
                if win - np.isnan(data).sum() < minp:
                    output[rkc__vva] = np.nan
                else:
                    output[rkc__vva] = kernel_func(data)
    ...
    def impl_series(output, in_arr, index_arr, win, minp, offset,
        l_recv_buff, l_recv_buff_idx, kernel_func, raw):
        enr__elsw = np.concatenate((l_recv_buff, in_arr[:win - 1]))
        aroey__zqow = np.concatenate((l_recv_buff_idx, index_arr[:win - 1]))
        for rkc__vva in range(0, win - offset - 1):
            data = enr__elsw[rkc__vva:rkc__vva + win]
            if win - np.isnan(data).sum() < minp:
                output[rkc__vva] = np.nan
...
def overload_roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
    ...
    def roll_fixed_apply_seq_impl(in_arr, index_arr, win, minp, center,
        ...
        output = np.empty(N, dtype=np.float64)
        offset = (win - 1) // 2 if center else 0
        for rkc__vva in range(0, N):
            start = max(rkc__vva - win + 1 + offset, 0)
            end = min(rkc__vva + 1 + offset, N)
            data = in_arr[start:end]
            if end - start - np.isnan(data).sum() < minp:
                output[rkc__vva] = np.nan
            else:
                output[rkc__vva] = apply_func(kernel_func, data, index_arr,
                    start, end, raw)
        return output
...
def roll_var_linear_generic(in_arr, on_arr_dt, win, minp, center, parallel,
    ...
    if parallel:
        ...
        khkz__bvsh = _border_icomm_var(in_arr, on_arr, rank, n_pes, win)
        (l_recv_buff, l_recv_t_buff, r_send_req, yifga__jiuxa, l_recv_req,
            muau__crt) = khkz__bvsh
    ...
    if parallel:
        ...
        _border_send_wait(yifga__jiuxa, yifga__jiuxa, rank, n_pes, True, False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(muau__crt, True)
            num_zero_starts = 0
            for rkc__vva in range(0, N):
                if start[rkc__vva] != 0:
                    break
            ...
            recv_starts = _get_var_recv_starts(on_arr, l_recv_t_buff,
                num_zero_starts, win)
            data = init_data()
            for wjod__vuqj in range(recv_starts[0], len(l_recv_t_buff)):
                data = add_obs(l_recv_buff[wjod__vuqj], *data)
            if right_closed:
                data = add_obs(in_arr[0], *data)
            output[0] = calc_out(minp, *data)
            for rkc__vva in range(1, num_zero_starts):
                s = recv_starts[rkc__vva]
                toev__auc = end[rkc__vva]
                for wjod__vuqj in range(recv_starts[rkc__vva - 1], s):
                    data = remove_obs(l_recv_buff[wjod__vuqj], *data)
                for wjod__vuqj in range(end[rkc__vva - 1], toev__auc):
                    data = add_obs(in_arr[wjod__vuqj], *data)
                output[rkc__vva] = calc_out(minp, *data)
    return output
...
def _get_var_recv_starts(on_arr, l_recv_t_buff, num_zero_starts, win):
    ...
    imyh__aif = cast_dt64_arr_to_int(on_arr)
    left_closed = False
    opcqf__onko = imyh__aif[0] - win
    if left_closed:
        opcqf__onko -= 1
    recv_starts[0] = halo_size
    for wjod__vuqj in range(0, halo_size):
        if l_recv_t_buff[wjod__vuqj] > opcqf__onko:
            recv_starts[0] = wjod__vuqj
            break
    for rkc__vva in range(1, num_zero_starts):
        opcqf__onko = imyh__aif[rkc__vva] - win
        if left_closed:
            opcqf__onko -= 1
        recv_starts[rkc__vva] = halo_size
        for wjod__vuqj in range(recv_starts[rkc__vva - 1], halo_size):
            if l_recv_t_buff[wjod__vuqj] > opcqf__onko:
                recv_starts[rkc__vva] = wjod__vuqj
                break
    return recv_starts
...
def roll_var_linear_generic_seq(in_arr, on_arr, win, minp, start, end,
    ...
    data = init_data()
    for wjod__vuqj in range(start[0], end[0]):
        data = add_obs(in_arr[wjod__vuqj], *data)
    output[0] = calc_out(minp, *data)
    for rkc__vva in range(1, N):
        s = start[rkc__vva]
        toev__auc = end[rkc__vva]
        for wjod__vuqj in range(start[rkc__vva - 1], s):
            data = remove_obs(in_arr[wjod__vuqj], *data)
        for wjod__vuqj in range(end[rkc__vva - 1], toev__auc):
            data = add_obs(in_arr[wjod__vuqj], *data)
        output[rkc__vva] = calc_out(minp, *data)
    return output
...
def roll_variable_apply_impl(in_arr, on_arr_dt, index_arr, win, minp,
    ...
    if parallel:
        ...
        (l_recv_buff, l_recv_t_buff, r_send_req, yifga__jiuxa, l_recv_req,
            muau__crt) = khkz__bvsh
        if raw == False:
            bbaw__seh = _border_icomm_var(index_arr, on_arr, rank, n_pes, win)
            (l_recv_buff_idx, bcnvv__driq, gdddi__wcbtl, keksh__jiwz,
                juu__imgo, vodei__zkim) = bbaw__seh
    start, end = _build_indexer(on_arr, N, win, left_closed, right_closed)
    ...
    if parallel:
        ...
        _border_send_wait(yifga__jiuxa, yifga__jiuxa, rank, n_pes, True, False)
        if raw == False:
            _border_send_wait(gdddi__wcbtl, gdddi__wcbtl, rank, n_pes, True,
                False)
            _border_send_wait(keksh__jiwz, keksh__jiwz, rank, n_pes, True, 
                False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(muau__crt, True)
            if raw == False:
                bodo.libs.distributed_api.wait(juu__imgo, True)
                bodo.libs.distributed_api.wait(vodei__zkim, True)
            num_zero_starts = 0
            for rkc__vva in range(0, N):
                if start[rkc__vva] != 0:
                    break
...
def overload_recv_left_var_compute(output, in_arr, index_arr,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, num_zero_starts, recv_starts,
            ...
            for rkc__vva in range(0, num_zero_starts):
                ueu__ibe = recv_starts[rkc__vva]
                cyp__guwr = np.concatenate((l_recv_buff[ueu__ibe:], in_arr[
                    :rkc__vva + 1]))
                if len(cyp__guwr) - np.isnan(cyp__guwr).sum() >= minp:
                    output[rkc__vva] = kernel_func(cyp__guwr)
                else:
                    output[rkc__vva] = np.nan
        return impl
    ...
    def impl_series(output, in_arr, index_arr, num_zero_starts, recv_starts,
        ...
        for rkc__vva in range(0, num_zero_starts):
            ...
            cyp__guwr = np.concatenate((l_recv_buff[ueu__ibe:], in_arr[:
                rkc__vva + 1]))
            ayx__muvo = np.concatenate((l_recv_buff_idx[ueu__ibe:],
                index_arr[:rkc__vva + 1]))
            if len(cyp__guwr) - np.isnan(cyp__guwr).sum() >= minp:
                output[rkc__vva] = kernel_func(pd.Series(cyp__guwr, ayx__muvo))
            else:
                output[rkc__vva] = np.nan
    return impl_series
...
def roll_variable_apply_seq_impl(in_arr, on_arr, index_arr, win, minp,
    ...
    for rkc__vva in range(0, N):
        s = start[rkc__vva]
        toev__auc = end[rkc__vva]
        data = in_arr[s:toev__auc]
        if toev__auc - s - np.isnan(data).sum() >= minp:
            output[rkc__vva] = kernel_func(data)
        else:
            output[rkc__vva] = np.nan
    return output
...
def roll_variable_apply_seq_impl_series(in_arr, on_arr, index_arr, win,
    ...
    for rkc__vva in range(0, N):
        ...
        data = in_arr[s:toev__auc]
        if toev__auc - s - np.isnan(data).sum() >= minp:
            output[rkc__vva] = kernel_func(pd.Series(data, index_arr[s:
                toev__auc]))
        else:
            output[rkc__vva] = np.nan
    return output
...
def _build_indexer(on_arr, N, win, left_closed, right_closed):
    ...
    for rkc__vva in range(1, N):
        nug__rlrvo = imyh__aif[rkc__vva]
        opcqf__onko = imyh__aif[rkc__vva] - win
        if left_closed:
            opcqf__onko -= 1
        start[rkc__vva] = rkc__vva
        for wjod__vuqj in range(start[rkc__vva - 1], rkc__vva):
            if imyh__aif[wjod__vuqj] > opcqf__onko:
                start[rkc__vva] = wjod__vuqj
                break
        if imyh__aif[end[rkc__vva - 1]] <= nug__rlrvo:
            end[rkc__vva] = rkc__vva + 1
        else:
            end[rkc__vva] = end[rkc__vva - 1]
        if not right_closed:
            end[rkc__vva] -= 1
    return start, end
...
def calc_mean(minp, nobs, sum_x, neg_ct):
    if nobs >= minp:
        kno__nmbz = sum_x / nobs
        if neg_ct == 0 and kno__nmbz < 0.0:
            kno__nmbz = 0
        elif neg_ct == nobs and kno__nmbz > 0.0:
            kno__nmbz = 0
    else:
        kno__nmbz = np.nan
    return kno__nmbz
...
def add_var(val, nobs, mean_x, ssqdm_x):
    if not np.isnan(val):
        nobs += 1
        cyqhk__nozj = val - mean_x
        mean_x += cyqhk__nozj / nobs
        ssqdm_x += (nobs - 1) * cyqhk__nozj ** 2 / nobs
...
def calc_var(minp, nobs, mean_x, ssqdm_x):
    tgnt__gnmb = 1.0
    kno__nmbz = np.nan
    if nobs >= minp and nobs > tgnt__gnmb:
        if nobs == 1:
            kno__nmbz = 0.0
        else:
            kno__nmbz = ssqdm_x / (nobs - tgnt__gnmb)
            if kno__nmbz < 0.0:
                kno__nmbz = 0.0
    return kno__nmbz
...
def calc_std(minp, nobs, mean_x, ssqdm_x):
    kjlu__ctr = calc_var(minp, nobs, mean_x, ssqdm_x)
    return np.sqrt(kjlu__ctr)
...
def shift_impl(in_arr, shift, parallel):
    ...
    if parallel:
        ...
        khkz__bvsh = _border_icomm(in_arr, rank, n_pes, halo_size,
    ...
    if parallel:
        if send_right:
            ...
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(hxbp__rifaz, True)
                for rkc__vva in range(0, halo_size):
                    if bodo.libs.array_kernels.isna(r_recv_buff, rkc__vva):
                        bodo.libs.array_kernels.setna(output, N - halo_size +
                            rkc__vva)
                        continue
                    output[N - halo_size + rkc__vva] = r_recv_buff[rkc__vva]
    return output
...
def shift_seq(in_arr, shift, output, is_parallel_str=False):
    N = len(in_arr)
    wbm__xmmo = 1 if shift > 0 else -1
    shift = wbm__xmmo * min(abs(shift), N)
    if shift > 0 and (not is_parallel_str or bodo.get_rank() == 0):
        bodo.libs.array_kernels.setna_slice(output, slice(None, shift))
    start = max(shift, 0)
    end = min(N, N + shift)
    for rkc__vva in range(start, end):
        if bodo.libs.array_kernels.isna(in_arr, rkc__vva - shift):
            bodo.libs.array_kernels.setna(output, rkc__vva)
            continue
        output[rkc__vva] = in_arr[rkc__vva - shift]
    if shift < 0:
...
def shift_left_recv(r_send_req, l_send_req, rank, n_pes, halo_size,
    ...
    if rank != 0:
        bodo.libs.distributed_api.wait(l_recv_req, True)
        for rkc__vva in range(0, halo_size):
            if bodo.libs.array_kernels.isna(l_recv_buff, rkc__vva):
                bodo.libs.array_kernels.setna(output, rkc__vva)
                continue
            output[rkc__vva] = l_recv_buff[rkc__vva]
    ...
    if parallel:
        if send_right:
            ...
            if rank != 0:
                bodo.libs.distributed_api.wait(l_recv_req, True)
                for rkc__vva in range(0, halo_size):
                    rwntl__xeq = l_recv_buff[rkc__vva]
                    output[rkc__vva] = (in_arr[rkc__vva] - rwntl__xeq
                        ) / rwntl__xeq
        else:
            _border_send_wait(r_send_req, l_send_req, rank, n_pes, False, True)
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(hxbp__rifaz, True)
                for rkc__vva in range(0, halo_size):
                    rwntl__xeq = r_recv_buff[rkc__vva]
                    output[N - halo_size + rkc__vva] = (in_arr[N -
                        halo_size + rkc__vva] - rwntl__xeq) / rwntl__xeq
    return output
...
def get_first_non_na(arr):
    ...
    if arr.dtype == types.float32:
        kwmq__jzvq = np.float32('nan')

    def impl(arr):
        for rkc__vva in range(len(arr)):
            if not bodo.libs.array_kernels.isna(arr, rkc__vva):
                return arr[rkc__vva]
        return kwmq__jzvq
    return impl
...
def get_last_non_na(arr):
    ...
    def impl(arr):
        rke__fga = len(arr)
        for rkc__vva in range(len(arr)):
            zci__huei = rke__fga - rkc__vva - 1
            if not bodo.libs.array_kernels.isna(arr, zci__huei):
                return arr[zci__huei]
        return kwmq__jzvq
    return impl
...
def pct_change_seq(in_arr, shift):
    ...
    if shift > 0:
        ...
        bodo.libs.array_kernels.setna_slice(output, slice(shift, None))
    if shift > 0:
        gufp__tduu = get_first_non_na(in_arr[:shift])
        eolf__wqli = get_last_non_na(in_arr[:shift])
    else:
        gufp__tduu = get_last_non_na(in_arr[:-shift])
        eolf__wqli = get_first_non_na(in_arr[:-shift])
    one = get_one_from_arr_dtype(output)
    start = max(shift, 0)
    end = min(N, N + shift)
    for rkc__vva in range(start, end):
        rwntl__xeq = in_arr[rkc__vva - shift]
        if np.isnan(rwntl__xeq):
            rwntl__xeq = gufp__tduu
        else:
            gufp__tduu = rwntl__xeq
        val = in_arr[rkc__vva]
        if np.isnan(val):
            val = eolf__wqli
        else:
            eolf__wqli = val
        output[rkc__vva] = val / rwntl__xeq - one
    return output
...
def _border_icomm(in_arr, rank, n_pes, halo_size, send_right=True,
    send_left=False):
    mymty__krpfz = np.int32(comm_border_tag)
...
def _border_icomm_var(in_arr, on_arr, rank, n_pes, win_size):
    ...
    for wjod__vuqj in range(-2, -N, -1):
        palyd__zgpau = on_arr[wjod__vuqj]
        if end - palyd__zgpau >= win_size:
            halo_size = -wjod__vuqj
            break
    if rank != n_pes - 1:
        bodo.libs.distributed_api.send(halo_size, np.int32(rank + 1),
            mymty__krpfz)
        r_send_req = bodo.libs.distributed_api.isend(in_arr[-halo_size:],
            np.int32(halo_size), np.int32(rank + 1), mymty__krpfz, True)
        yifga__jiuxa = bodo.libs.distributed_api.isend(on_arr[-halo_size:],
            np.int32(halo_size), np.int32(rank + 1), mymty__krpfz, True)
    if rank != 0:
        ...
        muau__crt = bodo.libs.distributed_api.irecv(l_recv_t_buff, np.int32
            (halo_size), np.int32(rank - 1), mymty__krpfz, True)
    return (l_recv_buff, l_recv_t_buff, r_send_req, yifga__jiuxa,
        l_recv_req, muau__crt)
...
def _is_small_for_parallel(N, halo_size):
    eiktl__nicsf = bodo.libs.distributed_api.dist_reduce(int(N <= 2 *
        halo_size + 1), np.int32(Reduce_Type.Sum.value))
    return eiktl__nicsf != 0
...
def _handle_small_data(in_arr, win, minp, center, rank, n_pes, init_data,
    add_obs, remove_obs, calc_out):
    N = len(in_arr)
    wxhb__gjf = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.int32
        (Reduce_Type.Sum.value))
    ubny__rwfy = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        lxy__zvib, tzs__jiru = roll_fixed_linear_generic_seq(ubny__rwfy,
            win, minp, center, init_data, add_obs, remove_obs, calc_out)
    else:
        lxy__zvib = np.empty(wxhb__gjf, np.float64)
    bodo.libs.distributed_api.bcast(lxy__zvib)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
    return lxy__zvib[start:end]
...
def _handle_small_data_apply(in_arr, index_arr, win, minp, center, rank,
    ...
    wxhb__gjf = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.int32
        (Reduce_Type.Sum.value))
    ubny__rwfy = bodo.libs.distributed_api.gatherv(in_arr)
    mpnp__ztjpt = bodo.libs.distributed_api.gatherv(index_arr)
    if rank == 0:
        lxy__zvib = roll_fixed_apply_seq(ubny__rwfy, mpnp__ztjpt, win, minp,
...
def _handle_small_data_shift(in_arr, shift, rank, n_pes):
    ...
    ubny__rwfy = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        lxy__zvib = alloc_shift(len(ubny__rwfy), ubny__rwfy, (-1,))
        shift_seq(ubny__rwfy, shift, lxy__zvib)
        uvg__rlwx = bcast_n_chars_if_str_binary_arr(lxy__zvib)
    else:
        uvg__rlwx = bcast_n_chars_if_str_binary_arr(in_arr)
        lxy__zvib = alloc_shift(wxhb__gjf, in_arr, (uvg__rlwx,))
    bodo.libs.distributed_api.bcast(lxy__zvib)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
...
def _handle_small_data_pct_change(in_arr, shift, rank, n_pes):
    N = len(in_arr)
    wxhb__gjf = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    ubny__rwfy = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        lxy__zvib = pct_change_seq(ubny__rwfy, shift)
    else:
        lxy__zvib = alloc_pct_change(wxhb__gjf, in_arr)
    bodo.libs.distributed_api.bcast(lxy__zvib)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
...
def _is_small_for_parallel_variable(on_arr, win_size):
    if len(on_arr) < 2:
        ...
        qabcc__woww = end - start
        mhinj__mdwfl = int(qabcc__woww <= win_size)
    eiktl__nicsf = bodo.libs.distributed_api.dist_reduce(mhinj__mdwfl, np.
...
def _handle_small_data_variable(in_arr, on_arr, win, minp, rank, n_pes,
    ...
    wxhb__gjf = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    ubny__rwfy = bodo.libs.distributed_api.gatherv(in_arr)
    kpbny__fng = bodo.libs.distributed_api.gatherv(on_arr)
    if rank == 0:
        start, end = _build_indexer(kpbny__fng, wxhb__gjf, win, False, True)
        lxy__zvib = roll_var_linear_generic_seq(ubny__rwfy, kpbny__fng, win,
...
def _handle_small_data_variable_apply(in_arr, on_arr, index_arr, win, minp,
    ...
    if rank == 0:
        start, end = _build_indexer(kpbny__fng, wxhb__gjf, win, False, True)
        lxy__zvib = roll_variable_apply_seq(ubny__rwfy, kpbny__fng,
            mpnp__ztjpt, win, minp, start, end, kernel_func, raw)
    else:
        lxy__zvib = np.empty(wxhb__gjf, np.float64)
    bodo.libs.distributed_api.bcast(lxy__zvib)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
...
def _dropna(arr):
    omj__fcren = len(arr)
    wrx__ypwri = omj__fcren - np.isnan(arr).sum()
    A = np.empty(wrx__ypwri, arr.dtype)
    sfebx__tjo = 0
    for rkc__vva in range(omj__fcren):
        val = arr[rkc__vva]
        if not np.isnan(val):
            A[sfebx__tjo] = val
            sfebx__tjo += 1

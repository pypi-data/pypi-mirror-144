...
def _column_filter_impl(B, ind):
    ney__rpj = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,))
    for qrjp__kkov in numba.parfors.parfor.internal_prange(len(ney__rpj)):
        if ind[qrjp__kkov]:
            ney__rpj[qrjp__kkov] = B[qrjp__kkov]
        else:
            bodo.libs.array_kernels.setna(ney__rpj, qrjp__kkov)
    return ney__rpj
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    dusk__tunhl = len(B)
    ajrq__xxbgf = 0
    for qrjp__kkov in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, qrjp__kkov):
            ajrq__xxbgf += 1
    rqedz__tljn = dusk__tunhl - ajrq__xxbgf
    gfr__zpbm = bodo.libs.str_arr_ext.num_total_chars(B)
    ney__rpj = bodo.libs.str_arr_ext.pre_alloc_string_array(rqedz__tljn,
        gfr__zpbm)
    bodo.libs.str_arr_ext.copy_non_null_offsets(ney__rpj, B)
    bodo.libs.str_arr_ext.copy_data(ney__rpj, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(ney__rpj, -1)
    return ney__rpj
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    cgmjf__gbtob = first_moment / count
    jncz__ypuhv = (third_moment - 3 * second_moment * cgmjf__gbtob + 2 *
        count * cgmjf__gbtob ** 3)
    bwpt__zfm = second_moment - cgmjf__gbtob * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2
        ) * jncz__ypuhv / bwpt__zfm ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    cgmjf__gbtob = first_moment / count
    dmjhm__fauw = (fourth_moment - 4 * third_moment * cgmjf__gbtob + 6 *
        second_moment * cgmjf__gbtob ** 2 - 3 * count * cgmjf__gbtob ** 4)
    htwnb__udady = second_moment - cgmjf__gbtob * first_moment
    aph__fwpl = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    fdp__rdxos = count * (count + 1) * (count - 1) * dmjhm__fauw
    jvxqm__fjbc = (count - 2) * (count - 3) * htwnb__udady ** 2
    s = (count - 1) * (fdp__rdxos / jvxqm__fjbc - aph__fwpl)
    s = s / (count - 1)
    return s

...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        erjb__ekq = []
        zsz__dosmk = h5g_get_num_objs(obj_id)
        for fjo__nwgxu in range(zsz__dosmk):
            tcqdf__zhfcn = h5g_get_objname_by_idx(obj_id, fjo__nwgxu)
            erjb__ekq.append(tcqdf__zhfcn)
        return erjb__ekq
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    uvp__pkud = parse_dtype(dtype)
    xjg__lwpy = np.int32(numba_to_c_type(uvp__pkud))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            lkfqg__zhz = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    skqc__xaxh = tuple_to_ptr(starts)
    psko__reqq = tuple_to_ptr(counts)
    zqdys__wbt = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, skqc__xaxh, psko__reqq,
        is_parallel, out_arr.ctypes, zqdys__wbt)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    psko__reqq = tuple_to_ptr(counts)
    zqdys__wbt = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, skqc__xaxh, psko__reqq,
        is_parallel, out_arr.ctypes, zqdys__wbt)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        rgm__ppzpk = types.Array(dtype, ndim, 'C')
        jxns__hfvss = args[3]
        rgm__ppzpk = self.context.resolve_function_type(operator.getitem, [
            rgm__ppzpk, jxns__hfvss], {}).return_type.copy(layout='C')
        return signature(rgm__ppzpk, *args)
...
def get_filter_read_indices(bool_arr):
    keu__hray = bool_arr.nonzero()[0]
    ehn__enude = bodo.libs.distributed_api.get_rank()
    jjysp__oyqm = bodo.libs.distributed_api.get_size()
    mpti__ndydp = np.empty(jjysp__oyqm, np.int64)
    cvpb__tbru = len(bool_arr)
    bodo.libs.distributed_api.allgather(mpti__ndydp, cvpb__tbru)
    cgwdj__wji = mpti__ndydp.cumsum()[ehn__enude] - cvpb__tbru
    keu__hray += cgwdj__wji
    zkvoz__wvw = bodo.libs.distributed_api.dist_reduce(len(keu__hray), np.
        int32(sum_op))
    bgwrg__func = bodo.libs.distributed_api.gatherv(keu__hray)
    if ehn__enude == 0:
        gtadj__hvjie = bgwrg__func
    else:
        gtadj__hvjie = np.empty(zkvoz__wvw, keu__hray.dtype)
    bodo.libs.distributed_api.bcast(gtadj__hvjie)
    oum__hkj = bodo.libs.distributed_api.get_start(zkvoz__wvw, jjysp__oyqm,
        ehn__enude)
    refhw__gjtqt = bodo.libs.distributed_api.get_end(zkvoz__wvw,
        jjysp__oyqm, ehn__enude)
    return gtadj__hvjie[oum__hkj:refhw__gjtqt]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        nvkvg__dtqi = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], nvkvg__dtqi)
        return builder.bitcast(nvkvg__dtqi, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    psko__reqq = tuple_to_ptr(counts)
    zqdys__wbt = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, skqc__xaxh,
        psko__reqq, is_parallel, out_arr.ctypes, zqdys__wbt, read_indices.

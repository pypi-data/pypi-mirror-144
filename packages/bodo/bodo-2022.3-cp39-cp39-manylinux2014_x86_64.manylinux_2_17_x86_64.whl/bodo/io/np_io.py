...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        qrsh__mvufi = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, qrsh__mvufi)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            lhhse__wfvt = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(lhhse__wfvt.dtype)
            file_write(unicode_to_utf8(fname), lhhse__wfvt.ctypes, 
                dtype_size * lhhse__wfvt.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            lhhse__wfvt = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(lhhse__wfvt.dtype)
            snjt__jgzlw = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(lhhse__wfvt.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), lhhse__wfvt.ctypes,
                start, count, snjt__jgzlw)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            nybg__ggr = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                nybg__ggr = min(nybg__ggr, count * dtype_size)
            if nybg__ggr < 0:
                return -1
            return nybg__ggr
        return impl

...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        iod__jwet = len(A.types)
        uruug__lzh = 'def f(A):\n'
        zwvhj__ppsr = ','.join(f'decode_if_dict_array(A[{i}])' for i in
            range(iod__jwet))
        uruug__lzh += '  return ({}{})\n'.format(zwvhj__ppsr, ',' if 
            iod__jwet == 1 else '')
        xcfr__ukv = {}
        exec(uruug__lzh, {'decode_if_dict_array': decode_if_dict_array},
            xcfr__ukv)
        impl = xcfr__ukv['f']
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            xanmq__hpsa = bodo.hiframes.pd_series_ext.get_series_data(A)
            qfwo__jcl = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            gza__wxue = decode_if_dict_array(xanmq__hpsa)
            return bodo.hiframes.pd_series_ext.init_series(gza__wxue,
                qfwo__jcl, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        kpyqq__hvz = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(kpyqq__hvz, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        gjy__gtd = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(gjy__gtd(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        upau__ipaoh = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=upau__ipaoh)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        ivpzb__bui = self.get_full_filename(fname)
        return self._get_schema(ivpzb__bui)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qia__oqwn = dmm.lookup(bodo.typeof(fe_type.fname))
        mtef__xqne = [(a, wmff__zvmwy) for a, wmff__zvmwy in zip(qia__oqwn.
            _fields, qia__oqwn._members)]
        super().__init__(dmm, fe_type, mtef__xqne)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    imgf__ddei = t.initial_value
    if isinstance(imgf__ddei, dict):
        imgf__ddei = imgf__ddei.values()
    return not any(isinstance(cruov__ifn, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for cruov__ifn in imgf__ddei)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (cruov__ifn) for cruov__ifn in val.types
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (cruov__ifn) for cruov__ifn in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        hahxs__rdhtv = []
        for cruov__ifn in val.literal_value:
            xujt__kzm = get_overload_const(cruov__ifn)
            if xujt__kzm == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                hahxs__rdhtv.append(xujt__kzm)
        return hahxs__rdhtv
    ...
    if isinstance(val, types.BaseTuple):
        hahxs__rdhtv = []
        for cruov__ifn in val.types:
            xujt__kzm = get_overload_const(cruov__ifn)
            if xujt__kzm == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                hahxs__rdhtv.append(xujt__kzm)
        return tuple(hahxs__rdhtv)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    xujt__kzm = get_overload_const(val)
    gtjtk__ptsiz = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(xujt__kzm, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {gtjtk__ptsiz} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    xujt__kzm = get_overload_const(val)
    if xujt__kzm not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{xujt__kzm}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    nbp__rpzt = False
    for a in args_dict:
        oplrh__hzrk = get_overload_const(args_dict[a])
        vqi__vdlfe = arg_defaults_dict[a]
        if (oplrh__hzrk is NOT_CONSTANT or oplrh__hzrk is not None and 
            vqi__vdlfe is None or oplrh__hzrk is None and vqi__vdlfe is not
            None or oplrh__hzrk != vqi__vdlfe or oplrh__hzrk is not
            _no_input and vqi__vdlfe is _no_input or oplrh__hzrk is
            _no_input and vqi__vdlfe is not _no_input):
            huomn__zxuhc = (
                f'{fn_str}: {a} parameter only supports default value {vqi__vdlfe}'
                )
            nbp__rpzt = True
            break
    if nbp__rpzt and package_name == 'pandas':
        if module_name == 'IO':
            huomn__zxuhc += """
...
"""
    elif nbp__rpzt and package_name == 'ml':
        huomn__zxuhc += """
...
"""
    elif nbp__rpzt and package_name == 'numpy':
        huomn__zxuhc += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(moj__yaqwr): get_literal_value(cruov__ifn
            ) for moj__yaqwr, cruov__ifn in val.literal_value.items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    sdpqa__htl = [get_overload_const(cruov__ifn) for cruov__ifn in val.
        types[1:]]
    return {sdpqa__htl[2 * i]: sdpqa__htl[2 * i + 1] for i in range(len(
        sdpqa__htl) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(cruov__ifn) for cruov__ifn in val.
            literal_value]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        kvek__acs = get_overload_const_str(dtype)
        if kvek__acs.startswith('Int') or kvek__acs.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(kvek__acs), None)
        if kvek__acs == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        return numba.np.numpy_support.from_dtype(np.dtype(kvek__acs))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    tsb__ocva = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        fza__bllb = []
        for i, mioa__nzqae in enumerate(t.names_typ):
            if mioa__nzqae == types.none:
                fza__bllb.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(mioa__nzqae):
                raise BodoError(tsb__ocva)
            fza__bllb.append(get_overload_const_str(mioa__nzqae))
        return tuple(fza__bllb)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        drxap__xxt = typ.literal_value
        vod__wrccb = []
        for ria__xgp in ('start', 'stop', 'step'):
            tsnm__erp = getattr(typ.literal_value, ria__xgp)
            aikr__dpmw = c.pyapi.make_none(
                ) if tsnm__erp is None else c.pyapi.from_native_value(types
                .literal(tsnm__erp), tsnm__erp, c.env_manager)
            vod__wrccb.append(aikr__dpmw)
        nvg__brv = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        wiz__fozuk = c.pyapi.call_function_objargs(nvg__brv, vod__wrccb)
        for a in vod__wrccb:
            c.pyapi.decref(a)
        c.pyapi.decref(nvg__brv)
        return wiz__fozuk
...
def box_list_literal(typ, val, c):
    dqn__uhr = typ.literal_value
    emn__zoe = [c.pyapi.from_native_value(types.literal(cruov__ifn),
        cruov__ifn, c.env_manager) for cruov__ifn in dqn__uhr]
    fbrk__jncmp = c.pyapi.list_pack(emn__zoe)
    for a in emn__zoe:
        c.pyapi.decref(a)
    return fbrk__jncmp
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    lfr__oxvbx = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(cruov__ifn) for cruov__ifn in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(moj__yaqwr): get_literal_value(
                cruov__ifn) for moj__yaqwr, cruov__ifn in t.literal_value.
                items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(cruov__ifn) for cruov__ifn in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(cruov__ifn) for cruov__ifn in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        xanmq__hpsa = types.Array(dtype, 1, 'C')
        if jtka__iytf:
            return to_nullable_type(xanmq__hpsa)
        return xanmq__hpsa
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    lxt__ipsk = dtype_to_array_type(f_return_type)
    lxt__ipsk = to_nullable_type(lxt__ipsk) if return_nullable else lxt__ipsk
    return lxt__ipsk
...
def types_equality_exists(t1, t2):
    axb__bimdi = numba.core.registry.cpu_target.typing_context
    try:
        axb__bimdi.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    vxu__echqn = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo.
        pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in vxu__echqn:
        return True
    axb__bimdi = numba.core.registry.cpu_target.typing_context
    try:
        axb__bimdi.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        vadq__byucc = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(vadq__byucc, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    mtyr__crro, fjhd__qofq = get_common_scalar_dtype(scalar_types)
    return fjhd__qofq
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        akdm__ydm = np.find_common_type([numba.np.numpy_support.as_dtype(t) for
            t in scalar_types], [])
        if akdm__ydm != object:
            return numba.np.numpy_support.from_dtype(akdm__ydm), True
    except numba.core.errors.NumbaNotImplementedError as vrd__kadh:
        pass
    ...
    tpyo__jtl = itertools.groupby(scalar_types)
    if next(tpyo__jtl, True) and not next(tpyo__jtl, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            pyoi__eob.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            pyoi__eob.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                pyoi__eob.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                pyoi__eob.append(bodo.libs.int_arr_ext.IntegerArrayType(typ
                    .dtype))
        pyoi__eob.append(typ)
    return pyoi__eob
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        aoeo__sju = getfullargspec(func)
        assert aoeo__sju.varargs is None, 'varargs not supported'
        assert aoeo__sju.varkw is None, 'varkw not supported'
        defaults = [] if aoeo__sju.defaults is None else aoeo__sju.defaults
        nkk__arezk = len(aoeo__sju.args) - len(defaults)
        args = aoeo__sju.args[1:] if attr_name else aoeo__sju.args[:]
        lvjo__akj = []
        for i, ahey__kpf in enumerate(aoeo__sju.args):
            if i < nkk__arezk:
                lvjo__akj.append(ahey__kpf)
            elif str(defaults[i - nkk__arezk]) != '<deprecated parameter>':
                lvjo__akj.append(ahey__kpf + '=' + str(defaults[i -
                    nkk__arezk]))
            else:
                args.remove(ahey__kpf)
        if aoeo__sju.kwonlyargs is not None:
            for ahey__kpf in aoeo__sju.kwonlyargs:
                args.append(f'{ahey__kpf}={ahey__kpf}')
                lvjo__akj.append(
                    f'{ahey__kpf}={str(aoeo__sju.kwonlydefaults[ahey__kpf])}')
        sig = ', '.join(lvjo__akj)
        args = ', '.join(args)
    else:
        sig = 'self'
    ...
    dhv__ecgx = f'self.{attr_name}' if attr_name else f'{func_name}'
    uruug__lzh = f'def overload_impl({sig}):\n'
    uruug__lzh += f'    def impl({sig}):\n'
    if single_rank:
        uruug__lzh += f'        if bodo.get_rank() == 0:\n'
        xoewx__rocx = '    '
    else:
        xoewx__rocx = ''
    uruug__lzh += (
        f"        {xoewx__rocx}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        uruug__lzh += f'            {xoewx__rocx}res = {dhv__ecgx}({args})\n'
    else:
        uruug__lzh += f'            {xoewx__rocx}res = {dhv__ecgx}\n'
    uruug__lzh += f'        return res\n'
    uruug__lzh += f'    return impl\n'
    xcfr__ukv = {}
    zylaq__sxzk = globals()
    if not attr_name:
        zylaq__sxzk[func_name] = func
    exec(uruug__lzh, zylaq__sxzk, xcfr__ukv)
    ekr__pbm = xcfr__ukv['overload_impl']
    return ekr__pbm


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        ekr__pbm = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(ekr__pbm)
    except Exception as vrd__kadh:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        ekr__pbm = _gen_objmode_overload(method, output_type, method_name, 
            True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(ekr__pbm)
    except Exception as vrd__kadh:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        ekr__pbm = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(ekr__pbm)
    except Exception as vrd__kadh:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, thj__puun = args
        if isinstance(thj__puun, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[thj__puun]), *args
                )
...
def construct_pysig(arg_names, defaults):
    uruug__lzh = f'def stub('
    for ahey__kpf in arg_names:
        uruug__lzh += ahey__kpf
        if ahey__kpf in defaults:
            if isinstance(defaults[ahey__kpf], str):
                uruug__lzh += f"='{defaults[ahey__kpf]}'"
            else:
                uruug__lzh += f'={defaults[ahey__kpf]}'
        uruug__lzh += ', '
    uruug__lzh += '):\n'
    uruug__lzh += '    pass\n'
    xcfr__ukv = {}
    exec(uruug__lzh, {}, xcfr__ukv)
    tpix__xfel = xcfr__ukv['stub']
    return numba.core.utils.pysignature(tpix__xfel)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    ced__cyj = len(arg_names)
    qxfv__krp = len(args) + len(kws)
    if qxfv__krp > ced__cyj:
        dzfc__vmvfe = 'argument' if ced__cyj == 1 else 'arguments'
        akr__sedk = 'was' if qxfv__krp == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {ced__cyj} {dzfc__vmvfe}, but {qxfv__krp} {akr__sedk} provided.'
            )
    rjank__rlv = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        erot__mrghk = bodo.utils.transform.fold_argument_types(rjank__rlv,
            args, kws)
    except Exception as nmv__giuwa:
        raise_bodo_error(f'{func_name}(): {nmv__giuwa}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                lhtz__zzggs[arg_name] = erot__mrghk[i]
                ahi__fjinn[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, lhtz__zzggs, ahi__fjinn)
    return rjank__rlv, erot__mrghk
...
class BodoArrayIterator(types.SimpleIteratorType):
    ...
    def __init__(self, arr_type):
        ...
        scirx__hbizl = arr_type.dtype
        super(BodoArrayIterator, self).__init__(name, scirx__hbizl)
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        mtef__xqne = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, mtef__xqne)
...
def iternext_bodo_array(context, builder, sig, args, result):
    ...
    [hzyqb__bpr] = sig.args
    [pmfp__ayyt] = args
    imiqc__vblxr = context.make_helper(builder, hzyqb__bpr, value=pmfp__ayyt)
    lwmsr__xertc = signature(types.intp, hzyqb__bpr.arr_type)
    zzwaw__kxfag = context.compile_internal(builder, lambda a: len(a),
        lwmsr__xertc, [imiqc__vblxr.array])
    qfwo__jcl = builder.load(imiqc__vblxr.index)
    lrmk__qkyqk = builder.icmp(lc.ICMP_SLT, qfwo__jcl, zzwaw__kxfag)
    result.set_valid(lrmk__qkyqk)
    with builder.if_then(lrmk__qkyqk):
        bro__xfhx = signature(hzyqb__bpr.arr_type.dtype, hzyqb__bpr.
            arr_type, types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            bro__xfhx, [imiqc__vblxr.array, qfwo__jcl])
        result.yield_(value)
        vgx__kdf = cgutils.increment_index(builder, qfwo__jcl)
        builder.store(vgx__kdf, imiqc__vblxr.index)


def index_typ_from_dtype_name(elem_dtype, name):
    dvbos__lfdp = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        uvg__cmkqs = None
    else:
        uvg__cmkqs = types.StringLiteral(name)
    if dvbos__lfdp == bodo.hiframes.pd_index_ext.NumericIndexType:
        rjt__rvm = dvbos__lfdp(elem_dtype, uvg__cmkqs)
    elif dvbos__lfdp == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        rjt__rvm = dvbos__lfdp(bodo.CategoricalArrayType(elem_dtype),
            uvg__cmkqs)
    else:
        rjt__rvm = dvbos__lfdp(uvg__cmkqs)
    return rjt__rvm
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(cruov__ifn, t) for cruov__ifn, t in
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        mahx__gkeja = pd.Index(val_typ.columns)
        ocstf__oqtz = pd.Index(typ.columns)
        atp__nba = mahx__gkeja.argsort()
        rebib__peah = ocstf__oqtz.argsort()
        wlkuy__ubs = val_typ.copy(data=tuple(np.array(val_typ.data)[
            atp__nba]), columns=tuple(mahx__gkeja[atp__nba]))
        xubu__gqd = typ.copy(data=tuple(np.array(typ.data)[rebib__peah]),
            columns=tuple(ocstf__oqtz[rebib__peah]))
        if wlkuy__ubs == xubu__gqd:
            val_typ = typ

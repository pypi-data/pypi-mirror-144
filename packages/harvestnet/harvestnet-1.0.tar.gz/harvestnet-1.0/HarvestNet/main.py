...
if os.path.exists(basepath + '\\' + 'ResNet50_SGD.pth') == False and os.path.exists(basepath + '\\' + 'potato4.h5') == False and os.path.exists(basepath + '\\' + 'model-Xception.h5') == False:
    from .ftp import totalmodels
    DL = totalmodels()

elif os.path.exists(basepath + '\\' + 'ResNet50_SGD.pth') == False:
    from .ftp import tomatomodel
    DL = tomatomodel()

elif os.path.exists(basepath + '\\' + 'potato4.h5') == False:
    from .ftp import potatomodel
    DL = potatomodel()

elif os.path.exists(basepath + '\\' + 'model-Xception.h5') == False:
    from .ftp import cornmodel
    DL = cornmodel()
...
transform_valid = transforms.Compose([transforms.Resize(225),
                                           ...
                                           transforms.Normalize([0.485, 0.456, 0.406],
                                                             [0.229, 0.224, 0.225])])
#马铃薯模型
model = load_model(basepath + '\\' + 'potato4.h5')
default_image_size = tuple((256, 256))

#番茄模型
model_tomato = torch.load(basepath + '\\' + 'ResNet50_SGD.pth')
use_cuda = torch.cuda.is_available()
if use_cuda:
    model_tomato = model_tomato.cuda()

#玉米模型
model_crop = load_model(basepath + '\\' + 'model-Xception.h5')
...
class predict(Resource):
    ...
    def post(self):
            ...
            if os.path.exists(upload_path) == False:
                ...
                f.save(upload_path + src_imgname)
            f_img = cv2.imread(os.path.join(basepath, 'static\one.jpg'))
...
class predict_tomato(Resource):
    ...
    def post(self):
            ...
            if os.path.exists(upload_path) == False:
                ...
                f.save(upload_path + src_imgname)
            img = Image.open(os.path.join(basepath, 'static\one.jpg')).convert('RGB')

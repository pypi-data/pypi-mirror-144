...
class TestCase(TlsSuiteTester):
    ...
    recorder_yaml = "recorder_client_simul_openssl3_0_0_Ed25519"
    path = pathlib.Path(__file__)
    server_cmd = (
        "utils/start_openssl --version {library} --port {server_port} "
        "--cert1 server-ed25519 -- -www -cipher ALL"
    )
    ...
    def run(self, tlsmate, is_replaying):
        ...
        assert client_prof.cert_type is tls.SignatureAlgorithm.ED25519
        assert client_prof.cert_pub_key_size == 256

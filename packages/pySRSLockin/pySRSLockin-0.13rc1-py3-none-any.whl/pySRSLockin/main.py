...
from pySRSLockin.abstract_interface import abstract_interface, abstract_gui
...
class interface(abstract_interface):
    ...
    def __init__(self, **kwargs):
        ...
        self.sensitivity_auto = True #Boolean, true if software-controlled automatic sensitivity is enabled
...
### Functions to talk with low-level driver
    ...
    def set_sensitivity(self,s): 
        ...
        try: 
            ...
            snew = self.instrument.sensitivity #read again sensitivity to confirm that it changed
            if snew == s:
                self.logger.info(f"Sensitivity changed to {self.sensitivities[s]} (old value was {self.sensitivities[self.sensitivity]}).")
                self.sensitivity = s
            else:
    ...
    def adjust_sensitivity_auto(self):
        ...
        if (current_sensitivity_value*min_bound) < current_mag < (current_sensitivity_value*max_bound):
            ...
            index_new_sensitivity = self.sensitivities.index (
                                                                min ([i for i in  self.sensitivities if i > target_sensitivity] )
                                                                )
            return self.set_sensitivity(self,index_new_sensitivity)
...
class gui(abstract_gui):
    def __init__(self,interface,parent,plot=False):
        ...
        for k in self.interface.output.keys():		
    ...
    def click_box_SensitivityAuto(self):
...
def main():
    ...
    Interface.create_gui(window,plot=False)

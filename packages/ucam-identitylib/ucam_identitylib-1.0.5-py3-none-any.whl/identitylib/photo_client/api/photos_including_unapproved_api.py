    ...
    def v1beta1_photos_create(
        ...
        **kwargs
        ...
        """Create a new photo with a set of identifiers  # noqa: E501
          ...
          # Create a new Photo  **NOTE** This endpoint is not currently implemented. Implementation of this functionality can be tracked under https://gitlab.developers.cam.ac.uk/uis/devops/iam/identity-photos-service/photo-api/-/issues/17.  This endpoint allows a new photo to be created and associated with a given set of identified.  At present only principals with the `PHOTO_CREATOR` permission are granted access to create new photos.  The image should be provided as a file upload on the request body and should be in any of the following formats:  * image/png, * image/jpeg, * image/heic,  Images should be at minimum 640 x 480 pixels and should not exceed 10MB in file size.  If the principal has the permission `PHOTO_CREATOR`, a list of identifiers for the person represented by a given photo can be provided on the request's query parameters in the `<value>@<scheme>` format, with each identifier separated by a comma. This is to allow an authorized client to create photos of other members of the University, linking a photo to an identity record held in another service. If identifiers are not provided, the identifier of the current principal will be attached to the photo, as it is assumed that the given photo represents the current principal.  Principals with the `PHOTO_APPROVER` permission can additionally provide the `status` query parameter which allows the photo to either be set as `APPROVED` or `UNAPPROVED`. If not provided or the principal does not have the `PHOTO_APPROVER` permission the photo's status will be `UNAPPROVED`.    # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        ...
        Keyword Args:
            ...
            status (str): The status of this photo. Only principals with the `PHOTO_APPROVER` permission can set the status to `APPROVED`. Defaults to `UNAPPROVED`.. [optional]
    ...
    def v1beta1_photos_destroy(
        ...
        **kwargs
        ...
        """Remove a photo  # noqa: E501
          ...
          # Remove a photo by UUID  **NOTE** This endpoint is not currently implemented. Implementation of this functionality can be tracked under https://gitlab.developers.cam.ac.uk/uis/devops/iam/identity-photos-service/photo-api/-/issues/20.  This endpoint allows a photo to be deleted from the Photo API, with the photo no longer being available for query from the API. Removing a photo is not reversible, once a photo has been removed it cannot be restored.  Only principals with the `PHOTO_REMOVER` permission can remove photos, and are limited to removing which were originally created by the same principal (as indicated by the `submittedBy`) field on a photo's record.    # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

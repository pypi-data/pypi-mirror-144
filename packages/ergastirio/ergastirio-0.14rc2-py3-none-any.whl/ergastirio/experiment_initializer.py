...
the experiment. After the experiment have been assigned a config file, the function load_config_and_setup_exp(exp) is called, where exp is the experiment() object.
'''
...
def load_config_and_setup_exp(exp):
    '''
    Import experiment settings from the .json file stored in exp.config_file, and then starts setting up the experiment 
    '''
    exp.logger.info(f"Loading the content of {exp.config_file}")
    try:
        with open(exp.config_file) as jsonfile:
            exp.config = json.load(jsonfile)
    except Exception as e:
        exp.logger.error(f"An error occurred while loading the file. Fix the error and restart this application\n: {e}")
        return
...
def set_up_experiment_gui(exp):
    ...
    '''
    exp.container_tabledata = exp.parent.containers['tabledata']['container']
    exp.container_plots = exp.parent.containers['plots']['scrollarea']
    exp.container_instruments = exp.parent.containers['instruments']['container']

    create_gui_table_data(exp)
    create_gui_plots(exp)
    
    if not create_gui_instruments(exp): #Create GUIs for all loaded instruments 
        return False
...
def create_gui_instruments(exp):
    '''
    ...
    for i,instrument in enumerate(exp.instruments):
        ...
        exp.instruments[i]['frame'].setStyleSheet(string + ergastirio.styles.instrument_box + string + ergastirio.styles.instrument_boxtitle) #This line changes the style of ONLY this QWdiget

        exp.instruments[i]['frame'].setTitle(f"{exp.instruments[i]['name']} ({exp.instruments[i]['type']} v{version(exp.instruments[i]['type'])})")
        
        exp.mainwindow.logging_text_area.add_logger(logger=logging.getLogger(exp.instruments[i]['fullname'])) #Connect the logger of this instrument to the text area for logging in the GUI
    ...
    exp.container_instruments.setLayout(box) #This line makes sure that all widgest defined so far are assigned to the widget defined in container
    exp.container_instruments.resize(exp.container_instruments.sizeHint())
    return True

def create_gui_plots(exp):
    '''
    ...
    for plotindex, plot in enumerate(exp.plots):
        ...
        mdi.addSubWindow(exp.plots[plotindex]['subwindow'])
    ...
    exp.container_plots.mdi = mdi
    exp.container_plots.setWidget(mdi)
    exp.container_plots.mdi.tileSubWindows()
    return True

def create_gui_table_data(exp):
    '''
    ...
    exp.container_tabledata.setLayout(layout)
...
def link_data_to_gui(exp):
    ...
    for plot in exp.plots:
        ...
        exp.data_std.add_syncronized_objects([  plot['plotobject'],  ergastirio.widgets.PlotObject.data_error])

...
from gm.csdk.c_sdk import py_gmi_current, py_gmi_schedule, py_gmi_set_data_callback, py_gmi_set_strategy_id, \
    ...
    py_gmi_set_mfp, py_gmi_user_timer, py_gmi_user_timer_stop, py_gmi_set_serv_addr_v5, py_gmi_set_account_id
...
def _py_gmi_unsubscribe_all():
    ...
    for symbol in context.tick_sub_symbols:
        ...
        c_status_fail(status, "py_gmi_unsubscribe")
    # 退订 bar
...
def run(strategy_id='', filename='', mode=MODE_UNKNOWN, token='',
    ...
    backtest_commission_unit = float(options.backtest_commission_unit)
    ...
    backtest_match_mode = int(options.backtest_match_mode)

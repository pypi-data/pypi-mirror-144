...
function dist_place(){
  ...
  local tmpdir=$5
  local idx=$6
  taskset -c $(((idx*4)%c))-$(((idx*4)%c+3)) depp_distance.py query_seq_file=$1 backbone_seq_file=${accessory_dir}/${data_type}_a.fasta outdir=../dist/$1 model_path=${accessory_dir}/${data_type}.ckpt replicate_seq=True recon_model_path=${accessory_dir}/${data_type}.recon.ckpt backbone_emb=${accessory_dir}/${data_type}_emb.pt backbone_gap=${accessory_dir}/${data_type}_gap.pt backbone_id=${accessory_dir}/${data_type}_id.pt recon_backbone_emb=${accessory_dir}/${data_type}_emb.recon.pt
  jplace_suffix="${seq_id##*.}"
  apples_core=`python -c "print(min($c,4))"`
  taskset -c $(((idx*4)%c))-$(((idx*4)%c+3)) run_apples.py -d ../dist/$1/depp.csv -t ${accessory_dir}/wol.nwk -o ${tmpdir}/${data_type}_${jplace_suffix}.jplace -f 0 -b 5 -T $apples_core 

...
# This is done just to provide a more helpful error message when needed.
try:
    import cvxopt
...
class ppxf:
    ...
    def __init__(self, templates, galaxy, noise, velscale, start,
        ...
        if (lam_temp is not None) and (lam is not None):
            ...
            vmax = 3e3      # km/s, maximum offset from starting guess
            lam_range = lam[[0, -1]]*np.exp(-(vmed + [vmax, -vmax])/c)
            lam_range = lam[[0, -1]]*np.exp(-(vmed + [vmax, -vmax])/c)
...
################################################################################
    ...
    def plot(self, gas_clip=False):
        ...
        if self.phot_npix:
            ...
            ll = min(ll, np.min(self.phot_lam_mean)/10)
            rr = max(rr, np.max(self.phot_lam_mean)/10)
        ...
        if self.phot_npix:
            ...
            if self.lam_temp is not None:
                ...
                start = self.sol[0]/self.velscale
                ...
                bestfit_full = np.fft.irfft(spec_rfft*lvd_rfft.squeeze(), npad)[:npix]

                if self.reddening is not None:
                    bestfit_full *= self.reddening_func(self.lam_temp_full, self.reddening)

                plt.plot(self.lam_temp_full/10, bestfit_full, 'gold', linewidth=2, zorder=1)

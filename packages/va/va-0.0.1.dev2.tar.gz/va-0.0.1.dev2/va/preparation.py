...
class PreParation:
    ...
    def new_combinemap(self, odd_file, even_file, rawmapfullname):
        ...
        # oddmap = mrcfile.open(odd_file, mode='r+')
        # evenmap = mrcfile.open(even_file, mode='r+')
        ...
        np.subtract(oddmap, oddmap.mean() * evenmap.std(), out=oddmap)
        np.add(oddmap, evenmap * oddmap.std(), out=oddmap)
        np.subtract(oddmap, oddmap.std() * evenmap.mean(), out=oddmap)
        np.divide(oddmap, oddmap.std() * evenmap.std(), out=oddmap)
        np.multiply(oddmap, 1.0, out=oddmap)
        np.add(oddmap, 1.0, out=oddmap)

...
class DataBlob:
    ...
    Currently, the only way to instantiate the DataBlob class is to call either `from_local`, `from_mysql`, or `from_s3`
    ...
    port, database name, table name.

    If the access to the database requires authentication, then the credentials can be passed as parameters to the `from_mysql`
    method or stored in the environment variables **AIRT_CLIENT_DB_USERNAME** and **AIRT_CLIENT_DB_PASSWORD**.

    Similarly, for establishing a connection to the S3 bucket, the `from_s3` method can be used and only requires the URI to the
    S3 bucket as input.
    ...
    def __init__(
        ...
        Warning:
            ...
            `from_mysql` or `from_local` methods instead.
    ...
    def from_local(
        ...
        Args:
            ...
            file_type: The file type to upload if the path is a directory. If not passed, then the
                default value **csv** will be used.
            tag: A string to tag the datablob. If not passed, then the tag **latest** will be assigned to the datablob.
        ...
        response = Client._post_data(
            relative_url=f"/datablob/from_local/start", data=dict(tag=tag)
        )

        # Step 2: download the csv to the s3 bucket
        p = Path(path)

        files = list(p.glob(f"*.{file_type}")) if p.is_dir() else [p]
    ...
    def as_df(dbx: List["DataBlob"]) -> pd.DataFrame:
        ...
        lists_df = pd.DataFrame(db_lists)[DataBlob.BASIC_DB_COLS]
...
@patch
def progress_bar(self: DataBlob, sleep_for: Union[int, float] = 5, timeout: int = 0):
    ...
    if self.type not in ["local"]:
        progress_status = ProgressStatus(
            relative_url=f"/datablob/{self.id}", sleep_for=sleep_for, timeout=timeout
        )
...
@patch
def wait(self: DataBlob, sleep_for: Union[int, float] = 1, timeout: int = 0):
...
def from_csv(
    ...
    *,
    ...
    Args:
        ...
        sort_by: The column to use for sorting the data.
        ...
        kwargs_json: Any additional parameters to be used while processing the data in a JSON string format. Example keys: usecols, parse_dates, skiprows, etc.,

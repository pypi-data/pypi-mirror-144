...
class PlaceholdersTestCase(unittest.TestCase):
    ...
    def test_file_placeholders(self) -> None:
        ...
        self.assertListEqual(get_file_placeholders('test.txt', type=str), ['num', 'action'])
        self.assertListEqual(get_file_placeholders('test.txt', type=[str, str]), ['num', 'action'])
        ...
        self.assertListEqual(get_file_placeholders('test2.txt', type=str), ['num', 'action', '42'])
        self.assertListEqual(get_file_placeholders('test2.txt', type=[str, str, int]), ['num', 'action', 42])
        with self.assertRaises(ValueError):
            self.assertListEqual(get_file_placeholders('test2.txt', type=[str, str]), ['num', 'action', 42])
        with self.assertRaises(ValueError):
            self.assertListEqual(get_file_placeholders('test2.txt', type=[str, str, int, str]), ['num', 'action', 42])
        self.assertEqual(get_file_placeholder('test2.txt', type=int, pos=2), 42)

    def test_no_placeholders(self) -> None:
        self.assertEqual(num_placeholders(''), 0)
        self.assertEqual(num_placeholders(None), 0)
        self.assertTupleEqual(parse_placeholders(None), (None, []))
        self.assertFalse(has_placeholders(''))
        self.assertFalse(has_placeholders(None))
        self.assertListEqual(get_placeholders(None), [])

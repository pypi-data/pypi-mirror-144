...
class AutoMLAbstractRunContext(ABC):
    ...
    def _save_mlflow_model(self,
        ...
        :return: Tuple of mlflow-associated filenames and file objects to upload.
        """
        try:
            import torch
        ...
        if isinstance(fitted_pipeline, sklearn.pipeline.Pipeline):
            ...
            logger.info("Saving model resources to {}.".format(output_dir))
            ...
            logger.info("Saving mlflow pyfunc model to {}.".format(output_dir))
            settings_file = open(os.path.join(working_dir, constants.MLFlowLiterals.MODEL_SETTINGS_FILENAME), 'w')
            self._save_dict_to_json_output(options[constants.MLFlowLiterals.WRAPPER]._model_settings, settings_file)
            mlflow.pyfunc.save_model(path=output_dir,
            ...
            for name, model in model_outputs.items():
                ...
                if save_as_mlflow and not isinstance(model, list):
                    try:
                        ...
                        mlflow_files, mlflow_paths = self._save_mlflow_model(fitted_pipeline=model,

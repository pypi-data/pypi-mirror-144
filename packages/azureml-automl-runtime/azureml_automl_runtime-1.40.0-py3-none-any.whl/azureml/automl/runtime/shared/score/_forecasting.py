...
class ForecastingMetric(Metric):
    ...
    def __init__(self,
                 ...
                 grain_column_names: Optional[Union[Any, List[Any]]] = None,
                 time_column_name: Optional[Any] = None) -> None:
        """
...
class ForecastTable(ForecastingMetric, NonScalarMetric):
    ...
    def compute(self) -> Dict[str, Any]:
        ...
        for igrain, (grain, df_one_train) in enumerate(df_train.groupby(grain_column_names)):
            ...
            df_one_train = df_one_train.iloc[-ForecastTable.MAX_FORECAST_TRAIN_DATA_POINTS:]
            df_one_valid = df_one_valid.iloc[:ForecastTable.MAX_FORECAST_VALID_DATA_POINTS]
            df_one = pd.concat([df_one_train, df_one_valid], sort=False, ignore_index=True)
            ...
            ci_bound = [stddev] * len(y_pred_list)
            for idx in range(len(y_pred_list)):
                ...
                ci_bound[idx] = abs(z_score * stddev * math.sqrt(idx + 1))

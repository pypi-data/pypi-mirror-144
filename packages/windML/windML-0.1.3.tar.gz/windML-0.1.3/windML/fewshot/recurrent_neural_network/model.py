...
class charRNN:
    ...
    def generate(self, prompt:str, condition_vector:Optional[ndarray]=None, generation_length:int=ModelSettings.SEQUENCE_LENGTH.value, greedy:bool=True) -> str:
        ...
        for character in prompt:
            encoded_input = one_hot_encode(token_ids[-1],ModelSettings.VOCABULARY_SIZE.value).reshape((-1,1))
    ...
    def fit(self, sentences:List[str], encoded_contexts:Optional[List[ndarray]]=None) -> None:
        ...
        for epoch in range(ModelSettings.EPOCHS.value):
            ...
            print(f'epoch {epoch} / {ModelSettings.EPOCHS.value} [loss = {loss}]') 

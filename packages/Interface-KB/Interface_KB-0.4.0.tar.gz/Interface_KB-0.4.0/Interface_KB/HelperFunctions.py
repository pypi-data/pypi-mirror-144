...
def updateKBv6(KB_path,model):
    ...
    #--DEBUG EXPERIMENT:use a blank template of optimizationProblem <includesOptimizationProblem/> and delete it
    postProcess_EmptyOptimization(KB_path)
    postProcess_EmptyProduct(KB_path)
    postProcess_EmptyProductPart(KB_path)
    postProcess_EmptyASG(KB_path)
    #postProcess_EmptyStopCriterion(KB_path)
    postProcess_EmptyDFARule(KB_path)
...
def create_noPlatformRoot( e_class,MM,model_instance):
    ...
    #root_reference = getattr(model_instance, 'includes' + e_class.name)
    #root_reference.append(e_object)            #original
    #root_reference.items.append(e_object)      #updated
...
#--DEBUG EXPERIMENT: hacking the empty subclass e.g. to update stopconditions or DFA rules

...
def test_repo_init_oebuild(oebuild_repo_init_object):
    ...
    assert oebuild.repo.url == "https://gitlab.com/vishalbhoj/test-manifest"
...
def test_validate(oebuild_git_object, oebuild_repo_init_object):

    # case - when everything is right and in proper format
    oebuild_git_obj = oebuild_git_object[0]
    oebuild_git_obj.validate()

    # case - similar with repo obj as it has repo instead of git_trees
    oebuild_obj = oebuild_repo_init_object
    oebuild_obj.validate()

    # handling all defined test in test_map
    for key in test_map:
        if key in oebuild_obj.__dataclass_fields__:
            if key == "git_trees":
                prev_val = getattr(oebuild_obj, "repo")
                setattr(oebuild_obj, "repo", None)
            else:
                prev_val = getattr(oebuild_obj, key)
            for test_case in test_map[key]:
                setattr(oebuild_obj, key, test_case)
                with pytest.raises(TuxbakeParsingError):
                    oebuild_obj.validate()
            if key == "git_trees":
                setattr(oebuild_obj, "git_trees", None)
                setattr(oebuild_obj, "repo", prev_val)
            else:
                setattr(oebuild_obj, key, prev_val)

    with pytest.raises(TuxbakeParsingError):
        # case - when both repo and git_trees are not present
        oebuild_obj.repo = None
        oebuild_obj.git_trees = None
        oebuild_obj.validate()

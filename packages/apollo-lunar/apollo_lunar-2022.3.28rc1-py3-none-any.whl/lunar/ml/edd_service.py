...
class EDDService:
    ...
    def copy_table_to_s3(
        ...
        - db_type: (str) Type of database to be loaded on Lunar ('dynamodb' | 'docdb' | 'es')
        - operation: (str) Type of operation ('put' | 'update' | 'upsert' (only for db_type 'dynamodb') | 'delete')
        - target_name: (optional) (str) Target name on database to be loaded (Default: Value of `table_name`)
        - s3_dt: (optional) (str) Set a dt partition on Lunar S3. If db_type is `docdb`, it is necessary
        ...
        assert db_type in ["dynamodb", "docdb", "es"], "`db_type` must be one of `dynamodb` / `docdb` / `es`"
        assert operation in [
            "put",
            "update",
            "upsert",

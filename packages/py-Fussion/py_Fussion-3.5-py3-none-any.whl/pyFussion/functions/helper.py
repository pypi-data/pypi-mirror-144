...
async def safeinstall(event):
    ...
    if plug in HELP:
        ...
        await eod(ok, f"✓ `Fussion - Installed`: `{plug}` ✓\n\n{output}")
    else:
        try:
...
def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>Fussion {ultroid_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"Fussion {ultroid_version} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
def make_html_telegraph(title, author, text):
    ...
    page = client.post(
        title=title,
        author=author,
        author_url="https://t.me/DarkFussion",
        text=text,
    )
    return page["url"]
...
async def restart(ult):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        ...
        os.execl(sys.executable, sys.executable, "-m", "pyFussion")


async def shutdown(ult, dynotype="Fussion"):
    from .. import LOGS
...
async def heroku_logs(event):
    ...
    await event.client.send_file(
        ...
        caption="**Fussion Heroku Logs.**",
    )

    os.remove("Fussion-heroku.log")
    await xx.delete()

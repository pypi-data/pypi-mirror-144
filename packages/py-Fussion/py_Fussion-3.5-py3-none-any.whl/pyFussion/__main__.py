...
if udB.get("UPDATE_ON_RESTART") and os.path.exists(".git") and updater():
    ...
    os.execl(sys.executable, "python3", "-m", "pyFussion")
...
suc_msg = """
            ----------------------------------------------------------------------
                Fussion has been deployed! Visit @DarkFussion for updates!!
            ----------------------------------------------------------------------
"""
...
if __name__ == "__main__":
    LOGS.info(
        f"Took {time_formatter((time.time() - start_time)*1000)} to start •FUSSION•"
    )
    LOGS.info(suc_msg)
    ultroid_bot.run()

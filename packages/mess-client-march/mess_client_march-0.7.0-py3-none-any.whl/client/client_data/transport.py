...
class ClientTransport(threading.Thread, QObject):
    ...
    def connection_init(self, port, ip):
        ...
        connected = False
        for i in range(5):
            logs.info(f'Попытка подключения №{i + 1}')
            try:
        ...
        if not connected:
            logs.critical('Не удалось установить соединение с сервером')
            raise ServerError('Не удалось установить соединение с сервером')

        logs.debug('Starting auth dialog.')
        ...
        logs.debug(f'Passwd hash ready: {passwd_hash_string}')
        ...
        with socket_lock:
            ...
            logs.debug(f"Presense message = {presense}")
            # Отправляем серверу приветственное сообщение.
            try:
                send_message(self.transport, presense)
                ans = get_message(self.transport)
                logs.debug(f'Server response = {ans}.')
                ...
                logs.debug(f'Connection error.', exc_info=err)
    ...
    def contacts_list_update(self):
        ...
        logs.debug(f'Запрос контакт листа для пользователся {self.name}')
        req = {
            ACTION: GET_CONTACTS,
            TIME: time.time(),
            USER: self.username
        }
        logs.debug(f'Сформирован запрос {req}')
        with socket_lock:
            send_message(self.transport, req)
            ans = get_message(self.transport)
        logs.debug(f'Получен ответ {ans}')
        if RESPONSE in ans and ans[RESPONSE] == 202:
            for contact in ans[LIST_INFO]:
                self.database.add_contact(contact)
        else:
            logs.error('Не удалось обновить список контактов.')

    def user_list_update(self):
        """Метод обновляющий с сервера список пользователей."""
        logs.debug(f'Запрос списка известных пользователей {self.username}')
        req = {
        ...
        if RESPONSE in ans and ans[RESPONSE] == 202:
            ...
            logs.error('Не удалось обновить список известных пользователей.')

    def key_request(self, user):
        """Метод запрашивающий с сервера публичный ключ пользователя."""
        logs.debug(f'Запрос публичного ключа для {user}')
        req = {
        ...
        if RESPONSE in ans and ans[RESPONSE] == 511:
            ...
            logs.error(f'Не удалось получить ключ собеседника{user}.')

    def add_contact(self, contact):
        """Метод отправляющий на сервер сведения о добавлении контакта."""
        logs.debug(f'Создание контакта {contact}')
        req = {
            ACTION: ADD_CONTACT,
            TIME: time.time(),
            USER: self.username,
    ...
    def remove_contact(self, contact):
        ...
        logs.debug(f'Удаление контакта {contact}')
        req = {
    ...
    def transport_shutdown(self):
        ...
        with socket_lock:
            try:
                ...
                pass
        logs.debug('Транспорт завершает работу.')
        time.sleep(0.5)
    ...
    def send_message(self, to, message):
        ...
        with socket_lock:
            ...
            self.process_server_ans(get_message(self.transport))
            logs.info(f'Отправлено сообщение для пользователя {to}')

    def run(self):
        """Метод содержащий основной цикл работы транспортного потока."""
        logs.debug('Запущен процесс - приёмник собщений с сервера.')
        while self.running:
            ...
            if message:
                logs.debug(f'Принято сообщение с сервера: {message}')

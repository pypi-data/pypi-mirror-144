...
class ASGIMiddleware(BaseMiddleware):
    ...
    async def __call__(self, scope, receive, send) -> None:
        path = scope.get("path")
        with Profiler(path):
            if self.is_agent_enabled() and not self.settings.ignore_request(path):
        ...
        await self.call_without_agent_async(scope, receive, send, path)
        return
    ...
    async def call_with_agent(self, context, request, send) -> None:
        ...
        try:
            ...
            await self.call_without_agent_async(
                request.scope, request.original_receive, send, path
            )
            return
    ...
    async def call_without_agent_async(self, scope, receive, send, path) -> None:

...
from contrast.api.dtm_pb2 import ApplicationUpdate, Activity
...
class LibraryReader(object):
    def __init__(self, settings, send_message_func=None):
        self.settings = settings
        # TODO: PYT-2140 remove send msg func once we deprecate SR
        ...
        if self.settings.config.is_service_bypassed:
            ...
            for library in libraries:
                if library.hash_code is not None:
                    app_update_msg.libraries[library.hash_code].CopyFrom(
                        library.to_dtm()
                    )

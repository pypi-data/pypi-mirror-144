...
class BokehBackend(BokehBackendBase):
    ...
    def process_pattern(self, ptn: Pattern) -> None:
        ...
        if isinstance(ptn.dim_shape, layout.LayoutBase):
            if isinstance(ptn.dim_shape, layout.DimLattice) and self._plot_reduced_latties:
                ...
                rect = shapes.Rect(bounding_box.width.m_as('µm'), bounding_box.height.m_as('µm'), center=bounding_box.center.vector_as(U_('µm'))) * U_('µm')

                self._bokeh_sites[-1].filled_curve(
                    Pattern(rect, ptn.mill, ptn.raster_style, **ptn.kwargs, description=ptn.description),
                    hatch_pattern='x'
                )
            else:
    ...
    def process_unknown(self, ptn: Pattern) -> None:
        if hasattr(ptn.dim_shape, 'shape') and hasattr(ptn.dim_shape.shape, 'to_arc_spline'):
            new_pattern = Pattern(

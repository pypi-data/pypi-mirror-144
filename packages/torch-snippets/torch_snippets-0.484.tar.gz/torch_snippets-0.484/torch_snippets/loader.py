...
class BB:
    ...
    def remap(self, og_dim: Tuple("h", "w"), new_dim: Tuple("H", "W")):
        h, w = og_dim
        H, W = new_dim
        sf_x = H / h
        sf_y = W / w
    ...
    def relative(self, dim: Tuple("h", "w")):
        h, w = dim
        return BB(self.x / w, self.y / h, self.X / w, self.Y / h)

    def absolute(self, dim: Tuple("h", "w")):
        h, w = dim
        return BB(self.x * w, self.y * h, self.X * w, self.Y * h)
...
def resize_old(im: np.ndarray, sz: Union[float, Tuple("H", "W")]):
    h, w = im.shape[:2]
...
def resize(im: np.ndarray, sz: Union[float, Tuple("H", "W"), Tuple(str, ("H", "W"))]):

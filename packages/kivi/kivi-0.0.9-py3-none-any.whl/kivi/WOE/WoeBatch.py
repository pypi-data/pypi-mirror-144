...
def WOEBatchWithRebin(
        ...
        max_bin=5, min_bin=2, columns_name=None, drop_columns=['uuid', 'target'],):
    """
    ...
    df_woe, fault_col = WOEBatch(df=df, columns_name=columns_name, target_name=target_name, WOEFun=WOEFun, bins=bins)
    ...
    while len(columns_rebin) > 0 and bins >= min_bin:
        df_woe_new, fault_col_new = WOEBatch(
            df=df, columns_name=columns_rebin, target_name=target_name, WOEFun=WOEFun, bins=bins)

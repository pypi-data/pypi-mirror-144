...
from janusmskit.constants import CRYPT_BASE, LOGGER_NAME
from janusmskit.utils import import_from
...
class CryptResource(ConfigResource):
    """This class works between `janusmskit.flask.create_app.Microservice` and `janusmskit.flask.services.[THESERVICE]`. Search
    ...
    def get_crypt(self, *args, **kwargs) -> CryptAbstract:
        if self.config.method == "fernet":
            crypt_object = import_from("janusmskit.crypt.fernet", CRYPT_RESOURCES_CLASS)
        elif self.config.method == "aws_kms":
            crypt_object = import_from("janusmskit.cloud.aws.kms", CRYPT_RESOURCES_CLASS)
        else:

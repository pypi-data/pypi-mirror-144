...
def getRecordByIdFirstbank(table, connector, id):
    selectRecordQuery = "SELECT {0},{1},{2},{3},{4} FROM {5} WHERE {6}={7}".format(
        TABLE_3FIELD_NAME,
        TABLE_6FIELD_NAME,
        TABLE_7FIELD_NAME,
        TABLE_10FIELD_NAME,
        ...
        TABLE_1FIELD_NAME,
        str(id))
    return __getRecordByIdSTR__(connector,selectRecordQuery)
...
def getCursorIDDataByConnector(parameters, connector):
...
def getListId(parameters):
    ...
    with openConnection(parameters) as connector:
        with getCursorIDDataByConnector(parameters, connector) as cursor:
            for (id_RUN) in cursor:

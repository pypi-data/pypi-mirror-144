...
from tests.utils.assets.bst import MulticlassIrisLGBM,MulticlassIrisXGB
...
def test_02_get_facts_xgb(caplog):
    caplog.set_level(logging.INFO)
    MulticlassIrisXGB(cls.facts_client)
    assert 'Successfully logged results to Factsheet service' in caplog.text


def test_03_get_facts_lgbm(caplog):
    MulticlassIrisLGBM(cls.facts_client)
    assert 'Successfully logged results to Factsheet service' in caplog.text

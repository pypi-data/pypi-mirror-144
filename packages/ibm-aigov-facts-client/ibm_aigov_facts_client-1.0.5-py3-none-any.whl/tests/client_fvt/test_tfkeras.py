...
from tests.utils.assets.tfkeras import MulticlassIrisKeras,MulticlassIrisTF
...
def test_02_get_facts_tf(caplog):
    caplog.set_level(logging.INFO)
    MulticlassIrisTF(cls.facts_client)
    assert 'Successfully logged results to Factsheet service' in caplog.text


def test_03_get_facts_keras(caplog):
    MulticlassIrisKeras(cls.facts_client)
    assert 'Successfully logged results to Factsheet service' in caplog.text

...
class Release:
    ...
    def update_tags(self, new_tags=None):
        new_tags = new_tags or self.equal_tags
        self.registry_client. \
            update_image_tags(self.name, self.version, new_tags)
        return True

    def pull(self):
        return self.registry_client.pull_image(self.name, self.version)

    def push(self, validate=True, checks=None):
        checks = checks or {}
        validate and self.validate_push(check_is_next=
                                        'validate_is_next' in checks,
                                        check_new_hash=
                                        'validate_new+hash' in checks)
    ...
    def rollback(self, prev_release=None,
        ...
        if prev_release:
            ...
            if tags_to_move:
                to_move.extend([tag for tag in tags_to_move if tag not in prev_release.equal_tags])
                to_move = list(set(to_move))
                prev_release.update_tags(to_move)
        if tags_to_move:
            to_move.extend(tags_to_move)
        self.delete(with_equal_tags=True, excl_tags=to_move)
    ...
    def sync_with_remote(self, images=None, version=False):
        if version:
            self.version = self.get_versions(last=True)
        self.equal_tags, self.hash = self.get_equal_tags_from_remote(with_hash=True, images=images)
        return self
    ...
    def _check_is_different_release(self):
        self.registry_client.tag_image(self.name, self.version, local_name=self.local_name)
        ...
        if result:
            raise ValueError(
                f'Already {len(list(result.values())[0])} tags: {list(result.values())[0]} with the same hash: {digset} has been deployed')
        return True
    ...
    def next(self, incr_version=None, remote_sync=False):
        if remote_sync:
            version = self.get_versions(last=True)
        else:
            version = self.version_sem()
        position = incr_version or self.incr_version
        self.version = str(getattr(version, f'next_{position}')())
        return self
    ...
    def from_remote(cls, name, client: AwsRegistry,
        ...
        newest = images[0]['tag']
        obj = cls(client, name, newest, local_name=local_name)
        obj.sync_with_remote()
        return obj

...
def cli():
    ...
    ** how to use **
    ...
    dsf-cli create-cluster

    dsf-cli stop-cluster

    dsf-cli start-cluster

    dsf-cli delete-cluster
...
def create_deploy_files():
    """Create deploy files (created through 'Create project', but we can use it as standalone function """
    copy_deploy_files('')


@cli.command()
def create_cloud_eval_files():
    """Create cloud evaluation files (created through 'Create project', but we can use it as standalone function """
    currentPipelineFolder = os.path.abspath(os.getcwd())
...
def create_tester_files():
    """!Create tester files (created through 'Create project', but there is an option
       We can use it as standalone function """
    create_tester_env()
...
def create_cluster():
    ...
    with open(current_folder + '/batch_files/batch_config.json') as config_file:
        ...
        batch_helper = ZIDSBatchHelper(config_dict)
...
def upload_batch_files(file_type: str):
    """!Upload all files needed for a batch project to the predefined bucket
        ARGS:
            file_type: String containing the types of files to upload (jars, stages, whl or "all")
    """
    batch_helper = config_batch_helper()
    batch_helper.upload_files_to_bucket(file_type)


def config_batch_helper() -> ZIDSBatchHelper:
            ...
            with open(os.path.join(source_dir, file_path), 'r', encoding="utf8") as file:
                try:
                    ...
                    if file_path.rsplit('.', 1)[-1] == 'jar':
                        shutil.copy(os.path.join(source_dir, file_path), f'{main_dir}/batch_files/{dir_path}/')
                    else:

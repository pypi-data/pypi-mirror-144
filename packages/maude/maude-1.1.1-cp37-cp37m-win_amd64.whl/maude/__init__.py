...
class Term(object):
    ...
    def vu_narrow(self, type, target, depth=-1, fold=False, filter=False, delay=False):
        r"""
        ...
        :type filter: boolean, optional
        :param filter: Whether to activate filtered variant unification (``filter`` option in the command).
        :type delay: boolean, optional
        :param delay: Whether variant unifiers are filtered before using the first one for narrowing (``delay`` option in the command).

        :rtype: :py:class:`NarrowingSequenceSearch3`
        :return: An object to iterate through solutions.
        """
        return _maude.Term_vu_narrow(self, type, target, depth, fold, filter, delay)

...
async def test_get_state(request, test_dev_controller):
    req = IDACRequestAsync(
        uuid=request.config.cache.get("created_uuid_nop-v1", None), controller=test_dev_controller
    )
...
async def test_cleanup_hello_world(request, test_dev_controller):
    ...
    await req.wait_for_status(wanted_state=["executed"], timeout=30, interval=5)

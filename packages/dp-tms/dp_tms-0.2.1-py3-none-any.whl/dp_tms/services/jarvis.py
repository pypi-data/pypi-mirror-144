...
def slice_list_in_to_buckets(
    items: List[Any], max_buckets: int = None, chunk_size: int = None
) -> List[List[Any]]:
...
def batch_and_fetch_vectors(
    records: List[Dict[str, str]],
    compiled_documents: List[Dict[str, str]],
    embedding_url: str,
    api_key: str,
    **kwargs,
):
    ...
    final_results = []
    final_errors = []
    chunk_index = slice_list_in_to_buckets(
        items=range(len(compiled_documents)), chunk_size=chunk_size
    )
    for chunk in chunk_index:
        ...
        results, errors = fetch_document_vectors_from_api(
            documents=compiled_documents_i, embedding_url=embedding_url, api_key=api_key
        )

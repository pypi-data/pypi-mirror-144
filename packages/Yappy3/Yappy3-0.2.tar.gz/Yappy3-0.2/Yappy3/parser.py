...
from types import *
import re, string
import sys, string, copy, time, operator

import os, os.path

import shelve, dbm, dbm
    ...
    def resolve_shift_reduce(self,i,a,s,r):
        ...
        try:
            if self.operators and a in self.operators and r in self.precedence and  self.precedence[r]:
                ...
                if (self.precedence[r][0] > prec_op) or (self.precedence[r][0] == prec_op and    self.precedence[r][1] =='left'):
...
class LR1table(LRtable):
    ...
    def items(self):
        ...
        """
        c = osets.Set([ self.closure(osets.Set([(len(self.gr.rules) - 1,0,self.gr.endmark)]))])
        symbols = self.gr.terminals + self.gr.nonterminals
        e = 1
        while e:
            e = 0
    ...
    def print_items(self,c):
        ...
        for i in range(len(c)):
                ...
                for item in list(c[i].keys()):
                    ...
                    rhs = self.gr.rules[r][1]
                    s = s + "\t %s -> %s . %s, %s \n" %(lhs,
        " ".join(rhs[:p]), " ".join(rhs[p:]),c[i][item])
        print(s)
        return s
...
class LRparser:
    ...
    def __init__(self,grammar,table_shelve,no_table=1,tabletype=LALRtable,operators=None,noconflicts=1,expect=0,**args):
        """
        @param grammar: is a list for productions;
        each production is a tuple C{(LeftHandside,RightHandside,SemFunc,Prec?)}
        with C{LeftHandside} nonterminal, C{RightHandside} list of symbols,
        ...
        if not(db==None or db=="" or no_table==0):
            try:
                ...
                else: raise  TableError(table_shelve)
        else:
            ...
            for i in osets.Set(l):
                ...
                for a in  self.terminals:
                    if (i,a) in self.ACTION:
                        if self.ACTION[i,a][0]=="shift": x="s"
                        else: x="r"
                        ...
                        for rest in rhss:
                            ...
                            if rhss == "[]":
                                ...
                                if not m:
                                    ...
                                    else: rhs =  str.split(rest[0:m.start()].strip())
                                    if  m.group('opsem'):
                                        opsem = str.split(m.group('opsem'),sym['opsym'])
                                        if len(opsem) == 1:
...
class Yappy(LRparser):
    ...
    def __init__(self,tokenize,grammar, table='YappyTab',no_table=1, 
        ...
        if 'usrdir' in args and  os.path.isdir(args['usrdir']):
                table =  str.rstrip(args['usrdir']) + '/' + table
    ...
    def inputfile(self,FileName,context={}):
        ...
        except IOError: raise YappyError()
...
def expandSemRule(strargs,strfun):
    ...
    for i in [(x.group(0),strargs+x.group(1)+"]") for x in matchargs]:
        strfun = str.replace(strfun,i[0],i[1])
    return strfun    
      ...
      ("OPSEM -> IDS",self.OPSEMRule1),
        ...
        tokenize = [
            ...
            #        ("{{",lambda x: ("semsym",x)),
                    ...
                    (".*",lambda x: ("ID",x))]
        if 'tmpdir' in args:
                args1 = {'usrdir':str.rstrip(args['tmpdir'],'/')}
        else:
            args1 = {}        
...
class Stack:
    ...
    #Funcion añadida por Álvaro Rodríguez para solucionar función cmp
    def compare(a, b):
        return (a > b) - (a < b)

    def __cmp__(self, other):
        return self.compare(self.stack, other.stack) #Uso de compare en vez de cmp

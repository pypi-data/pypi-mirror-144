...
class EksNodeGroup(AwsResource):
    ...
    def _create(self, aws_client: AwsApiClient) -> bool:
        ...
        try:
            ...
            try:
                print_info(f"Creating IamRole: {nodegroup_iam_role.name}")
            ...
            if nodegroup_iam_role_arn is None:
                print_error(
                    f"ARN for IamRole {nodegroup_iam_role_arn} is not available, please try again"
                )
                return False
            ...
            not_null_create_nodegroup_args = {}
            if scaling_config is not None:
                not_null_create_nodegroup_args["scalingConfig"] = scaling_config
            if self.disk_size is not None:
                not_null_create_nodegroup_args["diskSize"] = self.disk_size
            if self.instance_types is not None:
                not_null_create_nodegroup_args["instanceTypes"] = self.instance_types
            if self.ami_type is not None:
                not_null_create_nodegroup_args["amiType"] = self.ami_type
            if self.remote_access is not None:
                not_null_create_nodegroup_args["remoteAccess"] = self.remote_access
            if self.labels is not None:
                not_null_create_nodegroup_args["labels"] = self.labels
            if self.taints is not None:
                not_null_create_nodegroup_args["taints"] = self.taints
            if self.tags is not None:
                not_null_create_nodegroup_args["tags"] = self.tags
            if self.client_request_token is not None:
                not_null_create_nodegroup_args[
                    "clientRequestToken"
                ] = self.client_request_token
            if self.launch_template is not None:
                not_null_create_nodegroup_args["launchTemplate"] = self.launch_template
            if self.update_config is not None:
                not_null_create_nodegroup_args["updateConfig"] = self.update_config
            if self.capacity_type is not None:
                not_null_create_nodegroup_args["capacityType"] = self.capacity_type
            if self.version is not None:
                not_null_create_nodegroup_args["version"] = self.version
            if self.release_version is not None:
                not_null_create_nodegroup_args["release_version"] = self.release_version
            ...
            try:
                ...
                print_info(f"Creating EksNodeGroup: {self.name}")
                create_ng_response = service_client.create_nodegroup(
                ...
                # logger.debug(f"create_ng_response: {create_ng_response}")
                # logger.debug(f"create_ng_response type: {type(create_ng_response)}")

                ## Validate EksNodeGroup creation
                nodegroup_creation_time = create_ng_response.get("nodegroup", {}).get(
                    "createdAt", None
                )
                nodegroup_status = create_ng_response.get("nodegroup", {}).get(
                    "status", None
                )
                ...
                if nodegroup_creation_time is not None:
                    ...
                    self.active_resource = create_ng_response
                    self.active_resource_class = create_ng_response.__class__
                    return True
            except Exception as e:
                print_error(
                    "EksNodeGroup could not be created, this operation is known to be buggy."
                )
    ...
    def post_create(self, aws_client: AwsApiClient) -> bool:
        ...
        if self.wait_for_completion:
            try:
                print_info(
                    "Waiting for EksNodeGroup to be created, this can take upto 5 minutes"
                )
                ...
                print_error(
                    "Received errors while waiting for EksNodeGroup creation, this operation is known to be buggy."
                )
    ...
    def _read(self, aws_client: AwsApiClient) -> Optional[Any]:
        ...
        try:
            service_client = self.get_service_client(aws_client)
            describe_ng_response = service_client.describe_nodegroup(
                clusterName=self.eks_cluster.name,
                nodegroupName=self.name,
            )
            # logger.debug(f"describe_ng_response: {describe_ng_response}")
            # logger.debug(f"describe_ng_response type: {type(describe_ng_response)}")

            nodegroup_creation_time = describe_ng_response.get("nodegroup", {}).get(
                "createdAt", None
            )
            nodegroup_status = describe_ng_response.get("nodegroup", {}).get(
                "status", None
            )
            logger.debug(f"NodeGroup creation_time: {nodegroup_creation_time}")
            logger.debug(f"NodeGroup status: {nodegroup_status}")
            if nodegroup_creation_time is not None:
                ...
                self.active_resource = describe_ng_response
                self.active_resource_class = describe_ng_response.__class__
    ...
    def _delete(self, aws_client: AwsApiClient) -> bool:
        ...
        try:
            ...
            delete_ng_response = service_client.delete_nodegroup(
                clusterName=self.eks_cluster.name,
                nodegroupName=self.name,
            )
            # logger.debug(f"delete_ng_response: {delete_ng_response}")
            # logger.debug(
            #     f"delete_ng_response type: {type(delete_ng_response)}"
            # )
            print_info(f"EksNodeGroup deleted: {self.name}")
            return True
        except Exception as e:
            print_error(e)
        return False
    ...
    def post_delete(self, aws_client: AwsApiClient) -> bool:
        ...
        if self.wait_for_completion:
            try:
                print_info(
                    "Waiting for EksNodeGroup to be deleted, this can take upto 5 minutes"
                )
                waiter = self.get_service_client(aws_client).get_waiter(
                    "nodegroup_deleted"
                )
                ...
                print_error(
                    "Received errors while waiting for EksNodeGroup deletion, this operation is known to be buggy."
                )

...
class PostgresDbArgs(DbAppArgs):
    ...
    # Configure postgres
    # Provide POSTGRES_USER as db_user or POSTGRES_USER in env_file
    db_user: Optional[str] = None
    # Provide POSTGRES_PASSWORD as db_password or POSTGRES_PASSWORD in secrets_file
    db_password: Optional[str] = None
    # Provide POSTGRES_DB as db_schema or POSTGRES_DB in env_file
    ...
    ebs_volume_zone: Optional[str] = None
...
class PostgresDb(DbApp):
    def __init__(
        ...
        try:
            self.args: PostgresDbArgs = PostgresDbArgs(
                ...
                ebs_volume_zone=ebs_volume_zone,
    ...
    def get_db_user(self) -> Optional[str]:
        ...
        if db_user_var is None and self.args.env_file is not None:
            # read from env_file
            logger.debug(f"Reading db_user from env_file")
            env_data_from_file = self.get_env_data_from_file()
            db_user_var = env_data_from_file.get("POSTGRES_USER", db_user_var)
        return db_user_var
    ...
    def get_db_password(self) -> Optional[str]:
        ...
        if db_password_var is None and self.args.secrets_file is not None:
            ...
            logger.debug(f"Reading db_password from secrets_file")
            secret_data_from_file = self.get_secret_data_from_file()
            if secret_data_from_file is not None:
                encrypted_password = secret_data_from_file.get(
                    "POSTGRES_PASSWORD", db_password_var
                )
                if encrypted_password is not None:
                    import base64

                    # logger.debug(f"encrypted_password type: {type(encrypted_password)}")
                    db_password_var = (
                        base64.b64decode(encrypted_password)
                        .decode("ascii")
                        .replace("\n", "")
                    )
                    # logger.debug(f"db_password_var: {db_password_var}")
    ...
    def get_db_schema(self) -> Optional[str]:
        ...
        if db_schema_var is None and self.args.env_file is not None:
            # read from env_file
            logger.debug(f"Reading db_schema from env_file")
            env_data_from_file = self.get_env_data_from_file()
            db_schema_var = env_data_from_file.get("POSTGRES_DB", db_schema_var)
    ...
    def get_db_port_local(self) -> Optional[str]:
    ...
    def get_k8s_rg(
        ...
        if self.args.create_volume:
            ...
            if self.args.volume_type == PostgresVolumeType.EMPTY_DIR:
                ...
                if self.args.ebs_volume is not None:
                    ...
                    if pod_node_selector is None:
                        pod_node_selector = {}
                    # so these pods are assigned to the same region and zone as the ebs_volume
                    # https://kubernetes.io/docs/reference/labels-annotations-taints/#topologykubernetesiozone
                    if self.args.ebs_volume_region is not None:
                        pod_node_selector[
                            "topology.kubernetes.io/region"
                        ] = self.args.ebs_volume_region
                    if self.args.ebs_volume_zone is not None:
                        pod_node_selector[
                            "topology.kubernetes.io/zone"
                        ] = self.args.ebs_volume_zone
                else:
        ...
        k8s_container = CreateContainer(
            ...
            envs_from_secret=[container_env_secret.secret_name] if container_env_secret else None,

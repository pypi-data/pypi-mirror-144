...
class DataboxArgs(PhidataAppArgs):
    ...
    workspace_volume_name: str = "databox-workspace-volume"
    ...
    # eg: if ws name is: idata, workspace path would be: /usr/local/databox/idata
    workspace_parent_container_path: str = "/usr/local/databox"
    ...
    aws_config_volume_name: str = "databox-aws-config-volume"
    ...
    # Configure airflow
    # If init_airflow = True, this databox initializes airflow
...
class Databox(PhidataApp):
    ...
    def get_k8s_rg(
        ...
        if self.args.mount_aws_config:
            logger.debug(f"Mounting EmptyDir")
            logger.debug(f"\tto: {self.args.aws_config_container_path}")
            aws_config_volume = CreateVolume(
                volume_name=self.args.aws_config_volume_name,
                app_name=self.args.name,
                mount_path=self.args.aws_config_container_path,
                volume_type=VolumeType.EMPTY_DIR,
            )
            container_volumes.append(aws_config_volume)

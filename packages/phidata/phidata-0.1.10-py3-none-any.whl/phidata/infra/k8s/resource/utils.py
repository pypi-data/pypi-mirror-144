...
from phidata.infra.k8s.resource.types import (
...
def get_k8s_resources_from_group(
    ...
    List[K8sResourceType] attributes. Eg: it will flatten pvc, cm, secret and
    ...
    # Now we populate the k8s_resources list with the resources
    # we also flatten any list resources
    # This will loop over each key of the K8sResourceGroup model
    # resource = key
    # resource_data = value in the current K8sResourceGroup object
    for resource, resource_data in k8s_resource_group.__dict__.items():
        ...
        # When we check for isinstance(resource_data, K8sResource)
        # we filter out all keys which are not a subclass of the resource_data
        # thereby leaving out only K8sResource types

        # Check if the resource is a single K8sResourceType or a List[K8sResourceType]
        # If it is a List[K8sResourceType], flatten the resources, verify each element
        ...
        if isinstance(resource_data, list):
            for _r in resource_data:
                if isinstance(_r, K8sResource):
                    # If type_filter are provided then skip over K8sResources
        ...
        # K8sResource and add it to the k8s_resources list
            ...
            # If type_filter are provided then skip if K8sResource
...
def dedup_k8s_resources(
    k8s_resources_with_weight: List[Tuple[K8sResourceType, int]],
) -> List[Tuple[K8sResourceType, int]]:
    if k8s_resources_with_weight is None:
        raise ValueError

    deduped_resources: List[Tuple[K8sResourceType, int]] = []
    prev_rsrc: Optional[K8sResourceType] = None
...
def filter_and_flatten_k8s_resource_groups(
    ...
    k8s_resource_list_with_weight: List[Tuple[K8sResourceType, int]] = []
    ...
    deduped_k8s_resources: List[Tuple[K8sResourceType, int]] = dedup_k8s_resources(

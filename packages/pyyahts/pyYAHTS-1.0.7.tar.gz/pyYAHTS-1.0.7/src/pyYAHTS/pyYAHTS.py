...
class GetJson():
    ...
    def pdf_file(self, parsed_json):
        ...
        pdf = PDFDocument(f)
        pdf.init_report()
        pdf.h1(f"{self.hostname} {self.command}")
        pdf.p(print_json(parsed_json))
        pdf.generate()
...
@click.option('--filetype', prompt='Filetype', type=click.Choice(['json','yaml','html','datatable','pdf'], case_sensitive=True), help='Filetype to output captured network state to', required=False)

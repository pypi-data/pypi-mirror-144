...
class SafeReceiver:
    ...
    def __init__(self, _name, _debugging_option):
        ...
        packet_catcher_thread = threading.Thread(target=self.packet_catcher, name="PacketCatcher", args=[0], daemon=True)
        packet_catcher_thread.start()

        # Packet Queue Dispatcher Thread Initialization

        packet_queue_dispatcher_thread = threading.Thread(target=self.packet_queue_dispatcher, name="QueueDispatcher", args=[0], daemon=True)

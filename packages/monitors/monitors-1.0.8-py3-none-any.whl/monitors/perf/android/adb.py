...
from monitors.perf.android.constant import (DEFAULT_ADB_PATH, IP_PATTERN, SDK_VERISON_ANDROID7)
from monitors.perf.utils import (AdbError, AdbShellError, MonitorsError, DeviceConnectionError)
from monitors.perf.utils import decode_path, raisefrom, proc_communicate_timeout, SUBPROCESS_FLAG
from monitors.perf.utils import NonBlockingStreamReader
from monitors.perf.utils import retries
from monitors.perf.utils import get_std_encoding, reg_cleanup, split_cmd, make_file_executable
...
class ADB(object):
    ...
    def swipe(self, tuple_x0y0, tuple_x1y1, duration=500):
        ...
        Raises:
            MonitorsError: if SDK version is not supported
        ...
        if version <= 15:
            raise MonitorsError('swipe: API <= 15 not supported (version=%d)' % version)
        elif version <= 17:
    ...
    def get_top_activity(self):
        ...
        Raises:
            MonitorsError: if top activity cannot be obtained
        ...
        if m:
            ...
            raise MonitorsError("Can not get top activity, output:%s" % dat)
    ...
    def is_screenon(self):
        ...
        Raises:
            MonitorsError: if screen state can't be detected
        ...
        raise MonitorsError("Couldn't determine screen ON state")
    ...
    def is_locked(self):
        ...
        Raises:
            MonitorsError: if lock screen can't be detected
        ...
        if not m:
            raise MonitorsError("Couldn't determine screen lock state")
        return (m.group(1) == 'true')
    ...
    def path_app(self, package):
        ...
        Raises:
            ...
            MonitorsError: if package is not found on the device
        ...
        if 'package:' not in output:
            raise MonitorsError('package not found, output:[%s]' % output)
    ...
    def check_app(self, package):
        ...
        Raises:
            MonitorsError: if package is not found
        ...
        if match is None:
            raise MonitorsError('package "{}" not found'.format(package))
        return True
    ...
    def start_app_timing(self, package, activity):
        ...
        if not re.search(r"Status:\s*ok", out):
            raise MonitorsError("Starting App: %s/%s Failed!" % (package, activity))

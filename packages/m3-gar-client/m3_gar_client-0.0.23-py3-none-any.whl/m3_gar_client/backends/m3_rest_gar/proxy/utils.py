    ...
    def load_raw(self, page=None):
        ...
        for index, part in enumerate(filter_parts, start=1):
            ...
            if index == filter_count:
                ...
                # Переопределим родителей, чтобы не плодить их для каждого параметра фильтрации
                _parent_ids = deepcopy(parent_ids) or set([None])
                parent_ids = set()
                ...
                for _parent_id in _parent_ids:
                    ...
                    for page_number in count(start=1):
                        #  Не ограничиваем поиск количеством страниц, чтобы не потерять результаты
                        drf_data = self._load_page(params, page_number)

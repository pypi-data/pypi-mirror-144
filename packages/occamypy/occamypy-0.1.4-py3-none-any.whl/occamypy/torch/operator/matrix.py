...
class Matrix(Operator):
    ...
    def __init__(self, matrix, domain, range):
        ...
        if not isinstance(matrix, torch.Tensor):
            raise ValueError("matrix has to be a torch Tensor")
        self.M = matrix
        if not matrix.device == domain.device == range.device:
            raise ValueError("All the elements have to be in the same device")

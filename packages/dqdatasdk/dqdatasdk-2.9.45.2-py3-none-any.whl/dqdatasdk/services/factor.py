...
from dqdatasdk.services.calendar import (
...
def get_factor_return(
    ...
    df = get_client().execute(
        "get_factor_return_v2", start_date, end_date, factors, universe, method, market=market, industry_mapping=industry_mapping
    )
    if not df:
        return None
    df = pd.DataFrame(df)
...
@export_as_api
def get_factor_exposure(order_book_ids, start_date=None, end_date=None, factors=None, industry_mapping='sw2021', market="cn"):
    """获取因子暴露度
    ...
    results = get_client().execute(
        "get_factor_exposure_v2", order_book_ids, start_date, end_date, factors, industry_name, market
    )

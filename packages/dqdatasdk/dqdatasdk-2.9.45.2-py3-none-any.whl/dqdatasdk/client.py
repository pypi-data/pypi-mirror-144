...
import dqdatasdk
from dqdatasdk.dqmodules.dqcertificate import get_dqdata_license

URI_PATTERN = r"^(?P<schema>tcp|dqdatasdk?)://(?P<username>\S+?):(?P<password>\S+)@(?P<hostname>\S+):(?P<port>\d+$)"
...
class DummyClient:
    ...
    def execute(self, *args, **kwargs):
        raise RuntimeError("dqdatasdk is not initialized")

    def reset(self):
        pass

    def info(self):
        print('dqdatasdk is not initialized')
...
def reset(flush_cache=False):
    ...
    if flush_cache:
        from dqdatasdk.decorators import ttl_cache_clear
        ttl_cache_clear()


def initialized():
    """
    is dqdatasdk already initialized ?
...
def _set_sock_factory(proxy_info):
    from dqdatasdk.connection import Connection
...
def init(username=None, password=None, addr=("rqdatad-pro.ricequant.com", 16011), *_, **kwargs):
    ...
    dqdatasdk_CONF / dqdatasdk2_CONF: When init called with no argument, the value in dqdatasdk2_CONF is used as uri, then dqdatasdk_CONF.
    dqdatasdk_PROXY: proxy info, e.g. http://username:password@host:port
    ...
    '''
    这利用来获取点宽的登录信息，获取点宽登录信息后使用license连接
    '''
###############################################################################################
    license = get_dqdata_license(username=username, password=password)
    username = 'license'
    password = license
###############################################################################################
    extra_args = {k: kwargs.pop(k) for k in ('timeout', 'connect_timeout') if k in kwargs}
    proxy_info = kwargs.pop('proxy_info', None) or _parse_proxy_url(os.environ.get('dqdatasdk_PROXY'))
    ...
    if not (username or password or uri):
        uri = os.environ.get("dqdatasdk2_CONF") or os.environ.get("dqdatasdk_CONF")
    ...
    if _CLIENT is not _DUMMY:
        warnings.warn("dqdatasdk is already inited. Settings will be changed.", stacklevel=0)
        reset()
    ...
    auth_info = {'username': username, 'password': password, 'ver': dqdatasdk.__version__}
    ...
    def show_info():
        print('dqdatasdk version:', dqdatasdk.__version__)
    ...
    if username == "license":
        ...
        if is_trial or 0 <= remaining_days <= 14:
            warnings.warn("Your account will be expired after  {} days. "
                          "Please call us at 0755-8695-2080 to upgrade or purchase or "

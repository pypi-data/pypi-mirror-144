...
import dqdatasdk
from dqdatasdk.share.errors import OverwriteWarning
...
def export_as_api(f=None, name=None, namespace=None, priority=0):
    ...
    if namespace:
        if hasattr(dqdatasdk, namespace):
            namespace = getattr(dqdatasdk, namespace)
        else:
            namespace_name = namespace
            namespace = types.ModuleType(namespace_name)
            namespace.__file__ = 'dqdatasdk plugin'
            namespace.__module__ = "dqdatasdk"
            setattr(dqdatasdk, namespace_name, namespace)
            dqdatasdk.__all__.append(namespace_name)
    else:
        namespace = dqdatasdk
        dqdatasdk.__all__.append(name)

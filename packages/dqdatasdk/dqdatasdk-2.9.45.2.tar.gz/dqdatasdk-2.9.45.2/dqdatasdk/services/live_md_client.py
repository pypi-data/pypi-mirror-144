...
from dqdatasdk.services.basic import instruments, get_trading_hours
from dqdatasdk.services.live import current_minute
from dqdatasdk.validators import ensure_list_of_string
from dqdatasdk.decorators import export_as_api, retry
from dqdatasdk.utils import datetime_to_int14
from dqdatasdk.client import get_client
from dqdatasdk.share.errors import PermissionDenied
...
def get_trading_minutes(order_book_id, trading_date):
    ...
    for start, end in get_trading_hours(order_book_id, trading_date, expected_fmt='datetime'):
        dt = start
        while dt <= end:
            result.append(dt)
            dt += delta
...
class MinbarResampler:
    def __init__(self, order_book_id, freq):
        ...
        self._bar_dts = None
        self._trading_minutes = None
        self.channel = 'bar_' + order_book_id + '_' + str(freq) + 'm'

    def get_bar_from_dqdatasdk(self, dt):
        try:
    ...
    def enqueue(self, bar):
        ...
        if dt in self._bar_dts:
            if self._current_bar is None:
                self._current_bar = {}
                bar = self.get_bar_from_dqdatasdk(dt)
                if bar is not None:

...
def _make_error_logging_route_handler_class(*, msg_prefix="", logger_: Logger = logger) -> t.Type[APIRoute]:
    """
            ...
            async def custom_route_handler(request: Request) -> Response:
                try:
                    ...
                    if error_to_report is not None:
                        # Log the error.
                        logger_.exception(error_to_report)
...
def log_errors(app: FastAPI, msg_prefix="", logger_: Logger = logger):
    """
    ...
    logger_ : Logger, optional
        Custom :class:`loguru.Logger` logger to call when generating log lines. Defaults to the default loguru logger.
    """
    app.router.route_class = _make_error_logging_route_handler_class(msg_prefix=msg_prefix, logger_=logger_)

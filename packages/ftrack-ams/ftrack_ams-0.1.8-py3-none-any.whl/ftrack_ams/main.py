...
from ftrack_ams.functions import clearConsole, select_artist
...
def create_new_project(session):
    ...
    internal_project_handle = session.query('Project where name is "Team_Intern"').one()

    for c in internal_project_handle["children"]:
        if c["name"] == "Assets":
            for a in c["children"]:
                if a["name"] == "VFB-LOGO":
                    vfb_thumb = a["thumbnail_id"]
                if a["name"] == "AMSVFB-LOGO":
                    amsvfb_thumb = a["thumbnail_id"]
                if a["name"] == "AMS-LOGO":
                    ams_thumb = a["thumbnail_id"]
    ...
    if internal_project:
        departement_choice, departement_index = pick(["AMS", "VFB"],
    ...
    if num_ext > 0:
        ext_folder = session.create("Folder", {
                                    "name": f"{num}_EXT",
                                    "parent": proj,
                                    "thumbnail_id": project_thumb}
                                    )
    ...
    print(f"-- {num_int} INT tasks for {int_artist['username']}") if num_int > 0 else None
    print(f"-- {num_ext} EXT tasks for {ext_artist['username']}") if num_ext > 0 else None

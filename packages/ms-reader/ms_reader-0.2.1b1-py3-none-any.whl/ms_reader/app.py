...
@st.cache
def read_data(data_in, report_in):
    data_df = pd.read_excel(data_in, engine="openpyxl")
    if report_in:
        report_df = pd.read_excel(report_in, engine="openpyxl")
    else:
        report_df = None
    return data_df, report_df


st.title("Welcome to the MS_Reader App")
...
if data:
    data, report = read_data(data, report)
    ...
    with st.form("Table select"):
        ...
        if clicked:
            ...
            st.session_state.destination = Path(st.text_input(
                "Selected output folder:", filedialog.askdirectory(master=root)
            ))
            if st.session_state.destination == Path(".") or not st.session_state.destination.exists():
                raise RuntimeError("Please input a valid file path")
            destination = copy(st.session_state.destination)

        elif hasattr(st.session_state, "destination"):

            destination = Path(st.text_input(
                "Selected output folder:", st.session_state.destination
            ))

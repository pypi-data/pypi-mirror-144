...
def get_liveAnimal_lookup_values(product: dict, country_id: str, year: int = None):
    ...
    for animal_product in animal_products:
        grouping = LOOKUP_GROUPING.get(TermTermType.ANIMALPRODUCT.value, lambda *_: None)(MODEL, {
            '@id': animal_product
        })
...
def _run_by_liveAnimal(product: dict, country_id: str, year: int = None):
    ...
    debugValues(model=MODEL, term=term_id, by='liveAnimal',
...
def _run_by_country(product: dict, country_id: str, year: int = None):
    ...
    debugValues(model=MODEL, term=term_id, by='country',
                grouping=grouping)
...
def run(cycle: dict):
    ...
    return ((
        non_empty_list(map(
            ...
            list(filter(_should_run_product_by_country, cycle.get('products', [])))
        )) + non_empty_list(map(
            lambda p: _run_by_liveAnimal(p, country_id, year) if _should_run_product_by_liveAnimal(p) else None,
            list(filter(_should_run_product_by_liveAnimal, cycle.get('products', [])))
        ))

...
def generate_reports(
    ...
    for package in benchmark_df["package"].unique():
        ...
        for operation in package_df["operation"].unique():
            ...
            operation_descr = package_operation_df[
                    package_operation_df["run_datetime"] == package_operation_df["run_datetime"].max()
                    ]["operation_descr"].item()
            package_operation_df = package_operation_df[["package_version", "run_details", "secs_taken"]]
            package_operation_df["run_details"] = package_operation_df["run_details"].apply(lambda x: format_run_details(x))
            package_operation_df = package_operation_df.set_index(["package_version", "run_details"])
    ...
    benchmark_maxversions_df = (benchmark_df[["package", "operation", "package_version"]]

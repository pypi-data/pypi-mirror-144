...
import signal
from typing import (
    Any, FrozenSet, List, Literal, Mapping,
...
class KubernetesKernelCreationContext(AbstractKernelCreationContext[KubernetesKernel]):
    ...
    def get_runner_mount(
        ...
        return Mount(
                MountTypes.K8S_GENERIC, Path(src), Path(target),
    ...
    async def mount_vfolders(
        ...
        for i, vfolder in zip(range(len(vfolders)), vfolders):
            if len(vfolder) == 5:
            ...
            if kernel_path_raw := vfolder_mount_map.get(folder_name):
                if not kernel_path_raw.startswith('/'):  # type: ignore
                    raise ValueError(
                        f'Error while mounting {folder_name} to {kernel_path_raw}: '
                        'all vfolder mounts should be absolute path')
                kernel_path = Path(kernel_path_raw)  # type: ignore
            else:
                kernel_path = Path(f'/{folder_name}')
            ...
            mount = Mount(
                ...
                folder_perm,
                opts={
                    'name': f'kernel-{self.kernel_id}-hostPath-{i}',
                },
            )
            self.process_volumes([

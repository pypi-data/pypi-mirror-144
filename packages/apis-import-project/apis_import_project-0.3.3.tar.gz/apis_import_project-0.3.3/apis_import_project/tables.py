...
def get_generic_relation_listview_table(relation_name):
    ...
    class GenericRelationListViewTable(tables.Table):
        ...
        class Meta:
            ...
            attrs = {"class": "table table-sm table-hover table-striped table-condensed"}
    ...
    if detail:
        ...
        class RelationTableEdit(RelationTableBase):
            ...
            class Meta(RelationTableBase.Meta):
                ...
                if relation_class.__name__ == "PersonInstitution" and entity_class_name == "person":
                    fields = ["delete"] + RelationTableBase.Meta.fields + ["edit"] + ["autoDate"]
                else:
            ...
            def __init__(self, *args, **kwargs):
                ...
                # autoDate button
                if relation_class.__name__ == "PersonInstitution" and entity_class_name == "person":
                    self.base_columns['autoDate'] = tables.TemplateColumn(
                        template_name = "element_templates/autoDate_button_generic_ajax_form.html"
                    )
...
def get_generic_entity_progress_table(data, version=True):

    class GenericEntityProgressTable(tables.Table):
        if version:
            type = tables.columns.TemplateColumn(template_code=u"""{{record.version}}""", default="")
        name = tables.columns.TemplateColumn(template_code=u"""<a href='#' onclick='updateSessionEntity("{{record.entity_type}}","{{record.id}}")'> {{record.name}}</a>""", default="")
        start = tables.columns.TemplateColumn(template_code=u"""{{record.start_date_written}}""", default="")
        end = tables.columns.TemplateColumn(template_code=u"""{{record.end_date_written}}""", default="")
        labels = tables.columns.TemplateColumn(template_code=u"""{{record.labels}}""", default="")
        if "titles" in data[0].keys():
            titles = tables.columns.TemplateColumn(template_code=u"""{{record.titles}}""", default="")
        if "gender" in data[0].keys():
            gender = tables.columns.TemplateColumn(template_code=u"""{{record.gender}}""", default="")
        if "first_name" in data[0].keys():
            first_name = tables.columns.TemplateColumn(template_code=u"""{{record.first_name}}""", default="")
        if "kind" in data[0].keys():
            kind = tables.columns.TemplateColumn(template_code=u"""{{record.kind}}""", default="")


        class Meta:
            attrs = {"class": "table table-sm table-hover table-striped table-condensed pb-5"}
            fields = ["name", "start", "end", "labels"]
            if "first_name" in data[0].keys():
                fields.insert(1, "first_name")
            elif "kind" in data[0].keys():
                fields.insert(1, "kind")
            if "titles" in data[0].keys():
                fields += ["titles"]
            if "gender" in data[0].keys():
                fields.insert(4, "gender")
            if version:
                fields.insert(0, "type")
            sequence = fields

    return GenericEntityProgressTable(data)

def get_generic_relation_progress_table(data, version=True):

    A_class = data[0]["relation_source"]["class"]
    B_class = data[0]["relation_target"]["class"]
    ...
    class GenericRelationProgressTable(tables.Table):
        if version:
            type = tables.columns.TemplateColumn(template_code=u"""{{record.version}}""", default="")
        start = tables.columns.TemplateColumn(template_code=u"""{{record.start_date_written}}""", default="")
        end = tables.columns.TemplateColumn(template_code=u"""{{record.end_date_written}}""", default="")
        objectA = tables.columns.TemplateColumn(template_code=u"""<a href='#' onclick='updateSessionEntity("{{record.relation_source.class}}","{{record.relation_source.id}}")'> {{record.relation_source.name}}</a>""", verbose_name=A_class, default="")
        relation = tables.columns.TemplateColumn(template_code=u"""{{record.relation_type.name}}""", default="")
        objectB = tables.columns.TemplateColumn(template_code=u"""<a href='#' onclick='updateSessionEntity("{{record.relation_target.class}}","{{record.relation_target.id}}")'> {{record.relation_target.name}}</a>""", verbose_name=B_class, default="")

        class Meta:
            attrs = {
                "class": "table table-sm table-hover table-striped table-condensed pb-5"
            }
            fields = ["start", "end", "objectA", "relation", "objectB"]
            if version:
                fields.insert(0, "type")
            sequence = fields

    return GenericRelationProgressTable(data)

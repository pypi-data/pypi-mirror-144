...
class SpecSecSecurityAttr03(ScoreNode):
    ...
    def __init__(self, qspec, target_obj=None, attr_wt=9):
        ...
        self.fix_template = \
            '[CVSPS003] [%s]: Security field contains an empty security '\
            'requirement.'
    ...
    def compute_openapiv2(self):
        ...
        if self.qspec.G.has_successor(self.qspec.ROOT_NODE, security_node):
            for security_elem_idx in self.qspec.G.neighbors(security_node):
                if self.qspec.G.out_degree(security_elem_idx) == 0:
                    empty_req = True

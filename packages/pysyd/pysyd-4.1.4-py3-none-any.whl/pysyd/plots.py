...
def plot_estimates(star, highlight=True):
    """
...
def plot_parameters(star, n_peaks=10):
    ...
    if star.params['globe']:
        ax3.plot(params['frequency'][(params['frequency'] >= n[2])&(params['frequency'] <= n[-2])], params['pssm'][(params['frequency'] >= n[2])&(params['frequency'] <= n[-2])], color='yellow', linewidth=2.0, linestyle='dashed', zorder=6)
...
def plot_samples(star):
    ...
    for i, col in enumerate(params['df'].columns.values.tolist()):

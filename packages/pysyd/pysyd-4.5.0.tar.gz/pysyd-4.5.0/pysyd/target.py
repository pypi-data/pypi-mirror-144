            ...
            if not self.ps:
                ...
                if self.params['oversampling_factor'] is not None:
                    if int(oversampling_factor) != self.params['oversampling_factor']:
                        print("WARNING: calculated vs. provided oversampling factor do NOT match")
                else:
#                    if float(oversampling_factor%1) != 0.:
#                        raise InputError("ERROR: the oversampling factor calculated from the data is not an integer\nPlease check the input data and try again")
#                    else:

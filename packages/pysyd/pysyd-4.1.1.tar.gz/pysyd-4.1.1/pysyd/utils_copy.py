...
#except NameError:
INFDIR = os.path.join(os.path.abspath(os.getcwd()),'info')
...
class Parameters(Constants):
    ...
    def __init__(self, args=None, stars=None):
        ...
        if args is not None:
            self.add_cli(args)
        if (self.params['cli'] and args.stars is not None) or stars is not None:
            self.add_stars()
    ...
    def get_defaults(self, params={}):
        ...
        Attributes:
            ...
            cli : bool
                `True` if pysyd is ran via command line
    ...
    def get_main(self, stars=None, excess=True, background=True, globe=True, verbose=False, 
                 ...
                 todo='todo.txt', info='star_info.csv', cli=False):
        """
    ...
    def add_cli(self, args):
        self.params['cli'], self.params['mode'] = True, args.mode
        # Assert arrays are proper lengths and/or types
        self.check_cli(args)
    ...
    def add_stars(self, stars=None, mode='load',):
        """
        Add target stars
        ...
        """
        self.assign_stars(stars)
        # if no mode set yet, use load since it is "safe" mode
    ...
    def assign_stars(self, stars):
        """

        Add targets

        """
        # If no stars have been provided, read in from text file
        if stars is None:
            if self.params['stars'] is None or not self.params['stars']:
    ...
    def add_info(self):
        ...
        for star in self.params['stars']:
            if self.params[star]['numax'] is not None:
                ...
                self.params[star]['dnu'] = 0.22*(self.params[star]['numax']**0.797)
            else:
                if 'rs' in self.params[star] and self.params[star]['rs'] is not None and \
                  'logg' in self.params[star] and self.params[star]['logg'] is not None:
                    self.params[star]['ms'] = ((((self.params[star]['rs']*self.r_sun)**(2.0))*10**(self.params[star]['logg'])/self.G)/self.m_sun)
                    self.params[star]['numax'] = self.numax_sun*self.params[star]['ms']*(self.params[star]['rs']**(-2.0))*((self.params[star]['teff']/self.teff_sun)**(-0.5))
                    self.params[star]['dnu'] = self.dnu_sun*(self.params[star]['ms']**(0.5))*(self.params[star]['rs']**(-1.5))  
    ...
    def get_info(self):
        ...
        # Fill/copy rest of keys 
        for star in self.params['stars']:
            for column in columns['all']:
                if column not in self.params[star] or (column in self.params[star] and self.params[star][column] is None):
        ...
        for i, star in enumerate(self.params['stars']):
            for column in get_dict(type='columns')['override']:
                if self.params['cli'] and self.override[column] is not None:
...
def scrape_output(args):
    ...
    if glob.glob(os.path.join(args.params['outdir'],'**','estimates*csv')):
        ...
        df.sort_values(by='star', inplace=True)
    ...
    if glob.glob(os.path.join(args.params['outdir'],'**','global*csv')):
        ...
        for dir in dirs:
            ...
            if df_temp['uncertainty'].values.tolist()[0] != '--':
                columns += ['%s_err'%col for col in columns]
        ...
        columns = sorted(df.columns.values.tolist())
        idx = columns.index('star')
        del columns[idx]
        final_cols = ['star'] + columns
        df_final = pd.DataFrame(columns=final_cols)
        for col in final_cols:
            df_final[col] = df[col]
        df_final.to_csv(os.path.join(args.params['outdir'],'global.csv'), index=False)
        # reload and stars will likely be of integer type aka better sorting
        df = pd.read_csv(os.path.join(args.params['outdir'],'global.csv'))
        df.sort_values(by='star', inplace=True)

...
from pysyd.plots import make_plots
...
# 
    ...
    def estimate_parameters(self, excess=True,):
        ...
        if self.params['excess']:
            ...
            # execute function
            self.estimate_numax()
            # if still not a numax value, return warning
            if not self.check_numax():
                print('# WARNING: did not converge on a value for numax\nPlease try again.')
                return
    ...
    def derive_parameters(self, background=True, globe=True, mc_iter=1, converge=False,):
        ...
        if not self.check_numax():
            print('# WARNING: you are about to derive parameters but it is best to start\nwith an approximate value for numax.')
            return
        # get initial values and fix data
        self.initial_parameters()
        # if the first step converges, carry on
        if self.first_step():
    ...
    def load_time_series(self, nyquist=None):
        ...
        if os.path.exists(os.path.join(self.params['inpdir'], '%s_LC.txt' % str(self.name))):
            ...
            if not self.ps:
                ...
                self.params['oversampling_factor'] = int(round((1./((max(self.time)-min(self.time))*0.0864))/(self.frequency[1]-self.frequency[0])))
                self.frequency, self.power = self.fix_data(self.frequency, self.power)
            note += self.note
            self.freq_os, self.pow_os = np.copy(self.frequency), np.copy(self.power)
            self.freq_cs = np.array(self.frequency[self.params['oversampling_factor']-1::self.params['oversampling_factor']])
...
#
    ...
    def estimate_numax(self, n_trials=3, binning=0.005, bin_mode='mean', smooth_width=20.0, ask=False):
        ...
        if self.params['verbose']:
            print('------------------------------------------------------\nEstimating numax:\nPS binned to %d datapoints' % len(self.smooth_freq))
    ...
    def check_numax(self, columns=['numax', 'dnu', 'snr']):
        ...
        if self.params['numax'] is None:
            ...
            if glob.glob(os.path.join(self.params['path'],'estimates*')):
                ...
                if np.isnan(float(self.params['numax'])) or self.params['numax'] is None:
                    return False
...
#
    ...
    def solar_scaling(self, numax=None, scaling='tau_sun_single', max_laws=3, ex_width=1.0,
        ...
        # If not, uses entire power spectrum
            ...
            maxpower = [min(self.frequency), max(self.frequency)]
            if self.params['lower_ps'] is not None:
                maxpower[0] = self.params['lower_ps']
            if self.params['upper_ps'] is not None:
                maxpower[1] = self.params['upper_ps']
            self.params['ps_mask'] = [maxpower[0],maxpower[1]]
            scale = self.constants['numax_sun']/((maxpower[0]+maxpower[1])/2.)
#            scale = 1.0
    ...
    def first_step(self, converge=False, background=True, globe=True,):
        ...
        if self.params['background']:
            self.fit_background()
            if self.params['verbose']:
                print('------------------------------------------------------\nDetermining background model:\nPS binned to %d data points' % len(self.bin_freq))
            converge = self.get_best_model()
        else:
            converge = True
        ...
        # Requires bg convergence before doing global fit
        if converge and self.params['globe']:
            # get numax smooth
    ...
    def fit_global(self, acf_mask=None):
        ...
        # get numax
        self.get_numax_smooth()
        _ = self.get_numax_gaussian()
        # get dnu
        self.compute_acf()
        self.estimate_dnu()


    def fit_background(self, ind_width=20.0,):
        """
    ...
    def get_best_model(self, n_laws=None, fix_wn=False, basis='tau_sigma',):
        """
    ...
    def save_best_model(self, metric='bic'):
        """
    ...
    def get_red_noise(self):
        ...
        if self.params['selected'] != 0:
            ...
            self.bg_corr = self.random_pow/self.params['noise']
        return True
    ...
    def get_numax_gaussian(self):
        ...
        """
        guesses = [0.0,max(self.region_pow),self.params['obs_numax'],(max(self.region_freq)-min(self.region_freq))/np.sqrt(8.0*np.log(2.0))]
    ...
    def compute_acf(self, fft=True, smooth_ps=2.5, ps_mask=None,):
        ...
        mask = np.ma.getmask(np.ma.masked_inside(lag, self.params['dnu']/4., 2.*self.params['dnu']+self.params['dnu']/4.))
        lag = lag[mask]
        auto = auto[mask]
        auto -= min(auto)
        auto /= max(auto)
    ...
    def initial_dnu(self, force=None, method='D', n_peaks=10,):
        ...
        if self.params['dnu'] is not None:
            guess = self.params['dnu']
        else:
            guess = self.params['exp_dnu']
        if self.params['method'] == 'M':
            ...
            self.peaks_l, self.peaks_a = utils.max_elements(self.lag, self.auto, npeaks=self.params['n_peaks'], exp_dnu=guess)
        else:
            pass

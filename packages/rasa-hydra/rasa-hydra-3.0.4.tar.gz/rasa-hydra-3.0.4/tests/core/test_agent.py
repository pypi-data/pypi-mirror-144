...
async def test_load_agent(trained_model):
    ...
    assert agent[0].tracker_store is not None
    assert agent[0].interpreter is not None
    assert agent[0].model_directory is not None
    assert agent[1].tracker_store is not None
    assert agent[1].interpreter is not None
    assert agent[1].model_directory is not None
...
async def test_agent_update_model_none_domain(trained_model):
    agent = await load_agent(model_path=trained_model)
    agent[0].update_model(
        None, None, agent[0].fingerprint, agent[0].interpreter, agent[0].model_directory
    )

    sender_id = "test_sender_id"
    message = UserMessage("hello", sender_id=sender_id)
    await agent[0].handle_message(message)
    tracker = agent[0].tracker_store.get_or_create_tracker(sender_id)

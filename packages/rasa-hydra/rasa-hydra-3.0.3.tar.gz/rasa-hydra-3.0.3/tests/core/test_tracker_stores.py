...
from rasa.core.events import SlotSet, ActionExecuted, Restarted, UserUttered, BotUttered
...
from tests.core.utilities import get_tracker
...
def test_stream_events_step_assignment():
    actual_steps_sequence = []
    expected_steps_sequence = [0, 1, 2, 3, 4, 5, 6, 7, 8]
    broker = broker_utils.from_endpoint_config(read_endpoint_config("data/test_endpoints/event_brokers/pika_endpoint.yml", "event_broker"))
    tracker_store = InMemoryTrackerStore(domain, broker)
    events = [
        UserUttered("/greet", {"name": "greet", "confidence": 1.0}, []),
        BotUttered("one", data={"custom": {"type": "capture_utterance"}}),
        BotUttered("two", data={"custom": {"type": "dtmf"}}),
        UserUttered("sip_info", {"name": "goodbye", "confidence": 1.0}, []),
        BotUttered("three", data={"custom": {"type": "get_speech"}}),
        BotUttered("four", data={"custom": {"type": "send_sip_info"}}),
        UserUttered("/goodbye", {"name": "goodbye", "confidence": 1.0}, []),
        BotUttered("five", data={"custom": {"type": "not_app_event"}}),
        BotUttered("six", data={"custom": {"type": "hangup"}})
    ]
    tracker = get_tracker(events)
    tracker_store.stream_events(tracker)
    while broker._unpublished_messages:
        actual_steps_sequence.insert(0, loads(broker._unpublished_messages.pop())["step_number"])
    assert actual_steps_sequence == expected_steps_sequence
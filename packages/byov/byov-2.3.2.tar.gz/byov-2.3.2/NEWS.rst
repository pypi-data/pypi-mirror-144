...
=====
  ...
  * Turn `byov.path` option into the `byov.path` python list as this is
    needed to build config stacks and as such cannot be described inside a
    config file. This is for the benefit of projects like `byoci` that want
    to layout an additional set of config files on top of the `byov` ones
    via the `byov.conf.d/byov.py` extension point.

...
class ExperimentCompiler(Compiler):
    ...
    def get_tp(self, sigloc=None, tp=None, mode=None, *args, **kwargs) -> pd.Series:
        ...
        df = self.concat_signal(sigloc=sigloc, mode=mode, *args, **kwargs)
        if tp > df.shape[1]:
            print(
                f"Compiler:Warning: {tp} in meta is larger than real dataframe for signal {sigloc}"
            )
            tp = self.grouper.ntps - 1
        return df.iloc[:, tp]
    ...
    def compile_dmetrics(self, metrics=["max_dVol", "max_bud_dVol"], stages=None):
        ...
        input_signals = {
            k: self.grouper.concat_signal(v, pool=0) for k, v in names_signals.items()
        }
    ...
    def compile_growth_metrics(
        ...
        input_signals = {
            k: self.grouper.concat_signal(v) for k, v in tqdm(names_signals.items())
        }
...
for dir in dirs:
    try:
        ...
        print(f"Compiler:ERROR: Failed experiment {dir}")
        print(e)
# print(f"LOG:ERROR:{dir}: {e}")


# def try_report(dir, base_dir=base_dir):
#     try:
#         compiler = ExperimentCompiler(None, base_dir / dir)
#         dfs = compiler.run()
#         rep = Reporter(data=dfs, path=base_dir / (dir + "/report.pdf"))

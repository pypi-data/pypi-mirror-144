...
def get_max_s_val_ratio(s_vals):
    ...
    for i, s_vals in enumerate(s_vals):
        if len(s_vals) == 1:
            max_ratio_idx[i][:] = i, 0, 0
        else:
            ratio = (s_vals[0] / s_vals)
...
def truncate_s_vals(s_vals, index_to_drop):
    s_vals_truncated = []
    for m, s_vals in enumerate(s_vals):
        if m == int(index_to_drop[0]):
            s_vals_truncated.append(s_vals[:int(index_to_drop[1])])
        else:
            s_vals_truncated.append(s_vals)
...
def bit_string(nqubits, decimal):
    ...
    if np.abs(decimal) > (2**nqubits-1):
        ermsg = ("For a bit string of size {:d}".format(nqubits) +
                 ", |decimal| must be smaller than {:d}.".format(2**nqubits-1))
        raise ValueError(ermsg)

    str1 = "{{:0>{}b}}".format(nqubits)
...
def update_kwargs_dict(default_kwargs, kwargs=None):
    ...
    if kwargs is not None:
        options = dict([(key, val) for key, val in default_kwargs.items()

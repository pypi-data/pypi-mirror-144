...
bcpnp_case_stream = {
    'validate': 'list',
    'source': (['EE-Skilled Worker', 'EE-International Graduate', 'EE-International Post-Graduate', 'EE-Health Authority',
                'Skilled Worker', 'International Graduate', 'Entry-Level and Semi-Skilled Worker', 'International Post-Graduate', 'Health Authority']
               )
}
corporate_structure = {
    'validate': 'list',
    'source': ["Incorporated", "Limited Liability Partnership", "Extra-provincially-registered", "federally-incorporated", "Other"]
}
canada_provinces = {
    'validate': 'list',
    'source': ["AB", "BC", "MB", "NB", "NL", "NS", "NT", "NU", "ON", "PE", "QC", "SK", "YT"]
}
english_french = {
    'validate': 'list',
...
imm_status = {
    'validate': 'list',
    'source': ["Citizen", "Permanent Resident", "Worker", "Student", "Visitor", "Refugess", "Other"]
}
...
purpose_of_lmia = {
    'validate': 'list',
    'source': ["Support Permanent Resident only", "Support Work Permit only", "Support both Work Permit and Permanent Resident"]
}
stream_of_lmia = {
    'validate': 'list',
    'source': ["1. EE(Express Entry: 5593)",
...
workpermit_type = {
    'validate': 'list',
    'source': ["Co-op Work Permit", "Exemption from Labour Market Impact Assessment", "Labour Market Impact Assessment Stream", "Live-in Caregiver Program", "Open Work Permit", "Open work permit for vulnerable workers", "Other", "Post Graduation Work Permit", "Start-up Business Class"]
}
marital_status = {
    'validate': 'list',
    'source': []  # TODO:
...
education_level = {
    'validate': 'list',
    'source': ["Doctor", 'Master', 'Post-graduate diploma', 'Bachelor', 'Associate', 'Diploma/Certificate', 'High school', 'Less than high school']
}
relationship = {
    "validate": 'list',
    'source': ["Grand Parent", "Parent", "Spouse", "Child", "Grand Child", "Sibling", "Aunt", "Uncle", "Niece", "Newphew", "Friend"]
}
family_relationship = {
    'validate': 'list',
...
pr_imm_program = {
    'validate': 'list',
    'source': ["Economic", "Family"]
}
pr_imm_category = {
    'validate': 'list',
    'source': ["Agri-Food Pilot", "Home Support Worker Pilot", "Rural and Northern Immigration Program", "Provincial Nominee Program (PNP)", "Self-Employed Persons Class", "Start-Up Business Class", "Quebec Skilled Workers Program", "Quebec Selected Investor Program", "Quebec Entrepreneur Program", "Quebec Self-Employed Persons Program", "Atlantic Immigration Program", "Spouse", "Common-law Partner", "Conjugal Partner", "Dependent Child", "Other Relative", "Adopted child/Child to be adopted in Canada"]
}
...
wp_in_application_purpose = {
    'validate': 'list',
    'source': ['apply or extend work permit', 'restore status as worker', 'TRP']
}
wp_apply_wp_type = {
    'validate': 'list',
    'source': [

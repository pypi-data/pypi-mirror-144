...
def nestedModel(df, X_columns, y_column, model_regression='linear', X_trasformations_columns=[],
                ...
                return_metric=False, size=0.2, positive=False):
    metrics_values = {}
    ...
    if model_regression == 'linear':
        if return_metric:
            ...
            result, model = linear(df, X_columns, y_column, name_model, metric=metric, return_metric=return_metric, size=size, positive=positive)
        ...
        if return_metric:
            ...
            result, model = logLinear(df, X_columns, y_column, name_model, metric=metric, return_metric=return_metric, size=size, positive=positive)
...
def mainNestedModel(sub_df, sub_X_columns, sub_y_column, X_columns, y_column, sub_model_regression='linear',
                    ...
                    return_sub_metric=False, sub_size=0.2, sub_positive=False, model_regression='linear',
                    X_trasformations_columns=[], model_regression_log_log='linear', ridge_number=0,
                    metric=['rsq', 'nrmse', 'mape'], return_metric=False, size=0.2, positive=False):
    metrics_values = {}
    if return_sub_metric == True:
        sub_result, sub_model, sub_metric_values = nestedModel(sub_df, sub_X_columns, sub_y_column,
                                                               ...
                                                               sub_metric, return_sub_metric, sub_size=sub_size, sub_positive=sub_positive)
    else:
        sub_result, sub_model = nestedModel(sub_df, sub_X_columns, sub_y_column, sub_model_regression,
                                            sub_X_trasformations_columns,
                                            sub_model_regression_log_log, sub_ridge_number, sub_metric, sub_size=sub_size, sub_positive=sub_positive)
    ...
    if model_regression == 'linear':
        ...
        if return_metric:
            result, model, metrics_values = ridge(df, X_columns, y_column, name_model, ridge_number=ridge_number,
                                                  metric=metric, return_metric=return_metric, size=size, positive=positive)
        else:
            result, model = ridge(df, X_columns, y_column, name_model, ridge_number=ridge_number, metric=metric,
                                  return_metric=return_metric, size=size, positive=positive)

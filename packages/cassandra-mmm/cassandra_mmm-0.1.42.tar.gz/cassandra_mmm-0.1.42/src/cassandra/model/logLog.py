...
def logLog(df, X, y, target, name_model, medias = [], organic = [], metric = ['rsq', 'nrmse', 'mape'], return_metric = False):
    metrics_values = {}

    X_log = np.log(abs(X) + 1)
    y_log = np.log(y + 1)
    ...
    if medias or organic:
        ...
        result['prediction'] = np.exp(model.predict(X_log)) - 1

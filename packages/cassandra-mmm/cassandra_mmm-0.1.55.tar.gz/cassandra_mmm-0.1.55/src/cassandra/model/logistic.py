...
def logistic(df, X_columns, target, name_model, medias=[], organic=[], metric=['rsq', 'nrmse', 'mape'],
    ...
    if medias or organic:
        model = create_model(medias, organic, LogisticRegression(positive=positive))
    else:
        model = LogisticRegression(positive=positive)

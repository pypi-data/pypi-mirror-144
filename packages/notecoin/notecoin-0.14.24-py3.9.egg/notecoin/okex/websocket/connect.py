    ...
    def run(self):
        ...
        while True:
            if self.channel_listen is not None:
                new_channels = self.channel_listen()
                old_channels_str = [str(channel['channel']) for channel in self.channels]
                new_channels_str = [str(channel['channel']) for channel in new_channels]
                if new_channels_str is not None and len(new_channels_str) > 0:
                    if len(list(set(old_channels_str).intersection(set(new_channels_str)))) != len(new_channels_str):

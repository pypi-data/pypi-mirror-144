...
from notecoin.database.base import Base, BaseTable, create_all, create_session
...
class OkexClientAccountBalance(Base, BaseTable):
    ...
    def update_data():
        ...
        try:
            ...
            for detail in data:
                OkexClientAccountBalance.session.merge(OkexClientAccountBalance(**detail))
            OkexClientAccountBalance.session.commit()
            return {"success": len(data)}
        except Exception as e:
            OkexClientAccountBalance.session.rollback()
    ...
    def delete_dated():
        try:
            ...
            # 先查询，后删除
            OkexClientAccountBalance.session.query(OkexClientAccountBalance).filter(
                OkexClientAccountBalance.updateTime < update_time).delete()
            OkexClientAccountBalance.session.commit()
            OkexClientAccountBalance.session.query(OkexClientAccountBalance).filter(
                OkexClientAccountBalance.eqUsd < 1).delete()
            OkexClientAccountBalance.session.commit()

...
class GroupSummary(RunnableSummary):
    ...
    def is_config_consistent_failure(self, minimum_count: int = 3) -> Optional[bool]:
        ...
        for task in self.tasks:
            for result in task.results:
                if result.group == self.name:
                    if task.configuration == "test-macosx1015-64-qr/debug-*-fis-e10s" and self.name == "browser/components/privatebrowsing/test/browser/browser.ini":
                        print(task.id)
        ...
        if self.name == "browser/components/privatebrowsing/test/browser/browser.ini":
            for config, results in config_to_results.items():
                if len(results) >= minimum_count and not any(results):
                    print(f"{config} has {len(results)} failures")

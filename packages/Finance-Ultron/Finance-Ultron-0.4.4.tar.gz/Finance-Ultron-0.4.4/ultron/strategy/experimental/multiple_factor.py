...
class MultipleFactor(object):
    ...
    def _mixed_signal(self, columns):
        mixed_signal = np.sum(self._signal_data.set_index(['trade_date','code']
                    )[columns],axis=1)
        mixed_signal.name = 'signal'
        mixed_signal = mixed_signal.reset_index()
        ### 过滤叠加信号为Nan
    ...
    def _initialize_weight(self, mixed_signal, volatility_name):
        ...
        ### 多空组合，归零处理
        long_weighted = weighted[weighted.weight > 0] ## 多头权重
        short_weighted = weighted[weighted.weight < 0] ## 空头权重
        equal_weighted = weighted[weighted.weight == 0] ### 0 权重

        long_weighted = long_weighted.set_index(['trade_date']).groupby(
            level=['trade_date']).apply(
                lambda x: self._weighted(x, 'weight')

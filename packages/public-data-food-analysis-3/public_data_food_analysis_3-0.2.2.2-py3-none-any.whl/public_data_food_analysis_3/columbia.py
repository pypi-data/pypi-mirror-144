...
# Cell
def mean_daily_eating_duration(df, start_date='not_defined', end_date='not_defined'):
    """
    Description:\n
    ...
    breakfast_time = df.groupby('date')['float_time'].agg(min)
    dinner_time = df.groupby('date')['float_time'].agg(max)
    return (dinner_time-breakfast_time).mean()

# Cell
def std_daily_eating_duration(df, start_date='not_defined', end_date='not_defined'):
    """
    Description:\n
...
def make_table(food_data, ref_tbl, min_log_num=2, min_seperation=5, buffer_time= '15 minutes', h=4):
    ...
    for index, row in ref_tbl.iterrows():
        ...
        rows.append(mean_daily_eating_duration(df[df['PID']==id_], row['Start_Day'],row['End_day']))
        rows.append(std_daily_eating_duration(df[df['PID']==id_], row['Start_Day'],row['End_day']))

...
def check_coords(x: float, y: float) -> None:
    ...
    if x > 0 or y < 0:
        ...
        print(
            "Coordinates outside of COUNS. Submit a point within (-124.848974, "
...
def get_local_catchment(x: float, y: float) -> tuple[str, shapely.geometry]:
    ...
    try:
        r: requests.models.Response = requests.get(
            NLDI_GEOSERVER_URL, params=fixed_payload
        )
        resp = r.json()
...
def get_raindrop_path(  # noqa C901
    ...
    nhd_mask.flat[nhd_cell_list] = True  # type: ignore[index]

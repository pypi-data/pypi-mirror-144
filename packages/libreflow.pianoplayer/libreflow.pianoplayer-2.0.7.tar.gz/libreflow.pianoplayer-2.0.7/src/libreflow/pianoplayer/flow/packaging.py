...
class CreateCleanPackagesAction(CreateShotPackagesAction):
    ...
    def get_shots_data(self, refresh=False):
        if self._shots_data is None or refresh:
            ...
            regex = self.shot_name_regex.get()
            line_src_dir = site.package_line_dir.get()
            color_src_dir = site.package_color_dir.get()
            ...
            for sg_shot in sg_config.get_shots(task_filter['name'], task_filter['status']):
                ...
                if m is not None:
                    ...
                    if line_src_dir:
                        line_src_path = self.find_shot_source_dir(sequence_name, shot_name, line_src_dir)

                    color_src_path = self.find_shot_source_dir(sequence_name, shot_name, color_src_dir)
                    color_status = sg_config.get_shot_task_status(sg_shot_id, 'Color Ink & Paint')

                    status = 'warning'
                    if color_src_path is None: # clean files don't exist
                        status = 'error'
                    elif color_status == 'apr':
                        status = 'valid'
                    ...
                    self._shots_data.append({
                        ...
                        'publish_comment': None,
                        'status': status,
                        'available': color_src_path is not None,
                        'layout_src_path': None,
                        'layout_sg_status': 'na',
                        'color_src_path': color_src_path,
                        'color_sg_status': color_status,
                        'line_src_path': line_src_path,
                    })
    ...
    def create_shot_packages(self, shot_data, do_upload, dst_dir):
        created = False
        color_src_path = shot_data['color_src_path']
        line_src_path = shot_data['line_src_path']
        source_paths = []

        if color_src_path is not None:
            source_paths.append(color_src_path)
        if line_src_path is not None:
            source_paths.append(line_src_path)

        if source_paths:
            created = self._create_shot_package(shot_data, 'clean', 'sources', source_paths, do_upload, dst_dir)
        
        return created

    def create_packages(self, shots_data, do_upload=False):
        site = self.root().project().get_current_site()
        pkg_color_dir = site.package_clean_dir.get()
        dst_dir = os.path.join(
            site.package_target_dir.get(),
            datetime.now().strftime('%y%m%d')
        )
        ...
        if pkg_color_dir is None:
            print((
                'Packages :: Color package directory '
                'must be specified in the current site settings'
            ))
            return

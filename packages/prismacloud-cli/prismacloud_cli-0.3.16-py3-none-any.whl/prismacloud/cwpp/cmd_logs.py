...
def defender(limit=150, hostname=''):
    result = pc_lib.get_endpoint("logs/defender", { 'lines': limit, 'hostname': hostname} )
    cli_output(result)

@click.command()
@click.option('-l', '--limit', default=150, help='Number of documents to return')
def console(limit=150):
    result = pc_lib.get_endpoint("logs/console", { 'lines': limit } )
    cli_output(result)

@click.command()
def audit():
    result = pc_lib.get_endpoint("audits/mgmt")
    cli_output(result)

...
def cli(ctx, query, amount, unit, field=''):
    ...
    if query.startswith('config from'):
        result_list = pc_lib.pc_api.search_config_read(
            search_params=search_params)
    elif query.startswith('network from'):
        result_list = pc_lib.pc_api.search_network_read(
            search_params=search_params)
    elif query.startswith('event from'):
        result_list = pc_lib.pc_api.search_event_read(
            search_params=search_params)
    else:
        logging.error(
            'Unknown RQL query type (limited to: config|network|event).')
